...
public class CStoreService extends DicomService implements CStoreSCP {
	...
	private final Object identifySender(final Association association) {
		return new StringBuilder().append(association.getRemoteAET())
				.append("@")
		...
		try {
			try {
				final GradualDicomImporter importer = new GradualDicomImporter(
						this, userProvider.get(), fw, ImmutableMap.of(
								GradualDicomImporter.SENDER_ID_PARAM,
								identifySender(as),
								GradualDicomImporter.TSUID_PARAM, tsuid,
								GradualDicomImporter.SENDER_AE_TITLE_PARAM,
								as.getRemoteAET()));
	...
	public static final class Specifier {
		...
		public CStoreService build() {
			final CStoreService cstore = new CStoreService(identifier,
					userProvider);
			if (null != namer) {
		...
		public Provider<XDATUser> getUserProvider() {
			return userProvider;
		}
