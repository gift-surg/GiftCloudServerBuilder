...
public class ContainedAssignmentDicomIdentifier extends DelegateDicomIdentifier {
	private final static String START = "(?:\\A|(?:.*[\\s,;]))",
			OPTWS = "\\s*", END = "(?:(?:[\\s,;].*\\Z)|\\Z)",
	...
	public ContainedAssignmentDicomIdentifier(final int tag, final String id,
			final String op, final String valuePattern, int patternFlags) {
		super(new PatternDicomIdentifier(tag,
				Pattern.compile(
						new StringBuilder(START).append(id).append(OPTWS)
								.append(op).append(OPTWS).append(START_GROUP)
								.append(valuePattern).append(END_GROUP)
								.append(END).toString(), patternFlags), 1));
	}

	public ContainedAssignmentDicomIdentifier(final int tag, final String id,
			final String op, final int patternFlags) {
