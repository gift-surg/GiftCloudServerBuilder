...
public class DbBackedProjectIdentifier implements DicomProjectIdentifier {
	private final Logger logger = LoggerFactory
			.getLogger(DbBackedProjectIdentifier.class);
	...
	public DbBackedProjectIdentifier(
			final Iterable<DicomDerivedString> identifiers) {
		this.extractors = Lists.newArrayList(identifiers);
		final ImmutableSortedSet.Builder<Integer> b = ImmutableSortedSet
				.naturalOrder();
	...
	public final XnatProjectdata apply(final XDATUser user, final DicomObject o) {
		...
		for (final DicomDerivedString extractor : extractors) {
			...
			if (!Strings.isNullOrEmpty(alias)) {
				// added caching here to prevent duplicate project queries in
				// every file transaction
				// the cache is shared with the one in gradual dicom importer,
				// which does a similar query.
				final Element pe = cache.get(alias);
				if (null == pe) {
					// no cached value, look in the db
					final XnatProjectdata p = XnatProjectdata
							.getProjectByIDorAlias(alias.toString(), user,
									false);
					if (null != p && canCreateIn(user, p)) {
						cache.put(new Element(alias, p));
						return p;
					} else {
						// this alias is either not a project or not one we can
						// write to
						GradualDicomImporter.cacheNonWriteableProject(cache,
								alias);
					}
				} else if (!GradualDicomImporter
						.isCachedNotWriteableProject(pe)) {
	...
	private boolean canCreateIn(final XDATUser user, final XnatProjectdata p) {
		try {
			...
			logger.error(
					"Unable to check permissions for " + user + " in " + p, e);
			return false;
		}
	}
