...
public class TemplatizedDicomFileNamer implements DicomFileNamer {
	...
	/**
	 * Makes the file name for the given DICOM object based on the naming
	 * template specified during namer initialization.
	 * 
	 * @param dicomObject
	 *            The DICOM object for which the name should be calculated.
	 * @return The generated file name from the variable values extracted from
	 *         the DICOM object.
	 */
	public String makeFileName(DicomObject dicomObject) {
		...
		for (final String variable : _variables) {
			if (!variable.startsWith(HASH_PREFIX)) {
				final String tagValue = dicomObject.getString(Tag
						.forName(variable));
				values.put(variable, tagValue == null ? "no-value-for-"
						+ variable : tagValue);
			}
		}
	 ...
	 * 
	 * @param values
	 *            The various extracted variable values.
	...
	public String makeFileName(Map<String, String> values) {
		...
		try {
			...
			throw new RuntimeException(
					"Error trying to resolve naming template", exception);
		}
	 ...
	 * 
	 * @param variables
	 *            The variables for which the hash should be calculated.
	 * @param values
	 *            The map of all values extracted from the DICOM object.
	 * @return The calculated hash.
	 */
	private String calculateHash(final List<String> variables,
	...
	/**
	 * This tells you whether the template has an extension that is separated by
	 * the '.' character and consists of one or more alphanumeric characters.
	 * 
	 * @param template
	 *            The naming template to be tested.
	...
	private boolean hasExtension(final String template) {
		...
		final String lastElement = template
				.substring(template.lastIndexOf("."));
		return !StringUtils.isBlank(lastElement)
				&& lastElement.matches("^\\.[A-z0-9]+");
	}
			...
			if (variable.startsWith(HASH_PREFIX)) {
				if (!variable.contains(HASH_DELIMITER)) {
					throw new RuntimeException(
							"You can't specify a "
									+ HASH_PREFIX
									+ " without specifying at least two DICOM header values joined by the "
									+ HASH_DELIMITER + " delimiter.");
				}
				List<String> variables = Arrays.asList(variable.substring(4)
	...
	private void validate() {
		...
		try {
			...
			throw new RuntimeException("Illegal DICOM header tag specified: "
					+ lastVariable, exception);
		}
	}
