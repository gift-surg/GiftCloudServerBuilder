...
public class PipelineRepositoryManager {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(PipelineRepositoryManager.class);
	...
	public synchronized static PipePipelinerepository GetInstance() {
		if (pipelineRepository == null) {
			...
			ArrayList<PipePipelinerepository> pipelineRepos = PipePipelinerepository
					.getAllPipePipelinerepositorys(null, true);
	...
	public synchronized static void SetInfo(
			final PipePipelinedetails pipelineDetails) throws Exception {
		...
		if (!xmlFile.exists())
			throw new Exception("File at " + pipelineXml + " not found");
		...
		if (hasXnatInfo) {
			...
			if (pipeline.getXnatInfo().isSetGeneratesElements()) {
				GeneratesElements elementsGenerated = pipeline.getXnatInfo()
						.getGeneratesElements();
				for (int i = 0; i < elementsGenerated.sizeOfElementArray(); i++) {
					String elementGenerated = elementsGenerated
							.getElementArray(i);
		...
		if (pipeline.isSetDocumentation()) { // Extract the input parameters for
												// the pipeline
			Documentation documentation = pipeline.getDocumentation();
			if (documentation.isSetInputParameters()) {
				InputParameters inputParameters = documentation
						.getInputParameters();
				for (int i = 0; i < inputParameters.sizeOfParameterArray(); i++) {
					...
					if (param.isSetValues()) {
						if (param.getValues().isSetSchemalink()) {
							pipeParam.setValues_schemalink(param.getValues()
									.getSchemalink());
						} else if (param.getValues().isSetCsv()) {
							pipeParam.setValues_csvvalues(param.getValues()
									.getCsv());
						}
					} else {
	...
	public synchronized static void RemoveReferenceToPipelineFromProject(
			PipePipelinedetails pipeline, UserI user, String pID,
			final EventUtils.TYPE type, String reason) {
		// Check to see if the project has some study protocol data
		// corresponding to the pipeline
		// If it does do nothing
		// If no data has been generated so far remove the reference to the
		// study protocol generated by the pipeline
		XnatProjectdata proj = XnatProjectdata.getXnatProjectdatasById(pID,
				user, false);
		...
		if (proj != null) {
			...
			try {
				wrk = PersistentWorkflowUtils.buildOpenWorkflow(
						(XDATUser) user, XnatProjectdata.SCHEMA_ELEMENT_NAME,
						proj.getId(), proj.getId(), EventUtils
								.newEventInstance(CATEGORY.PROJECT_ADMIN, type,
			...
			ArcProject arcProject = ArcSpecManager.GetFreshInstance()
					.getProjectArc(proj.getId());
			removePipeline(pipeline.getPath(), arcProject, user, c);
		}
	}

	private static void removePipeline(String pipelinePath,
			ArcProject aProject, UserI user, EventMetaI c) {
		List<ArcProjectPipelineI> projectPipelines = aProject
				.getPipelines_pipeline();
		if (logger.isDebugEnabled()) {
			logger.debug(
					"Checking the {} pipelines configured for project {} to remove pipeline {}.",
					projectPipelines.size(), aProject.getId(), pipelinePath);
		}
		for (ArcProjectPipelineI projectPipeline : projectPipelines) {
			...
			if (pipelinePath.equals(path)) {
				try {
					if (logger.isDebugEnabled()) {
						logger.debug(
								"Removing the {} pipeline from project {}.",
								pipelinePath, aProject.getId());
					}
					SaveItemHelper.authorizedRemoveChild(
							instance.getCurrentDBVersion(), null,
							pipeline.getCurrentDBVersion(), user, c);
					instance.getItem().removeItem(pipeline.getItem());
					// DBAction.DeleteItem(pipeline.getItem(), user);
					break;
				} catch (Exception e) {
					logger.error("Couldn't delete pipelines located at "
							+ pipelinePath + " for project " + aProject.getId());
				}
			} else if (logger.isDebugEnabled()) {
				logger.debug(
						"Found a pipeline {}, but that doesn't match the target pipeline {} so I'm not removing it.",
						path, pipelinePath);
			}
		}

		List<ArcProjectDescendantI> descendants = aProject
		...
		for (ArcProjectDescendantI descendant : descendants) {
			...
			List<ArcProjectDescendantPipelineI> pipelines = instance
					.getPipeline();
			if (logger.isDebugEnabled()) {
				logger.debug(
						"Now processing {} pipelines related to descendant {}.",
						pipelines.size(), instance.getArcProjectDescendantId());
			}
			for (ArcProjectDescendantPipelineI pipeline1 : pipelines) {
				...
				if (pipelinePath.equals(path)) {
					try {
						...
						SaveItemHelper.authorizedRemoveChild(
								instance.getCurrentDBVersion(), null,
								descPipeline.getCurrentDBVersion(), user, c);
						instance.getItem().removeItem(descPipeline.getItem());
						break;
					} catch (Exception e) {
						logger.error("Couldn't delete pipelines located at "
								+ pipelinePath + " for project "
								+ aProject.getId() + " descendant  "
								+ instance.getXsitype());
					}
				}
			}
		...
		/*
		 * if (deleted) { try { ValidationResults vr =
		 * aProject.getCurrentDBVersion().validate(); if (vr.isValid()){
	...
	public synchronized static void RemoveReferenceToPipelineFromProjects(
			String pipelinePath, UserI user, EventMetaI c) {
		ArcArchivespecification arcSpec = ArcSpecManager.GetFreshInstance();
		List<ArcProjectI> arcProjects = arcSpec.getProjects_project();
		if (logger.isDebugEnabled()) {
			logger.info(
					"I will now attempt to remove the pipeline {} from the {} projects on this system.",
					pipelinePath, arcProjects.size());
		}
	...
	// Reinstated this method that was removed (possibly because it was thought
	// to be unused??)
	// ...because without it CNDA builds breaks due to their reference to it in:
	// src.org.apache.turbine.app.cnda_xnat.modules.screens.PipelineScreen_BoldSeedBasedAnalysis.java
	// (line #42)
	public static String GetPathToPipeline(String partialPipelinePath,
			ArcProject aProject) {
		String rtn = null;
		List<ArcProjectPipelineI> projectPipelines = aProject
				.getPipelines_pipeline();
		for (int j = 0; j < projectPipelines.size(); j++) {
			ArcProjectPipeline projectPipeline = (ArcProjectPipeline) projectPipelines
					.get(j);
			ArcPipelinedata pipeline = (ArcPipelinedata) projectPipeline
					.getPipelinedata();
			String path = pipeline.getLocation();
		...
		if (rtn == null) {
			List<ArcProjectDescendantI> projectDescs = aProject
					.getPipelines_descendants_descendant();
			for (int j = 0; j < projectDescs.size(); j++) {
				ArcProjectDescendant projectDesc = (ArcProjectDescendant) projectDescs
						.get(j);
				List<ArcProjectDescendantPipelineI> pipelines = projectDesc
						.getPipeline();
				for (int k = 0; k < pipelines.size(); k++) {
					ArcProjectDescendantPipeline descPipeline = (ArcProjectDescendantPipeline) pipelines
							.get(k);
					ArcPipelinedata pipeline = (ArcPipelinedata) descPipeline
							.getPipelinedata();
					String path = pipeline.getLocation();
					if (path.endsWith(partialPipelinePath)) {
						rtn = path;
