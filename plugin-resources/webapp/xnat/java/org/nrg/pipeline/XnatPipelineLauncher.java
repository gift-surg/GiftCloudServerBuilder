...
public class XnatPipelineLauncher {
	private static final Logger logger = LoggerFactory
			.getLogger(XnatPipelineLauncher.class);
	private static final Logger launchLogger = LoggerFactory
			.getLogger("org.nrg.pipeline.launch");
	...
	public static final Pattern DEPASSWORDIFIZER = Pattern
			.compile("(.*)(-pwd )\\S+(.*)");
	...
	/**
	 * @param useAlias
	 *            Indicates whether the alias should be used.
	 */
	...
	public boolean launch(String cmdPrefix) {
		return runPipelineInProcess ? launchInProcessPipelineExecution()
				: launchExternalPipelineExecution(cmdPrefix);
	}
	...
	private boolean launchInProcessPipelineExecution() {
		...
		try {
			...
			if (launchLogger.isInfoEnabled()) {
				launchLogger
						.info("Launching pipeline in-process with parameters: "
								+ depasswordifize(convertArgumentListToCommandLine(parameters)));
			}
			XNATPipelineLauncher launcher = new XNATPipelineLauncher(parameters);
			success = launcher.run();
		} catch (Exception exception) {
			logger.error(exception.getMessage()
					+ " for in-process execution of pipeline " + pipelineName
					+ " with parameters:\n" + depasswordifize(parameters),
					exception);
			success = false;
		}
		return success;
	...
	private boolean launchExternalPipelineExecution(String cmdPrefix) {
		...
		String command = buildPipelineLauncherScriptCommand(cmdPrefix) + " "
				+ convertArgumentListToCommandLine(getCommandLineArguments());
		...
		try {
			...
			if (launchLogger.isInfoEnabled()) {
				launchLogger.info("Launching pipeline with command: "
						+ depasswordifize(command));
			}
			...
			logger.error(e.getMessage() + " for command "
					+ depasswordifize(command), e);
			success = false;
		}
	...
	private String depasswordifize(final String command) {
		...
		if (matcher.matches()) {
			return matcher.group(1) + matcher.group(2) + NOT_A_PASSWORD
					+ matcher.group(3);
		} else {
			return command;
		}
	...
	private String buildPipelineLauncherScriptCommand(String cmdPrefix) {
		...
		command += XFT.GetPipelinePath() + "bin" + File.separator
	...
	private List<String> getPipelineConfigurationArguments() {
		...
		try {
			String pipelinePath = new File(XFT.GetPipelinePath())
					.getCanonicalPath();
			boolean requiresQuotes = pipelinePath.contains(" ");
			arguments.add("-config");
			String configPath = pipelinePath + File.separator
					+ "pipeline.config";
			arguments.add(requiresQuotes ? "\"" + configPath + "\""
					: configPath);
			arguments.add("-log");
			String logConfigPath = pipelinePath + File.separator
					+ "log4j.properties";
			arguments.add(requiresQuotes ? "\"" + logConfigPath + "\""
					: logConfigPath);
			arguments.add("-catalogPath");
			String catalogPath = pipelinePath + File.separator + "catalog";
			arguments.add(requiresQuotes ? "\"" + catalogPath + "\""
					: catalogPath);
		} catch (IOException e) {
	 ...
	 * in-process and external launch mode. Use the
	 * {@link #convertArgumentListToCommandLine(List)} method to convert the
	 * returned list to a command line.
	 *
	 * @return A list of the submitted command-line arguments.
	 */
	private List<String> getCommandLineArguments() {
		AliasToken token = XDAT.getContextService()
				.getBean(AliasTokenService.class).issueTokenForUser(user);
		...
		for (String param : params) {
			...
			for (String value : values) {
				if (!StringUtils.isBlank(value)) {
					paramArg.append(escapeSpecialShellCharacters(value))
							.append(",");
				}
			}
	...
	public static String getUserName(UserI user) {
		...
		try {
			if (user.getFirstname() != null && user.getLastname() != null)
				rtn = user.getFirstname().substring(0, 1) + "."
						+ user.getLastname();
	...
	private void setBuildDir() {
		...
		String buildPath = ArcSpecManager.GetFreshInstance()
				.getGlobalBuildPath();
		if (buildPath.endsWith(File.separator)) {
			buildPath = buildPath.substring(0, buildPath.length() - 1);
		}
		if (needsBuildDir) {
			parameters.put("builddir",
					Arrays.asList(buildPath + File.separator + "Pipeline"));
		}
	}
	...
	public static XnatPipelineLauncher GetLauncherForExperiment(RunData data,
			Context context, XnatExperimentdata imageSession) throws Exception {
		...
		xnatPipelineLauncher.setParameter("cachepath", QCImageCreator
				.getQCCachePathForSession(imageSession.getProject()));

		List<String> emails = new ArrayList<String>();
		emails.add(TurbineUtils.getUser(data).getEmail());

		String extraEmails = (String) TurbineUtils.GetPassedParameter(
				"emailField", data);
	...
	public static XnatPipelineLauncher GetBareLauncherForExperiment(
			RunData data, Context context, XnatExperimentdata imageSession)
			throws Exception {
		XnatPipelineLauncher xnatPipelineLauncher = new XnatPipelineLauncher(
				data, context);
		xnatPipelineLauncher.setSupressNotification(true);
	...
	public static XnatPipelineLauncher GetLauncher(RunData data,
			Context context, XnatImagesessiondata imageSession)
			throws Exception {
		XnatPipelineLauncher xnatPipelineLauncher = GetLauncherForExperiment(
				data, context, imageSession);
