...
public class PipelineAdder {
	...
	public void prepareScreen(RunData data, Context context) {
		...
		try {
			if (edit) {
				...
				if (dataType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) { // It's
																			// a
																			// project
																			// level
																			// pipeline
				...
				if (dataType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) { // It's
					...
					// ArcProjectDescendantPipeline newPipeline = new
	...
	private void redirect(RunData data, String templateFile) throws Exception {
		if (templateFile != null) {
			...
			String template = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("template", data));
