...
public abstract class BaseArcPipelinedata extends AutoArcPipelinedata {
	...
	public String getCommand(XFTItem itemOfExpectedXsiType)
			throws ElementNotFoundException, FieldNotFoundException,
			XFTInitException {
		String rtn = "";
		if (getLocation() != null) {
		...
		// rtn += getName();
		List<ArcPipelineparameterdataI> parameters = this
				.getParameters_parameter();
		for (int i = 0; i < parameters.size(); i++) {
			ArcPipelineparameterdata param = (ArcPipelineparameterdata) parameters
					.get(i);
