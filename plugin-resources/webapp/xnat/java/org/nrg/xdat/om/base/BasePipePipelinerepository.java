...
public abstract class BasePipePipelinerepository extends
		AutoPipePipelinerepository {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(BasePipePipelinerepository.class);
	...
	/**
	 * Helper method to output a list of the pipelines registered to this
	 * arcProject.
	 * 
	 * [0]: Display name of XSI type [1]: Elements generated by this pipeline
	 * [2]: Pipeline name [3]: Pipeline description [4]: Pipeline location [5]:
	 * Applies To [6]: xsi:type [7]: stepId
	 * 
	 * @param arcProject
	 * @return
	 */
	public List<String[]> listPipelines(ArcProject arcProject) {
		...
		if (arcProject != null) {
			final List<ArcProjectPipelineI> pipelines = arcProject
					.getPipelines_pipeline();
			if (pipelines.size() > 0) {
				for (int i = 0; i < pipelines.size(); i++) {
					final PipePipelinedetails pipeline = getPipeline(pipelines
							.get(i).getLocation());
			...
			final List<ArcProjectDescendantI> descendants = arcProject
					.getPipelines_descendants_descendant();
			if (descendants.size() > 0) {
				for (int i = 0; i < descendants.size(); i++) {
					final List<ArcProjectDescendantPipelineI> pipelinesDesc = descendants
							.get(i).getPipeline();
					for (int j = 0; j < pipelinesDesc.size(); j++) {
						final PipePipelinedetails pipeline = getPipeline(pipelinesDesc
								.get(j).getLocation());
						final String[] rowEntries = new String[8];
						rowEntries[0] = getDisplayName(descendants.get(i)
								.getXsitype());
						rowEntries[1] = getElementsGeneratedBy(pipeline);
	...
	public PipePipelinedetails getPipeline(String path) {
		...
		for (int i = 0; i < pipelines.size(); i++) {
			PipePipelinedetails pipeline = (PipePipelinedetails) pipelines
					.get(i);
		...
		if (pipelines.size() > 0) {
			for (int i = 0; i < pipelines.size(); i++) {
				...
				if (rtn.containsKey(pipeline.getAppliesto())) {
					ArrayList<PipePipelinedetails> groupedByAppliesTo = rtn
							.get(pipeline.getAppliesto());
	...
	public Hashtable<String, ArrayList<ArcPipelinedata>> getPipelinesForProject(
			XnatProjectdata project) throws Exception {
		...
		while (keys.hasMoreElements()) {
			...
			if (xsiType.equalsIgnoreCase(XnatProjectdata.SCHEMA_ELEMENT_NAME))
				pipelinesForDataType = getArcPipelinesForProject(repository
						.get(xsiType));
			else
				pipelinesForDataType = getArcPipelinesForDescendant(repository
						.get(xsiType));
	...
	public ArrayList<ArcProjectPipeline> getArcPipelinesForProject(
			ArrayList<PipePipelinedetails> pipelines) throws Exception {
		...
		for (int i = 0; i < pipelines.size(); i++) {
			...
			if (description != null)
				description += " XNAT DATATYPE: " + pipeline.getAppliesto()
						+ " Applies To: "
						+ getDisplayName(pipeline.getAppliesto())
						+ " Generates: " + getElementsGeneratedBy(pipeline);
			else
				description = " XNAT DATATYPE: " + pipeline.getAppliesto()
						+ " Applies To: "
						+ getDisplayName(pipeline.getAppliesto())
			...
			List<PipePipelinedetailsParameterI> parameters = pipeline
	...
	public boolean delete(ArcProject arcProject, String pathToPipeline,
			String dataType, XDATUser user) throws Exception {
		...
		try {
			if (dataType.equalsIgnoreCase(XnatProjectdata.SCHEMA_ELEMENT_NAME)) {
				List<ArcProjectPipelineI> pipelines = arcProject
						.getPipelines_pipeline();
				for (int i = 0; i < pipelines.size(); i++) {
					ArcProjectPipeline pipeline = (ArcProjectPipeline) pipelines
							.get(i);
				...
				ArcProjectDescendant descendant = arcProject
						.getDescendant(dataType);
				if (descendant == null)
					success = false;
				else {
					List<ArcProjectDescendantPipelineI> pipelines = descendant
							.getPipeline();
					for (int i = 0; i < pipelines.size(); i++) {
						ArcProjectDescendantPipeline pipeline = (ArcProjectDescendantPipeline) pipelines
								.get(i);
						if (pipeline.getLocation().equals(pathToPipeline)) {
			...
			if (save) {
				SaveItemHelper.authorizedSave(arcProject, user, false, true,
						EventUtils.newEventInstance(
								...
								EventUtils.TYPE.STORE_XML,
								"Modified project pipelines"));
	...
	public ArrayList<ArcProjectDescendantPipeline> getArcPipelinesForDescendant(
			ArrayList<PipePipelinedetails> pipelines) throws Exception {
		...
		for (int i = 0; i < pipelines.size(); i++) {
			...
			if (parameters != null && parameters.size() > 0) {
				for (int j = 0; j < parameters.size(); j++) {
					PipePipelinedetailsParameter parameter = (PipePipelinedetailsParameter) parameters
							.get(j);
					ArcPipelineparameterdata paramData = new ArcPipelineparameterdata();
					paramData.setName(parameter.getName());
					description = parameter.getDescription();
	...
	public ArcPipelineparameterdata convertToArcPipelineParameter(
			PipePipelinedetailsParameter parameter) {
		ArcPipelineparameterdata paramData = new ArcPipelineparameterdata();
		paramData.setName(parameter.getName());
	...
	public Hashtable<String, String> getGeneratedElementsByAllPipelines() {
		...
		while (enume.hasMoreElements()) {
			...
			ArrayList<PipePipelinedetails> pipelinesForDataType = repository
					.get(dataType);
			for (int i = 0; i < pipelinesForDataType.size(); i++) {
				PipePipelinedetails pipeline = pipelinesForDataType.get(i);
				List<PipePipelinedetailsElementI> generatedElements = pipeline
						.getGenerateselements_element();
	...
	private boolean exists(List<ArcProjectPipelineI> pipelines, String path) {
		...
		if (pipelines == null || path == null || pipelines.size() == 0)
			return false;
		for (int i = 0; i < pipelines.size(); i++) {
			String pipelinePath = pipelines.get(i).getLocation(); // +
																	// File.separator
																	// +
																	// pipelines.get(i).getName();
	...
	private boolean existsDesc(List<ArcProjectDescendantPipelineI> pipelines,
			String path) {
		boolean exists = false;
		if (pipelines == null || path == null || pipelines.size() == 0)
			return exists;
	...
	public ArcProject createNewArcProject(XnatProjectdata proj)
			throws Exception {
		ArcProject arcProject = new ArcProject();
		if (proj == null)
			return arcProject;
		Hashtable<String, ArrayList<ArcPipelinedata>> pipelinesHash = PipelineRepositoryManager
				.GetInstance().getPipelinesForProject(proj);
		if (pipelinesHash != null && pipelinesHash.size() > 0) {
			...
			while (keys.hasMoreElements()) {
				...
				if (dataType.equals(proj.SCHEMA_ELEMENT_NAME)) {
					ArrayList<ArcPipelinedata> projectPipelines = pipelinesHash
							.get(proj.SCHEMA_ELEMENT_NAME);
					if (projectPipelines != null && projectPipelines.size() > 0) {
						for (int i = 0; i < projectPipelines.size(); i++) {
							arcProject.setPipelines_pipeline(projectPipelines
									.get(i));
						}
					}
				} else {
					...
					if (projectPipelines != null && projectPipelines.size() > 0) {
						...
						arcProject
								.setPipelines_descendants_descendant(arcProjectDesc);
					}
				}
			}
		}
	...
	public ArcProject getAdditionalPipelines(XnatProjectdata project)
			throws Exception {
		// ArcProject arcProject = new ArcProject();
		...
		ArcProject arcProjectFromSpec = ArcSpecManager.GetFreshInstance()
				.getProjectArc(project.getId());
		if (arcProjectFromSpec == null)
			return createNewArcProject(project);
		...
		// Clear all the existing pipelines as we want only the additional
		// pipelines that are applicable for this project
		// rtn.removeAllPipelines();
		if (pipelinesHash != null && pipelinesHash.size() > 0) { // There are
																	// some site
																	// pipelines
			List<ArcProjectPipelineI> projectSelectedPipelines = arcProjectFromSpec
					.getPipelines_pipeline();
			...
			ArrayList<ArcPipelinedata> pipelines = pipelinesHash
					.get(project.SCHEMA_ELEMENT_NAME);
			...
			while (keys.hasMoreElements()) {
				String xsiType = keys.nextElement();
				ArrayList<ArcPipelinedata> descpipelines = pipelinesHash
						.get(xsiType);
				List<ArcProjectDescendantPipelineI> projectSelectedDescPipelines = arcProjectFromSpec
						.getPipelinesForDescendant(xsiType);
				...
				if (descpipelines != null) {
					...
					additionalPipelineHash
							.put(xsiType, additionalDescPipelines);
				}
			}
			Enumeration<String> dataTypes = additionalPipelineHash.keys();
			while (dataTypes.hasMoreElements()) {
				String dataType = dataTypes.nextElement();
				ArrayList<ArcPipelinedata> additionalDescPipelines = additionalPipelineHash
						.get(dataType);
				ArcProjectDescendant arcProjectDesc = new ArcProjectDescendant();
				arcProjectDesc.setXsitype(dataType);
	...
	public List getAdditionalPipelinesForDatatype(XnatProjectdata project,
