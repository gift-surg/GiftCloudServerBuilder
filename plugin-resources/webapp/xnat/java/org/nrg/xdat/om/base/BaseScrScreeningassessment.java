...
public abstract class BaseScrScreeningassessment extends
		AutoScrScreeningassessment implements ScanAssessorI {
	...
	public ScrScreeningscandata getScanScreeningAssessment(String scanId) {
		...
		if (scans != null && scans.size() > 0) {
			for (int i = 0; i < scans.size(); i++) {
				ScrScreeningscandata aScan = (ScrScreeningscandata) scans
						.get(i);
		...
		if (rtn == null)
			throw new NullPointerException(
					"Couldnt find screening assessment for scan id " + scanId);
		return rtn;
	}
