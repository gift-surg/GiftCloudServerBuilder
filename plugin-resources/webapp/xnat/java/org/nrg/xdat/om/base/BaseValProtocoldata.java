...
public abstract class BaseValProtocoldata extends AutoValProtocoldata implements
		ScanAssessorI {
	...
	public ValProtocoldataScanCheck getScanProtocolValidation(String scanId)
		...
		if (scans != null && scans.size() > 0) {
			for (int i = 0; i < scans.size(); i++) {
				ValProtocoldataScanCheck aScan = (ValProtocoldataScanCheck) scans
						.get(i);
		...
		if (rtn == null)
			throw new NullPointerException(
					"Couldnt find scan protocol validation for scan id "
							+ scanId);
		return rtn;
	}
