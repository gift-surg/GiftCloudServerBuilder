...
public class BaseXnatDicomseries extends AutoXnatDicomseries {
	...
	public ArrayList getCorrespondingFiles(String rootPath) {
		if (files == null) {
			...
			for (int i = 0; i < this.getImageset_image().size(); i++) {
				org.nrg.xdat.om.XnatDicomseriesImage image = (org.nrg.xdat.om.XnatDicomseriesImage) this
						.getImageset_image().get(i);
	...
	public String getFullPath(String rootPath) {
		...
		fileNames = new ArrayList();
		for (org.nrg.xdat.model.XnatDicomseriesImageI image : this
				.getImageset_image()) {
			path = image.getUri();
			break;
		}
		...
		String fullPath = StringUtils.ReplaceStr(
				FileUtils.AppendRootPath(rootPath, path), "\\", "/");
	...
	public void moveTo(File newSessionDir, String existingSessionDir,
		...
		for (XnatDicomseriesImageI img : this.getImageset_image()) {
			((XnatDicomseriesImage) img).moveTo(newSessionDir,
					existingSessionDir, rootPath, user, ci);
		}
