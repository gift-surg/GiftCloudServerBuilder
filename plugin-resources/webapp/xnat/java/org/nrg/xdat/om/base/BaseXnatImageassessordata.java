...
public abstract class BaseXnatImageassessordata extends
		...
		if (session_id != null) {
			...
			try {
				final String idCOUNT = (String) PoolDBUtils
						.ReturnStatisticQuery(query + session_id + "';", "id",
				...
				if (project != null) {
					...
					String new_session_id = (String) PoolDBUtils
					...
					query = "SELECT sharing_share_xnat_experimentda_id FROM xnat_experimentData_share WHERE label='"
							+ session_id + "' AND project='" + project + "';";
					new_session_id = (String) PoolDBUtils.ReturnStatisticQuery(
							query, "sharing_share_xnat_experimentda_id",
							this.getDBName(), login);
					if (new_session_id != null) {
						this.setImagesessionId(new_session_id);
						return true;
	...
	public void deleteFiles(UserI u, EventMetaI ci) throws Exception {
		...
		final String rootPath = ArcSpecManager.GetInstance()
		...
		if (!StringUtils.IsEmpty(sessionDir)
				&& !StringUtils.IsEmpty(assessorDir)) {
	...
	public void preSave() throws Exception {
		if (StringUtils.IsEmpty(this.getId())) {
			throw new IllegalArgumentException(
					"Please specify an ID for your experiment.");
		}

		if (XFT.getBooleanProperty("security.require_image_assessor_labels",
				false) && StringUtils.IsEmpty(this.getLabel())) {
			throw new IllegalArgumentException(
					"Please specify a label for your experiment.");
		}
		...
		if (!StringUtils.IsEmpty(this.getLabel())
				&& !StringUtils.IsAlphaNumericUnderscore(getLabel())) {
			throw new IllegalArgumentException(
