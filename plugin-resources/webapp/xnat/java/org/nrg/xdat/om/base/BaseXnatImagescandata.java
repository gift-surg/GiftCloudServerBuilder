...
public class BaseXnatImagescandata extends AutoXnatImagescandata {
	...
	public void deleteFilesFromFileSystem(String rootPath, UserI u,
			EventMetaI ci) throws Exception {
		List<XnatAbstractresourceI> files = this.getFile();
		if (!files.isEmpty())
			for (XnatAbstractresourceI resource : files) {
				((XnatAbstractresource) resource).deleteWithBackup(rootPath, u,
						ci);
			}
	}
			...
			for (XnatAbstractresourceI xnatFile : this.getFile()) {
				if (xnatFile instanceof org.nrg.xdat.om.XnatResource) {
					...
					if (last_dir == null) {
						...
						if (index != -1) {
							if (last_dir.toUpperCase().indexOf(
									"/" + this.getId().toUpperCase() + "/",
									index) > -1) {
								scan_dir = last_dir.substring(
										0,
										last_dir.toUpperCase().indexOf(
												"/"
														+ this.getId()
																.toUpperCase()
														+ "/", index)
		...
		if (this.getFile().size() > 0) {
			...
			for (XnatAbstractresourceI res : this.getFile()) {
				if (res.getLabel() != null
						&& res.getLabel().equalsIgnoreCase("SNAPSHOTS")) {
					path = "/data/experiments/" + ses.getId() + "/scans/"
							+ this.getId() + "/resources/SNAPSHOTS/files/";
					if (res instanceof XnatResourcecatalog) {
						CatCatalogI cat = ((XnatResourcecatalog) res)
								.getCleanCatalog(ses.getProjectData()
										.getRootArchivePath(), false, null,
										null);
			...
			if (path == null) {
				path = "/app/template/QualityControl.vm?project="
						+ ses.getProject() + "&session_label=" + label
						+ "&session_id=" + ses.getId() + "&scan="
						+ this.getId();
	...
	public XnatQcscandataI getManualQC() {
		...
		if (session.getManualQC() != null
				&& session.getManualQC().getScans_scan() != null) {
	...
	public static List<XnatImagescandata> getScansByIdORType(
			final String scanID, final XnatImagesessiondata session,
			UserI user, boolean preLoad) {
		...
		return XnatImagescandata
				.getXnatImagescandatasByField(cc, user, preLoad);
	}
	...
	public Map<String, Number> getPrearchiveFileStats() {
		...
		for (XnatAbstractresourceI resource : this.getFile()) {
			...
			try {
				...
				prearcPath = this.getParent().getStringProperty(
						"prearchivepath");
				if (null == prearcPath || prearcPath.equals("")) {
					throw new Exception(
							"Unable to determine the prearchive path for scan");
				}
			} catch (Exception e) {
				// If the prearchive path is null or empty, count and size
				// should be 0
			...
			if (resource instanceof XnatResourcecatalogI) {
				...
				final String rootPath = CatalogUtils
						.getCatalogFile(prearcPath,
								((XnatResourcecatalogI) resource))
						.getParentFile().getAbsolutePath();
				CatalogUtils.Stats c_stats = CatalogUtils.getFileStats(
						CatalogUtils.getCleanCatalog(prearcPath,
								(XnatResourcecatalogI) resource, false),
						rootPath);
				if (null != c_stats) {
					...
					logger.error(
							...
							new Exception(
									"Unable to retrieve CatalogUtils.Stats for the given resource."));
				}
			} else {
				logger.error(
						"Unable to retrieve file counts.",
						new Exception(
								"Prearchive resource is not an instance of XnatResourcecatalogI. Skipping..."));
			}
		}
	...
	public List<String> getReadableFileStats() {
		...
		// If the scan is in the prearchive, retrieve the count & file sizes
		// from CatalogUtils.getFileStats()
		if (this.getImageSessionId() == null) {
			Map<String, Number> prearcStats = this.getPrearchiveFileStats();
			stats.add(CatalogUtils.formatFileStats("TOTAL",
					prearcStats.get("count").intValue(), prearcStats
							.get("size").longValue()));
		} else { // If this scan is already archived, retrieve the count & file
					// sizes from the resource
			int totalCount = 0;
			long totalSize = 0;
			...
			stats.add(0, CatalogUtils.formatFileStats("TOTAL", totalCount,
					totalSize));
		}
	...
	/**
	 * It's annoying to do this, but in some cases it's easier to get this
	 * formatted display directly from the scan object rather than trying to
	 * manage it in the Velocity template.
	 * 
	 * @return A formatted string that will display the scans in a tip text
	 *         format.
	 */
	public String getFormattedReadableFileStats(XDATUser user) throws Exception {
		if (getFile().size() > 0) {
			List<String> stats = getReadableFileStats();
			if (stats == null || stats.size() == 0 || stats.get(0) == null
					|| stats.get(0).equals("")
					|| stats.get(0).equals("0 B in 0 files")) {
	...
	static public String getListAsTipText(List<String> list) {
		...
		StringBuilder buffer = new StringBuilder("<span class=\"tip_text\">")
				.append(list.get(0))
				.append("<span style=\"top:20px;white-space:nowrap;left:-10;width:auto;\" class=\"tip shadowed\">");
