...
public abstract class BaseXnatImagesessiondata extends AutoXnatImagesessiondata {
	...
	private static final Map<String, String> CUSTOM_SCAN_FIELDS = ImmutableMap
			.of("Scan Time", "startTime");
	...
	public String getArchivePath(String rootPath)
		...
		for (XnatImagescandataI scan : this.getScans_scan()) {
			...
			if (files.size() > 0) {
				...
				while (fIter.hasNext()) {
					XnatAbstractresource file = (XnatAbstractresource) fIter
							.next();
					...
					if (filePath != null && !filePath.equals("")) {
						try {
							...
							if (index == -1) {
								index = filePath.indexOf(dirName.toLowerCase());
							}
							if (index == -1) {
								index = filePath.indexOf(dirName.toUpperCase());
							}

							if (index < 0) {
								path = FileUtils.AppendRootPath("",
										getArchiveRootPath());
								return path;
							}
							if (index != -1) {
					...
					for (String filePath : file.getUnresolvedPaths()) {
						if (filePath != null && !filePath.equals("")) {
							try {
								...
								int index = upperFilePath.indexOf(dirName
										.toUpperCase());
								if (index == 0) {
		...
		if (arcFiles == null) {
			...
			if (this.hasSRBData()) {
				...
				for (XnatImagescandataI scan : this.getScans_scan()) {
					...
					try {
						XNATMetaData meta = new XNATMetaData();
						meta.setCategory("SCAN"); // match operator is = by
													// default
						...
						String stats = count
								+ " files, "
								+ formatter
										.format(((float) ((float) size / 1048576)))
								+ "MB";
				...
				for (XnatImagescandataI scan : this.getScans_scan()) {
					...
					long size = 0;
					try {
						String parsedScanID = StringUtils.ReplaceStr(
								StringUtils.ReplaceStr(scan.getId(), "-", ""),
								"*", "AST");

						ArrayList fileGroup = (ArrayList) getFileGroups().get(
								SCAN_ABBR + parsedScanID);
						if (fileGroup == null) {
							...
							while (iter.hasNext()) {
								String fID = (String) iter.next();
								int idIndex = this.getFileTracker().getIDIndex(
										fID);
				...
				for (XnatReconstructedimagedataI recon : this
						.getReconstructions_reconstructedimage()) {
					int count = 0;
					long size = 0;
					try {
						String parsedScanID = StringUtils.ReplaceStr(
								StringUtils.ReplaceStr(recon.getId(), "-", ""),
								"*", "AST");

						ArrayList fileGroup = (ArrayList) getFileGroups().get(
								RECON_ABBR + parsedScanID);
						if (fileGroup == null) {
				...
				for (XnatImageassessordataI assess : this
						.getAssessors_assessor()) {
					int count = 0;
					long size = 0;
					try {
						String parsedScanID = StringUtils
								.ReplaceStr(StringUtils.ReplaceStr(
										assess.getId(), "-", ""), "*", "AST");

						ArrayList fileGroup = (ArrayList) getFileGroups().get(
								ASSESSOR_ABBR + parsedScanID);
						if (fileGroup == null) {
				...
				for (XnatAbstractresourceI res : this.getResources_resource()) {
					...
					long size = 0;
					try {
						String parsedScanID = res.getXnatAbstractresourceId()
								.toString();

						ArrayList fileGroup = (ArrayList) getFileGroups().get(
								RESOURCES_ABBR + parsedScanID);
						if (fileGroup == null) {
				...
				try {
					...
					if (fileGroup != null && !fileGroup.isEmpty()) {
						...
						while (iter.hasNext()) {
							String fID = (String) iter.next();
							size += this.getFileTracker().getSize(
									this.getFileTracker().getIDIndex(fID));
						}
						count = fileGroup.size();
	...
	/**
	 * @param id
	 *            The ID of the assessor to retrieve.
	...
	public List<XnatReconstructedimagedata> getReconstructionsByType(String type) {
		...
		if (al != null) {
			for (int i = 0; i < al.size(); i++) {
				XnatReconstructedimagedata scan = (XnatReconstructedimagedata) al
						.get(i);
				try {
			...
			if (getItem().isPreLoaded()) {
				...
				try {
					...
					while (table.hasMoreRows()) {
						...
						try {
							XFTItem child = XFTItem.NewItem(element,
									this.getUser());
							...
							if (lastname != null) {
								try {
									child.setProperty("investigator.lastname",
											lastname);
								} catch (XFTInitException e) {
									logger.error("", e);
								} catch (ElementNotFoundException e) {
	 ...
	 * 
	 * Some servers will want to fully load the data, others won't. Separating
	 * this into its own method allows sites to just override this method,
	 * instead of the old thing (like they used to have to).
	 * 
	 * @param xsiType
	 * @param child
	 ...
	 * @throws Exception
	 */
	public void addMinLoadAssessor(String xsiType, XFTItem child,
			List<XnatImageassessordataI> minLoadAssessors)
			throws InvalidItemException, Exception {
		XnatImageassessordata assessor = (XnatImageassessordata) BaseElement
				.GetGeneratedItem(child);

		if (xsiType
				.equalsIgnoreCase(XnatQcmanualassessordata.SCHEMA_ELEMENT_NAME)) {
			if (this.getUser().canRead(child)) {
				this.manQC = new XnatQcmanualassessordata(
						child.getCurrentDBVersion(false));
				minLoadAssessors.add(this.manQC);
			} else {
				minLoadAssessors.add(new XnatQcmanualassessordata(child));
			}
		} else if (xsiType
				.equalsIgnoreCase(XnatQcassessmentdata.SCHEMA_ELEMENT_NAME)) {
			if (this.qc == null) {
				if (this.getUser().canRead(child)) {
					this.qc = new XnatQcassessmentdata(
							child.getCurrentDBVersion(false));
			...
			minLoadAssessors.add((XnatImageassessordata) BaseElement
					.GetGeneratedItem(child.getCurrentDBVersion(false)));
		} else {
	...
	public void loadSRBFiles() {
		if (this.fileGroups.size() == 0) {
			...
			Iterator reconIter = this.getReconstructions_reconstructedimage()
					.iterator();
			while (reconIter.hasNext()) {
				XnatReconstructedimagedata recon = (XnatReconstructedimagedata) reconIter
						.next();
				...
				String parsedScanID = StringUtils.ReplaceStr(
						...
						"AST");
				fileGroups
						.put(RECON_ABBR + parsedScanID, dir.filterLocal(meta));
			...
			for (XnatImageassessordataI assess : this.getAssessors_assessor()) {
				...
				String parsedScanID = StringUtils.ReplaceStr(
						StringUtils.ReplaceStr(assess.getId(), "-", ""), "*",
						"AST");
	...
	private void loadDefinedFiles(String rootPath) {
		...
		for (XnatImagescandataI scan : this.getSortedScans()) {
			...
			if (!scan.getFile().isEmpty()) {
				for (XnatAbstractresourceI xnatFile : scan.getFile()) {
					for (File f : ((XnatAbstractresource) xnatFile)
							.getCorrespondingFiles(rootPath)) {
						if (f.exists()) {
							String fileID = this._files.addFile(f.getPath(), f,
									FileTracker.KNOWN);
					...
					if (xnatFile instanceof XnatResourcecatalog) {
						File f = ((XnatResourcecatalog) xnatFile)
								.getCatalogFile(rootPath);
						if (f.exists()) {
							String fileID = this._files.addFile(f.getPath(), f,
									FileTracker.KNOWN);
							fileGrouping.add(fileID);
		...
		while (reconIter.hasNext()) {
			...
			if (outFiles.size() > 0) {
				...
				while (files.hasNext()) {
					XnatAbstractresource xnatFile = (XnatAbstractresource) files
							.next();
		...
		while (resourceIter.hasNext()) {
			XnatAbstractresource xnatFile = (XnatAbstractresource) resourceIter
					.next();
			ArrayList fileGrouping = new ArrayList();
			ArrayList jFiles = xnatFile.getCorrespondingFiles(rootPath);
			...
			if (fileGrouping.size() > 0) {
				String parsedScanID = xnatFile.getXnatAbstractresourceId()
						.toString();
	...
	public void loadDirectoryFiles(File dir, String parents, boolean raw,
			ArrayList miscRaw) {
		if (dir.exists()) {
			parents += dir.getName() + "/";

			if (!raw) {
				if (dir.getName().equalsIgnoreCase("RAW")
						|| dir.getName().equalsIgnoreCase("SCANS")) {
					raw = true;
				}
			}
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (child.isDirectory()) {
					...
					if (_files.indexOf(child) == -1) {
						if (!child.getName().equalsIgnoreCase("dcmtoxnat.log")
								&& !child.getName().equalsIgnoreCase(
										"dcmtoxnat.log.gz")) {
							String fileID = this._files.addFile(
									parents + child.getName(), child,
									FileTracker.MISC);
							if (raw) {
								miscRaw.add(fileID);
							}
	...
	public String listArchiveToHTML(String server) {
			...
			if (scanFiles.size() > 0) {
				...
				while (files.hasNext()) {
					...
					if (miscDir == null) {
						...
						int mrIndex = miscDir.toLowerCase().indexOf(
								"/" + getArchiveDirectoryName().toLowerCase()
										+ "/");
						if (mrIndex == -1) {
							String sPath = rootPath.replace('\\', '/');
							sPath = sPath.replace("//", "/");
							if (miscDir.startsWith(sPath)) {
								int index = miscDir.indexOf(File.separator,
										sPath.length() + 1);
								if (index == -1) {
									index = miscDir.indexOf("/",
											sPath.length() + 1);
									if (index == -1) {
										index = miscDir.indexOf("\\",
												sPath.length() + 1);
					...
					while (iter.hasNext()) {
						...
						if (f.exists()) {
							String fileID = this._files.addFile(
									f.getAbsolutePath(), f, FileTracker.KNOWN);
							scanLinkBuffer.append("<b>").append(f.getName())
									.append("</b><BR>");
							fileGrouping.add(fileID);
							hasContent = true;
				...
				if (hasContent) {
					sb.append("\n");
					if (hasFunctionText) {
						sb.append("<INPUT type=\"checkbox\" id=\"scan")
								.append(parsedScanID).append("\" name=\"scan")
								.append(parsedScanID)
								.append("\" VALUE=\"CHECKED\" CHECKED/>&nbsp;");
						this.fileGroups
								.put("scan" + parsedScanID, fileGrouping);
					}
					sb.append(
							"<span class=\"trigger\" onClick=\"blocking('scan")
							.append(parsedScanID).append("');\">");
					sb.append("<img ID=\"IMGscan").append(parsedScanID)
							.append("\" src=\"").append(server)
							.append("/images/plus.jpg\" border=0/>&nbsp;");
					sb.append("<b>").append(scan.getId()).append("</b>&nbsp;");
					if (scan.getType() != null) {
						sb.append("(" + scan.getType() + ")");
					}
					sb.append("</span>");
					sb.append("<BR><span class=\"branch\" ID=\"spanscan")
							.append(parsedScanID).append("\">");
					...
					if (hasFunctionText) {
						allFiles.append("       \n");
						allFiles.append("       change = \"scan")
								.append(parsedScanID).append("\";\n");
						allFiles.append("       node = document.getElementById(change);\n");
						allFiles.append("       node.checked=checkAll.checked;\n");
					}
				} else {
					sb.append("<span class=\"trigger\"><b>"
							+ scan.getId()
							+ "</b>&nbsp;("
							+ scan.getType()
							+ ")</span><br><span class=\"branch\" style=\"display: block;\">No files found for this scan.</span><BR>");
				}
			} else {
				sb.append("<span class=\"trigger\"><b>"
						+ scan.getId()
						+ "</b>&nbsp;("
						+ scan.getType()
						+ ")</span><br><span class=\"branch\" style=\"display: block;\">No files defined for this scan.</span><br>");
			}
		}
		sb.append("</TD></TR>");
		...
		while (reconIter.hasNext()) {
			...
			if (outFiles.size() > 0) {
				...
				if (hasContent) {
					...
					sb.append("\n");
					if (hasFunctionText) {
						sb.append("<INPUT type=\"checkbox\" id=\"recon")
								.append(parsedScanID).append("\" name=\"recon")
								.append(parsedScanID)
								.append("\" CHECKED/>&nbsp;");
						this.fileGroups.put("recon" + parsedScanID,
								fileGrouping);
					}
					sb.append(
							"<span class=\"trigger\" onClick=\"blocking('recon")
							.append(parsedScanID).append("');\">");
					sb.append("<img ID=\"IMGrecon").append(parsedScanID)
							.append("\" src=\"").append(server)
							.append("/images/plus.jpg\" border=0/>&nbsp;<b>")
							.append(recon.getId()).append("</b>&nbsp;");
					if (recon.getType() != null)
						sb.append("(" + recon.getType() + ")");
					sb.append("</span>");
					sb.append("<BR><span class=\"branch\" ID=\"spanrecon")
							.append(parsedScanID).append("\">");
					sb.append(scanLinkBuffer);
					...
					sb.append("<span class=\"trigger\"><b>"
							...
							+ "</b>&nbsp;("
							+ recon.getType()
							+ ")</span><br><span class=\"branch\" style=\"display: block;\">No files found for this reconstruction.</span><BR>");
				}
			} else {
				if (c++ == 0)
					sb.append("<b>RECONSTRUCTIONS</b></TD><TD>");
				sb.append("<span class=\"trigger\"><b>"
						...
						+ "</b>&nbsp;("
						+ recon.getType()
						+ ")</span><br><span class=\"branch\" style=\"display: block;\">No files defined for this reconstruction.</span><BR>");
			}
		}
		sb.append("</TD></TR>");
			...
			for (XnatImageassessordataI assess : this.getAssessors_assessor()) {
				...
				if (outFiles.size() > 0) {
						...
						while (iter.hasNext()) {
							...
							if (f.exists()) {
								String fileID = this._files.addFile(
										f.getAbsolutePath(), f,
										FileTracker.KNOWN);
								scanLinkBuffer.append("<b>")
										.append(f.getName()).append("</b><BR>");
								fileGrouping.add(fileID);
								hasContent = true;
								hasFunctionText = true;
								fileCount++;
					...
					if (hasContent) {
						...
						sb.append("\n");
						if (hasFunctionText) {
							sb.append("<INPUT type=\"checkbox\" id=\"assess")
									.append(parsedScanID)
									.append("\" name=\"assess")
									.append(parsedScanID)
									.append("\" VALUE=\"CHECKED\" CHECKED/>&nbsp;");
							this.fileGroups.put("assess" + parsedScanID,
									fileGrouping);
						}
						sb.append(
								"<span class=\"trigger\" onClick=\"blocking('assess")
								.append(parsedScanID).append("');\">");
						sb.append("<img ID=\"IMGassess")
								.append(parsedScanID)
								.append("\" src=\"")
								.append(server)
								.append("/images/plus.jpg\" border=0/>&nbsp;<b>")
								.append(assess.getId()).append("</b>&nbsp;");
						sb.append("("
								+ ((XnatImageassessordata) assess).getItem()
										.getProperName() + ")");
						sb.append("</span>");
						sb.append(
								"<BR>\n<span class=\"branch\" ID=\"spanassess")
								.append(parsedScanID).append("\">");
						sb.append(scanLinkBuffer);
						sb.append("</span><BR>");
						...
						sb.append("<span class=\"trigger\"><b>"
								...
								+ ((XnatImageassessordata) assess).getItem()
										.getProperName()
								+ ")</span><br><span class=\"branch\" style=\"display: block;\">No files found for this assessment.</span><BR>");
					}
				} else {
					if (c++ == 0)
						sb.append("<b>ASSESSMENTS</b></TD><TD>");
					sb.append("<span class=\"trigger\"><b>"
							...
							+ ((XnatImageassessordata) assess).getItem()
									.getProperName()
							+ ")</span><br><span class=\"branch\" style=\"display: block;\">No files defined for this assessment.</span><BR>");
				}
			}
			sb.append("</TD></TR>");
		...
		while (uploadsIter.hasNext()) {
			XnatAbstractresource xnatFile = (XnatAbstractresource) uploadsIter
					.next();
			ArrayList fileGrouping = new ArrayList();
			ArrayList jFiles = xnatFile.getCorrespondingFiles(rootPath);
			...
			while (iter.hasNext()) {
				File f = (File) iter.next();
				if (f.exists()) {
					String fileID = this._files.addFile(f.getAbsolutePath(), f,
							FileTracker.KNOWN);
					scanLinkBuffer.append("<b>").append(f.getName())
							.append("</b><BR>");
					fileGrouping.add(fileID);
			...
			if (hasContent) {
				...
				sb.append("\n");
				if (hasFunctionText) {
					sb.append("<INPUT type=\"checkbox\" id=\"uploads")
							.append(parsedScanID).append("\" name=\"uploads")
							.append(parsedScanID)
							.append("\" VALUE=\"CHECKED\" CHECKED/>&nbsp;");
					this.fileGroups.put("uploads" + parsedScanID, fileGrouping);
				}
				sb.append("<span class=\"trigger\" onClick=\"blocking('uploads")
						.append(parsedScanID).append("');\">");
				sb.append("<img ID=\"IMGuploads").append(parsedScanID)
						.append("\" src=\"").append(server)
						.append("/images/plus.jpg\" border=0/>&nbsp;<b>")
						.append(label).append("</b>&nbsp;");
				sb.append("");
				sb.append("</span>");
				sb.append("<BR>\n<span class=\"branch\" ID=\"spanuploads")
						.append(parsedScanID).append("\">");
				sb.append(scanLinkBuffer);
				sb.append("</span><BR>");
				...
				if (hasFunctionText) {
					allFiles.append("       \n");
					allFiles.append("       change = \"uploads")
							.append(parsedScanID).append("\";\n");
					allFiles.append("       node = document.getElementById(change);\n");
					allFiles.append("       node.checked=checkAll.checked;\n");
				}
				...
				sb.append("<span class=\"trigger\"><b>"
						+ label
						+ "</b>&nbsp;</span><br><span class=\"branch\" style=\"display: block;\">No files found for this upload.</span><BR>");
			}
		}
		sb.append("</TD></TR>");
		...
		if (miscDir != null) {
			...
			if (misc.exists()) {
				...
				String dirListing = listDirectoryToHTML(misc, server, "", 0)
						.toString();
				...
				if (dirListing.length() > 0) {
					fileCount++;
					allFiles.append("       \n");
					allFiles.append("       f").append(misc.getName() + "0")
							.append("CheckAll(checkAll);\n");
					allFiles.append("       change = \"dir_")
		...
		if (fileCount > 0) {
			sb.insert(
					0,
					"<INPUT type=\"checkbox\" name=\"all_files\" VALUE=\"CHECKED\" ONCLICK=\"allFilesCheckAll(this)\" CHECKED/>&nbsp;Select All");
		}
		sb.insert(0, "  <TR>");
		sb.insert(0, "<TABLE>");
	...
	public StringBuffer listDirectoryToHTML(File dir, String server,
			String parents, int count) {
		StringBuffer sb = new StringBuffer();
		...
		if (dir.exists()) {
			...
			function.append("   function f")
					.append(dir.getName() + local_count)
					.append("CheckAll(checkAll)\n");
			function.append("   {\n");
			function.append("       var change=null;\n");
			function.append("       var node=null;\n");

			sb.append("<INPUT type=\"checkbox\" id=\"dir_").append(parents)
					.append("\" name=\"dir_").append(parents)
					.append("\" VALUE=\"CHECKED\" ONCLICK=\"f")
					.append(dir.getName())
					.append(local_count + "CheckAll(this)\" CHECKED/>");
			sb.append("&nbsp;<span class=\"trigger\" onClick=\"blocking('")
					.append(dir.getName() + local_count).append("');\">");
			sb.append("<img ID=\"IMG").append(dir.getName() + local_count)
					.append("\" src=\"").append(server)
					.append("/images/plus.jpg\" border=0/>&nbsp;<b>")
					.append(dir.getName()).append("</b></span>");
			sb.append("<span class=\"branch\" ID=\"span")
					.append(dir.getName() + local_count).append("\">");
			File[] children = dir.listFiles();
			for (int i = 0; i < children.length; i++) {
				File child = children[i];
				if (child.isDirectory()) {
					...
					functionTemp.append("       \n");
					functionTemp.append("       f")
							.append(child.getName() + count)
							.append("CheckAll(checkAll);\n");
					functionTemp.append("       change = \"dir_")
							.append(parents + child.getName() + "/")
							.append("\";\n");
					functionTemp
							.append("       node = document.getElementById(change);\n");
					functionTemp
							.append("       node.checked=checkAll.checked;\n");
					StringBuffer temp = listDirectoryToHTML(child, server,
							parents, count);
					if (temp.length() > 0) {
					...
					if (_files.indexOf(child) == -1) {
						if (!child.getName().equalsIgnoreCase("dcmtoxnat.log")
							...
							String fileID = this._files.addFile(
									...
									FileTracker.MISC);
							function.append("       \n");
							function.append("       change = \"dir_file_")
									.append(fileID).append("\";\n");
							function.append("       node = document.getElementById(change);\n");
							function.append("       node.checked=checkAll.checked;\n");
							sb.append("<INPUT type=\"checkbox\" id=\"dir_file_")
									.append(fileID)
									.append("\" name=\"dir_file_")
									.append(fileID)
									.append("\" VALUE=\"CHECKED\" CHECKED/>&nbsp;")
									.append(child.getName()).append("<BR>");
							fileCount++;
						}
					}
				}
					...
					if (xnatFile instanceof org.nrg.xdat.om.XnatResource) {
						...
						if (last_dir == null) {
							...
							if (index != -1) {
								...
								index = last_dir.toUpperCase().indexOf(
										"/SCANS/");
								if (index != -1) {
				...
				while (files.hasNext()) {
					...
					if (xnatFile instanceof org.nrg.xdat.om.XnatResource) {
						...
						if (last_dir == null) {
							...
							int index = UPPER_dir.indexOf("/"
									+ getArchiveDirectoryName().toUpperCase()
									+ "/");
							if (index != -1) {
								session_dir = last_dir.substring(0, index
										+ (2 + getArchiveDirectoryName()
												.length()));

								logger.debug("MATCHED BY getArchiveDirectoryName():"
										+ session_dir);
								return session_dir;
							} else {
								index = UPPER_dir
										.indexOf(getArchiveDirectoryName()
												.toUpperCase());
								if (index != -1) {
									int unixSepIndex = last_dir.indexOf("/",
											index);
									int winSepIndex = last_dir.indexOf("\\",
											index);
									if (unixSepIndex == -1 && winSepIndex == -1) {
										session_dir = last_dir + File.separator;
									} else if (unixSepIndex == -1) {
										session_dir = last_dir.substring(0,
												winSepIndex + 1);
	...
	public List<XnatAbstractresourceI> getAllResources() {
		...
		while (recons.hasNext()) {
			XnatReconstructedimagedata scan = (XnatReconstructedimagedata) recons
					.next();
			Iterator outfiles = scan.getOut_file().iterator();
			while (outfiles.hasNext()) {
				XnatAbstractresource file = (XnatAbstractresource) outfiles
						.next();
				resources.add(file);
			}
			...
			while (infiles.hasNext()) {
				XnatAbstractresource file = (XnatAbstractresource) infiles
						.next();
				resources.add(file);
			}
		}
				...
				for (Iterator iter = groupFiles.iterator(); iter.hasNext();) {
					...
					if (o instanceof String) {
						...
						if (path.indexOf(File.separator + project.getId()) != -1) {
							...
							if (path.indexOf(File.separator
									+ getArchiveDirectoryName()) != -1) {
								path = path
										.substring(path.indexOf(File.separator
												+ getArchiveDirectoryName()) + 1);
							}
						}
						...
						for (Map.Entry<String, GeneralFile> entryF : dir
								.getRelativeFiles().entrySet()) {
							...
							meta = new CatEntryMetafieldBean();
							meta.setMetafield(new Long(entryF.getValue()
									.length()).toString());
							meta.setName("SIZE");
							entry.addMetafields_metafield(meta);
		...
		if (al != null) {
			for (int i = 0; i < al.size(); i++) {
				...
				if (!mappers.containsKey(scan.getXSIType())) {
					mappers.put(scan.getXSIType(),
							scan.getScanTypeMapping(project, this.getDBName()));
				}
				...
				if (scan.getFile().size() > 0) {
					...
					if (abstRes instanceof XnatResource) {
						if (((XnatResource) abstRes).getContent() == null
								|| ((XnatResource) abstRes).getContent()
										.equals("")) {
							((XnatResource) abstRes).setContent("RAW");
						}
						if (abstRes.getLabel() != null
								&& ((XnatResource) abstRes).getFormat() == null) {
							((XnatResource) abstRes).setFormat(abstRes
									.getLabel());
						}
					}
				}
	 ...
	 * 
	 * @param other
	 *            object from which assigned values will be copied
	 * @throws Exception
	 *             from XnatExperimentdata.setProjects_project()
	 */
	public void copyValuesFrom(final XnatImagesessiondata other)
			throws Exception {
		...
		for (final XnatExperimentdataFieldI otherField : other
				.getFields_field()) {
		...
		for (final XnatImagescandataI scan : this.getScans_scan()) {
			...
			if (null != otherScan) {
				...
				if (null != otherScan.getQuality())
					((XnatImagescandata) scan).setQuality(otherScan
							.getQuality());
	...
	// try {
	// Long idCOUNT= (Long)PoolDBUtils.ReturnStatisticQuery(query + newID +
	// "';", "id_count", this.getDBName(), this.getUser().getUsername());
	// while (idCOUNT > 0){
	// i++;
	// newID=this.getId() + "_"+ visitNum + "_"+ i;
	// idCOUNT= (Long)PoolDBUtils.ReturnStatisticQuery(query + newID + "';",
	...
	public boolean validateSubjectId() {
		...
		if (subjectid != null) {
			...
			try {
				final String idCOUNT = (String) PoolDBUtils
						.ReturnStatisticQuery(query + subjectid + "';", "id",
								this.getDBName(), login);
				if (idCOUNT != null) {
					return true;
				}
				...
				if (project != null) {
					...
					String new_subjectid = (String) PoolDBUtils
							.ReturnStatisticQuery(query, "id",
									this.getDBName(), login);
					...
					query = "SELECT subject_id FROM xnat_projectParticipant WHERE label='"
							+ subjectid + "' AND project='" + project + "';";
					new_subjectid = (String) PoolDBUtils.ReturnStatisticQuery(
							query, "subject_id", this.getDBName(), login);
					if (new_subjectid != null) {
						this.setSubjectId(new_subjectid);
						return true;
					}
	...
	public static List<String> getPrearchiveSessionReadableScanStats(
			Collection<XnatImagescandataI> scans) {
		List<String> stats = new ArrayList<String>();
		int count = 0;
		long size = 0;
		for (XnatImagescandataI scan : scans) {
			if (scan instanceof BaseXnatImagescandata) {
				Map<String, Number> prearcStats = ((BaseXnatImagescandata) scan)
						.getPrearchiveFileStats();
				count += prearcStats.get("count").intValue();
				size += prearcStats.get("size").longValue();
			} else {
				logger.error(
						"Unable to retrieve file counts.",
						new Exception(
								"Prearchive scan is not an instance of BaseXnatImagescandata. Skipping..."));
			}
		}
	...
	public List<String> getSessionReadableScanStats(boolean useLabel) {
		...
		if (this.getId() == null) {
			...
			stats = this.getPrearchiveSessionReadableScanStats(this
					.getSortedScans());
		} else {
			...
			stats.add(CatalogUtils.formatFileStats(useLabel ? getLabel()
					: "TOTAL", count, size));
		}
		if (accumulator.containsKey("DICOM")) {
			List<Long> values = accumulator.get("DICOM");
			stats.add(CatalogUtils.formatFileStats("DICOM", values.get(0),
					values.get(1)));
			accumulator.remove("DICOM");
		}
		for (String modality : accumulator.keySet()) {
			List<Long> values = accumulator.get(modality);
			stats.add(CatalogUtils.formatFileStats(modality, values.get(0),
					values.get(1)));
		}
		return stats;
	}
	...
	public void moveToProject(final XnatProjectdata newProject,
			final String label, final XDATUser user, final EventMetaI c,
			final List<String> assessorsToMove) throws Exception {
		if (!this.getProject().equals(newProject.getId())) {
			...
			final File rootBackup = MoverMaker.createPrimaryBackupDirectory(
					"move", this.getProject(), getId());
			final String existingRootPath = this.getProjectData()
					.getRootArchivePath();
			// FIXME: Is this correct?
			final String newLabel = label == null ? (this.getLabel() == null ? this
					.getId() : this.getLabel())
					: label;
			final File newSessionDir = new File(
					new File(newProject.getRootArchivePath(),
							newProject.getCurrentArc()), newLabel);
			final String current_label = this.getLabel() == null ? this.getId()
					: this.getLabel();
			...
			CopyOp scanOp = new CopyOp(new OperationI<Map<String, File>>() {
				public void run(Map<String, File> fs) throws Exception {
					new ProjectAnonymizer(base, newProject.getId(),
							base.getArchivePath(existingRootPath)).call();
					for (XnatImagescandataI scan : getScans_scan()) {
						for (XnatAbstractresourceI abstRes : scan.getFile()) {
							MoverMaker.Mover m = MoverMaker.moveResource(
									abstRes, current_label, base,
									newSessionDir, existingRootPath, user, c);
					...
					for (XnatReconstructedimagedataI recon : base.getReconstructions_reconstructedimage()) {
						for (XnatAbstractresourceI abstRes : recon
								.getOut_file()) {
							MoverMaker.Mover m = MoverMaker.moveResource(
									abstRes, current_label, base,
									newSessionDir, existingRootPath, user, c);
					...
					for (XnatImageassessordataI assessor : base
							.getAssessors_assessor()) {
						// assessors need to be updated
						if (assessorsToMove != null
								&& assessorsToMove.contains(assessor.getId())) {
							EventDetails event = EventUtils.newEventInstance(
									EventUtils.CATEGORY.DATA,
									EventUtils.TYPE.WEB_SERVICE,
									"Moved assessor to new project.");
							XnatImageassessordata.ChangePrimaryProject(user,
									(XnatImageassessordata) assessor,
									newProject, null, event, null);
						}

						for (XnatAbstractresourceI abstRes : assessor
								.getOut_file()) {
							MoverMaker.Mover m = MoverMaker.moveResource(
									abstRes, current_label, base,
									newSessionDir, existingRootPath, user, c);
					...
					BaseXnatImagesessiondata.super.moveToProject(newProject,
							newLabel, user, c, assessorsToMove);
				}
	...
	public ArrayList getCatalogSummary() throws Exception {
		String query = "SELECT * FROM (SELECT xnat_abstractresource_id,label,element_name, 'resources'::TEXT AS category, NULL::TEXT AS cat_id"
				...
				+ " WHERE res_map.xnat_experimentdata_id='"
				+ this.getId()
				+ "'"
				+ "  UNION"
	...
	public String canDelete(XnatProjectdata proj, XDATUser user) {
		...
		if (!expt.hasProject(proj.getId())) {
			...
			for (XnatImageassessordataI sad : expt.getAssessors_assessor()) {
				String msg = ((XnatImageassessordata) sad)
						.canDelete(proj, user);
				if (msg != null) {
					return msg;
				}
	...
	public String delete(XnatProjectdata proj, XDATUser user,
		...
		if (!expt.getProject().equals(proj.getId())) {
			try {
				...
				final List<XnatImageassessordata> expts = (expt)
						.getAssessors_assessor();
	...
	public void deleteFiles(UserI user, EventMetaI ci) throws Exception {
		...
		String rootPath = ArcSpecManager.GetInstance()
				.getArchivePathForProject(this.getProject());

		for (final XnatImagescandataI scan : this.getScans_scan()) {
			for (XnatAbstractresourceI abstRes : scan.getFile()) {
				((XnatAbstractresource) abstRes).deleteWithBackup(rootPath,
						user, ci);
			}
		}

		for (XnatReconstructedimagedataI scan : this
				.getReconstructions_reconstructedimage()) {
	...
	public XnatQcmanualassessordataI getManualQC() {
		...
		if (assessors != null && assessors.size() > 0) {
			return (XnatQcmanualassessordata) assessors
					.get(assessors.size() - 1);
		}
		return null;
	}
	...
	public XnatQcassessmentdataI getQCByType(String type) {
		...
		for (int i = 0; i < assessors.size(); i++) {
			if (((AutoXnatQcassessmentdata) assessors.get(i)).getType().equals(
					type)) {
				qcassessorOfType.add(assessors.get(i));
			}
		}
		if (qcassessorOfType != null && qcassessorOfType.size() > 0) {
			return (XnatQcassessmentdata) qcassessorOfType.get(qcassessorOfType
					.size() - 1);
		}
		return null;
	}
	...
	public ScrScreeningassessmentI getScreeningAssessment() {
		...
		if (screeningAssessment != null && screeningAssessment.size() > 0) {
			return (ScrScreeningassessmentI) screeningAssessment
					.get(screeningAssessment.size() - 1);
		}
		return null;
	}
