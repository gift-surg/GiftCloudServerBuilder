...
public class BaseXnatMrscandata extends AutoXnatMrscandata {
	...
	public ScanTypeMappingI getScanTypeMapping(final String project,
			final String dbName) {
			...
			if (imgtype == null) {
				...
				for (ScanType scanType : types) {
					if (scanType.getImgtype() == null) {
						if (candidate == null
								|| (candidate.getType() == null && scanType
										.getType() != null)) {
							candidate = scanType;
						}
					}
	...
	/**
	 * For MR, we also use the DICOM image type parameters for matching scan
	 * types.
	 *
	 */
	public static class MRScanTypeMapping extends
			AbstractScanTypeMapping<ScanTypeHistory> implements
			ScanTypeMappingI {
		 ...
		 * 
		 * @see
		 * org.nrg.xnat.helpers.scanType.AbstractScanTypeMapping#getMappedType
		 * (org.nrg.xdat.model.XnatImagescandataI, java.util.Map)
		 */
		protected String getMappedType(XnatImagescandataI scan,
			...
			final String formatted = AbstractScanTypeMapping
					.standardizeFormat(seriesDescription);
			final String imgType = ((XnatMrscandataI) scan)
					.getParameters_imagetype();

			final ScanTypeHistory history = histories.get(formatted);
			return null == history ? null : history.match(seriesDescription,
					imgType, scan.getFrames());
		}
		 ...
		 * 
		 * @see
		 * org.nrg.xnat.helpers.scanType.AbstractScanTypeMapping#newScanHistory
		 * ()
		 */
		protected ScanTypeHistory newScanHistory() {
			return new ScanTypeHistory();
		}
		 ...
		 * 
		 * @see
		 * org.nrg.xnat.helpers.scanType.AbstractScanTypeMapping#update(java
		 * .lang.Object, java.util.Hashtable)
		 */
		protected void update(final ScanTypeHistory h, final Hashtable<?, ?> row) {
			h.add((String) row.get("type"),
					(String) row.get("parameters_imagetype"),
					(Integer) row.get("frames"));
		}
	}
}
