...
public class BaseXnatMrsessiondata extends AutoXnatMrsessiondata {
	...
	public String getWorkflowStatus() {
		...
		ArrayList al = WrkWorkflowdata.getWrkWorkflowdatasByField(cc,
				getUser(), false);
		if (al.size() > 0) {
			...
			if (wkdata.getPipelineName().toLowerCase().indexOf("transfer") != -1) {
				...
				if (wkdata.getStatus().equalsIgnoreCase("Running")) {
					...
					return "processed";
				} else if (wkdata.getStatus().equalsIgnoreCase(
						"Awaiting Action")) {
					return "waiting";
				} else if (wkdata.getStatus().equalsIgnoreCase("Queued")) {
					return "queued";
	 ...
	 * @throws Exception
	 */
	public void copyValuesFrom(final XnatImagesessiondata otherImageSession)
			throws Exception {
		...
		if (chronological) {
			...
			for (XnatImagescandataI scan : this.getScans_scan()) {
				if (scan.getType() != null
						&& scanTypes.containsKey(scan.getType())) {
					_return.add(scan);
				}
			}
			_return.trimToSize();
	...
	public ArrayList getUnionOfScansByType(String csvType, String chronological) {
		return getUnionOfScansByType(csvType,
				new Boolean(chronological).booleanValue());
	}
