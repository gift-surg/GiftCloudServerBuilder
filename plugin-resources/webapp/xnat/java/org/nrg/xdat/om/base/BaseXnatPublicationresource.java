...
public class BaseXnatPublicationresource extends AutoXnatPublicationresource {
	...
	public ArrayList<File> getCorrespondingFiles(String rootPath) {
		if (files == null) {
			...
			/*
			 * files = new ArrayList(); File f = new
			 * File(org.nrg.xft.utils.FileUtils
			 * .AppendRootPath(rootPath,this.getUri())); if
			 * (!f.getPath().startsWith("srb:") &&
			 * !f.getPath().startsWith("http:")){ if (!f.exists() &&
			 * !getUri().endsWith(".gz")) { f = new
			 * java.io.File(org.nrg.xft.utils
			 * .FileUtils.AppendRootPath(rootPath,this.getUri()) + ".gz"); } }
			 * files.add(f);
			 */
	...
	public ArrayList getCorrespondingFileNames(String rootPath) {
		if (fileNames == null) {
			...
			File f = new File(org.nrg.xft.utils.FileUtils.AppendRootPath(
					rootPath, this.getUri()));
			if (!f.exists() && !getUri().endsWith(".gz")) {
				f = new java.io.File(
						org.nrg.xft.utils.FileUtils.AppendRootPath(rootPath,
								this.getUri()) + ".gz");
			}
	...
	public String getFullPath(String rootPath) {
		...
		String fullPath = StringUtils.ReplaceStr(
				FileUtils.AppendRootPath(rootPath, this.getUri()), "\\", "/");
	...
	/**
	 * Gets the files associated with an image. For an uri which is say like:
	 * /data/a.img This method will return all files which match the pattern
	 * /data/a.*
	 *
	 * @return
	 */
	...
	protected ArrayList<File> getAssociatedFilesFromRemoteFileSystem(
			String fullPath, File tempDir) {
		...
		if (lastIndexOfSlash != -1) {
			...
			try {
				...
				XNATDirectory srbDir = XNATSrbSearch
						.getFilesAssociatedWith(uri.getPath(),
								fullPath.substring(lastIndexOfSlash + 1));
				srbDir.importFiles(tempDir);
				ArrayList srbAssociatedFiles = srbDir.getFiles();
				for (int i = 0; i < srbAssociatedFiles.size(); i++) {
					associatedFiles.add(new File(tempDir.getAbsolutePath()
							+ File.separator
							+ ((GeneralFile) srbAssociatedFiles.get(i))
									.getName()));
				}
			} catch (Exception e) {
	...
	protected ArrayList<File> getAssociatedFilesOnLocalFileSystem(
			String fullPath) {
		ArrayList<File> associatedFiles = new ArrayList();
		...
		if (dir.exists() && dir.isDirectory()) {
			...
			for (int i = 0; i < associatedFileNames.length; i++) {
				associatedFiles.add(new File(dir.getAbsolutePath()
						+ File.separator + associatedFileNames[i]));
			}
		}
		return associatedFiles;
	}
	...
	public void moveTo(File newSessionDir, String existingSessionDir,
			String rootPath, XDATUser user, EventMetaI c) throws IOException,
			Exception {
		String uri = this.getUri();
