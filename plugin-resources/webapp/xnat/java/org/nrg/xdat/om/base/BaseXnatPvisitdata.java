...
public class BaseXnatPvisitdata extends AutoXnatPvisitdata implements
		Comparable {
	...
	public static XnatPvisitdata GetVisitByProjectIdentifier(String project,
		...
		ArrayList al = XnatPvisitdata.getXnatPvisitdatasByField(cc, user,
				preLoad);
	...
	public XnatSubjectdata getSubjectData() {
		...
		if (getSubjectId() != null) {
			ArrayList al = XnatSubjectdata.getXnatSubjectdatasByField(
					"xnat:subjectData/ID", this.getSubjectId(), this.getUser(),
					false);
			if (al.size() > 0) {
	...
	public String delete(BaseXnatProjectdata proj, XDATUser user,
		...
		if (visit.getProject() != null
			...
			try {
				...
				for (XnatExperimentdataShareI pp : visit.getSharing_share()) {
					if (pp.getProject().equals(proj.getId())) {
						SaveItemHelper.authorizedRemoveChild(visit.getItem(),
				...
				// TODO:at this point, retrieve all experiments associated with
				// this visit and disassociate them.
			...
			try {
				...
				// find any experiments that reference this visit and delete the
				// reference.
				try {

					final QueryOrganizer qo = new QueryOrganizer(
							"xnat:experimentdata", user, ViewManager.ALL);
					...
					cc2.addClause("xnat:experimentdata/sharing/share/project",
							proj.getId());
					cc2.addClause("xnat:experimentdata/sharing/share/visit",
							visit.getId());
					...
					String exptIdColumn = qo
							.getFieldAlias("xnat:experimentdata/ID");

					XFTTable table = XFTTable.Execute(query, user.getDBName(),
							user.getUsername());
					// TODO: these are the visits we're going to have to
					// disassociate.
					...
					while (table.hasMoreRows()) {
						...
						try {
							final Object exptID = row.get(exptIdColumn
									.toLowerCase()); // fun, we have to do a
														// toLowerCase because
														// getFieldAlias returns
							...
							XnatExperimentdata frank = XnatExperimentdata
									.getXnatExperimentdatasById(
											exptID.toString(), user, true);
							if (org.apache.commons.lang.StringUtils
									.equalsIgnoreCase(frank.getVisit(), visitId)) {
								XnatExperimentdata new_expt = frank
										.getLightCopy();
								new_expt.setProperty(
										"xnat:experimentdata/visit", "NULL");
								SaveItemHelper.authorizedSave(new_expt, user,
										false, false, c);
							} else {
								// the visit must be in the shared project,
								// then. So, we need to clear that out...
								// TODO: implement this.
								throw new Exception(
										"Unable to delete a shared project's visit from an experiment because the feature is not implemented.");
							}
				...
				SaveItemHelper.authorizedDelete(visit.getItem()
	...
	public int hashCode() {
		return new HashCodeBuilder(11, 37)
				...
				append(this.getVisitName()).append(this.getSubjectId())
