...
public class BaseXnatResource extends AutoXnatResource {
		...
		if (files == null || files.size() == 0) {
			if (getUri().startsWith("file:")) {
				try {
					...
					files = getAssociatedFilesOnLocalFileSystem(file
							.getAbsolutePath());
				} catch (Exception e) {
	...
	protected ArrayList<File> getAssociatedFilesOnLocalFileSystem(
		...
		int indexOfDot = (fileroot.toLowerCase().lastIndexOf(".ima") != -1 ? fileroot
				.toLowerCase().lastIndexOf(".ima") : fileroot.toLowerCase()
				.lastIndexOf(".img"));
		if (indexOfDot != -1) {
