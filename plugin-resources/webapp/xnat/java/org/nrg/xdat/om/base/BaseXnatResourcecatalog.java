...
public abstract class BaseXnatResourcecatalog extends AutoXnatResourcecatalog {
		...
		if (files == null) {
			...
			if (cat != null) {
				for (CatEntryI entry : cat.getEntries_entry()) {
					String entryPath = StringUtils.ReplaceStr(FileUtils
							.AppendRootPath(parentPath, entry.getUri()), "\\",
							"/");
	...
	public static void backupEntry(String parentPath, CatCatalogBean cat,
			UserI user, EventMetaI c, String timestamp)
		...
		if (cat != null) {
			for (CatEntryI entry : cat.getEntries_entry()) {
				...
				((CatEntryBean) entry).setModifiedtime(EventUtils.getEventDate(
						c, false));
				if (user != null) {
				...
				if (c != null && c.getEventId() != null) {
					entry.setModifiedeventid(Integer.valueOf(c.getEventId()
							.intValue()));
				}
			}
		}
	...
	public void backupToHistory(String rootPath, UserI user, EventMetaI c)
			throws Exception {
		...
		if (cat != null) {
			...
			CatalogUtils.writeCatalogToFile(cat,
					FileUtils.BuildHistoryFile(f, timestamp));
		}
	}
	...
	public CatCatalogBean getCleanCatalog(String rootPath,
	...
	public Integer getCount(String rootPath) {
		if (count == null) {
			...
			while (files.hasNext()) {
				...
				if (f != null && f.exists()
						&& !f.getName().endsWith("catalog.xml")) {
					countI++;
					sizeI += f.length();
				}
			}
	...
	public void moveTo(File newSessionDir, String existingSessionDir,
		...
		if (base instanceof CatCatalogBean) {
			moveCatalogEntries((CatCatalogBean) base, catalog.getParent(),
					newFile.getParent());
		}
	...
	public void moveCatalogEntries(CatCatalogI cat, String existingRootPath,
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
				...
				if (entry.getUri().startsWith(existingRootPath)) {
					...
					if (FileUtils.IsAbsolutePath(entry.getUri())) {
						if (entry.getUri().indexOf("/") > 0) {
							relativePath = uri.substring(entry.getUri()
									.indexOf("/") + 1);
						} else if (uri.indexOf("\\") > 0) {
							relativePath = uri.substring(entry.getUri()
									.indexOf("\\") + 1);
			...
			if (!existingLocation.exists())
				existingLocation = getFileOnLocalFileSystem(existingLocation
						.getAbsolutePath());
