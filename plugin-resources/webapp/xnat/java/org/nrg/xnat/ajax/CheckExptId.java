...
public class CheckExptId {
	...
	public void execute(HttpServletRequest req, HttpServletResponse response)
		...
		if (user != null) {
			...
			if (expt_id != null && !expt_id.trim().equals("")
					&& !(expt_id.contains("\\") || expt_id.contains("'"))) {
				try {
					GenericWrapperElement e = GenericWrapperElement
							.GetElement("xnat:experimentData");

					XFTTable table = XFTTable
							.Execute(
									"SELECT id, insert_date, login, me.element_name FROM xnat_experimentData ed "
