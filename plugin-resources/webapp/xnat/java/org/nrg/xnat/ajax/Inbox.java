...
public final class Inbox {
	...
	private static final Map<String, StatusQueue> active = Collections
			.synchronizedMap(new HashMap<String, StatusQueue>());
	private static final Map<String, StatusQueue> complete = Collections
			.synchronizedMap(new HashMap<String, StatusQueue>());

	// This is configurable from inside the webapp, so it might change from one
	// instantiation to the next.
	private final File inboxRoot = new File(ArcSpecManager.GetInstance()
			.getGlobalpaths().getFtppath());

	private final org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory
			.getLog(Inbox.class);

	public void startImport(final HttpServletRequest req,
		...
		final File prearc = new File(ArcSpecManager.GetInstance()
		...
		try {
			...
			try {
				response.sendError(
						HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
						"unable to create lock file " + lockFile);
			} catch (IOException ignore) {
			}
			return;
		}
		...
		try {
			...
			try {
				response.sendError(
						HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
						"unable to acquire inbox lock: " + e.getMessage());
				lockch.close();
				fos.close();
			} catch (IOException ignore) {
			}
		...
		final File tsdir = new File(prearc, formatter.format(Calendar
				.getInstance().getTime()));
		...
		final PrearcImporter importer = PrearcImporterFactory.getFactory()
				.getPrearcImporter(project, tsdir, projectInbox,
						getPaths(req, projectInbox));
		...
		if (null != prevListener) {
			...
			try {
				response.sendError(HttpServletResponse.SC_CONFLICT,
						"importer already active for " + projectInbox);
				lock.release();
				fos.close();
			} catch (IOException ignore) {
			}
	...
	public void monitorImport(final HttpServletRequest req,
		...
		if (null == listener) {
			...
			try {
				if (null == finished) { // no matching operation found, status
										// unknown
	...
	private void sendEmptyListing(final HttpServletResponse response,
			final String object) {
		response.setContentType("text/xml");
		response.setHeader("Cache-Control", "no-cache");
		try {
	 ...
	 * 
	 * @param f
	 *            Root of the file tree to be deleted.
	...
	public void list(final HttpServletRequest req,
		...
		log.debug("received FTP inbox list request for user " + login
				+ " project " + project);
		...
		final File folder = (path != null && path.length() > 0) ? new File(
				projectInbox, path) : projectInbox;
		...
		if (null != path) { // verify that nothing funny happened resolving the
							// path
			final int lastf = path.lastIndexOf('/');
			final int lastb = path.lastIndexOf('\\');
			final int lastsep = lastf > lastb ? lastf : lastb;
			if (lastsep > -1
					&& !folder.getName().equals(path.substring(lastsep)))
				log.warn("requested inbox path " + path
						+ " does not match retrieved " + folder);
			root.addAttribute("name", path);
		} else {
			root.addAttribute("name", "");
		}

		if (folder.exists()) { // nonexistent project folder is fine -- just act
								// like it's empty
		...
		if (log.isDebugEnabled()) {
			log.debug("Directory message validation "
					+ (validate("Directory", document) ? "successful"
							: "failed"));
		}
	...
	private void writeStatus(final Writer writer, final StatusQueue queue)
			throws IOException {
		writer.write("<status>");
		synchronized (queue) {
	...
	private boolean validate(final String schemaName, final Document document) {
		...
		final File schemaFile = new File(context.getRealPath("schemas/ws/"
				+ schemaName + XSD_SUFFIX));
