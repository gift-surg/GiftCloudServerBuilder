...
public final class Prearchive {
	...
	private static final FileFilter isXMLFileFilter = new FileFilter() {
		public boolean accept(final File f) {
			return f.isFile() && f.getName().matches(".*\\.[xX][mM][lL]\\z");
		}
	};
	...
	private final SimpleDateFormat XS_DATETIME_FORMAT = new SimpleDateFormat(
	...
	private boolean canCreate(final XDATUser user, final String project,
			final String type) {
		try {
		...
		final Map<String, Boolean> projectTypes = createAllowedTypes
				.get(project);
		if (!projectTypes.containsKey(type)) {
			try {
				...
				log.error("Failed to verify security permissions for "
						+ project + "-" + type, e);
				projectTypes.put(type, Boolean.FALSE);
			}
		}
	...
	/**
	 * Handles a "prearchives" request by returning a list of prearchives, with
	 * session counts.
	 */
	@SuppressWarnings("unchecked")
	public void prearchives(final HttpServletRequest req,
			final HttpServletResponse response, final ServletConfig config) {
		...
		if (null == login) {
			...
			try {
				response.sendError(HttpServletResponse.SC_CONFLICT,
						"no user in session: who are you?");
		...
		final List<List> projects = user.getQueryResults("xnat:projectData/ID",
				"xnat:projectData");
		for (final List<String> row : projects) {
			...
			if (prearcs.containsKey(id))
				continue;
			try {
				if (user.canAction("xnat:mrSessionData/project", id,
						SecurityManager.CREATE)) {
					prearcs.put(id, new File(ArcSpecManager.GetInstance()
							.getPrearchivePathForProject(id)));
				}
			} catch (Exception e) {
		...
		try {
			if (user.checkRole(ROLE_SITE_ADMIN)) {
				final String commonPath = ArcSpecManager.GetInstance()
		...
		for (final Map.Entry<String, File> e : prearcs.entrySet()) {
			...
			dir.mkdirs();
			if (!dir.isDirectory()) {
				log.error("prearchive " + prearc + " path " + dir
						+ " is not a directory");
				try {
					response.sendError(
							HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
							prearc + " prearchive is improperly configured");
				} catch (IOException ignore) {
				}
				return;
			}
	...
	/**
	 * Checks for obvious problems with a session XML: existence, lock,
	 * permissions.
	 * 
	 * @param sessionDir
	 *            Directory holding the session
	 * @return
	 */
	...
	/**
	 * Exception-free method for getting a canonical file representation. If
	 * f.getCanonicalFile() fails, returns f.getAbsoluteFile() instead.
	 * 
	 * @param f
	 *            file for which we want canonical representation
	...
	private static final File getCanonical(final File f) {
		try {
			...
			Logger.getLogger(Prearchive.class).warn(
					"unable to get canonical path for " + f, e);
	...
	/**
	 * Handles a "sessions" request by returning a list of sessions in the
	 * specified prearchive.
	 */
	public void sessions(final HttpServletRequest req,
			final HttpServletResponse response, final ServletConfig config) {
		final HttpSession session = req.getSession();
		final XDATUser user = XDAT.getUserDetails();
		final String login = user.getLogin();
		...
		log.debug("received session list request for user " + login
				+ " prearchive " + name);

		if (!canCreate(user, name, null)) {
			log.info("Unauthorized user " + login
					+ " tried to access prearchive " + name);
			try {
		...
		if (!prearc.canRead()) {
			...
			try {
				response.sendError(HttpServletResponse.SC_FORBIDDEN,
						"prearchive " + name + " cannot be read");
			} catch (IOException ignore) {
			}
			return;
		}
		...
		// Extract the sessions. This code depends on the following prearchive
		// structure:
		// prearc/
		// +--- TimeStamp1/
		// | +--- SessionDir1/
		...
		// +--- TimeStamp2/
		// ...
		// The only exception is an explicit special case: this prearchive
		// directory may contain
		// other prearchive directories, which we ignore here. This special case
		// should only arise
		...
		for (final Object apo : arcspec.getProjects_project()) {
			...
			projectPrearcs.add(getCanonical(arcspec
					.getPrearchivePathForProject(apname)));
		}
		...
		for (final File tsdir : prearc.listFiles(isDirectoryFilter)) {
			...
			for (final File sessdir : tsdir.listFiles(isDirectoryFilter)) {
				...
				if (null != status) {
					...
					if (null == item) {
						log.error("Unable to load image session data for "
								+ sessdir);
						imgSession = null;
						status = PrearcStatus.ERROR;
						...
						if (!canCreate(user, name, type)) {
							// Oops. User doesn't have access to this session.
							// Move along.
							root.remove(se);
							continue;
						}
						...
						sessionDateTime = (String) imgSession.getDate() + "T"
								+ (String) imgSession.getTime();
						...
						for (final String prop : new String[] { "ID" })
							try {
								final String value = imgSession
						...
						if (null != sessLabel) { // We don't want the session
													// label in the alternate
													// IDs
						...
						if (!altIDs.isEmpty()) {
							...
							for (final String id : altIDs) {
								// Don't include IDs already embedded in the
								// session name.
								if (sessDirName.endsWith(id))
									continue;
								if (sb.length() > 0)
									sb.append(",");
								sb.append(id);
				...
				try {
					se.addAttribute("uploadDateTime", XS_DATETIME_FORMAT
							.format(TIMESTAMP_FORMAT.parse(ts)));
				} catch (ParseException e) {
					log.error("unable to convert timestamp folder name " + ts
							+ " to xs:dateTime", e);
				}
				...
				final StringBuilder archiveURL = new StringBuilder(
						TurbineUtils.GetContext());
	...
	private File getPrearcRoot(final String project) {
		if (null == project || COMMON.equals(project)) {
			return new File(ArcSpecManager.GetInstance()
					.getGlobalPrearchivePath());
		} else {
			final String path = ArcSpecManager.GetInstance()
					.getPrearchivePathForProject(project);
			return null == path ? null : new File(path);
		}
	}

	private void removeSession(final XDATUser user,
			final HttpServletResponse response, final String project,
			final File prearc, final String path) throws PrearcOpException {
		if (null == prearc || !prearc.isDirectory()) {
		...
		if (!canCreate(user, project, type)) {
			throw new PrearcOpException(HttpServletResponse.SC_FORBIDDEN,
					"user " + user + " not allowed to delete " + path
							+ " from prearchive " + project);
		}

		new File(sessdir.getPath() + XML_SUFFIX).delete(); // remove the session
															// XML
	 ...
	 */
	public void remove(final HttpServletRequest req,
			final HttpServletResponse response, final ServletConfig config) {
		final HttpSession session = req.getSession();
		final XDATUser user = XDAT.getUserDetails();
		final String login = user.getLogin();
		...
		try {
			removeSession(user, response, name, prearc,
					(String) req.getParameter("path"));
	...
	/**
	 * Handles a "move" request by moving a session from one prearchive to
	 * another.
	 */
	public void move(final HttpServletRequest req,
			final HttpServletResponse response, final ServletConfig config) {
		final HttpSession session = req.getSession();
		final XDATUser user = XDAT.getUserDetails();
		final String login = user.getLogin();
		...
		if (null == fromPrearc || !fromPrearc.isDirectory()) {
			...
			try {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
						"source prearchive " + fromProject + " does not exist");
			} catch (IOException ignore) {
			}
			return;
		}
		...
		if (null == toPrearc || !toPrearc.isDirectory()) {
			...
			try {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
						"destination prearchive " + toProject
								+ " does not exist");
			} catch (IOException ignore) {
			}
			return;
		...
		if (fromPrearc.equals(toPrearc)) {
			log.warn("attempted to move session from prearchive " + fromPrearc
					+ " to same prearchive.");
			try {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
						"cannot move session from and to the same prearchive");
			} catch (IOException ignore) {
			}
			return;
		}
		...
		if (path == null || "".equals(path)) {
			log.info("received session move request from " + login
					+ " with empty path");
			try {
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
		...
		if (!canCreate(user, fromProject, type)) {
			log.info("rejected move request for " + login
					+ ": no permission on source project " + fromProject);
			try {
				response.sendError(HttpServletResponse.SC_FORBIDDEN,
						"not allowed to remove session from project "
								+ fromProject);
			} catch (IOException ignore) {
			}
			return;
		...
		if (!canCreate(user, toProject, type)) {
			log.info("rejected move request for " + login
					+ ": no permission on destination project " + toProject);
			try {
				response.sendError(HttpServletResponse.SC_FORBIDDEN,
						"not allowed to add session to project " + toProject);
			} catch (IOException ignore) {
			}
			return;
		}

		// Remove the old session documents (including session build logs and
		// image catalog files),
		...
		for (final File file : fromSession
				.listFiles(PrearcUtils.isSessionGeneratedFileFilter)) {
			file.delete();
		}
		final File newTsdir = new File(toPrearc, tsdir.getName());
		PrearcImporterFactory.getFactory()
				.getPrearcImporter(toProject, newTsdir, fromSession).run();

		// If everything was moved, we can remove the session and timestamp
		// directories.
		fromSession.delete();
		if (fromSession.isDirectory()) {
			log.warn("moved session " + fromSession + " to " + toPrearc
					+ ", but unable to delete original directory.");
		}
		tsdir.delete(); // timestamp directory might contain another session, so
						// no warning if deletion fails.
	}
}
