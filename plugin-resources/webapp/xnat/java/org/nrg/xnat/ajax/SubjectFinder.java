...
public class SubjectFinder {
	...
	public void execute(HttpServletRequest req, HttpServletResponse response)
		...
		if (s != null) {
			...
			try {
				final GenericWrapperElement element = GenericWrapperElement
				...
				final ItemSearch is = new ItemSearch(user, "xnat:subjectData",
						cc);
				...
				if (matches.size() == 0) {
					...
					cc.addClause("xnat:subjectData/label", " LIKE ", "%" + s
							+ "%");
					cc.addClause("xnat:subjectData/sharing/share/label",
							" LIKE ", "%" + s + "%");
				}
				final ItemCollection items = ItemSearch.GetItems(cc, user,
						false);
				for (final ItemI i : items.getItems()) {
					if (i.getXSIType().equals("xnat:subjectData")) {
						...
						if (!user.canRead(i)) {
							...
							for (int j = 0; j < subject.getSharing_share()
									.size(); j++) {
								i.setProperty(subject.getXSIType()
										+ "/sharing/share[" + j + "]/label",
										"*****");
							}
						}

						sb.append("<subject ID=\"").append(subject.getId())
								.append("\" label=\"")
								.append(subject.getLabel())
								.append("\" project=\"")
								.append(subject.getProject()).append("\">");

						if (!subject.getSharing_share().isEmpty()) {
							sb.append("<sharing>");
							for (final XnatProjectparticipantI pp : subject
									.getSharing_share()) {
								sb.append("<share ");
								if (pp.getLabel() == null) {
						...
						if (!subject.getAddid().isEmpty()) {
							for (final XnatSubjectdataAddidI pp : subject
									.getAddid()) {
								sb.append("<addID ");
								sb.append("name=\"").append(pp.getName())
										.append("\">");
