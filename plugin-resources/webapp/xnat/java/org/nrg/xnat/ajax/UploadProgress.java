...
public class UploadProgress {
	public void monitor(HttpServletRequest req, HttpServletResponse response)
		...
		if (uploadID != null) {
			...
			Object extract = req.getSession()
					.getAttribute(uploadID + "Extract");
			ArrayList<String[]> status = (ArrayList<String[]>) req.getSession()
					.getAttribute(uploadID + "status");
			if (upload == null) {
				// System.out.println("0");
				StringBuffer sb = new StringBuffer();
				sb.append("<uploadStatus upload=\"0\" extract=\"0\"/>");

				req.getSession().setAttribute(uploadID + "Upload",
						new Integer(0));
				req.getSession().setAttribute(uploadID + "Extract",
						new Integer(0));
	...
	public void start(HttpServletRequest req, HttpServletResponse response)
			throws IOException {
		String uploadID = req.getParameter("ID");
		if (uploadID != null) {
			...
			if (o == null) {
				...
				req.getSession().setAttribute(uploadID + "Extract",
						new Integer(0));
				req.getSession().setAttribute(uploadID + "status",
						new ArrayList());
				response.setContentType("text/xml");
				response.setHeader("Cache-Control", "no-cache");
				response.getWriter().write(sb.toString());
