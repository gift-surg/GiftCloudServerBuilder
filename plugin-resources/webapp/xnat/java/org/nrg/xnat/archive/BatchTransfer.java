...
public class BatchTransfer extends Thread {
	...
	public BatchTransfer(final String server, final String system,
		...
		for (int i = 0, nSessions = sessions.size(); i < nSessions; i++) {
			...
			try {
				...
				try {
					...
					cc.addClause(subCC);
					List<WrkWorkflowdata> al = WrkWorkflowdata
							.getWrkWorkflowdatasByField(cc, user, false);
					if (al.size() > 0) {
						wkdata = (WrkWorkflowdata) al.get(al.size() - 1);
						if (XFT.VERBOSE)
							System.out.println("FOUND Transfer Workflow:"
									+ wkdata.getWrkWorkflowdataId());
					} else {
						if (XFT.VERBOSE)
						...
						try {
							...
							wkdata.setLaunchTime(java.util.Calendar
									.getInstance().getTime());
					...
					try {
						...
						wkdata.setCurrentStepId("Store");
						wkdata.setLaunchTime(java.util.Calendar.getInstance()
								.getTime());
						wkdata.setPipelineName("Transfer");
					} catch (Exception e) {
						logger.error("", e);
				...
				if (XFT.VERBOSE)
					System.out
							.print("Reading " + xml.getAbsolutePath() + "...");
				...
				final XnatImagesessiondata session = (XnatImagesessiondata) BaseElement
				...
				partialMR = (XnatImagesessiondata) BaseElement
						.GetGeneratedItem(session.getCurrentDBVersion());

				try {
					wkdata.setCurrentStepId("Copy To Arc");
					wkdata.setCurrentStepLaunchTime(java.util.Calendar
							.getInstance().getTime());
				...
				try {
					...
					logger.error("Unable to identify current archive location",
							e);
				...
				final StringBuilder arcPath = new StringBuilder(
						partialMR.getArchiveRootPath());
				if (null != currentarc)
				...
				if (XFT.VERBOSE)
					System.out.print("Copying from " + dir.getAbsolutePath()
							+ " to " + arcF.getAbsolutePath() + "...");
				try {
				...
				if (fileerrors.size() > 0) {
					...
					try {
						...
						wkdata.setCurrentStepLaunchTime(java.util.Calendar
								.getInstance().getTime());
						PersistentWorkflowUtils.save(wkdata,
								wkdata.buildEvent());
					} catch (Exception e) {
						logger.error("", e);
					}
				...
				if (partialMR instanceof XnatMrsessiondata) {
					...
					try {
						QCImageCreator qcImageCreator = new QCImageCreator(
								(XnatMrsessiondata) partialMR, (XDATUser) user);
						boolean _success = qcImageCreator
								.createQCImagesForScans();
						if (!_success) {
							AdminUtils
									.sendAdminEmail((XDATUser) user,
											"Archiving Failed",
											"Couldnt complete creation of Quality Control Images Successfully");
						}
					} catch (Exception e) {
						StringBuffer sb = new StringBuffer();
						sb.append("Archiving Failed.<BR>Quality Control Image Creation Failed.<BR>");
						sb.append(e.getMessage());
						AdminUtils.sendAdminEmail((XDATUser) user,
								"Archiving Failed", sb.toString());
					}
					if (XFT.VERBOSE)
						System.out.print("Done");
				}
				/*
				 * try { wkdata.setCurrentStepId("Zipping");
				 * wkdata.setCurrentStepLaunchTime
				 * (java.util.Calendar.getInstance().getTime());
				 * wkdata.setNextStepId("Cache Upload"); wkdata.save(user,
				 * false, false); } catch (Exception e) { logger.error("",e); }
				 * 
				 * try { System.out.print("Zipping archive files...");
				 * FileUtils.GZIPFiles(arcF); System.out.println("done."); }
				 * catch (RuntimeException e1) { System.out.println("failed.");
				 * logger.error("zip failed", e1); try {
				 * wkdata.setStatus("Failed");
				 * wkdata.setCurrentStepLaunchTime(java
				 * .util.Calendar.getInstance().getTime()); wkdata.save(user,
				 * false, false); } catch (Exception e) { logger.error("",e); }
				 * Collection<String> error = new ArrayList<String>(2);
				 * error.add(partialMR.getId()); error.add(
				 * "Error transfering files.  Failed to zip archive files.");
				 * errors.add(error); _successful=false; continue; }
				 */
				...
				try {
					...
					while (prearcpath.endsWith("/")) {
						prearcpath = prearcpath.substring(0,
								prearcpath.length() - 1);
					}
					while (prearcpath.endsWith("\\")) {
						prearcpath = prearcpath.substring(0,
					...
					org.nrg.xft.utils.FileUtils.MoveFile(prearcXML, cacheXML,
							true);
	...
	public String getEmailCompletionMessage(UserI user, List<String> messages,
			List<List<String>> errors, String system, String admin_email) {
		try {
			...
			Template template = Velocity
					.getTemplate("/screens/BatchWorkflowCompleteEmail.vm");
