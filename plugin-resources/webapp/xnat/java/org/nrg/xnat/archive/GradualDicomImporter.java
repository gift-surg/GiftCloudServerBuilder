...
public class GradualDicomImporter extends ImporterHandlerA {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(GradualDicomImporter.class);
	private static final Object NOT_A_WRITABLE_PROJECT = new Object();
	private static final String DEFAULT_TRANSFER_SYNTAX = TransferSyntax.ImplicitVRLittleEndian
			.uid();
	...
	public GradualDicomImporter(final Object listenerControl, final XDATUser u,
	...
	private boolean canCreateIn(final XnatProjectdata p) {
		try {
			...
			logger.error(
					"Unable to check permissions for " + user + " in " + p, t);
			return false;
		}
	}

	/**
	 * Does e contain the sentinel value indicating that its alias does not map
	 * to a project where user has create perms?
	 * 
	 * @param e
	 *            cache element (not null)
	...
	/**
	 * Indicate in the provided cache that the provided name does not describe a
	 * writable project.
	 * 
	 * @param cache
	 *            The cache to use
	 * @param name
	 *            The name of the project to cache as non-writable.
	 */
	public static void cacheNonWriteableProject(final Cache cache,
			final String name) {
		cache.put(new Element(name, NOT_A_WRITABLE_PROJECT));
	}

	private XnatProjectdata getProject(final Object alias,
			final Callable<XnatProjectdata> lookupProject) {
		if (null == projectCache) {
			projectCache = getUserProjectCache(user);
		}
		if (null != alias) {
			logger.debug(
					"looking for project matching alias {} from query parameters",
					alias);
			final Element pe = projectCache.get(alias);
			if (null != pe) {
				if (isCachedNotWriteableProject(pe)) {
					// this alias is cached as a non-writable project name, but
					// user is specifying it.
					// maybe they know something we don't; clear cache entry so
					// we can try again.
				...
				logger.trace(
						"cache miss for project alias {}, trying database",
						alias);
				final XnatProjectdata p = XnatProjectdata
						.getXnatProjectdatasById(alias, user, false);
				if (null != p && canCreateIn(p)) {
					projectCache.put(new Element(alias, p));
					return p;
				} else {
					for (final XnatProjectdata pa : XnatProjectdata
			...
			logger.info(
					"storage request specified project {}, which does not exist or user does not have create perms",
					alias);
		} else {
			logger.trace("no project alias found in query parameters");
		}
		// No alias, or we couldn't match it to a project. Run the identifier to
		// see if that can get a project name/alias.
		// (Don't cache alias->identifier-derived-project because we didn't use
		// the alias to derive the project.)
		try {
	...
	private File getSafeFile(File sessionDir, String scan, String name,
			DicomObject o, boolean forceRename) {
		...
		if (reqFile.exists()) {
			try {
				...
				try {
					...
					if (Objects.equal(o.get(Tag.SOPInstanceUID),
							o1.get(Tag.SOPInstanceUID))
							&& Objects.equal(o.get(Tag.SOPClassUID),
									o1.get(Tag.SOPClassUID))) {
						return reqFile; // object are equivalent; ok to
										// overwrite
					} else {
						return safeFile;
					}
	...
	private static <K, V> String getString(final Map<K, V> m, final K k,
	...
	public List<String> call() throws ClientException, ServerException {
		...
		try {
			...
			try {
				dis = null == ts ? new DicomInputStream(bis)
						: new DicomInputStream(bis, ts);
				final int lastTag = Math.max(dicomObjectIdentifier.getTags()
						.last(), SeriesImportFilter.LAST_TAG) + 1;
				logger.trace("reading object into memory up to {}",
						TagUtils.toString(lastTag));
				dis.setHandler(new StopTagInputHandler(lastTag));
				dicom = dis.readDicomObject();
				if (seriesImportFilter.isEnabled()
						&& !seriesImportFilter.shouldIncludeDicomObject(dicom)) {
					return new ArrayList<String>();
					/**
					 * TODO: Return information to user on rejected files.
					 * Unfortunately throwing an exception causes DicomBrowser
					 * to display a panicked error message. Some way of
					 * returning the information that a particular file type was
					 * not accepted would be nice, though. Possibly record the
					 * information and display on an admin page. Work to be done
					 * for 1.7
					 */
				}
				try {
				...
				if (Strings.isNullOrEmpty(dicom.getString(Tag.SOPClassUID))) {
					throw new ClientException("object " + name
							+ " contains no SOP Class UID");
				}
				if (Strings.isNullOrEmpty(dicom.getString(Tag.SOPInstanceUID))) {
					throw new ClientException("object " + name
							+ " contains no SOP Instance UID");
				}
			} catch (Throwable t) {
				try {
					bis.close();
				...
				throw new ClientException(Status.CLIENT_ERROR_BAD_REQUEST,
						"unable to read DICOM object " + name, t);
			}
		} catch (ClientException e) {
			throw e;
		} catch (Throwable t) {
		...
		if (null == project) {
			...
			root = new File(ArcSpecManager.GetInstance()
					.getGlobalPrearchivePath() + "/" + project.getId());
		...
		if (params.containsKey(URIManager.EXPT_LABEL)) {
			...
			logger.trace("using provided experiment label {}",
		...
		if (params.containsKey(URIManager.VISIT_LABEL)) {
			...
			logger.trace("using provided visit label {}",
					params.get(URIManager.VISIT_LABEL));
		} else {
			visit = null;
		}
		...
		if (null == subject) {
			logger.trace("subject is null for session {}/{}", tsdir,
					sessionLabel);
		}
		...
		session.setPreventAnon(Boolean.valueOf((String) params
				.get(URIManager.PREVENT_ANON)));
		session.setPreventAutoCommit(Boolean.valueOf((String) params
				.get(URIManager.PREVENT_AUTO_COMMIT)));

		// Query the cache for an existing session that has this Study Instance
		// UID, project name, and optional modality.
		// If found the SessionData object we just created is over-ridden with
		// the values from the cache.
		// Additionally a record of which operation was performed is contained
		// in the Either<SessionData,SessionData>
		// object returned.
		//
		// This record is necessary so that, if this row was created by this
		// call, it can be deleted if anonymization
		// goes wrong. In case of any other error the file is left on the
		// filesystem.
		Either<SessionData, SessionData> getOrCreate;
		try {
			getOrCreate = PrearcDatabase.eitherGetOrCreateSession(session,
					tsdir, shouldAutoArchive(project, dicom));
		...
		try {
			...
			// this code builds and executes the sql directly, because the APIs
			// for doing so generate multiple SELECT statements (to confirm the
			// row is there)
			// we've confirmed the row is there in line 338, so that shouldn't
			// be necessary here.
			// this code executes for every file received, so any unnecessary
			// sql should be eliminated.
			if (!PrearcUtils.PrearcStatus.RECEIVING.equals(session.getStatus())) {
				// update the last modified time and set the status
				PoolDBUtils
						.ExecuteNonSelectQuery(DatabaseSession
								.updateSessionStatusSQL(session.getName(),
					...
					.getInstance()
					.getTime()
					.after(DateUtils.addSeconds(session.getLastBuiltDate(), 15))) {
				PoolDBUtils.ExecuteNonSelectQuery(DatabaseSession
						.updateSessionLastModSQL(session.getName(),
								session.getTimestamp(), session.getProject()),
						null, null);
			}
		} catch (Exception e) {
			logger.error(
					"An error occurred while trying to set the session status to RECEIVING",
					e);
			// not exactly sure what we should do here. should we throw an
			// exception, and the received file won't be stored locally? Or
			// should we let it go and let the file be saved but unreferenced.
		...
		sessdir = new File(new File(root, session.getTimestamp()),
		...
		final String source = getString(params, SENDER_ID_PARAM,
				user.getLogin());
		...
		try {
			...
			if (dicom.contains(Tag.TransferSyntaxUID)) {
				...
				final String sopInstanceUID = dicom
				...
				fmi.initFileMetaInformation(sopClassUID, sopInstanceUID,
						transferSyntaxUID);
				if (params.containsKey(SENDER_AE_TITLE_PARAM)) {
					fmi.putString(Tag.SourceApplicationEntityTitle, VR.AE,
							(String) params.get(SENDER_AE_TITLE_PARAM));
				}
			}

			final File f = getSafeFile(sessdir, scan, name, dicom,
					Boolean.valueOf((String) params.get(RENAME_PARAM)));
			f.getParentFile().mkdirs();

			try {
				lock = PrearcUtils.lockFile(session.getSessionDataTriple(),
						f.getName());
				...
				throw new ClientException(
						"Concurrent file sends of the same data is not supported.");
			}
			try {
				// check to see of this session came in through the upload
				// applet
				Boolean uploadedViaApplet = Importer.getUploadFlag(this.params);
				if (!uploadedViaApplet) {
					// I can't use the SiteWideAnonymizer here because it
					// expects an XnatImagesessionI
					String path = DicomEdit.buildScriptPath(
							DicomEdit.ResourceScope.SITE_WIDE, null);
					Configuration c = AnonUtils.getService().getScript(path,
							null);
					boolean enabled = AnonUtils.getService().isEnabled(path,
							null);
					if (enabled) {
						if (c == null) {
							throw new Exception(
									"Unable to retrieve the site-wide script.");
						} else {
							Anonymize.anonymize(f, session.getProject(),
									session.getSubject(),
									session.getFolderName(), true, c.getId(),
									c.getContents());
						}
					} else {
				...
				try {
					// if we created a row in the database table for this
					// session
					// delete it.
					if (getOrCreate.isRight()) {
						PrearcDatabase.deleteSession(session.getFolderName(),
								session.getTimestamp(), session.getProject());
					} else {
						f.delete();
					}
		...
		logger.trace("Stored object {}/{}/{} as {} for {}", project,
				studyInstanceUID, dicom.getString(Tag.SOPInstanceUID),
				session.getUrl(), source);
		return Collections.singletonList(session.getExternalUrl());
	}

	private PrearchiveCode shouldAutoArchive(final XnatProjectdata project,
			final DicomObject o) {
		if (null == project) {
			return null;
		}
		...
		if (fromDicomObject != null) {
			return fromDicomObject ? PrearchiveCode.AutoArchive
					: PrearchiveCode.Manual;
		}
		return PrearchiveCode.code(project.getArcSpecification()
				.getPrearchiveCode());
	}

	/**
	 * Adds a cache of project objects on a per-user basis. This is currently
	 * used by GradualDicomImporter and DbBackedProjectIdentifier
	 * 
	 * @param user
	 *            The user for whom to retrieve the cache.
	...
	public GradualDicomImporter setIdentifier(
	...
	private static DicomObject read(final InputStream in, final String name)
		...
		try {
			...
			throw new ClientException(Status.CLIENT_ERROR_BAD_REQUEST,
					"unable to parse DICOM object", ioexception);
		} finally {
			try {
				bis.close();
			} catch (IOException e) {
				if (null == ioexception) {
					throw new ClientException(Status.CLIENT_ERROR_BAD_REQUEST,
							"unable to close DICOM object input", e);
			...
			try {
				...
				try {
					if (Decompress.needsDecompress(tsuid) && canDecompress) {
						try {
							// Read the rest of the object into memory so the
							// pixel data can be decompressed.
							final DicomInputStream dis = new DicomInputStream(
									remainder, tsuid);
							try {
							...
							final ByteArrayInputStream bis = new ByteArrayInputStream(
									Decompress
											.dicomObject2Bytes(dataset, tsuid));
							final DicomObject d = Decompress.decompress_image(
									bis, tsuid);
							final String dtsdui = Decompress.getTsuid(d);
							try {
								fmi.putString(Tag.TransferSyntaxUID, VR.UI,
										dtsdui);
								...
								if (t instanceof IOException) {
									...
									slog().error(
											"Unable to write decompressed dataset",
											t);
								}
								try {
									dos.close();
								} catch (IOException e) {
									throw ioexception = null == ioexception ? e
											: ioexception;
								}
							}
						} catch (ClientException e) {
							...
							slog().error(
									"Decompression failed; storing in original format "
											+ tsuid, t);
							dos.writeFileMetaInformation(fmi);
							dos.writeDataset(dataset, tsuid);
							if (null != remainder) {
								final long copied = ByteStreams.copy(remainder,
										bos);
								slog().trace(
										"copied {} additional bytes to {}",
										copied, f);
							}
						}
					} else {
						...
						dos.writeDataset(dataset, tsuid);
						if (null != remainder) {
							final long copied = ByteStreams
									.copy(remainder, bos);
							slog().trace("copied {} additional bytes to {}",
									copied, f);
						}
					}
				} catch (NoClassDefFoundError t) {
					slog().error(
							"Unable to check compression status; storing in original format "
									+ tsuid, t);
					dos.writeFileMetaInformation(fmi);
					dos.writeDataset(dataset, tsuid);
					if (null != remainder) {
				...
				try {
					dos.close();
					LoggerFactory.getLogger("org.nrg.xnat.received").info(
							"{}:{}", source, f);
				} catch (IOException e) {
