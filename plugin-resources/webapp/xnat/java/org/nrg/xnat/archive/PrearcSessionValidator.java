...
public final class PrearcSessionValidator extends PrearcSessionArchiver {
	...
	protected PrearcSessionValidator(final XnatImagesessiondata src,
			final PrearcSession prearcSession, final XDATUser user,
			final String project, final Map<String, Object> params) {
		super(src, prearcSession, user, project, params, false, true, false,
				false);
	}

	public PrearcSessionValidator(final PrearcSession session,
	...
	/**
	 * This method overwrites the one in archiver so that multiple exceptions
	 * can be recorded, rather than just the first one.
	 * 
	 * @return
	 */
			...
			for (final XnatImagescandataI newScan : src.getScans_scan()) {
				...
				if (match != null) {
					if (StringUtils.equals(match.getUid(), newScan.getUid())) {
						conflict(16, "Session already contains a scan ("
								+ match.getId()
						...
						conflict(
								...
								"Session already contains a scan ("
										+ match.getId()
										+ ") with the same number, but a different UID. - New scan will be renamed to "
										+ match.getId() + "_1.");
					}
				}
				...
				if (match2 != null) {
					if (match == null
						...
						conflict(
								...
								"Session already contains a scan ("
										+ match2.getId()
										+ ") with the same UID, but a different number.");
					}
				}
			}
		}
	...
	/**
	 * Mimics the behavior of PrearcSessionArchiver.call(), but tracks the
	 * exceptions, rather then failing on them.
	 * 
	 * @return
	 */
	public List<PrearcSessionValidator.Notice> validate()
		...
		try {
			...
			fail(8, e1.getMessage());// this means the code couldn't identify
										// the session label.
		}
		...
		if (existing == null) {
			...
			try {
				...
				conflict(10, e.getMessage());// this means there is an open
												// workflow entry
			}
		}
		...
		try {
			...
			fail(11, e1.getMessage());// this means the action couldn't identify
										// the subject
		} catch (ServerException e1) {
			warn(12, e1.getMessage());// this just means the action was going to
		...
		try {
			...
			fail(13, e1.getMessage());// this is some sort of schema validation
										// exception
		}
		...
		MergePrearcToArchiveSession merger = new MergePrearcToArchiveSession(
				src.getPrearchivePath(), this.prearcSession.getSessionDir(),
				src, src.getPrearchivepath(), arcSessionDir, existing,
				arcSessionDir.getAbsolutePath(), true, false, saveImpl, user,
				EventUtils.TEST_EVENT(user));
	...
	// prearcSessionArchiver will fail (throw exception) on the first issue it
	// finds
