...
public class Rename implements Callable<File> {
	enum STEP {
		PREPARING, PREPARE_SQL, COPY_DIR, ANONYMIZE, EXECUTE_SQL, DELETE_OLD_DIR, COMPLETE
	};
	...
	public Rename(final XnatProjectdata p, final ArchivableItem e,
			final String lbl, final XDATUser u, final String reason,
			final EventUtils.TYPE type) {
		proj = p;
		i = e;
		newLabel = lbl;
		user = u;
	...
	/**
	 * Rename the label for the corresponding session and modify the file URIs
	 * for the adjusted path.
	 * 
	 ...
	 * @throws Exception
	 */
	public File call() throws FieldNotFoundException, ProcessingInProgress,
			DuplicateLabelException, IllegalAccessException,
			LabelConflictException, FolderConflictException,
			InvalidArchiveStructure, URISyntaxException, Exception {
		final File newSessionDir = new File(new File(proj.getRootArchivePath(),
				proj.getCurrentArc()), newLabel);
		...
		try {
			...
			if (!checkPermissions(i, user)) {
				throw new org.nrg.xdat.exceptions.IllegalAccessException(
						"Invalid Edit permissions for project: " + proj.getId());
			}

			// confirm if new label is already in use
			if (i instanceof XnatSubjectdata) {
				final XnatSubjectdata match = XnatSubjectdata
						.GetSubjectByProjectIdentifier(proj.getId(), newLabel,
								null, false);
				if (match != null) {
					throw new LabelConflictException();
				}
			} else {
				final XnatExperimentdata match = XnatExperimentdata
						.GetExptByProjectIdentifier(proj.getId(), newLabel,
								null, false);
				if (match != null) {
					throw new LabelConflictException();
				}
			...
			final Collection<? extends PersistentWorkflowI> open = PersistentWorkflowUtils
					.getOpenWorkflows(user, id);
			if (!open.isEmpty()) {
				throw new ProcessingInProgress(
						((WrkWorkflowdata) CollectionUtils.get(open, 0))
								.getPipelineName());
			}

			// confirm if new directory already exists w/ stuff in it
			if (newSessionDir.exists() && newSessionDir.list() != null
					&& newSessionDir.list().length > 0) {
			...
			final String message = String.format("Renamed from %s to %s",
					current_label, newLabel);

			// add workflow entry
			final PersistentWorkflowI workflow = PersistentWorkflowUtils
					.buildOpenWorkflow(user, i.getXSIType(), i
							.getStringProperty("ID"), proj.getId(), EventUtils
							.newEventInstance(EventUtils.CATEGORY.DATA, type,
									EventUtils.RENAME, reason, null));
			workflow.setDetails(message);
			EventMetaI c = WorkflowUtils
					.setStep(workflow, getStep().toString());
			...
			try {
				...
				if (moveFiles)
					org.nrg.xft.utils.FileUtils.CopyDir(oldSessionDir,
							newSessionDir, false);

				if (i instanceof XnatImagesessiondata) {
					this.updateStep(workflow, setStep(STEP.ANONYMIZE));
					new ProjectAnonymizer(newLabel, (XnatImagesessiondata) i,
							proj.getId(),
							((XnatImagesessiondata) i)
									.getArchivePath(((XnatImagesessiondata) i)
											.getArchiveRootPath())).call();
				}
				...
				if (moveFiles)
					org.nrg.xnat.utils.FileUtils.moveToCache(proj.getId(),
							SUCCESSFUL_RENAMES, oldSessionDir);
				...
				if (!getStep().equals(STEP.DELETE_OLD_DIR)) {
					try {
						if (moveFiles)
							org.nrg.xnat.utils.FileUtils.moveToCache(
									proj.getId(), FAILED_RENAME, newSessionDir);
	...
	public EventMetaI updateStep(final PersistentWorkflowI wrk, final STEP step)
			throws Exception {
	...
	public static boolean checkPermissions(final ArchivableItem i,
			final XDATUser user) throws Exception {
		return user.canEdit(i);
	}

	/**
	 * Generate the SQL update logic for all of the items resources. Checks
	 * permissions for assessments, if they were modified.
	 * 
	 * @param current_label
	 * @return
	 ...
	 */
	public static void generateURISQL(final ItemI i, final URI expected,
			final String newArchive, final DBItemCache cache,
			final XDATUser user) throws UnsupportedResourceType, SQLException,
			Exception {
		final SecurityManager sm = SecurityManager.GetInstance();
		// set label and modify URI
		if (i instanceof XnatSubjectdata) {
			for (final XnatAbstractresourceI res : ((XnatSubjectdata) i)
					.getResources_resource()) {
				modifyResource((XnatAbstractresource) res, expected,
						newArchive, user, sm, cache);
			}
		} else {
			for (final XnatAbstractresourceI res : ((XnatExperimentdata) i)
					.getResources_resource()) {
				modifyResource((XnatAbstractresource) res, expected,
						newArchive, user, sm, cache);
			}
			...
			if (i instanceof XnatImagesessiondata) {
				for (final XnatImagescandataI scan : ((XnatImagesessiondataI) i)
						.getScans_scan()) {
				...
				for (final XnatReconstructedimagedataI recon : ((XnatImagesessiondataI) i)
				...
				for (final XnatImageassessordataI assess : ((XnatImagesessiondataI) i)
						.getAssessors_assessor()) {
					boolean checkdPermissions = false;
					for (final XnatAbstractresourceI res : assess
							.getResources_resource()) {
						if (modifyResource((XnatAbstractresource) res,
								expected, newArchive, user, sm, cache)) {
							if (!checkdPermissions) {
								if (checkPermissions(
										(XnatImageassessordata) assess, user)) {
									checkdPermissions = true;
								} else {
									throw new org.nrg.xdat.exceptions.IllegalAccessException(
											"Invalid Edit permissions for assessor in project: "
													+ assess.getProject());
								}
							}
						}
	...
	/**
	 * Modifies the resource to point to the new path, if the old path is in the
	 * expected place.
	 * 
	 * @param res
	 * @param expected
	 ...
	 * @throws ElementNotFoundException
	 */
	protected static boolean modifyResource(final XnatAbstractresource res,
			final URI expected, final String newArchive, final XDATUser user,
			final SecurityManager sm, final DBItemCache cache)
			throws UnsupportedResourceType, ElementNotFoundException,
			XFTInitException, FieldNotFoundException, SQLException, Exception {
		...
		if (relative.equals(current)) {
			...
			if (path.replace('\\', '/').contains(
					"/" + oldSessionDir.getName() + "/")) {
				// session contains resource which is not in the standard
				// format, but is in a directory with the old label.
			...
			// properly in place
			setPath(res,
					(new File(newArchive, relative.getPath()))
							.getAbsolutePath());
			DBAction.StoreItem(res.getItem(), user, false, false, false, false,
					sm, cache);
	 ...
	 * @throws Exception
	 */
	protected static void generateLabelSQL(final ArchivableItem i,
			final String newLabel, final DBItemCache cache,
			final XDATUser user, EventMetaI message) throws SQLException,
			Exception {
		i.getItem().setProperty("label", newLabel);

		DBAction.StoreItem(i.getItem(), user, false, false, false, false,
				SecurityManager.GetInstance(), message);
	}
	 ...
	 * @throws Exception
	 */
	protected static void executeSQL(final DBItemCache cache,
			final XDATUser user, final String logFileName) throws Exception {
	 ...
	 * @throws UnsupportedResourceType
	 */
	protected static String getPath(final XnatAbstractresource res)
			throws UnsupportedResourceType {
	 ...
	 * @throws UnsupportedResourceType
	 */
	protected static void setPath(final XnatAbstractresource res,
			final String newPath) throws UnsupportedResourceType {
		if (res instanceof XnatResource) {
