...
public class Transfer {
	...
	public boolean execute() {
		...
		try {
			...
			try {
				...
				if (!prearcF.exists())
					throw new FileNotFoundException(prearc);
			...
			XnatPipelineLauncher xnatPipelineLauncher = new XnatPipelineLauncher(
					(XDATUser) user);
			// Modified by MR - 2010/03/11 AdminEmail is set in pipeline setup
			// setup
			// xnatPipelineLauncher.setAdmin_email(AdminUtils.getAdminEmailId());
			// Modified bt MR - 2010/03/11 There is a setting for Site Admin to
			// set email notification for Transfer pipeline
			...
			xnatPipelineLauncher.setParameter("logDir", XFT.GetCachePath()
					+ "logs" + "/" + "transfer");
			...
			if (cache) {
				...
				cachePath += "transfer_bk" + File.separator + parent.getName()
						+ File.separator + prearcF.getName();
			 ...
			 * 
			 * xnatPipelineLauncher.setParameter("tbpath",
			 * QCImageCreator.getQCThumbnailPathForSession(mr.getProject()));
			 * xnatPipelineLauncher.setParameter("cpath",
			 * QCImageCreator.getQCCachePathForSession(mr.getProject())); }
			 */
			...
			try {
				AdminUtils.sendAdminEmail((XDATUser) user, "Archiving Failed: "
						+ mr.getId(), sb.toString());
	...
	/*
	 * public void run() { super.run(); execute(); }
	 */

	public String getEmailCompletionMessage(UserI user, String message,
			String system, String admin_email) throws Exception {
		...
		Template template = Velocity
				.getTemplate("/screens/WorkflowCompleteEmail.vm");
	...
	/**
	 * @param clearExistingWorkflows
	 *            the clearExistingWorkflows to set
	 */
