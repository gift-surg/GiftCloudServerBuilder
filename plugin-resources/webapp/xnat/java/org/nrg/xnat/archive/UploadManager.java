...
public class UploadManager extends Thread {
	...
	public UploadManager(XDATUser user, String project, String outputPath,
			String server) {
		this.user = user;
		projectID = project;
	...
	public void execute() {
		...
		// }
		//
		// final String[] command = {"archiveIma"}; // or maybe
		// {"/usr/bin/perl", "/data/cninds01/data2/arc-tools/archiveIma"}
		//
		// final String[] env = {"LD_LIBRARY_PATH=/usr/local/lib"};
		//
		//
		// final PrearcImporter pw = new PrearcImporter(tempFile, prearc,
		// command, env);
		//
		// pw.run();
		//
		...
		// {
		// File newDir = new File(prearc.getAbsolutePath() + File.separator +
		// child.getName());
		// while (newDir.exists())
		// {
		...
		// }
		//
		// File images = new File(prearc.getAbsolutePath() + File.separator +
		// fileToFolder);
		//
		// while (images.exists())
		// {
		...
		// images.mkdirs();
		// try {
		// FileUtils.MoveFile(child, new File(images.getAbsolutePath() +
		// File.separator + child.getName()), true);
		// } catch (FileNotFoundException e) {
		// logger.error("",e);
		// continue;
		// } catch (IOException e) {
		...
		//
		//
		// File xml = new File(prearc.getAbsolutePath() + File.separator +
		// child.getName() + ".xml");
		...
		// Template template
		// =Velocity.getTemplate("/screens/BatchUploadEmail.vm");
