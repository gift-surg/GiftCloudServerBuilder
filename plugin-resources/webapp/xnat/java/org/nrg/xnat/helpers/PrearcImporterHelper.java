...
public class PrearcImporterHelper extends PrearcImporterA {
	...
	static org.apache.log4j.Logger logger = Logger
			.getLogger(PrearcImporterHelper.class);
	 ...
	 */
	public PrearcImporterHelper(final Object uID2, final XDATUser u,
			final FileWriterWrapperI fi, Map<String, Object> params,
			boolean allowSessionMerge, boolean overwriteFiles) {
		super((uID2 == null) ? u : uID2, u, fi, params, allowSessionMerge,
				overwriteFiles);
		this.user = u;
	...
	public List<PrearcSession> call() throws ActionException {
		...
		final String old_session_folder = (String) params
				.get(PrearcUtils.PREARC_SESSION_FOLDER);
		String old_timestamp = (String) params
		...
		// IF timestamp,session is specified then push to a temporary space and
		// merge
		// ELSE import directly to project.
		SessionData sd = null;
		boolean destination_specified = false;
		if (StringUtils.isNotEmpty(old_timestamp)
				&& StringUtils.isNotEmpty(old_session_folder)) {
			...
			if (StringUtils.isEmpty(project)) {
				...
				throw new ClientException(
						...
						"User must specify project portion of prearchive path, if timestamp/session portion is specified.",
						new IllegalArgumentException());
			}

			try {
				sd = PrearcDatabase.getSession(old_session_folder,
						old_timestamp, project);
				if (sd != null) {
					if (!PrearcDatabase.setStatus(old_session_folder,
							old_timestamp, project, PrearcStatus._RECEIVING)) {
						this.failed("Prearc destination is locked.");
						throw new ClientException(Status.CLIENT_ERROR_CONFLICT,
								"Prearc destination is locked.",
								new IllegalArgumentException());
					}
				}
			} catch (Exception e) {
			...
			if (StringUtils.isEmpty(old_timestamp)) {
				...
				throw new ClientException(
						Status.CLIENT_ERROR_BAD_REQUEST,
						"User must specify timestamp portion of prearchive path, if session portion is specified.",
						new IllegalArgumentException());
			}
		}
		...
		try {
			...
			final Map<String, Object> additionalValues = XMLPathShortcuts
					.identifyUsableFields(params,
							XMLPathShortcuts.EXPERIMENT_DATA, false);
			if (params.containsKey(SUBJECT)) {
				additionalValues.put(
						"xnat:subjectAssessorData/subject_ID".intern(),
						params.remove(SUBJECT));
			}

			if (params.containsKey(SESSION)) {
				additionalValues.put("xnat:experimentData/label".intern(),
			...
			if (destination_specified || project == null) {
				File projectPrearc;
				if (project == null) {
					projectPrearc = new File(ArcSpecManager.GetInstance()
							.getGlobalPrearchivePath(), PrearcUtils.TEMP_UNPACK);
				} else {
					projectPrearc = new File(ArcSpecManager.GetInstance()
				...
				List<File> tempFiles = reorganize(cacheDIR, new File(
						projectPrearc, new_timestamp), null, additionalValues);

				// should 2-srcs to project=null also be merged?
				// Merge_to_destination will write it to separate folders if
				// old_session_folder is null
				files = new ArrayList<File>();
				for (final File f : tempFiles) {
					File builtF = merge_to_destination(project, old_timestamp,
							old_session_folder, f);
					if (!files.contains(builtF)) {
						files.add(builtF);
					}
				}
			} else {
				files = reorganize(cacheDIR, new File(ArcSpecManager
						.GetInstance().getPrearchivePathForProject(project),
						old_timestamp), project, additionalValues);
			}
		} catch (ActionException e1) {
			if (sd != null) {
				try {
					PrearcUtils.resetStatus(user, project, old_timestamp,
							old_session_folder, true);
				} catch (Exception e) {
					logger.error("", e);
				}
			}
	...
	private File write_to_cache(final XDATUser user,
			final FileWriterWrapperI fi, final String timestamp)
			throws ServerException {
		final String filename = fi.getName();
		this.processing("Importing file (" + filename + ")");

		// BUILD CACHE PATH
		final File cacheDIR = new File(new File(user.getCachedFile("uploads"),
				user.getXdatUserId().toString()), timestamp);
		if (!cacheDIR.exists()) {
	...
	private File merge_to_destination(String project, final String timestamp,
			String sessionFolder, File srcDIR) throws ClientException,
			ServerException {
		...
		String prearc_path;
		if (project != null)
			prearc_path = ArcSpecManager.GetInstance()
					.getPrearchivePathForProject(project);
		else {
			prearc_path = ArcSpecManager.GetInstance()
		...
		final File destDIR = new File(new File(prearc_path, timestamp),
				sessionFolder);
		...
		ListenerUtils.addListeners(
				this,
				new MergePrearchiveSessions(uID, srcDIR, src, src
						.getPrearchivepath(), destDIR, dest, destDIR
						.getAbsolutePath(), allowSessionMerge, overwriteFiles,
						saveImpl, user)).call();
		...
		if (!FileUtils.HasFiles(srcDIR.getParentFile())) {
			org.nrg.xft.utils.FileUtils
					.deleteDirQuietly(srcDIR.getParentFile());
		}
	...
	private static String identifyProject(File f, final XDATUser user)
			throws ClientException, ServerException {
		try {
			if (!f.getName().endsWith(".xml"))
				f = new File(f.getParentFile(), f.getName() + ".xml");
			final XnatImagesessiondataI session = PrearcTableBuilder
					.parseSession(f);
	...
	public List<File> reorganize(final File cacheDIR, final File prearcDIR,
			final String project, Map<String, Object> additionalValues)
			throws ClientException, ServerException {
		if (!prearcDIR.exists()) {
		...
		return ListenerUtils.addListeners(
				this,
				new ImageUploadHelper(uID, this.user, project, cacheDIR,
						prearcDIR, additionalValues)).call();
	}
