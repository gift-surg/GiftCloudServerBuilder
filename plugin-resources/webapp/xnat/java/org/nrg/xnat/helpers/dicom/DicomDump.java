...
public final class DicomDump extends SecureResource {
	...
	private static final ElementDictionary TAG_DICTIONARY = ElementDictionary
			.getDictionary();
	...
	/**
	 * A global environment that contains the type of request has made and a map
	 * of the parsed "src" uri.
	 * 
	 * @author aditya
	 *
	 */
	...
	/**
	 * The dicom dump requested, either a specific file, or a general summary of
	 * the session
	 * 
	 * @author aditya
	 *
	...
	private static enum HeaderType {
		...
		SCAN(
				"/archive/projects/{PROJECT_ID}/subjects/{SUBJECT_ID}/experiments/{EXPT_ID}/scans/{SCAN_ID}",
		...
		private HeaderType(final String... templates) {
			...
			final ImmutableMap.Builder<Template, ResourceType> builder = ImmutableMap
					.builder();
		...
		/**
		 * Returns the filter used to determine whether to use a provided
		 * catalog entry. Default implementation always passes.
		 * 
		 * @param env
		 * @param user
		 * @return
		 */
		CatFilterWithPath getFilter(Env env, XDATUser user) {
		...
		String retrieve(final Env env, final XDATUser user) throws Exception {
			final Iterable<File> matches = env.r.getFiles(env, user,
					getFilter(env, user), 1);
	 ...
	 * 
	 * There are two global class variables, one holding the root path to a
	 * resource and the image session object associated with requested session.
	 * These variables are updated *implicitly* by functions in the class, so
	 * the call order of the methods in this class is important.
	 * 
	...
	private static enum ArchiveType {
		PREARCHIVE() {
			...
			CatCatalogI getCatalog(XnatResourcecatalogI r) {
				this.rootPath = CatalogUtils
						.getCatalogFile(this.x.getPrearchivepath(),
								((XnatResourcecatalogI) r)).getParentFile()
						.getAbsolutePath();
				final CatCatalogI catalog = CatalogUtils.getCleanCatalog(
						this.x.getPrearchivepath(), r, false);
				return catalog;
			}

			@Override
			XnatImagesessiondataI retrieve(Env env, XDATUser user)
					throws Exception, IOException, InvalidPermissionException {
				...
				sessionDIR = PrearcUtils.getPrearcSessionDir(user, project,
						timestamp, experiment, false);
				srcXML = new File(sessionDIR.getAbsolutePath() + ".xml");
				XnatImagesessiondataI x = PrearcTableBuilder
						.parseSession(srcXML);
				this.x = x;
				return x;
			}
		...
		ARCHIVE() {
			...
			XnatImagesessiondataI retrieve(Env env, XDATUser user)
				...
				String project = (String) env.attrs.get("PROJECT_ID");
				String experiment = (String) env.attrs.get("EXPT_ID");
				XnatImagesessiondata x = (XnatImagesessiondata) XnatExperimentdata
						.GetExptByProjectIdentifier(project, experiment, user,
								false);
				if (x == null || null == x.getId()) {
					x = (XnatImagesessiondata) XnatExperimentdata
							.getXnatExperimentdatasById(experiment, user, false);
				...
				if (x == null) {
					throw new ClientException(Status.CLIENT_ERROR_NOT_FOUND,
							"Experiment or project not found", new Exception(
		...
		/**
		 * Retrieve the catalog for this resource. Additionally this also
		 * updates the "rootPath" global class variable. This function is
		 * dependent on the {@link XnatImageassessordataI} having been
		 * populated.
		 * 
		 * @param r
		 * @return
		...
		/**
		 * Retrieve the image session object for this session. Additionally this
		 * also updates the XnatImagesessiondataI global.
		 * 
		 * @param env
		 * @param user
		 * @return
		 ...
		 * @throws Exception
		 */
		abstract XnatImagesessiondataI retrieve(Env env, XDATUser user)
				throws ClientException, IOException,
				InvalidPermissionException, Exception;
	};
	...
	private static enum ResourceType {
		SCAN {
			Iterable<File> getFiles(Env env, XDATUser user,
					CatFilterWithPath filter, int enough) throws Exception {
				...
				for (final XnatImagescandataI scan : x.getScans_scan()) {
					if (null == scanID || scanID.equals(scan.getId())) {
						final List<XnatResourcecatalogI> resources = scan
								.getFile();
						files.addAll(this.findMatchingFile(env, resources,
								filter, enough));
		...
		ASSESSOR {
			Iterable<File> getFiles(Env env, XDATUser user,
				...
				final List<File> files = new ArrayList<File>();
				for (XnatImageassessordataI assessor : x
						.getAssessors_assessor()) {
					if (null == id || id.equals(assessor.getId())) {
						final List<XnatResourcecatalogI> resources = assessor
						...
						final List<XnatResourcecatalogI> in_resources = assessor
								.getIn_file();
						files.addAll(this.findMatchingFile(env, in_resources,
								filter, enough));
						if (files.size() >= enough) {
							return files;
						}
						final List<XnatResourcecatalogI> out_resources = assessor
								.getOut_file();
						files.addAll(this.findMatchingFile(env, out_resources,
								filter, enough));
						if (files.size() >= enough) {
							return files;
						}
		...
		RECON {
			Iterable<File> getFiles(Env env, XDATUser user,
				...
				for (XnatReconstructedimagedataI recon : x
						.getReconstructions_reconstructedimage()) {
					if (null == id || id.equals(recon.getId())) {
						List<XnatResourcecatalogI> in_resources = recon
								.getIn_file();
						files.addAll(this.findMatchingFile(env, in_resources,
								filter, enough));
						if (files.size() >= enough) {
						...
						List<XnatResourcecatalogI> out_resources = recon
								.getOut_file();
						files.addAll(this.findMatchingFile(env, out_resources,
								filter, enough));
						if (files.size() >= enough) {
		...
		UNKNOWN {
			Iterable<File> getFiles(Env env, XDATUser user,
					CatFilterWithPath filter, int enough) {
				return Collections.emptyList();
			}
		};

		List<File> findMatchingFile(final Env env,
				final List<XnatResourcecatalogI> resources,
				final CatFilterWithPath filter, final int enough) {
			...
			for (XnatResourcecatalogI resource : resources) {
				...
				if (type.equals(DicomDump.imageType)) {
					...
					for (CatEntryI match : CatalogUtils.getEntriesByFilter(
							catalog, filter)) {
		 ...
		 * @throws Exception
		 */
		abstract Iterable<File> getFiles(Env env, XDATUser user,
				CatFilterWithPath filter, int enough) throws ClientException,
				IOException, InvalidPermissionException, Exception;
	}

	private static ImmutableMap<Integer, Set<String>> getFields(
			String[] fieldVals) {
		ImmutableMap.Builder<Integer, Set<String>> fieldsb = ImmutableMap
				.builder();
			...
			try {
				...
				try {
					...
					throw new IllegalArgumentException(
							"not a valid DICOM attribute tag: " + tag_s, e1);
				}
			}
	...
	/**
	 * Enhance {@link CatEntryFilterI} to include a path in its environment.
	 * 
	 * This is required because by the time we get down to iterating through the
	 * catalog entries we've lost access to the absolute path to the resource.
	 * 
	 * @author aditya
	 *
	 */
	...
	final static CatFilterWithPath alwaysCatWithPath = new CatFilterWithPath() {
		...
		public boolean accept(CatEntryI _) {
			return true;
		}
	};
