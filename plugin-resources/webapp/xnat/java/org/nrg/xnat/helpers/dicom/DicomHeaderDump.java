...
public final class DicomHeaderDump {
	...
	private static final String[] columns = { "tag1", // tag name, never empty.
			"tag2", // for normal, non-sequence DICOM tags this is the empty
					// string.
	...
	private final Logger logger = LoggerFactory
			.getLogger(DicomHeaderDump.class);
	 ...
	 * 
	 * @param o
	 *            Necessary so we can get to the description of the tag
	 * @param e
	 *            The current DICOM element
	 * @param parentTag
	 *            If non null, this is a nested DICOM tag.
	 * @param maxLen
	 *            The maximum number of characters to read from the description
	 *            and value
	 * @return
	 */
	String[] makeRow(DicomObject o, DicomElement e, String parentTag, int maxLen) {
		...
		// extract one using dcm4che will result in an
	...
	/**
	 * Render the DICOM header to an XFTTable supporting one level of tag
	 * nesting.
	 * 
	 * @return
	 * @throws IOException
	...
	public XFTTable render() throws IOException, FileNotFoundException {
		...
		DicomObjectToStringParam formatParams = DicomObjectToStringParam
				.getDefaultParam();
			...
			if (fields.isEmpty() || fields.containsKey(e.tag())) {
				if (e.hasDicomObjects()) {
					for (int i = 0; i < e.countItems(); i++) {
						...
						t.insertRow(makeRow(header, e,
								TagUtils.toString(e.tag()),
								formatParams.valueLength));
						for (Iterator<DicomElement> it1 = o.iterator(); it1
								.hasNext();) {
							DicomElement e1 = it1.next();
							t.insertRow(makeRow(header, e1,
									TagUtils.toString(e.tag()),
									formatParams.valueLength));
						}
					}
				} else if (SiemensShadowHeader.isShadowHeader(header, e)) {
					SiemensShadowHeader.addRows(t, header, e,
							fields.get(e.tag()));
				} else {
					t.insertRow(makeRow(header, e, null,
							formatParams.valueLength));
				}
			}
		}
