...
public class SiemensShadowHeader {
	...
	private static String getStringZ(final byte[] bytes, final int offset,
			final int length) {
	...
	public static boolean isShadowHeader(final DicomObject o,
			final DicomElement e) {
		final int tag = e.tag();
		...
		if (IMAGE_NUM_4.equals(version) && SIEMENS_CSA_HEADER.equals(csa)
			...
			return true;
				...
				&& SIEMENS_CSA_NON_IMAGE.equals(csa)
				&& (tag == 0x00291210 || tag == 0x00291220 || tag == 0x00291110 || tag == 0x00291120)) {
			return true;
		} else {
			return false;
		}
	...
	public static XFTTable addRows(final XFTTable table, final DicomObject o,
		...
		if (IMAGE_NUM_4.equals(version)) {
			if (SIEMENS_CSA_HEADER.equals(csa)) {
				...
				throw new IllegalArgumentException(
						"invalid Siemens CSA HEADER identifier " + csa);
			}
		} else if (SPEC_NUM_4.equals(version)) {
			...
			throw new IllegalArgumentException("invalid NUMARIS version "
					+ version);
		}
		...
		for (int i = 0; i < n; i++) {
			final String fieldName = getStringZ(bytes, offset, 64);
			offset += 64;
			int vm = getLEint(bb, offset);
			offset += INT32_SIZE;
			final String vr = getStringZ(bytes, offset, 4);
			offset += 4;
			offset += INT32_SIZE; // skip SyngoDT (int32)
			final int numItems = getLEint(bb, offset);
			offset += INT32_SIZE;
			if (0 == vm) {
			...
			final String value;
			if (numItems > 1) {
				final Collection<String> values = Lists
						.newArrayListWithExpectedSize(numItems);
				offset += INT32_SIZE; // skip unknown int32 (= 77)
				for (int j = 0; j < vm; j++) {
					offset += 3 * INT32_SIZE; // skip three int32s
					final int fieldWidth = 4 * (int) Math.ceil(getLEint(bb,
							offset) / 4.0);
					offset += INT32_SIZE;
			...
			if (null == only || only.isEmpty() || only.contains(fieldName)) {
				table.insertRow(new String[] { tagName, fieldName, vr, value,
						desc });
			}
		}
		return table;
