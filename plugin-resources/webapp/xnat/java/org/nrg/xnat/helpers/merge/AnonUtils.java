...
public class AnonUtils {
	public AnonUtils() throws Exception {
		if (_instance != null) {
			throw new Exception(
					"The AnonUtils service is already initialized, try calling getInstance() instead.");
		}
		_instance = this;
	}
	...
	public Configuration getScript(String path, Long project) {
		if (logger.isDebugEnabled()) {
			logger.debug("Retrieving script for {}, {} for project: {}",
					new Object[] { DicomEdit.ToolName, path, project });
		}

		return _configService.getConfig(DicomEdit.ToolName, path, project);
	}

	public boolean isEnabled(String path, Long project) {
		Configuration config = _configService.getConfig(DicomEdit.ToolName,
				path, project);
		final boolean enabled = config.getStatus().equals(
				Configuration.ENABLED_STRING);
		if (logger.isDebugEnabled()) {
			logger.debug("Retrieved status {} for {}, {} for project: {}",
					new Object[] { enabled, DicomEdit.ToolName, path, project });
		}
	...
	public List<Configuration> getAllScripts(Long project) {
		List<Configuration> scripts = _configService.getConfigsByTool(
				DicomEdit.ToolName, project);
		if (logger.isDebugEnabled()) {
			if (scripts == null) {
				logger.debug("Retrieved no scripts for {}, {} for project: {}",
						new Object[] { DicomEdit.ToolName, project });
			} else if (scripts.size() == 0) {
				logger.debug("Retrieved no scripts for {}, {} for project: {}",
						new Object[] { DicomEdit.ToolName, project });
			} else {
				logger.debug("Retrieved {} scripts for {}, {} for project: {}",
						new Object[] { scripts.size(), DicomEdit.ToolName,
								project });
			}
		}
	...
	public void setProjectScript(String login, String path, String script,
			Long project) throws ConfigServiceException {
		if (logger.isDebugEnabled()) {
			logger.debug("Setting script for {}, {} for project: {}",
					new Object[] { DicomEdit.ToolName, path, project });
		}
		_configService.replaceConfig(login, "", DicomEdit.ToolName, path,
				script, project);
	}

	public void setSiteWideScript(String login, String path, String script)
	...
	public static File getDefaultScript() throws FileNotFoundException {
		...
		if (def.exists()) {
			...
			throw new FileNotFoundException("Default anon script: "
					+ DEFAULT_ANON_SCRIPT + " not found in " + XFT.GetConfDir());
		}
	}

	private static final Logger logger = LoggerFactory
			.getLogger(AnonUtils.class);
