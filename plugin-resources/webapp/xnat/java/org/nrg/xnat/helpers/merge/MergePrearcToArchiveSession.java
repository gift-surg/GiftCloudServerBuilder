...
public class MergePrearcToArchiveSession extends
		MergeSessionsA<XnatImagesessiondata> {
	public MergePrearcToArchiveSession(Object control, final File srcDIR,
			final XnatImagesessiondata src, final String srcRootPath,
			final File destDIR, final XnatImagesessiondata existing,
			final String destRootPath, boolean addFilesToExisting,
			boolean overwrite_files, SaveHandlerI<XnatImagesessiondata> saver,
			final UserI u, final EventMetaI now) {
		super(control, srcDIR, src, srcRootPath, destDIR, existing,
				destRootPath, addFilesToExisting, overwrite_files, saver, u,
				now);
		super.setAnonymizer(new PrearcSessionAnonymizer(src, src.getProject(),
	...
	public void finalize(XnatImagesessiondata session) throws ClientException,
		...
		for (XnatImagescandataI scan : session.getScans_scan()) {
			for (final XnatAbstractresourceI file : scan.getFile()) {
				...
				if (XNATUtils.isNullOrEmpty(((XnatAbstractresource) file)
						.getContent())) {
				...
				if (file instanceof XnatResourcecatalog) {
					...
					CatalogUtils
							.populateStats((XnatResourcecatalog) file, root);
				}
			}
		}
	}
			...
			for (final XnatAbstractresourceI file : scan.getFile()) {
				if (file instanceof XnatResourcecatalog) {
					...
					try {
						...
						if (CatalogUtils.formalizeCatalog(cat, f
								.getParentFile().getAbsolutePath(), user, c,
								checksums, false)) {
							CatalogUtils.writeCatalogToFile(cat, f, checksums);
						}
					} catch (Exception exception) {
						logger.error(
								"An error occurred trying to write catalog data for "
										+ ((XnatResourcecatalog) file).getUri(),
								exception);
					}
				}
			}
	...
	public org.nrg.xnat.helpers.merge.MergeSessionsA.Results<XnatImagesessiondata> mergeSessions(
		...
		if (dest == null)
			return new Results<XnatImagesessiondata>(src);
		...
		try {
			for (final XnatImagescandataI srcScan : srcScans) {
				final XnatImagescandataI destScan = MergeUtils.getMatchingScan(
						srcScan, destScans);
				if (destScan == null) {
					dest.addScans_scan(srcScan);
				} else {
					final List<XnatAbstractresourceI> source = srcScan
							.getFile();
					final List<XnatAbstractresourceI> destination = destScan
							.getFile();

					for (final XnatAbstractresourceI srcRes : source) {
						final XnatAbstractresourceI destRes = MergeUtils
								.getMatchingResource(srcRes, destination);
						if (destRes == null) {
							destScan.addFile(srcRes);
						} else {
							if (destRes instanceof XnatResourcecatalogI) {
								MergeSessionsA.Results<File> r = mergeCatalogs(
										srcRootPath,
								...
								if (r != null) {
									...
									CatalogUtils.populateStats(
											(XnatAbstractresource) srcRes,
											srcRootPath);
								}
		...
		if (!backup.mkdirs() && !backup.exists()) {
			throw new ServerException("Unable to create back-up folder: "
					...
					for (File f : toDelete) {
						...
						if (!catBkDir.mkdirs() && !catBkDir.exists()) {
							throw new ServerException(
									"Unable to create back-up folder: "
											+ catBkDir.getAbsolutePath());
						}

						FileUtils.MoveFile(f, new File(catBkDir, f.getName()),
								false);
					}
					return Boolean.TRUE;
				} catch (Exception e) {
