...
public class MergePrearchiveSessions extends
		MergeSessionsA<XnatImagesessiondataBean> {

	public MergePrearchiveSessions(Object control, final File srcDIR,
			final XnatImagesessiondataBean src, final String srcRootPath,
			final File destDIR, final XnatImagesessiondataBean existing,
			final String destRootPath, boolean allowSessionMerge,
			boolean overwriteFiles,
			SaveHandlerI<XnatImagesessiondataBean> saver, final UserI u) {
		super(control, srcDIR, src, srcRootPath, destDIR, existing,
				destRootPath, allowSessionMerge, overwriteFiles, saver, u, null);
	...
	public org.nrg.xnat.helpers.merge.MergeSessionsA.Results<XnatImagesessiondataBean> mergeSessions(
			final XnatImagesessiondataBean src, final String srcRootPath,
			final XnatImagesessiondataBean dest, final String destRootPath,
			final File rootbackup) throws ClientException, ServerException {
		if (dest == null)
			return new Results<XnatImagesessiondataBean>(src);

		final Results<XnatImagesessiondataBean> result = new Results<XnatImagesessiondataBean>(
				dest);
		...
		try {
			for (final XnatImagescandataI srcScan : srcScans) {
				...
				if (destScan == null) {
					...
					final List<XnatAbstractresourceI> srcRess = srcScan
							.getFile();
					final List<XnatAbstractresourceI> destRess = destScan
							.getFile();

					for (final XnatAbstractresourceI srcRes : srcRess) {
						final XnatAbstractresourceI destRes = MergeUtils
								.getMatchingResource(srcRes, destRess);
						if (destRes == null) {
