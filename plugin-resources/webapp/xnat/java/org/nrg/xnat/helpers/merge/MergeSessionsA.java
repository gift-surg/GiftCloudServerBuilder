...
public abstract class MergeSessionsA<A extends XnatImagesessiondataI> extends
		StatusProducer implements Callable<A> {
	...
	static org.apache.log4j.Logger logger = Logger
			.getLogger(MergeSessionsA.class);

	public MergeSessionsA(Object control, final File srcDIR, final A src,
			final String srcRootPath, final File destDIR, final A existing,
			final String destRootPath, boolean allowSessionMerge,
			boolean overwriteFiles, SaveHandlerI<A> saver, final UserI u,
			final EventMetaI c) {
		super(control);
		...
		if (destDIR.exists() || dest != null) {
			if (!allowSessionMerge) {
				...
				throw new ClientException(Status.CLIENT_ERROR_CONFLICT,
						HAS_FILES, new Exception());
			}

			if (!overwiteFiles) {
				if ((new SessionOverwriteCheck(src, dest,
						src.getPrearchivepath(), dest.getPrearchivepath(),
						user, c)).call()) {
					failed(CAT_ENTRY_MATCH);
					t = new ClientException(Status.CLIENT_ERROR_CONFLICT,
							CAT_ENTRY_MATCH, new IOException());
				}
			}
		}
			...
			try {
				if (FileUtils.FindFirstMatch(srcDIR, destDIR, new FileFilter() {
					public boolean accept(File pathname) {
						return !(pathname.getName().endsWith(".xml") || pathname
								.getName().endsWith(".log"));
					}
				}) != null) {
					failed(HAS_FILES);
					t = new ClientException(Status.CLIENT_ERROR_CONFLICT,
	...
	public A call() throws ClientException, ServerException {
		...
		rootBackup = createPrimaryBackupDirectory(this.getCacheBKDirName(),
				src.getProject(), destDIR.getName());
		if (destDIR.exists()) {
		...
		if (dest != null) {
			if (dest instanceof XnatImagesessiondata) {
				...
				// it looks like the XML write is invalidating the cached list
				// of scans stored within the Imagesessiondata object.
				// so the next thing that tries to access them (the merge)
				// doesn't find any,
				// this was exposed because we started accessing the scans
				// before this point (checkForConflict) which populated the
				// cached list of scans in the session object, which was then
				// invalidated by the sax write.
				// when we didn't access the scans prior to this line, then none
				// of this was necessary
				// as a temporary workaround, we'll generate the xml off of a
				// copy of the session.
				// a more permanent solution will probably be Hibernate related.
				A full_copy = (A) BaseElement
						.GetGeneratedItem((((XnatImagesessiondata) dest)
								.getCurrentDBVersion()));
		...
		// merge session xmls... nothing is modified until after file system is
		// merged.
		final Results<A> update = mergeSessions(src, srcRootPath, dest,
				destRootPath, rootBackup);
		...
		try {
			...
			// If we wrote to the src directory's catalogs, would the overwrite
			// persist them into the new space (overwriting the old ones).
			// What if the same catalog had two different catalog file names.
			// This would cause duplicate catalogs.
			// Could merge the catalogs based on label, delete the old one,
			// write the new one to the src space, then copy it all in.
			// DONE
	...
	private File backupDestDIR(File destDIR2, File rootBackup)
	...
	private File backupSourceDIR(File sourceDIR2, File rootBackup)
			throws ServerException {
	...
	private File createPrimaryBackupDirectory(String cacheBKDirName,
		...
		File f = org.nrg.xnat.utils.FileUtils.buildCachepath(project,
				cacheBKDirName, folderName);
		f.mkdirs();
		return f;
	}

	private void rollback(File backupDIR, File destDIR2, File rootBackup)
			throws ServerException {
	 ...
	 * @throws Exception
	 */
	public abstract void finalize(A session) throws ClientException,
			ServerException;

	public abstract String getCacheBKDirName();

	public abstract Results<A> mergeSessions(final A src,
			final String srcRootPath, final A dest, final String destRootPath,
			final File rootbackup) throws ClientException, ServerException;

	public MergeSessionsA.Results<File> mergeCatalogs(final String srcRootPath,
			final XnatResourcecatalogI srcRes, final String destRootPath,
			final XnatResourcecatalogI destRes) throws DCMEntryConflict,
			ServerException, Exception {
		final CatCatalogBean srcCat = CatalogUtils.getCleanCatalog(srcRootPath,
				(XnatResourcecatalogI) srcRes, false, user, c);

		// WARNING: this command will create a catalog if it doesn't already
		// exist
		final CatCatalogBean cat = CatalogUtils.getCleanCatalog(destRootPath,
				(XnatResourcecatalogI) destRes, false, user, c);

		MergeCatCatalog merge = new MergeCatCatalog(srcCat, cat,
				allowSessionMerge, c, CatalogUtils.getCatalogFile(destRootPath,
						(XnatResourcecatalogI) destRes));

		MergeSessionsA.Results<Boolean> r = merge.call();
		if (r.result != null && r.result) {
			try {
				// write merged destination file to src directory for merge
				// process to move
				CatalogUtils.writeCatalogToFile(cat, CatalogUtils
						.getCatalogFile(srcRootPath,
								(XnatResourcecatalogI) srcRes));

				return new MergeSessionsA.Results<File>(
						CatalogUtils.getCatalogFile(destRootPath, destRes), r);
			} catch (Exception e) {
				failed("Failed to update XML Specification document.");
				throw new ServerException(e.getMessage(), e);
			}
	...
	public void mergeDirectories(File srcDIR2, File destDIR2, boolean overwrite)
			...
			FileUtils.MoveDir(srcDIR2, destDIR2, overwrite, new FileFilter() {
				...
				public boolean handle(File f) {
					if (CatalogUtils.maintainFileHistory()) {
						try {
							FileUtils.MoveToHistory(f,
									EventUtils.getTimestamp(c));
