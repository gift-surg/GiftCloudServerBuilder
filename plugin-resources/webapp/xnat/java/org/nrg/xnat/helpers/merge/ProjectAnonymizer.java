...
public class ProjectAnonymizer extends AnonymizerA implements
		Callable<java.lang.Void> {
	 ...
	 * @param sessionPath
	 *            The root path of this project's session directory
	 */
	public ProjectAnonymizer(XnatImagesessiondataI s, String projectId,
			String sessionPath) {
		this.s = s;
		this.projectId = projectId;
		this.sessionPath = sessionPath;
		this.label = s.getLabel();
		this.path = DicomEdit.buildScriptPath(DicomEdit.ResourceScope.PROJECT,
				projectId);
		this.subjectLabel = null;
	}

	public ProjectAnonymizer(String label, XnatImagesessiondataI s,
			String projectId, String sessionPath) {
		this.s = s;
		this.projectId = projectId;
		this.sessionPath = sessionPath;
		this.label = label;
	...
	public ProjectAnonymizer(XnatImagesessiondataI s, String subjectLabel,
			String projectId, String sessionPath) {
		this.s = s;
		this.projectId = projectId;
		this.sessionPath = sessionPath;
	...
	String getSubject() {
		...
		if (s instanceof XnatImagesessiondata) {
			XnatSubjectdata d = ((XnatImagesessiondata) this.s)
					.getSubjectData();
			if (d != null) {
				label = d.getLabel();
			}
	...
	/**
	 * Retrieve a list of files that need to be anonymized. By default the files
	 * are retrieved from the project's archive space.
	 * 
	 * @return
	 */
	@Override
				...
				if (res instanceof XnatResource) {
					...
					if (abs.getFormat().equals("DICOM")) {
						for (final File f : abs
								.getCorrespondingFiles(this.sessionPath)) {
							ret.add(f);
						}
					}
				}
	...
	Configuration getScript() {
		return AnonUtils.getService().getScript(this.path,
				BaseXnatProjectdata.getProjectInfoIdFromStringId(projectId));
	}
