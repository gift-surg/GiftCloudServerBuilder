...
public class SessionOverwriteCheck implements Callable<Boolean> {
	...
	public SessionOverwriteCheck(XnatImagesessiondataI src,
			XnatImagesessiondataI dest, String srcRootPath,
			String destRootPath, UserI user, EventMetaI now) {
		this.src = src;
		this.dest = dest;
				...
				for (final XnatAbstractresourceI srcRes : srcRess) {
					...
					if (destRes == null) {
						...
						if (destRes instanceof XnatResourcecatalogI) {
							final CatCatalogBean srcCat = CatalogUtils
									.getCleanCatalog(srcRootPath,
											(XnatResourcecatalogI) srcRes,
											false, u, c);

							final CatCatalogBean destCat = CatalogUtils
									.getCleanCatalog(destRootPath,
	...
	private static boolean detectOverwrite(final CatCatalogI src,
			final CatCatalogI dest) {
		boolean merge = false;
		...
		for (final CatEntryI entry : src.getEntries_entry()) {
			if (entry instanceof CatDcmentryI
					&& !StringUtils.isEmpty(((CatDcmentryI) entry).getUid())) {
				final CatDcmentryI destEntry = CatalogUtils.getDCMEntryByUID(
