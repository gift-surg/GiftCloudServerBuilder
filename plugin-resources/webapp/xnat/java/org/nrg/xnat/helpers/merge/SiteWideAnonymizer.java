...
public class SiteWideAnonymizer extends AnonymizerA {
	...
	public List<File> getFilesToAnonymize() throws IOException {
		...
		if (located_in_prearchive) {
			PrearcSessionAnonymizer p = new PrearcSessionAnonymizer(s,
					s.getProject(), s.getPrearchivepath());
			fs.addAll(p.getFilesToAnonymize());
		} else {
			ProjectAnonymizer p = new ProjectAnonymizer(this.s,
					this.getProjectName(), s.getPrearchivepath());
			fs.addAll(p.getFilesToAnonymize());
		}
		return fs;
	}
