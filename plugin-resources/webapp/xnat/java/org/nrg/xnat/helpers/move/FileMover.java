...
public class FileMover {
	...
	public Boolean call(org.nrg.xnat.helpers.uri.URIManager.UserCacheURI src,
			ResourceURII dest, EventMetaI ci) throws Exception {
		File srcF;
		if (src.getProps().containsKey(UriParserUtils._REMAINDER)) {
			srcF = UserUtils.getUserCacheFile(user, (String) src.getProps()
					.get(URIManager.XNAME),
					(String) src.getProps().get(UriParserUtils._REMAINDER));
		} else {
			srcF = UserUtils.getUserCacheFile(user, (String) src.getProps()
					.get(URIManager.XNAME));
		}
		...
		this.buildResourceModifier(dest, overwrite, type, ci)
				.addFile(
						(List<? extends FileWriterWrapperI>) Lists.newArrayList(new StoredFile(
								srcF, overwrite)), label, type, filepath,
						this.buildResourceInfo(ci), overwrite);
	...
	public XnatResourceInfo buildResourceInfo(EventMetaI ci) {
		...
		return XnatResourceInfo.buildResourceInfo(description, format, content,
				tags, user, now, now, EventUtils.getEventId(ci));
	}

	protected ResourceModifierA buildResourceModifier(
			final ResourceURII arcURI, final boolean overwrite,
			final String type, final EventMetaI ci) throws ActionException {
		XnatImagesessiondata assessed = null;

		if (arcURI instanceof AssessedURII)
			assessed = ((AssessedURII) arcURI).getSession();
		...
		if (arcURI instanceof ReconURII) {
			...
			builder.setAssess((XnatImagesessiondata) assessed,
					((AssessorURII) arcURI).getAssessor(), type);
