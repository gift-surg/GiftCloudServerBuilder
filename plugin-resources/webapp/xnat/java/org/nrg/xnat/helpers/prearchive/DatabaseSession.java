...
public enum DatabaseSession {
	...
	STATUS("status", ColType.STATUS, false) {
		...
		@Override
		public String updateSessionSql(String sess, String timestamp,
				String proj, Object newVal) {
			String s = "UPDATE "
					+ PrearcDatabase.tableWithSchema
					+ " SET "
					+ this.searchSql(newVal)
					+ ", "
					+ DatabaseSession.LASTMOD.searchSql(Calendar.getInstance()
							.getTime()) + " WHERE "
	...
	private enum ColType {
		PROJECTNAME() {
			...
			public <T extends Object> T getFromResult(int columnIndex,
		...
		STATUS() {
			...
			public void setInsertStatement(int columnIndex,
					PreparedStatement s, Object o) throws SQLException {
				if (o != null) {
					s.setString(columnIndex,
							((PrearcUtils.PrearcStatus) o).name());
				} else {
			...
			public <T extends Object> T getFromResult(int columnIndex,
					ResultSet r) throws SQLException {
				return (T) PrearcUtils.PrearcStatus.valueOf(r
						.getString(columnIndex));
			}

			@Override
			public String typeToString(Object o) {
				return null == o ? null : DatabaseSession
						.singleQuote(((PrearcUtils.PrearcStatus) o).name());
			}
		...
		VARCHAR() {
			...
			@Override
			public String typeToString(Object o) {
				return null == o ? null : DatabaseSession.singleQuote(o
						.toString());
			}
		...
		TIMESTAMP() {
			...
			@Override
			public String typeToString(Object o) {
				return null == o ? null : DatabaseSession.singleQuote(this
						.date2Timestamp(o).toString());
			}
		 ...
		 * @throws SQLException
		 */
		public abstract void setInsertStatement(int columnIndex,
				PreparedStatement s, Object o) throws SQLException;
		 ...
		 * @throws SQLException
		 */
		public abstract <T extends Object> T getFromResult(int columnIndex,
				ResultSet r) throws SQLException;

		/**
		 * Extract this columns data from the given row and convert it to a
		 * string.
		 * 
		 * @param columnIndex
		 * @param r
		 ...
		 * @throws SQLException
		 */
		public abstract String resultToString(int columnIndex, ResultSet r)
				throws SQLException;

		/**
		 * Convert this columns data to a string. Returns null if the given
		 * object is null.
		 * 
		 * @param o
		 * @return
	 ...
	 * 
	 * @param columnName
	 *            The name of the column (no spaces)
	 * @param columnType
	 *            The kind of value this column can hold
	 * @param canBeNull
	 *            Whether this column can hold a null value
	 */
	private DatabaseSession(String columnName,
			DatabaseSession.ColType columnType, boolean canBeNull) {
	...
	/**
	 * Generate the <column=value> SQL eg.
	...
	/**
	 * Generate the "column IS NULL" in eg.
	...
	/**
	 * In Postgres setting a column to NULL uses "=". Hope this is somewhat
	 * portable.
	 * 
	 * @return
	 */
	...
	/**
	 * Generate the <column selection> SQL in eg. "SELECT ... FROM ... WHERE
	 * <column selection>. The value to select on is pulled directly from the
	...
	/**
	 * Generate the <column selection> SQL eg. "SELECT ... FROM ... WHERE
	 * <column selection>
	 * 
	 * @param value
	 *            value of column
	...
	/**
	 * Generate <column selection> SQL for all columns eg.
	 * "SELECT ... FROM ... WHERE <column selection> operator <column selection> operator ..."
	 * 
	 * @param s
	 *            Session object containing values
	 * @param operator
	 *            boolean SQL operator eg. "AND", "OR" etc
	 * @return
	 */
	 ...
	 * 
	 * @param values
	 *            list of values for this column
	 * @return
	 */
	...
	public String updateSessionSql(String sess, String timestamp, String proj,
			Object newVal) {
	...
	public String findSql(String sess, String timestamp, String proj) {
		return "SELECT " + this.columnName + " FROM "
				+ PrearcDatabase.tableWithSchema + " WHERE "
				+ DatabaseSession.sessionSql(sess, timestamp, proj);
	}

	public String findSql(final Object o) {
		return "SELECT * FROM " + PrearcDatabase.tableWithSchema + " WHERE "
				+ this.searchSql(o);
	}
	...
	public static String findSessionSql(SessionData s) {
		return "SELECT * FROM " + PrearcDatabase.tableWithSchema + " WHERE "
				+ DatabaseSession.searchSql(s, " AND ");
	}

	/**
	 * Take a list of SQL (column = 'value') constraints and return SQL with an
	 * appended a "SELECT ... WHERE " statement.
	 * 
	 * If there are no constraints a SQL statement that gets all the rows will
	 * be generated.
	 * 
	 * @param sql
	 *            An array of strings containing SQL constraints
	 * @return A complete SQL statement constructed from the submitted
	 *         constraints.
	 */
	...
	/**
	 * Generate SQL to find a row where that matches the given session and
	 * project
	 * 
	 * @param session
	 *            The session ID on which to search
	 * @param timestamp
	 *            The timestamp on which to search
	 * @param project
	 *            The project ID on which to search
	 * @return A complete SQL statement constructed from the submitted criteria.
	 */
	public static String findSessionSql(String session, String timestamp,
			String project) {
		return "SELECT * FROM " + PrearcDatabase.tableWithSchema + " WHERE "
				+ DatabaseSession.sessionSql(session, timestamp, project);
	}
	 ...
	 * @return A complete SQL statement constructed from the submitted criteria.
	 */
	public static String countSessionSql(String session, String timestamp,
			String project) {
	...
	public static String countSessionSql(final String sess,
			final String timestamp, final String proj, final String suid) {
		String s = "SELECT COUNT(*) FROM " + PrearcDatabase.tableWithSchema
	...
	/**
	 * Generate SQL that deletes a row that matches the given session and
	 * project
	 * 
	 * @param sess
	 * @param proj
	 * @return
	 */
	public static String deleteSessionSql(String sess, String timestamp,
			String proj) {
		return "DELETE FROM " + PrearcDatabase.tableWithSchema + " WHERE "
				+ DatabaseSession.sessionSql(sess, timestamp, proj);
	}

	/**
	 * Generates SQL that deletes all rows that do not match any of the given
	 * timestamps
	 * 
	...
	public static String deleteUnusedSessionsSql(String usedSessionTimestamps) {
		return "DELETE FROM " + PrearcDatabase.tableWithSchema
				+ " WHERE timestamp NOT IN (" + usedSessionTimestamps + ")";
	}

	/**
	 * Generate SQL that queries any rows matching the given names. This query
	 * will fail if this column is not a string.
	 * 
	 * @param names
	 * @return
	 */
	...
	/**
	 * Extract the value which corresponds to this column from the given session
	 * and insert it into the given PreparedStatement.
	 * 
	 * @param st
	 * @param s
	 * @throws SQLException
	 */
	public void setInsertStatement(PreparedStatement st, SessionData s)
			throws SQLException {
		this.columnType.setInsertStatement(this.ordinal() + 1, st,
				this.readSession(s));
	}
	...
	/**
	 * Generates SQL to be used to update the prearchive status without any
	 ...
	 * @return
	 */
	public static String updateSessionStatusSQL(String sess, String timestamp,
			String proj, PrearcUtils.PrearcStatus status) {
		return "UPDATE "
				+ PrearcDatabase.tableWithSchema
				+ " SET "
				+ DatabaseSession.STATUS.updateSql(status)
				+ ", "
				+ DatabaseSession.LASTMOD.updateSql(Calendar.getInstance()
						.getTime()) + " WHERE "
				+ DatabaseSession.sessionSql(sess, timestamp, proj);
	}

	/**
	 * Generates SQL to be used to update the prearchive last modified time
	 * without any unnecessary SELECT statements.
	 * 
	 * @param sess
	 * @param timestamp
	 * @param proj
	 * @return
	 */
	public static String updateSessionLastModSQL(String sess, String timestamp,
			String proj) {
		return "UPDATE "
				+ PrearcDatabase.tableWithSchema
				+ " SET "
