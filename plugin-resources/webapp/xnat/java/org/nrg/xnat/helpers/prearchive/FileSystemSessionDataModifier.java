...
public class FileSystemSessionDataModifier implements SessionDataModifierI {
	static Logger logger = Logger
			.getLogger(FileSystemSessionDataModifier.class);
	...
	/**
	 * Represents each step of the operation can either be run or rolled back in
	 * case of exception.
	 * 
	 ...
	 *
	 * @param <T>
	 *            the type of value returned from a running the op.
	 */
	...
	static class Move {
		...
		/**
		 * Each step of the move is a Transaction that can be rolled back. It is
		 * expected that the steps are executed in the order in which they
		 * appear below because each step depends on the success of the previous
		 * one. So an error in 'setXml' step for example invokes a rollback on
		 * the 'copy' step.
		 */
		...
		class SetXml implements Transaction<XnatImagesessiondataBean> {
			...
			public XnatImagesessiondataBean run() throws SyncFailedException {
				XnatImagesessiondataBean doc = null;
				try {
					XnatProjectdata xpd = XnatProjectdata
							.getXnatProjectdatasById(newProj, null, false);
					Long projectId = DicomEdit.getDBId(xpd);
					Configuration c = AnonUtils.getService().getScript(
							DicomEdit.buildScriptPath(
									DicomEdit.ResourceScope.PROJECT, newProj),
							projectId);
					if (c != null) {
						final String anonScript = c.getContents();
						final ScriptApplicator scriptapplicator = new ScriptApplicator(
								new ByteArrayInputStream(
										anonScript.getBytes("UTF-8")));
						final XnatAttrDef[] params = { new XnatAttrDef.Constant(
								"project", newProj) };
						final DICOMSessionBuilder db = new DICOMSessionBuilder(
								f, params,
								new Function<DicomObject, DicomObject>() {
									public DicomObject apply(final DicomObject o) {
										try {
											Anonymize.anonymize(o, newProj,
													subject, sess,
													scriptapplicator);
		...
		public Move(String basePath, final String sess, String uri,
				String subject, final String newProj) {
			super();
			this.basePath = basePath;
			this.sess = sess;
			this.uri = uri;
		...
		void run() throws SyncFailedException {
			...
			if (f.exists()) {
				logger.warn("moved session " + sess + " to " + newProj
						+ ", but unable to delete original directory.");
			}
			// timestamp directory might contain another session, so no warning
			// if deletion fails.
	...
	public void move(final SessionData sd, final String newProj)
			throws SyncFailedException {
		this._move(new Move(this.basePath, sd.getFolderName(), sd.getUrl(), sd
				.getSubject(), newProj) {
		});
	}
	...
	public void delete(SessionData sd) {
		...
		if (!FileUtils.HasFiles(tsdir)) {
			FileUtils.deleteDirQuietly(tsdir); // delete timestamp parent only
												// if empty.
		}
	}
