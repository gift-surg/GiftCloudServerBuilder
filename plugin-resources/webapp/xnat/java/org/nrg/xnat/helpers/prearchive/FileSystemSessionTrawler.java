...
public final class FileSystemSessionTrawler implements SessionDataProducerI {
	private final Logger logger = LoggerFactory
			.getLogger(FileSystemSessionTrawler.class);
	...
	static FileFilter hiddenAndDatabaseFileFilter = new FileFilter() {
		public boolean accept(File pathname) {
			return !hidden.contains(pathname.getName())
					&& doesNotContainDot.accept(pathname)
	...
	public Collection<SessionData> get() throws IllegalStateException {
		...
		File[] files = new File(this.prearcPath)
				.listFiles(hiddenAndDatabaseFileFilter);
		...
		if (files != null) {
			for (final File tsdir : files) {
				...
				if (!invalidProjects.contains(projectId)) {
					try {
						sessions = new PrearcTableBuilder()
								.getPrearcSessions(tsdir);
					} catch (IOException e) {
						logger.error(
								"Error getting prearchive sessions from the filesystem",
								e);
					...
					for (final Collection<PrearcTableBuilder.Session> ss : sessions
							.values()) {
						for (PrearcTableBuilder.Session s : ss) {
							SessionData _s = s.getSessionData(StringUtils
									.join(new String[] { PrearcDatabase
											.projectPath(s.getProject()) }));
							sds.add(_s);
						}
					}
				}
		...
		if (invalidProjects.size() > 0) {
			...
			final String intro = "During a prearchive rebuild operation, the system found sessions in the prearchive storage ("
					+ prearcPath
					+ ") that are associated with invalid project IDs: ";
			AdminUtils.sendAdminEmail("Invalid projects in the prearchive",
					"<p>" + intro + "</p><ul><li>"
							+ buffer.toString().replaceAll(", ", "</li><li>")
							+ "</li></ul>");
