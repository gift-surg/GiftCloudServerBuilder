...
public final class PrearcDatabase {
        ...
        new Thread() {
            public void run() {
                ...
                while(i.hasNext()){
                    ...
                    try {
                        PrearcDatabase._archive(_s,allowDataDeletion,overwrite,user,listeners,true);
                    } catch (SyncFailedException e) {
                        logger.error("",e);
                    }
                }		
    ...
    public static String archive (PrearcSession session, boolean allowDataDeletion, boolean overwrite, XDATUser user, Set<StatusListenerI> listeners) throws SyncFailedException {
    	return PrearcDatabase._archive(session, allowDataDeletion, overwrite, user, listeners,false);
    }

    private static String _archive (PrearcSession session, boolean allowDataDeletion, boolean overwrite, XDATUser user, Set<StatusListenerI> listeners, boolean waitFor) throws SyncFailedException {
        final PrearcSessionArchiver archiver;
        try {
            archiver = Archiver.buildArchiver(session, allowDataDeletion, overwrite, user, waitFor);
        }catch (Exception e1) {
