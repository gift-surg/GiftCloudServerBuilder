...
public final class PrearcUriParserUtils {
	...
	/**
	 * A uri parser that reads prearchive uri that requests an action on
	 * multiple projects.
	 * 
	 * @author aditya
	 *
	...
	static class ProjectsParser implements UriParserI<List<String>> {
		...
		/**
		 * Read multiple projects and throw an error if the session label or
		 * timestamp are specified.
		 */
		public List<String> readUri(String uri)
			...
			if (null == so.get("PROJECT_ID")) {
				throw new java.util.MissingFormatArgumentException(
						"No projects specified in uri " + uri);
			}
			Reference ref = new Reference(uri);
			List<String> segs = ref.getSegments();
			if (segs.size() > 3) {
				throw new java.util.MissingFormatArgumentException(
						"More in the uri past the project_id");
			}

			List<String> ls = new ArrayList<String>();
			String[] sa = StringUtils.split((String) so.get("PROJECT_ID"), ',');
			for (int i = 0; i < sa.length; i++) {
				if (StringUtils.upperCase(sa[i]).equals(
						StringUtils.upperCase(PrearcUtils.COMMON))) {
					ls.add(null);
				} else {
					ls.add(sa[i]);
				}
	...
	static class SessionParser implements UriParserI<Map<String, String>> {
		...
		/**
		 * Read a single session from the uri. If the project name field
		 * contains more than one project an error is thrown.
		 */
		public Map<String, String> readUri(String uri)
			...
			if (null == so.get("PROJECT_ID")) {
				throw new java.util.MissingFormatArgumentException(
						"Unable to parse PROJECT_ID using template from uri: "
								+ uri);
			}

			if (null == so.get("SESSION_LABEL")) {
				throw new java.util.MissingFormatArgumentException(
						"Unable to parse SESSION_LABEL using template from uri: "
								+ uri);
			}
			if (null == so.get("SESSION_TIMESTAMP")) {
				throw new java.util.MissingFormatArgumentException(
						"Unable to parse TIMESTAMP using template from uri: "
								+ uri);
			}
			...
			if (segs.size() > 5) {
				throw new java.util.MissingFormatArgumentException(
						"More in the uri past the TIMESTAMP");
			}
			...
			while (i.hasNext()) {
				...
				if (key.equals("PROJECT_ID")) {
					if (StringUtils.upperCase((String) so.get(key)).equals(
							StringUtils.upperCase(PrearcUtils.COMMON))) {
						ss.put(key, null);
					} else {
	...
	static class UriParser implements UriParserI<Map<String, Object>> {
		...
		/**
		 * Parse the uri with the given template. No errors are thrown for
		 * fields, instead they are set to null. Users of this method beware.
		 * 
		 * It also parses the attributes and stores them in locally.
		 * 
		 */
		public Map<String, Object> readUri(String uri) {
			Template t = new Template(template, Template.MODE_STARTS_WITH,
