...
public class SessionXMLRebuilderJob implements JobInterface {
	...
	public void execute() throws JobExecutionException {
		final Provider<XDATUser> provider = (Provider<XDATUser>) _map
				.get("user");
		...
		try {
			...
			// Swallow this message so it doesn't fill the logs before the
			// prearchive is initialized.
			if (!e.getMessage().contains(
					"relation \"xdat_search.prearchive\" does not exist")) {
				logger.error("", e);
			}
		} catch (Exception e) {
			logger.error("", e);
		...
		if (sds != null && sds.size() > 0) {
			for (final SessionData sessionData : sds) {
				total++;
				if (sessionData.getStatus().equals(
						PrearcUtils.PrearcStatus.RECEIVING)
						&& !sessionData.getPreventAutoCommit()
						&& !StringUtils.trimToEmpty(sessionData.getSource())
								.equals("applet")) {
					...
					try {
						sessionDir = PrearcUtils.getPrearcSessionDir(user,
								...
								sessionData.getFolderName(), false);
					} catch (IOException e) {
						logger.error("", e);
					...
					if (diff >= interval
							&& !PrearcUtils.isSessionReceiving(sessionData
									.getSessionDataTriple())) {
						updated++;
						try {
							if (PrearcDatabase.setStatus(
									sessionData.getFolderName(),
									sessionData.getTimestamp(),
									sessionData.getProject(),
									PrearcUtils.PrearcStatus.QUEUED_BUILDING)) {
								logger.debug(
										"Creating JMS queue entry for {} to archive {}",
										user.getUsername(),
						...
						logger.error(String
								.format("Prearchive session locked for an abnormally large time within CACHE_DIR/prearc_locks/%1$s/%2$s/%3$s",
										sessionData.getProject(),
										sessionData.getTimestamp(),
										sessionData.getName()));
					}
	...
	private Logger logger = LoggerFactory
			.getLogger(SessionXMLRebuilderJob.class);
