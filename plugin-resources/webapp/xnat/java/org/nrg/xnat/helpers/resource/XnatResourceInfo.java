...
public class XnatResourceInfo implements Serializable {
	...
	public static XnatResourceInfo buildResourceInfo(final String description,
			final String format, final String content, final String[] tags,
			final UserI user, Date created, Date modified, Number i) {
			...
			for (String tag : tags) {
				...
				if (!tag.equals("")) {
					for (String s : StringUtils
						...
						if (!s.equals("")) {
							if (s.indexOf("=") > -1) {
								info.addMeta(s.substring(0, s.indexOf("=")),
