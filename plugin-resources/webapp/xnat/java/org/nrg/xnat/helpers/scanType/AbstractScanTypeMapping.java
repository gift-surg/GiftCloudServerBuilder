...
public abstract class AbstractScanTypeMapping<HistoryType> implements
		ScanTypeMappingI {
	private final Logger logger = LoggerFactory
			.getLogger(AbstractScanTypeMapping.class);
	 ...
	 * 
	 * @param dbName
	 *            name of the database in which scan type mapping is stored
	 * @param scanSelectSql
	 *            SQL statement to extract scan type history
	 */
	public AbstractScanTypeMapping(final String projectId, final String dbName,
			final String scanSelectSql) {
		this.dbName = dbName;
		this.scanSelectSql = scanSelectSql;
		XnatProjectdata project = XnatProjectdata.getProjectByIDorAlias(
				projectId, null, false);
	...
	private void loadScanTypes() {
		...
		if (!Strings.isNullOrEmpty(scanSelectSql)) {
			try {
				final XFTTable t = XFTTable.Execute(scanSelectSql, dbName,
						"system");
				t.resetRowCursor();
				while (t.hasMoreRows()) {
					final Hashtable<?, ?> rowHash = t.nextRowHash();
					final String sd = (String) rowHash
	 ...
	 * 
	 * @see
	 * org.nrg.xnat.helpers.scanType.ScanTypeMappingI#setType(org.nrg.xdat.model
	 * .XnatImagescandataI)
	 */
		...
		if (Strings.isNullOrEmpty(scan.getType())) {
			...
			if (Strings.isNullOrEmpty(seriesDescription)) {
				...
				if (Strings.isNullOrEmpty(seriesDescription)) {
					...
					scan.setType(Strings.isNullOrEmpty(mappedType) ? seriesDescription
							: mappedType);
				}
			}
		}
	...
	/**
	 * For the given scan, with the provided scan type histories, return the
	 * mapped scan type.
	 * 
	 * @param scan
	 * @param histories
	 ...
	 *            series description as given in image metadata
	 * @return standardized form; if originalString is null, returns the empty
	 *         string.
	 */
