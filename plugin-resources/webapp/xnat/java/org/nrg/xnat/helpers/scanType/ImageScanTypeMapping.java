...
public class ImageScanTypeMapping extends
		AbstractScanTypeMapping<ImageScanTypeMapping.ScanTypeHistory> implements
		ScanTypeMappingI {
	...
	private static final String buildSql(final String project) {
		if (Strings.isNullOrEmpty(project)) {
			...
			return "SELECT DISTINCT REPLACE(REPLACE(REPLACE(REPLACE(UPPER(scan.series_description),' ',''),'_',''),'-',''),'*','') AS series_description,scan.type,frames FROM xnat_imagescandata scan LEFT JOIN xnat_experimentData isd ON scan.image_session_id=isd.id WHERE scan.series_description IS NOT NULL AND isd.project='"
					+ project + "';";
		}
	}
