...
public final class UriParserUtils {
	...
	public static URIManager.DataURIA parseURI(String s)
		...
		if (s.startsWith("/prearchive")) {
			...
			for (final URIManager.TemplateInfo template : URIManager
					.getTemplates(URIManager.TEMPLATE_TYPE.PREARC)) {
				Map<String, Object> map = new UriParser(template.key,
						template.MODE).readUri(s);
				if (map.size() > 0) {
			...
			for (final URIManager.TemplateInfo template : URIManager
					.getTemplates(URIManager.TEMPLATE_TYPE.ARC)) {
				Map<String, Object> map = new UriParser(template.key,
						template.MODE).readUri(s);
				if (map.size() > 0) {
					return template.wrap(map, s);
			...
			for (final URIManager.TemplateInfo template : URIManager
					.getTemplates(URIManager.TEMPLATE_TYPE.CACHE)) {
				Map<String, Object> map = new UriParser(template.key,
						template.MODE).readUri(s);
				if (map.size() > 0) {
					return template.wrap(map, s);
	...
	public static class UriParser implements UriParserI<Map<String, Object>> {
		 ...
		 * fields, instead they are set to null. Users of this object beware.
		 */
		public Map<String, Object> readUri(String uri) {
			final Template t = new Template(template, mode,
