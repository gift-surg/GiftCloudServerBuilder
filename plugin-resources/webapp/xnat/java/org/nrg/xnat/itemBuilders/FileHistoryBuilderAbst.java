...
public abstract class FileHistoryBuilderAbst {
	...
	public void modify(XFTItem i, FlattenedItemA.HistoryConfigI includeHistory,
		...
		if (root.instanceOf(XnatResourcecatalog.SCHEMA_ELEMENT_NAME)) {
			...
			if ((!StringUtils.isEmpty(uri)) && FileUtils.IsAbsolutePath(uri)) {
				...
				fi.addChildCollection("system.file", "file",
						getFiles(uri, idGenerator, local));

			}
		}
	}

	private List<FlattenedItemI> getFiles(String uri,
		...
		if (!found.containsKey(uri)) {
			...
			if (catFile.exists()) {
				try {
					files.addAll(handleCatFile(catFile, false, idGenerator,
							parents));
				} catch (Exception e) {
					logger.error("", e);
				}
	...
	public abstract List<FlattenedItemI> handleCatFile(File catFile,
			boolean isHistory, Callable<Integer> idGenerator,
