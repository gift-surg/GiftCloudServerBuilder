...
public class WorkflowBasedHistoryBuilder implements
		Callable<Map<Number, WorkflowView>> {
	...
	public WorkflowBasedHistoryBuilder(ItemI i, String id, XDATUser user,
			boolean includeFiles, boolean includeDetails) {
		this.i = i;
		this.id = id;
		this.user = user;
	...
	public Map<Number, WorkflowView> call() throws Exception {
		if (result.size() == 0) {
			...
			if (includeDetails) {
				...
				final List<FlattenedItemI> items = Arrays.asList(ItemMerger
						.merge(ItemPropBuilder.build(i.getItem(),
								FlattenedItemA.GET_ALL, modifiers)));

				final EventBuilderI builder;
				if (!includeFiles) {
					builder = new DefaultBuilder() {
						public List<ItemEventI> build(ItemObject io,
								String action, FieldEventI fe, String username,
								List<ItemObject> parents, FlattenedItemI fi) {
							List<ItemEventI> items = Lists.newArrayList();
							boolean _continue = true;
							if (io.getXsiTypes().contains(
									XnatAbstractresource.SCHEMA_ELEMENT_NAME)) {
								ItemObject new_io = new ItemObject("files", "",
										"", Arrays.asList("system:file"));
								if (action
										.equals(ChangeSummaryBuilderA.MODIFIED)
										&& fe != null
										&& fe.getXsiPath().equals(FILE_COUNT)) {
									...
									if (_new > _old) {
										...
										.equals(ChangeSummaryBuilderA.MODIFIED)) {
									int _new = convertToInt(fi.getFields()
											.getParams().get(FILE_COUNT));
									if (_new > 0) {
										items.add(new FileEvent(new_io, action,
												fe, parents, username, (_new)));
									}
								}
							}

							if (_continue) {
								items.addAll(super.build(io, action, fe,
										username, parents, fi));
							}
				...
				changesByDate = (new DateBasedSummaryBuilder(builder))
						.call(items);
			} else {
				changesByDate = Maps.newHashMap();
			}

			final Collection<? extends PersistentWorkflowI> workflows = getWorkflows(
					i, id, user);
	...
	private void addCS(Map<Number, WorkflowView> current, ChangeSummary cs) {
		...
		for (WorkflowView wv : current.values()) {
			...
			Number n2 = (wv.getWorkflow() == null) ? null : wv.getWorkflow()
					.getWorkflowId();
		...
		if (!matched) {
			// query for workflow
			if (cs.getDate() != null
					&& !cs.getNumber().equals(cs.getDate().getTime())) {
	...
	public PersistentWorkflowI getWorkflowById(Number n) {
		if (!queried.contains(n)) {
			queried.add(n);
			return WrkWorkflowdata.getWrkWorkflowdatasByWrkWorkflowdataId(n,
					user, false);
		}
	...
	public static Collection<? extends PersistentWorkflowI> getWorkflows(
			ItemI i, String id, XDATUser user) {
		...
		try {
			if (i.getItem().instanceOf("xnat:projectData")) {
				try {
					...
					wrks.addAll(WrkWorkflowdata.getWrkWorkflowdatasByField(cc,
							user, false));
				} catch (Exception e) {
					logger.error("", e);
				}
				...
				if (i.getItem().instanceOf("xnat:imageSessionData")) {
					try {
						List<List> expts = XFTTable
								.Execute(
										String.format(
												"SELECT DISTINCT id FROM (SELECT iad.id FROM xnat_imageassessordata iad WHERE iad.id IS NOT NULL AND iad.imagesession_id='%1$s' UNION SELECT iad.id FROM xnat_imageassessordata_history iad WHERE iad.id IS NOT NULL AND iad.imagesession_id='%1$s') SRCH;",
												id), i.getDBName(),
										user.getLogin()).toArrayListOfLists();
						for (List expt : expts) {
								...
								.Execute(
										String.format(
												""
														...
														+ "SELECT iad.id FROM xnat_subjectassessordata_history sad LEFT JOIN xnat_imageassessordata_history iad ON sad.id=iad.imagesession_id WHERE iad.id IS NOT NULL AND subject_id='%1$s') SRCH;",
												id), i.getDBName(),
										user.getLogin()).toArrayListOfLists();
						for (List expt : expts) {
							ids.add((String) expt.get(0));
	...
	public class WorkflowView {
		...
		public Date getDate() throws XFTInitException,
		...
		public String getUsername() throws XFTInitException,
				ElementNotFoundException, FieldNotFoundException,
				ParseException {
			if (workflow != null) {
				return workflow.getUsername();
			} else {
				return getChangeSummaries().get(0).getEvents().get(0)
						.getUsername();
			}
		}
		...
		public Map<String, Map<String, ChangeSummaryGroup>> buildSummary() {
			Map<String, Map<String, ChangeSummaryGroup>> sum = Maps
					.newHashMap();
			for (ChangeSummary cs : getChangeSummaries()) {
				for (Map.Entry<String, Map<String, ChangeSummaryGroup>> outer : cs
						.getSummary().entrySet()) {
					for (Map.Entry<String, ChangeSummaryGroup> inner : outer
						...
						if (!sum.containsKey(dataType)) {
							Map<String, ChangeSummaryGroup> temp = Maps
									.newHashMap();
							sum.put(dataType, temp);
						}

						if (!sum.get(dataType).containsKey(action)) {
							sum.get(dataType).put(action,
									new ChangeSummaryGroup(dataType, action));
						}
	...
	public JSONObject toJSON(String dateFormat) throws Exception {
		...
		for (Map.Entry<Number, WorkflowBasedHistoryBuilder.WorkflowView> entry : changes
				.entrySet()) {
			...
			if (entry.getValue().getDate() != null) {
				if (dateFormat == null) {
					...
					o.put("event_date", DateUtils.format(entry.getValue()
							.getDate(), dateFormat));
				}
			}
			...
			if (entry.getValue().getWorkflow() != null) {
				...
				o.put("event_reason", entry.getValue().getWorkflow()
						.getJustification());
				o.put("event_category", entry.getValue().getWorkflow()
						.getCategory());
				o.put("event_status", entry.getValue().getWorkflow()
						.getStatus());
			}

			JSONArray a = new JSONArray();
			o.put("changesets", a);
			for (ChangeSummaryBuilderA.ChangeSummary cs : entry.getValue()
					.getChangeSummaries()) {
