...
public class NotifyProjectListeners implements Callable<Boolean> {
	...
	public NotifyProjectListeners(XnatExperimentdata expt, String template,
			String subject, UserI user, Map params, String action,
			List<String> emails, ProjectListenersI listeners) {
		...
		this._action = action;
		this._emails = emails;
		this.listenersBuilder = (listeners != null) ? listeners
				: new ResourceBasedProjectListeners();
	}
	...
	public static interface ProjectListenersI {
		public List<String> call(String action, XnatProjectdata project,
				XnatExperimentdata expt);
	}

	@Override
	public Boolean call() throws Exception {
		try {
			List<String> email = listenersBuilder.call(_action,
					_expt.getProjectData(), _expt);
			...
			if (email.size() > 0) {
				...
				XDAT.getMailService().sendHtmlMessage(
						from,
						email.toArray(new String[email.size()]),
						TurbineUtils.GetSystemName() + " update: "
								+ _expt.getLabel() + " " + _subject, body);
				return true;
			} else {
				return false;
			}
	...
	public static class ResourceBasedProjectListeners implements
			ProjectListenersI {
		public List<String> call(final String action,
				final XnatProjectdata project, final XnatExperimentdata expt) {
			final String fileName = action;

			List<String> names = Lists.newArrayList();
			try {
				names.add(expt.getItem().getGenericSchemaElement().getSQLName()
						+ "_" + fileName);
				names.add(expt.getItem().getGenericSchemaElement().getXSIType()
						+ "_" + fileName);
				names.add(fileName);
			...
			for (XnatAbstractresourceI r : project.getResources_resource()) {
				if (r instanceof XnatResourcecatalog
						&& NOTIFICATIONS.equals(r.getLabel())) {
			...
			if (res != null) {
				final CatCatalogBean cat = CatalogUtils.getCatalog(
						project.getRootArchivePath(), res);
				final File catalog_xml = CatalogUtils.getCatalogFile(
						project.getRootArchivePath(), res);
				...
				if (cat != null && catalog_xml != null) {
					...
					if (entry != null) {
						matchedFile = CatalogUtils.getFile(entry,
								catalog_xml.getParent());
					}
				}
			}
