...
public abstract class ChangeSummaryBuilderA extends ItemHistoryBuilder {
	...
	public static class ChangeSummary {
		...
		public int getDominantChangeType() {
			int level = 0;
			for (Map.Entry<String, Map<String, ChangeSummaryGroup>> entry : getSummary()
					.entrySet()) {
				if (entry.getValue().containsKey(ChangeSummaryBuilderA.REMOVED)) {
					return 2;
				} else if (entry.getValue().containsKey(
						ChangeSummaryBuilderA.MODIFIED)) {
					level = 1;
				}
			}
			return level;
		...
		public static String toString(
				Map<String, Map<String, ChangeSummaryGroup>> map) {
			StringBuilder sb = new StringBuilder();
			int counter = 0;
			for (Map.Entry<String, Map<String, ChangeSummaryGroup>> outer : map
					.entrySet()) {
				for (Map.Entry<String, ChangeSummaryGroup> inner : outer
						.getValue().entrySet()) {
					if (counter++ > 0)
						sb.append(", ");
					sb.append(inner.getKey()).append(" ")
							.append(inner.getValue().getCount()).append(" ")
							.append(outer.getKey());
			...
			if (summary == null) {
				...
				for (ItemEventI e : getEvents()) {
					if (!summary.containsKey(e.getObjectHeader())) {
						summary.put(e.getObjectHeader(),
								new Hashtable<String, ChangeSummaryGroup>());
					}

					if (!summary.get(e.getObjectHeader()).containsKey(
							e.getAction())) {
						summary.get(e.getObjectHeader()).put(
								e.getAction(),
								new ChangeSummaryGroup(e.getObjectHeader(), e
										.getAction()));
					}

					summary.get(e.getObjectHeader())
							.get(e.getAction())
							.increment(
									(e instanceof FileEvent) ? ((FileEvent) e)
											.getDiff() : 1);
				}
			}
			return summary;
	...
	public Map<Date, ChangeSummary> call(List<FlattenedItemI> items)
			throws Exception {
		...
		if (fi.getHistory().size() > 0) {
			...
			Collections.sort(grouped, new Comparator<FlattenedItemI>() {
				public int compare(FlattenedItemI o1, FlattenedItemI o2) {
					return DateUtils.compare(o1.getStartDate(),
							o2.getStartDate());
				}
			});
		}
	...
	protected void addMessage(Map<Date, ChangeSummary> sb, Date date, Number n,
			ItemEventI s) {
		if (date != null) {
	...
	protected boolean contains(Map<Date, ChangeSummary> sb, Date d,
			String action, FlattenedItemA.ItemObject io) {
	...
	public void renderChildren(
			Map<String, FlattenedItemA.ChildCollection> items,
			Map<Date, ChangeSummary> sb, Integer parent) throws Exception {
		for (Map.Entry<String, FlattenedItemA.ChildCollection> entry : items
				.entrySet()) {
	...
	protected Date getRelevantDate(FlattenedItemI params, String action) {
		if (ADDED.equals(action)) {
			...
			return (params.getEndDate() == null) ? params.getStartDate()
					: params.getEndDate();
		} else {
	...
	protected void registerEvent(Map<Date, ChangeSummary> sb, ItemObject io,
			Number change_id, String action, FieldEventI fe, String username,
			Date d, List<ItemObject> parents, FlattenedItemI fi) {
		registerEvent(sb, io, change_id, d,
				builder.build(io, action, fe, username, parents, fi));
	}

	protected void registerEvent(Map<Date, ChangeSummary> sb, ItemObject io,
			Number change_id, Date d, List<ItemEventI> ies) {
		for (ItemEventI ie : ies) {
			if (MODIFIED.equals(ie.getAction())
					|| !contains(sb, d, ie.getAction(), io)) {
				addMessage(sb, d, change_id, ie);
			}
		}
	}

	protected void registerEvent(Map<Date, ChangeSummary> sb,
			FlattenedItemI params, FileSummary fs) {
		FlattenedItemA.ItemObject io = new FlattenedItemA.ItemObject("file",
				null, null, Arrays.asList("system:file".intern()));
		List<FlattenedItemA.ItemObject> local = new ArrayList<FlattenedItemA.ItemObject>();
		local.addAll(params.getParents());
		local.add(io);
		for (ItemEventI ie : builder.build(io, fs.action, null, "", local,
				params)) {
			addMessage(sb, fs.date, fs.change_id, ie);
		}
	}

	public FieldEventI buildFieldEvent(String path, String name,
			FlattenedItemI f1, FlattenedItemI f2) {
		if (DateUtils.isOnOrAfter(f1.getStartDate(), f2.getStartDate())) {
			return new FieldEvent(path, name, f1.getFields().getParams()
					.get(path), f2.getFields().getParams().get(path));
		} else {
			return new FieldEvent(path, name, f2.getFields().getParams()
					.get(path), f1.getFields().getParams().get(path));
		}
	}

	public void handleRow(Map<Date, ChangeSummary> sb,
			List<FlattenedItemI> params, Integer parent, String label,
		...
		for (int i = 0; i < params.size(); i++) {
			...
			if (fi instanceof FlattenedItem) {
				if (i == 0) {
					// first
					registerEvent(sb, fi.getItemObject(),
							((FlattenedItem) fi).getCreateEventId(), ADDED,
							(FieldEventI) null,
							((FlattenedItem) fi).getCreateUsername(),
							fi.getInsert_date(), fi.getParents(), fi);
				} else {
					if (fi.isDeleted()
							&& DateUtils.isEqualTo(fi.getChange_date(),
									fi.getLast_modified())) {
						// last
						registerEvent(sb, fi.getItemObject(),
								((FlattenedItem) fi).getModifiedEventId(),
								REMOVED, null,
								((FlattenedItem) fi).getModifiedUsername(),
								fi.getEndDate(), fi.getParents(), fi);

						if (!DateUtils.isEqualTo(fi.getInsert_date(),
								fi.getPrevious_change_date())) {
							FlattenedItemI previous = params.get(i - 1);
							markModifications(sb, headers, (FlattenedItem) fi,
									(FlattenedItem) previous);
						}
					} else {
						FlattenedItemI previous = params.get(i - 1);
						markModifications(sb, headers, (FlattenedItem) fi,
	...
	protected void handleFlattenedFile(Map<Date, ChangeSummary> sb,
			FlattenedFile f) {
		if (f.getCreateEventId() != null) {
			registerEvent(sb, f.getItemObject(), f.getCreateEventId(), ADDED,
					(FieldEventI) null, f.getCreateUsername(),
					f.getInsert_date(), f.getParents(), f);
		} else if (f.getInsert_date() != null) {
			registerEvent(sb, f.getItemObject(), f.getInsert_date().getTime(),
					ADDED, (FieldEventI) null, f.getCreateUsername(),
					f.getInsert_date(), f.getParents(), f);
		}

		if (f.getModifiedEventId() != null) {
			registerEvent(sb, f.getItemObject(), f.getModifiedEventId(),
					REMOVED, (FieldEventI) null, f.getModifiedUsername(),
					f.getEndDate(), f.getParents(), f);
		} else if (f.getEndDate() != null) {
			registerEvent(sb, f.getItemObject(), f.getEndDate().getTime(),
					REMOVED, (FieldEventI) null, f.getModifiedUsername(),
					f.getEndDate(), f.getParents(), f);
		}
	}

	// final static String[] addfiles=new
	// String[]{"file_count".intern(),"file_size".intern()};
	...
	protected void markModifications(Map<Date, ChangeSummary> sb,
			Map<String, String> headers, FlattenedItem fi,
			FlattenedItem previous) {
		for (Map.Entry<String, String> header : headers.entrySet()) {
			if (!conceal.contains(header.getKey())) {
				if (differBy(fi, previous, header.getKey())) {
					registerEvent(sb, fi.getItemObject(),
							fi.getCreateEventId(), fi.getStartDate(),
							builder.build(
									fi.getItemObject(),
									MODIFIED,
									buildFieldEvent(header.getKey(),
											header.getValue(), fi, previous),
									(fi.getModifiedUsername() == null) ? fi
											.getCreateUsername() : fi
											.getModifiedUsername(), fi
											.getParents(), fi));
				}
			}
		}
	...
	protected void build(FlattenedItemA.ChildCollection all_props,
			final String label, Map<Date, ChangeSummary> sb, Integer parent)
			throws Exception {
		...
		for (final FlattenedItemI params : all_props.getChildren()) {
			if (params.getHistory().size() > 0) {
				...
				if (params instanceof FlattenedItem) {
					registerEvent(sb, params.getItemObject(),
							((FlattenedItem) params).getCreateEventId(), ADDED,
							null, params.getCreateUsername(),
							params.getInsert_date(), params.getParents(),
							params);
					if (params.isDeleted()
							&& DateUtils.isEqualTo(params.getChange_date(),
									params.getLast_modified())) {
						// last
						registerEvent(sb, params.getItemObject(),
								((FlattenedItem) params).getModifiedEventId(),
								REMOVED, null, params.getModifiedUsername(),
								params.getEndDate(), params.getParents(),
								params);
					}
	...
	public static class ItemEvent implements ItemEventI {
		...
		public ItemEvent(FlattenedItemA.ItemObject o, String action,
				FieldEventI fe, List<FlattenedItemA.ItemObject> parents,
				String username) {
			super();
			this.object = o;
			this.action = action;
	...
	public static class FieldEvent implements FieldEventI {
		...
		public FieldEvent(String xsiPath, String fieldLabel, Object newValue,
				Object oldValue) {
			super();
	...
	public interface EventBuilderI {
		public List<ItemEventI> build(ItemObject io, String action,
				FieldEventI fe, String username, List<ItemObject> parents,
				FlattenedItemI fi);
	}
	...
	public static class DefaultBuilder implements EventBuilderI {
		public List<ItemEventI> build(ItemObject io, String action,
			...
			items.add(new ItemEvent(io, action, fe, parents, username));
			return items;
		}
	}
}
