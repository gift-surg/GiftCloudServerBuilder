...
public class XNATRestletFactory {
	...
	// # add additional VirtualHosts using this (If you customize it, you'll
	// need to manually register the STANDARD implementation as well as your
	// custom one.
	...
	static {
		try {
			config = PropertiesHelper.RetrieveConfiguration(new File(XFT
					.GetConfDir(), XNAT_RESTLET_PROPERTIES));
		} catch (ConfigurationException e) {
			logger.error("", e);
		}
	}

	public synchronized static Collection<VirtualHost> buildVirtualHosts(
			final org.restlet.Context context) {
		List<VirtualHost> virtualHosts = new ArrayList<VirtualHost>();

		if (config != null) {
			virtualHosts.addAll(((new PropertiesHelper<VirtualHost>())
					.buildObjectsFromProps(config,
							RESTLET_VIRTUAL_HOST_IDENTIFIER,
							RESTLET_VIRTUAL_HOST_PROP_OBJECT_FIELDS,
							CLASS_NAME, RESTLET_VIRTUAL_HOST_PARAMETER_TYPES,
							new Object[] { context })).values());
		}
	...
	public synchronized static Application buildDefaultApplication(
			final org.restlet.Context context) {
		Application app = ((new PropertiesHelper<Application>())
				.buildObjectsFromProps(config, RESTLET_APP_IDENTIFIER,
						RESTLET_APP_PROP_OBJECT_FIELDS, CLASS_NAME,
						RESTLET_APP_PARAMETER_TYPES, new Object[] { context })
				.get(DEFAULT));
		if (app == null) {
