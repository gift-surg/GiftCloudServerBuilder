...
public class PullSessionDataFromHeaders implements Callable<Boolean> {
	...
	public PullSessionDataFromHeaders(final XnatImagesessiondata mr,
			final XDATUser user, boolean allowDataDeletion,
			final boolean overwrite, final boolean isInPrearchive, EventMetaI c) {
		this.tempMR = mr;
		this.user = user;
	 ...
	 * session xml accordingly. It assumes the files are already in the archive
	 * and properly referenced from the session xml. This would usually be run
	 * after you've added the files via the REST API.
	 * 
	 * @throws IOException
	 * @throws SAXException
	...
	public Boolean call() throws IOException, SAXException,
		...
		// identify session directory location based on existing scans, the
		// deriveSessionDir() method will return null if it can't load it from
		// the scans.
		...
		final String timestamp = (new java.text.SimpleDateFormat(
				...
				.getInstance().getTime());
		final File xml = new File(sessionDir, tempMR.getLabel() + "_"
				+ timestamp + ".xml");

		final XNATSessionBuilder builder = new XNATSessionBuilder(sessionDir,
				xml, tempMR.getProject(), isInPrearchive);
		builder.call();
		...
		if (overwrite) {
			// this will ignore the pre-existing session and store the newly
			// generated xml in place of the old one.
			// this will delete references added resources (snapshots,
			// reconstructions, assessments, etc)
			allowDataDeletion = false;// why is this set to false when you want
										// to override?
			...
			for (final XnatImagescandataI newscan : newmr.getSortedScans()) {
				final XnatImagescandata oldScan = tempMR.getScanById(newscan
						.getId());
				// copy values from old session, to new session
				// if oldScan is null, then a new scan has been discovered and
				// old values are not present to maintain.
				if (oldScan != null) {
					((XnatImagescandata) newscan)
							.setXnatImagescandataId(oldScan
									.getXnatImagescandataId());

					// if allowDataDeletion=true, then new file tags will
					// replace old ones (modifications to content, format, etc
					// will not be preserved).
					if (!allowDataDeletion) {
						// in the current code, the new file entries should not
						// be maintained. The old ones are assumed to be correct
						// and not needing updates.
						// the content, format, and description of the new file
						// entries will be preserved if the old ones were null.
						if (newscan.getFile().size() > 0) {
							final XnatResource newcat = (XnatResource) newscan
									.getFile().get(0);

							final XnatAbstractresourceI oldCat = oldScan
									.getFile().get(0);
							if (oldCat instanceof XnatResource) {
								if (StringUtils.IsEmpty(((XnatResource) oldCat)
										.getContent())
										&& !StringUtils.IsEmpty(newcat
												.getContent()))
									((XnatResource) oldCat).setContent(newcat
											.getContent());
								if (StringUtils.IsEmpty(((XnatResource) oldCat)
										.getFormat())
										&& !StringUtils.IsEmpty(newcat
												.getFormat()))
									((XnatResource) oldCat).setFormat(newcat
											.getFormat());
								if (StringUtils.IsEmpty(((XnatResource) oldCat)
										.getDescription())
										&& !StringUtils.IsEmpty(newcat
												.getDescription()))
									((XnatResource) oldCat)
											.setDescription(newcat
													.getDescription());
							}

							while (newscan.getFile().size() > 0)
								((XnatImagescandata) newscan).removeFile(0);

							// replace new files (catalogs) with old ones.
							((XnatImagescandata) newscan)
									.setFile((XnatAbstractresource) oldCat);
						} else {
							while (newscan.getFile().size() > 0)
								((XnatImagescandata) newscan).removeFile(0);
		...
		// if any scan types are null, they will be filled according to the
		// standard logic.
		newmr.fixScanTypes();
		...
		if (vr != null && !vr.isValid()) {
			...
			if (SaveItemHelper.authorizedSave(newmr, user, false,
					allowDataDeletion, c)) {
				try {
