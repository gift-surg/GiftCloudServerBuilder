...
public class XarImporter extends ImporterHandlerA implements
	 ...
	 * @param session
	 * @param overwrite
	 *            : 'append' means overwrite, but preserve un-modified content
	 *            (don't delete anything) 'delete' means delete the pre-existing
	 *            content.
	 * @param additionalValues
	 *            : should include project, subject_ID and label (if session is
	 *            null)
	 */
	public XarImporter(final Object listenerControl, final XDATUser u,
	...
	private List<String> processXarFile() throws ClientException,
			ServerException {
		...
		try {
			...
			throw new ClientException(
					"Archive file is corrupt or not a valid archive file type.");
		}
		...
		if (destination.listFiles().length == 1
				&& destination.listFiles()[0].isDirectory()) {
			...
			if (f.isDirectory()) {
				...
				if (f.getName().toLowerCase().endsWith(".xml")) {
					...
					try {
						...
						ItemI om = org.nrg.xdat.base.BaseElement
								.GetGeneratedItem(item);
						urlList.add("/archive/experiments/"
								+ ((XnatExperimentdata) om).getId());
						if (om instanceof XnatImagesessiondata) {
							if (!itemsByType.containsKey("SESSION")) {
								itemsByType.put("SESSION",
										new ArrayList<ItemI>());
							}
							...
							if (!itemsByType.containsKey("ASSESSOR")) {
								itemsByType.put("ASSESSOR",
										new ArrayList<ItemI>());
							}

							ArrayList<ItemI> items = itemsByType
									.get("ASSESSOR");
		...
		if (itemsByType.containsKey("SESSION")
				&& itemsByType.get("SESSION").size() > 1) {
			if (fileList == null || fileList.length == 0) {
				throw new ClientException(
						"XAR can only include data for one imaging session.");
			}
		}
		...
		if (itemsByType.size() == 0) {
			...
			ArrayList<ItemI> items = itemsByType.get(itemsByType.keys()
					.nextElement());
			...
			if (itemsByType.containsKey("SESSION")) {
				...
				if (items.size() == 1) {
					...
					if (session.getProject() == null) {
						throw new ClientException(
								"Could not process XAR file - Invalid project tag.");
					}

					if (session.getSubjectData() == null) {
						throw new ClientException(
								"Could not process XAR file - Invalid subject.");
					}
					...
					try {
						...
						throw new ServerException(
								"Server Error:  Invalid Archive Structure");
					}
				...
				for (XFTItem resource : session.getItem().getChildrenOfType(
						"xnat:abstractResource")) {
					XnatAbstractresource res = (XnatAbstractresource) org.nrg.xdat.base.BaseElement
							.GetGeneratedItem(resource);
					...
					if (session != null)
						try {
							dest_path = FileUtils.AppendRootPath(
									session.getCurrentSessionFolder(true),
									"SCANS/" + uploadID + "/");
						} catch (Exception e) {
						...
						throw new ClientException(
								"All XNAT xml documents must reference a valid Imaging Session.");
					}

					for (XFTItem resource : scan.getItem().getChildrenOfType(
							"xnat:abstractResource")) {
						XnatAbstractresource res = (XnatAbstractresource) org.nrg.xdat.base.BaseElement
								.GetGeneratedItem(resource);
						res.prependPathsWith(FileUtils.AppendSlash(dest_path));
			...
			if (multiSession) {
				...
				try {
						...
						if (children != null) {
							for (File child : children) {
								if (child.isDirectory())
									FileUtils.MoveDir(child, new File(dest,
											child.getName()), true);
								else
									FileUtils.MoveFile(child, new File(dest,
											child.getName()), true);
							}
						}
					} else {
						for (File dir : dirs) {
							FileUtils.MoveDir(dir,
									new File(dest, dir.getName()), true);
						}
						for (File f : extraFiles) {
							FileUtils.MoveFile(f, new File(dest, f.getName()),
									true);
						}
					}
					for (ItemI item : items) {
								...
								.buildOpenWorkflow(
										...
										EventUtils
												.newEventInstance(
														...
														EventUtils.getType(
																(String) params
																		.get(EventUtils.EVENT_TYPE),
																EventUtils.TYPE.WEB_SERVICE),
														EventUtils.STORE_XAR,
														(String) params
						...
						try {
							SaveItemHelper.unauthorizedSave(item, user, false,
									true, c);
			...
			throw new ClientException(
					"Multiple data types cannot share a single XAR.  Please separate files into separate XARs");
