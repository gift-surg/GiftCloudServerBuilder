...
public abstract class ImporterHandlerA extends StatusProducer implements
		Callable<List<String>> {

	public ImporterHandlerA(final Object listenerControl, final XDATUser u,
	...
	static {
		...
		try {
			IMPORTERS.putAll((new PropertiesHelper<ImporterHandlerA>())
					.buildClassesFromProps(IMPORTER_PROPERTIES,
							PROP_OBJECT_IDENTIFIER, PROP_OBJECT_FIELDS,
							CLASS_NAME));

			if (!IMPORTERS.containsKey(SESSION_IMPORTER))
				IMPORTERS.put(SESSION_IMPORTER, SessionImporter.class);
			if (!IMPORTERS.containsKey(XAR_IMPORTER))
				IMPORTERS.put(XAR_IMPORTER, XarImporter.class);
			if (!IMPORTERS.containsKey(GRADUAL_DICOM_IMPORTER))
				IMPORTERS.put(GRADUAL_DICOM_IMPORTER,
						GradualDicomImporter.class);
			if (!IMPORTERS.containsKey(DICOM_ZIP_IMPORTER))
				IMPORTERS.put(DICOM_ZIP_IMPORTER, DicomZipImporter.class);
			if (!IMPORTERS.containsKey(BLANK_PREARCHIVE_ENTRY))
				IMPORTERS.put(BLANK_PREARCHIVE_ENTRY, PrearcBlankSession.class);
		} catch (Exception e) {
			logger.error("", e);
		}
	}

	public static ImporterHandlerA buildImporter(String format,
			final Object uID, final XDATUser u, final FileWriterWrapperI fi,
		...
		if (importerImpl == null) {
			...
			throw new ImporterNotFoundException(
					"Unknown importer implementation specified: " + format,
					new IllegalArgumentException());
		}

		final Constructor con = importerImpl.getConstructor(Object.class,
				XDATUser.class, FileWriterWrapperI.class, Map.class);
		final ImporterHandlerA handler = (ImporterHandlerA) con.newInstance(
				uID, u, fi, params);
		...
		if (GradualDicomImporter.class.equals(importerImpl)) {
			...
			gdi.setIdentifier(context.getBean("dicomObjectIdentifier",
					DicomObjectIdentifier.class));
			try {
				final DicomFileNamer namer = context.getBean("dicomFileNamer",
						DicomFileNamer.class);
				if (null != namer) {
					gdi.setNamer(namer);
				}
			...
			dzi.setIdentifier(context.getBean("dicomObjectIdentifier",
					DicomObjectIdentifier.class));
			try {
				final DicomFileNamer namer = context.getBean("dicomFileNamer",
						DicomFileNamer.class);
