...
public class AuthenticationRestlet extends Resource {
	...
	public AuthenticationRestlet(Context context, Request request,
	...
	private void runAuthenticate() {
		...
		if (StringUtils.isEmpty(_authMethod) && !StringUtils.isEmpty(_username)) {
			...
			_authMethod = XnatAuthenticationFilter
					.retrieveAuthMethod(_username);
			if (StringUtils.isEmpty(_authMethod)) {
				throw new BadCredentialsException(
						"Missing login method parameter.");
			}
		}

		UsernamePasswordAuthenticationToken authRequest = XnatAuthenticationFilter
				.buildUPTokenForAuthMethod(_authMethod, _username, _password);
		XnatProviderManager manager = XDAT.getContextService().getBean(
				XnatProviderManager.class);
	...
	private void extractCredentials(String text) {
		...
		for (String entry : entries) {
			...
			if (atoms == null || atoms.length < 2) {
				...
				try {
					if (atoms[0].equals("username")
							|| atoms[0].equals("j_username")) {
						_username = URLDecoder.decode(atoms[1], "UTF-8");
					} else if (atoms[0].equals("password")
							|| atoms[0].equals("j_password")) {
						_password = URLDecoder.decode(atoms[1], "UTF-8");
					} else if (atoms[0].equals("provider")
							|| atoms[0].equals("login_method")) {
						_authMethod = URLDecoder.decode(atoms[1], "UTF-8");
					} else {
						// TODO: Just ignoring for now, should we do something
						// here?
					}
				} catch (UnsupportedEncodingException e) {
					// This is the dumbest exception in the history of humanity:
					// the form of this method that doesn't
					// specify an encoding is deprecated, so you have to specify
					// an encoding. But the form of the method
					// that takes an encoding (http://bit.ly/yX56fe) has an note
					// that emphasizes that you should only
					// use UTF-8 because
					// "[n]ot doing so may introduce incompatibilities." Got it?
					// You have to specify
					// it, but it should always be the same thing. Oh, and BTW?
					// You have to catch an exception for
					// unsupported encodings because you may specify that one
					// acceptable encoding or... something.
					//
					// I hate them.
				}
			}
