	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		if (null == dicomSCPManager) {
			getResponse().setStatus(Status.CLIENT_ERROR_FAILED_DEPENDENCY,
					"DicomSCP was not properly initialized.");
			return new StringRepresentation(
					"ERROR: DicomSCP was not properly initialized.");
	...
	private void respondToInvalidAction(String action) {
		getResponse()
				.setStatus(
						...
						String.format(
								"Action '%s' is not supported by this resource.  Valid actions are: %s",
								action, Joiner.on(",").join(allowedActions)));
	}
