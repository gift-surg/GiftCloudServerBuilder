	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		try {
			return new StringRepresentation(
					XDAT.getWhitelistConfiguration(user));
	...
	public void handlePut() {
		try {
			...
			List<String> addresses = new ArrayList<String>(
					Arrays.asList(whitelist.split("[\\s,]+")));
			for (String localhost : XDAT.getLocalhostIPs()) {
				if (!addresses.contains(localhost)) {
					addresses.add(localhost);
				}
			}
			XDAT.getConfigService().replaceConfig(user.getLogin(), "",
					XDAT.IP_WHITELIST_TOOL, XDAT.IP_WHITELIST_PATH,
					Joiner.on("\n").join(addresses));
		} catch (IOException e) {
			getResponse().setStatus(Status.SERVER_ERROR_INTERNAL, e,
					"Error occurred trying to handle the incoming data");
			_log.error("Error occurred trying to handle the incoming data", e);
		} catch (ConfigServiceException e) {
			getResponse().setStatus(Status.SERVER_ERROR_INTERNAL, e,
					"Error occurred writing to the configuration service");
