...
public class SendEmailVerification extends Resource {
	...
	private final EmailRequestLogService requests = XDAT.getContextService()
			.getBean(EmailRequestLogService.class);

	public SendEmailVerification(Context context, Request request,
			...
			try {
				if (requests.isEmailBlocked(email)) {
					...
					throw new ExceededRequestsException(
							"Exceeded maximum number of email requests.");
				}

				// Send email and log request.
				AdminUtils
						.sendNewUserVerificationEmail(getXDATUser(StringEscapeUtils
								.escapeSql(email)));
