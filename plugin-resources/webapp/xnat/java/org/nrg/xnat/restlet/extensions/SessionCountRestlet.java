...
public class SessionCountRestlet extends SecureResource {
	...
	private static final Log _log = LogFactory
			.getLog(SessionCountRestlet.class);

	private final UserI _user;

	public SessionCountRestlet(Context context, Request request,
			Response response) {
		super(context, request, response);
		this.getVariants().add(new Variant(MediaType.ALL));

		final String username = (String) getRequest().getAttributes().get(
				PARAM_USERNAME);

		// You can't request another user's session count unless you're a site
		// admin.
			...
			if (username.equals(user.getLogin())) {
				...
				getResponse()
						.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
								"Only site admins can request the session count for another user.");
				_user = null;
			} else {
				// If you are an admin and this isn't you, then let's get that
				// account.
				final XdatUser xdatUser = XDATUser.getXdatUsersByLogin(
						username, user, false);
				if (xdatUser == null) {
					getResponse().setStatus(
							Status.CLIENT_ERROR_BAD_REQUEST,
							"The user identified by " + username
									+ " can not be found in the system.");
					_user = null;
				} else {
	...
	private int getSessionCount(UserI user) {
		...
		if (sessionRegistry != null) {
			List<SessionInformation> l = sessionRegistry.getAllSessions(user,
					false);
			if (l != null) {
				sessionCount = l.size();
			}
