 ...
 *
 *         Implementation of the User Roles functionality. The post method adds
 *         or removes role for the specified user.
 */
...
public class UserRolesRestlet extends SecureResource {
	...
	public void handlePost() {
		...
		try {
			// relies on a configuration parameter which defines the available
			// roles.
			...
			// remove roles and save one at a time so that there is a separate
			// workflow entry for each one
			...
			// add roles and save one at a time so that there is a separate
			// workflow entry for each one
			for (String dRole : allDefinedRoles) {
				if (roles.contains(dRole)) {
