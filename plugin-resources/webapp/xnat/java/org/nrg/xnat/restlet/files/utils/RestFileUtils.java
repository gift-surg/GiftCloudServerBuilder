...
public class RestFileUtils {
	...
	public static String replaceResourceLabel(String path, Object id,
			String label) {
		if (StringUtils.isEmpty(label) || id == null) {
			...
			if (i > -1) {
				return StringUtils.replace(path, '/' + id.toString()
						+ "/files/", '/' + label + "/files/");
			} else {
				return path;
			}
	...
	public static Map<String, String> getReMaps(List<XnatImagescandata> scans,
			List<XnatReconstructedimagedata> recons) {
		...
		if (scans != null) {
			for (final XnatImagescandata scan : scans) {
				if (!StringUtils.isEmpty(scan.getType())) {
					valuesToReplaceInPath.put(
							scan.getId(),
							scan.getId()
									+ '-'
									+ scan.getType().replaceAll("\\W", "_")
											.replaceAll(" ", "_"));
				}
			}
		}
	...
	public static String buildRelativePath(String uri,
			Map<String, String> session_mapping,
			Map<String, String> valuesToReplace, Object resourceID,
			String resourceLabel) {
		String relative = RestFileUtils.getRelativePath(uri.replace('\\', '/'),
				session_mapping);

		relative = RestFileUtils.replaceResourceLabel(relative, resourceID,
				resourceLabel);

		for (Map.Entry<String, String> e : valuesToReplace.entrySet()) {
			relative = RestFileUtils.replaceInPath(relative, e.getKey(),
					e.getValue());
		}
	...
	public static Map<String, String> getSessionMaps(
			List<XnatExperimentdata> assesseds, List<XnatExperimentdata> expts,
			XnatSubjectdata sub, XnatProjectdata proj) {
