...
public class RESTHTMLPresenter extends PresentationA {
	static org.apache.log4j.Logger logger = Logger
			.getLogger(RESTHTMLPresenter.class);
	...
	public RESTHTMLPresenter(String serverLocal, boolean canClickHeaders,
			String searchURI, XDATUser u, String sortBy) {
		server = serverLocal;
		if (!server.endsWith("/"))
			server = server + "/";
	...
	public RESTHTMLPresenter(String serverLocal, String searchURI, XDATUser u,
			String sortBy) {
		server = serverLocal;
		if (!server.endsWith("/"))
			server = server + "/";
	...
	public XFTTableI formatTable(XFTTableI table, DisplaySearch search,
			boolean allowDiffs) throws Exception {
		logger.debug("BEGIN HTML FORMAT");
		XFTTable csv = new XFTTable();
		ElementDisplay ed = DisplayManager.GetElementDisplay(getRootElement()
				.getFullXMLName());
		ArrayList visibleFields = search.getVisibleFields(this
				.getVersionExtension());
		...
		while (fields.hasNext()) {
			...
			if (dfr.getDisplayField() != null
					&& dfr.getDisplayField().getDescription() != null) {
				title = dfr.getDisplayField().getDescription();
			}

			// The id must be unique across tabs or document.getElementByID will
			// not work as expected.
			// We add a random number to the id to make it *likely* unique.
			headerLink.append("<th class=\"" + tHclass + "\" id=\"" + id
					+ random + "\" name=\"" + id + "\" align=\"left\"");
			diffLink.append("<th align=\"left\" class=\"x_rs_th\" id=\"diff_"
					+ id + random + "\"");
			if (dfr.getHeaderCellWidth() != null) {
				headerLink
						.append(" width=\"" + dfr.getHeaderCellWidth() + "\"");
			}
			...
			if (dfr.getHeaderCellAlign() != null) {
				headerLink
						.append(" align=\"" + dfr.getHeaderCellAlign() + "\"");
			}
			if (dfr.getHeaderCellVAlign() != null) {
				headerLink.append(" valign=\"" + dfr.getHeaderCellVAlign()
						+ "\"");
			}
			headerLink.append("><div class=\"yui-dt-liner\" title=\"")
					.append(TurbineUtils.GetInstance().escapeHTML(title))
					.append("\">");
			diffLink.append(">Diff</th>");

			if (this.searchURI != null && !dfr.getHeader().equalsIgnoreCase("")
					&& clickableHeaders) {
				headerLink.append("<a href='" + this.searchURI
						+ "?format=xList&sortBy=");
				headerLink.append(dfr.getElementName()).append(".");
				headerLink.append(dfr.getSortBy());
				if (!search.getSortBy().equalsIgnoreCase(
					...
					if (search.getSortOrder().equalsIgnoreCase("DESC")) {
						...
						headerLink
								.append(dfr.getHeader())
								.append(" <img border=\"0\" src=\"" + server
										+ "images/black-down-arrow.gif\"/></a>");
					} else {
						headerLink.append("&sortOrder=DESC'>");
						headerLink.append(dfr.getHeader()).append(
								" <img border=\"0\" src=\"" + server
										+ "images/black-up-arrow.gif\"/></a>");
					}
				}
			} else {
				...
				if (this.sortBy != null && this.sortBy.equals(id)) {
					// headerLink.append(" <IMAGE border=0 src=\"" + server +
					// "images/black-down-arrow.gif\"/>");
				}
			}
			...
			if (allowDiffs) {
				if (!diffs.contains(dfr.getElementName())) {
					...
					SchemaElementI foreign = SchemaElement.GetElement(dfr
							.getElementName());
					if (search.isMultipleRelationship(foreign)) {
						String temp = StringUtils.SQLMaxCharsAbbr(search
								.getRootElement().getSQLName()
								+ "_"
								+ foreign.getSQLName() + "_DIFF");
		...
		while (table.hasMoreRows()) {
			...
			// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
			// table.getRowCursor() + ") WHILE: 1");

			if (search.getInClauses().size() > 0) {
				for (int i = 0; i < search.getInClauses().size(); i++) {
					Object v = row.get("search_field" + i);
					if (v != null) {
						newRow[i] = "<td class=\"x_rs_td x_rs_td_in\">" + v
								+ "</td>";
					} else {
			...
			// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
			// table.getRowCursor() + ") WHILE: 2");
			diffs = new ArrayList();
			counter = search.getInClauses().size();
			int fieldC = 0;
			while (fields.hasNext()) {
				// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
				// table.getRowCursor() + ") FIELDS: 1");
				DisplayFieldReferenceI dfr = (DisplayFieldReferenceI) fields
						.next();
				...
				if (dfr.getElementName().equalsIgnoreCase(
					...
					field_row_id = dfr.getElementSQLName().toLowerCase() + "_"
							+ dfr.getRowID().toLowerCase();
				}
				...
				if (dfr.getElementName().equalsIgnoreCase(
					...
					v = row.get(dfr.getElementSQLName().toLowerCase() + "_"
							+ dfr.getRowID().toLowerCase());
				}
				...
				if (allowDiffs) {
					if (!diffs.contains(dfr.getElementName())) {
						// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
						// table.getRowCursor() + ") FIELDS: diffs :1");
						diffs.add(dfr.getElementName());
						SchemaElementI foreign = SchemaElement.GetElement(dfr
								.getElementName());
						// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
						// table.getRowCursor() + ") FIELDS: diffs :2");
						if (search.isMultipleRelationship(foreign)) {
							// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
							// table.getRowCursor() + ") FIELDS: diffs :IF:1");
							String temp = StringUtils.SQLMaxCharsAbbr(search
									.getRootElement().getSQLName()
									+ "_"
									+ foreign.getSQLName() + "_DIFF");
							Integer index = ((XFTTable) table)
									.getColumnIndex(temp);
							if (index != null) {
								String diff = "<td";
							...
							// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
							// table.getRowCursor() + ") FIELDS: diffs :IF:2");
						}
						// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
						// table.getRowCursor() + ") FIELDS: diffs :3");
					}
				}
				// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
				// table.getRowCursor() + ") FIELDS: 2");

				if (v != null && !v.toString().trim().equals("")) {
					// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
					// table.getRowCursor() + ") FIELDS: IF : 1");
					...
					if (dfr.getHTMLLink() != null) {
						// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
						// table.getRowCursor() + ") FIELDS: IF : IF 1");
						// HAS HTML LINK - CREATE ANCHOR

						HTMLLink link = dfr.getHTMLLink();
						// INSERT SECURITY VALIDATION
						if (link.isSecure()
								&& dfr.getElementName() != null
								&& !dfr.getElementName().equals(
										getRootElement().getFullXMLName())) {
							SchemaElementI secureElement = SchemaElement
									.GetElement(link.getSecureLinkTo());
							XDATUser user = (XDATUser) search.getUser();

							SecurityValues values = new SecurityValues();
							Enumeration secureKeys = link.getSecureProps()
									.keys();
							while (secureKeys.hasMoreElements()) {
								...
								if (!dfr.getElementName().equalsIgnoreCase(
										search.getRootElement()
												.getFullXMLName())) {
									secureVariable = row.get(dfr
											.getElementSQLName().toLowerCase()
											+ "_" + key.toLowerCase());
								} else {
								...
								if (secureVariable != null) {
									if (secureVariable.toString().indexOf("<") != -1) {
										secureVariable = StringUtils
												.ReplaceStr(secureVariable
														.toString(), "<", "");
									}
									if (secureVariable.toString().indexOf(">") != -1) {
										secureVariable = StringUtils
									...
									values.put((String) link.getSecureProps()
											.get(key), secureVariable);
								}
							}
							...
							if (values.getHash().size() > 0) {
								if (user.canReadByXMLPath(secureElement, values)) {
									hasLink = true;
									sb.append("<a");
									Iterator iter = link.getProperties()
											.iterator();
									while (iter.hasNext()) {
										HTMLLinkProperty prop = (HTMLLinkProperty) iter
												.next();
						...
						// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
						// table.getRowCursor() + ") FIELDS: IF : IF 2");
					}

					// SET IMAGE
					if (dfr.isImage()) {
						sb.append("<img");
						v = StringUtils.ReplaceStr((String) v, "/@WEBAPP/",
								server);
						v = StringUtils.ReplaceStr((String) v, "@WEBAPP/",
								server);
						v = StringUtils.ReplaceStr((String) v, "/@WEBAPP",
						...
						if (dfr.getDisplayField().getHtmlImage().getWidth() != null) {
							sb.append(" width=\""
									+ dfr.getDisplayField().getHtmlImage()
											.getWidth() + "\"");
						}
						...
						sb.append(" src=\"").append(v.toString())
								.append("\" border=\"0\"/>");
					} else {
						if (v instanceof Timestamp) {
							// String s =
							// DateFormat.getDateInstance(DateFormat.SHORT,
							// Locale.ENGLISH).format(DateUtils.parseDateTime(v.toString()));
					...
					// XFT.LogCurrentTime("BEGIN HTML FORMAT :: ROWS(" +
					// table.getRowCursor() + ") FIELDS: IF : 2");
				} else {
