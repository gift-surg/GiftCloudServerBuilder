...
public abstract class AutomationResource extends SecureResource {
	...
	public AutomationResource(final Context context, final Request request,
			final Response response) throws ResourceException {
		super(context, request, response);
		final String projectId = (String) getRequest().getAttributes().get(
				PROJECT_ID);
	...
	protected String getRequestContext(final String header) {
		...
		if (StringUtils.isNotBlank(getResourceId())) {
			buffer.append("\n * ").append(getResourceType()).append(" ID: ")
	...
	private Map<String, String> validateEntityId(final String entityId)
		...
		case Project:
			if (StringUtils.isBlank(entityId)) {
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
						"You must specify an ID for the project scope.");
			}
			final Map<String, String> ids = new HashMap<String, String>();
			// Check to see if entityId is actually a project ID. If so, convert
			// it to a Long.
			final Long resolved = XnatProjectdata
					.getProjectInfoIdFromStringId(entityId);
			if (resolved != null) {
				...
				try {
					XFTTable table = XFTTable.Execute(
							"SELECT id FROM xnat_projectdata WHERE projectdata_info = "
									+ entityId, user.getDBName(), userName);
					if (table.size() != 1) {
						throw new ResourceException(
								Status.CLIENT_ERROR_NOT_FOUND,
								"Couldn't find a project with the ID or alias of "
										+ entityId);
					}
					ids.put(KEY_PROJECTDATAINFO, entityId);
					ids.put(KEY_PROJECTID, (String) table
							.convertColumnToArrayList("id").get(0));
				} catch (Exception e) {
		...
		default:
			throw new ResourceException(
					...
					"The specified scope "
							+ getScope().code()
							+ " is not currently supported. Supported scopes include: "
							+ Scope.Site.code() + " and "
							+ Scope.Project.code());
		}
	}
