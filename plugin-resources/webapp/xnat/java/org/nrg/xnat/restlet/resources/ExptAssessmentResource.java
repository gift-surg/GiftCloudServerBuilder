...
public class ExptAssessmentResource extends ItemResource {
	...
	public ExptAssessmentResource(Context context, Request request,
			Response response) {
		...
		if (assessedID != null) {
			...
			response.setStatus(
					...
					"Unable to find assessed experiment '"
							+ TurbineUtils.escapeParam(assessedID) + "'");
		}

		this.fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XMLPathShortcuts.DERIVED_DATA, false));
	}
	...
	public void handlePut() {
		...
		try {
			...
			if (existing != null
					&& !this.isQueryVariableTrue("allowDataDeletion")) {
			...
			if (item.instanceOf("xnat:imageAssessorData")) {
				assessor = (XnatImageassessordata) BaseElement
				...
				if (filepath != null && !filepath.equals("")) {
					if (filepath.startsWith("projects/")) {
						...
						if (newProject != null) {
							...
							int index = 0;
							for (XnatExperimentdataShareI pp : assessor
									.getSharing_share()) {
							...
							if (this.getQueryVariable("primary") != null
								...
								if (matched == null) {
									...
									if (user.canCreate(assessor.getXSIType()
										...
										BaseXnatExperimentdata
												.SaveSharedProject(
														...
														newEventInstance(
																EventUtils.CATEGORY.DATA,
																(getAction() != null) ? getAction()
																		: "Shared into additional project"));
									} else {
										this.getResponse()
												.setStatus(
														Status.CLIENT_ERROR_FORBIDDEN,
					...
					if (this.proj == null && assessor.getProject() != null) {
						proj = XnatProjectdata.getXnatProjectdatasById(
								assessor.getProject(), user, false);
					}

					if (this.proj != null) {
						if (assessor.getProject() == null
								|| assessor.getProject().equals("")) {
							assessor.setProject(this.proj.getId());
						} else if (assessor.getProject().equals(
								this.proj.getId())) {
						} else {
					...
					if (this.assesed != null) {
						...
						if (assessor.getImagesessionId() != null
								&& !assessor.getImagesessionId().equals("")) {
							this.assesed = XnatExperimentdata
									.getXnatExperimentdatasById(
											assessor.getImagesessionId(), user,
											false);

							if (this.assesed == null
									&& assessor.getProject() != null
									&& assessor.getLabel() != null) {
								this.assesed = XnatExperimentdata
										.GetExptByProjectIdentifier(
												assessor.getProject(),
												assessor.getImagesessionId(),
												user, false);
							}
							...
							if (this.assesed == null) {
								for (XnatExperimentdataShareI pp : assessor
										.getSharing_share()) {
									this.assesed = XnatExperimentdata
											.GetExptByProjectIdentifier(pp
													.getProject(), assessor
													.getImagesessionId(), user,
													false);
									if (this.assesed != null) {
										break;
									}
					...
					if (existing == null) {
						...
						if (existing == null && assessor.getProject() != null
								&& assessor.getLabel() != null) {
							existing = (XnatImageassessordata) XnatExperimentdata
									.GetExptByProjectIdentifier(
											assessor.getProject(),
											assessor.getLabel(), user,
											completeDocument);
						}
						...
						if (existing == null) {
							for (XnatExperimentdataShareI pp : assessor
								...
								existing = (XnatImageassessordata) XnatExperimentdata
										.GetExptByProjectIdentifier(
												pp.getProject(), pp.getLabel(),
												user, completeDocument);
								if (existing != null) {
									break;
								}
							}
					...
					if (existing == null) {
						...
						if (!user.canEdit(assessor)) {
							this.getResponse()
									.setStatus(
											Status.CLIENT_ERROR_FORBIDDEN,
											"Specified user account has insufficient edit privileges for subjects in this project.");
							return;
						}
						// MATCHED
					}
					...
					if (!StringUtils.IsEmpty(assessor.getLabel())
							&& !StringUtils.IsAlphaNumericUnderscore(assessor
									.getId())) {
						this.getResponse().setStatus(
								Status.CLIENT_ERROR_EXPECTATION_FAILED,
					...
					if (existing != null
							&& !org.apache.commons.lang.StringUtils.equals(
									existing.getLabel(), assessor.getLabel())) {
						this.getResponse().setStatus(
								Status.CLIENT_ERROR_BAD_REQUEST,
	...
	public void handleDelete() {
		...
		if (assessor == null && exptID != null) {
			assessor = (XnatImageassessordata) XnatExperimentdata
					.getXnatExperimentdatasById(exptID, user, false);
		...
		// if we attempt to delete an assessment from a shared project, the
		// assessment shouldn't be deleted (just unshared)
		...
		if (filepath != null && !filepath.equals("")) {
			if (filepath.startsWith("projects/")) {
				...
				String newProjectS = filepath.substring(9);
				deleteFromProject = XnatProjectdata.getXnatProjectdatasById(
			...
			// if no project is referenced... assume its the default one
			// (/REST/experiments/ID)
		...
		try {
			wrk = WorkflowUtils.buildOpenWorkflow(
					...
					newEventInstance(
							EventUtils.CATEGORY.DATA,
							(getAction() != null) ? getAction() : EventUtils
									.getDeleteAction(assessor.getXSIType())));
			EventMetaI c = wrk.buildEvent();

			try {
				String msg = assessor.delete(
						(deleteFromProject != null) ? deleteFromProject : proj,
						user, this.isQueryVariableTrue("removeFiles"), c);
				if (msg != null) {
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (assessor != null) {
			String filepath = this.getRequest().getResourceRef()
					.getRemainingPart();
			...
			if (filepath != null && filepath.equals("status")) {
				...
				for (Map.Entry<XnatProjectdataI, String> entry : assessor
