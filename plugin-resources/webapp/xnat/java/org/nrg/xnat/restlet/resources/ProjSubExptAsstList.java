...
public class ProjSubExptAsstList extends QueryOrganizerResource {
	...
	public ProjSubExptAsstList(Context context, Request request,
			Response response) {
		...
		if (pID != null) {
			...
			assessed = XnatExperimentdata.getXnatExperimentdatasById(exptID,
					user, false);
			if (assessed != null) {
		...
		this.fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XMLPathShortcuts.DERIVED_DATA, true));
	}
			...
			if (item.instanceOf("xnat:imageAssessorData")) {
				...
				if (this.assessed != null) {
					...
					if (assessor.getImagesessionId() != null
						...
						this.assessed = XnatExperimentdata
				...
				if (assessor.getId() != null) {
					existing = (XnatImageassessordata) XnatExperimentdata
							.getXnatExperimentdatasById(assessor.getId(), user,
									completeDocument);
				}
				...
				if (existing == null && assessor.getProject() != null
						&& assessor.getLabel() != null) {
					existing = (XnatImageassessordata) XnatExperimentdata
							.GetExptByProjectIdentifier(assessor.getProject(),
									assessor.getLabel(), user, completeDocument);
				}
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (assessed != null) {
			...
			try {
				...
				QueryOrganizer qo = new QueryOrganizer(rootElementName, user,
						ViewManager.ALL);
				...
				cc.addClause("xnat:imageAssessorData/imagesession_id",
						assessed.getId());
				where.addClause(cc);

				if (user.getGroup("ALL_DATA_ADMIN") == null) {
					CriteriaCollection projects = new CriteriaCollection("OR");
					List<Object> ps = user.getAllowedValues("xnat:subjectData",
							"xnat:subjectData/project",
							org.nrg.xdat.security.SecurityManager.READ);
					for (Object p : ps) {
						projects.addClause(rootElementName + "/project", p);
						projects.addClause(rootElementName
