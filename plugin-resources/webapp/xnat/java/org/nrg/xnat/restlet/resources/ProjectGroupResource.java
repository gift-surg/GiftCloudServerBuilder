...
public class ProjectGroupResource extends SecureResource {
	...
	public ProjectGroupResource(Context context, Request request,
			Response response) {
	...
	private static List<String> protected_displayNames = Lists.newArrayList(
			...
			try {
				if (user.canDelete(proj)) {
					...
					try {
						...
						PopulateItem populator = new PopulateItem(
								props,
								user,
								XdatUsergroup.SCHEMA_ELEMENT_NAME,
								true,
								(group == null) ? XFTItem
										.NewItem(
												XdatUsergroup.SCHEMA_ELEMENT_NAME,
												user)
										: group.getItem());
						XdatUsergroup tempGroup = new XdatUsergroup(
								populator.getItem());

						// tag must be for this project
						if (!StringUtils.equals(proj.getId(),
								tempGroup.getTag())) {
						...
						if (StringUtils.isEmpty(tempGroup.getId())) {
							tempGroup.setId(proj.getId() + "_"
									+ tempGroup.getDisplayname());
						}

						final List<ElementSecurity> elements = ElementSecurity
								.GetSecureElements();

						final UserGroup ug = new UserGroup(tempGroup.getId());
						ug.init(tempGroup);
						for (ElementSecurity es : elements) {
							final List<String> permissionItems = es
									.getPrimarySecurityFields();
							for (String securityField : permissionItems) {
								...
								final String s = es.getElementName() + "_"
										+ securityField + "_" + ug.getTag();
								...
								final String wasSet = (String) props.get(s
										+ "_wasSet");

								if ((wasSet != null && wasSet.equals("1"))
										|| pc.getCreate() || pc.getRead()
										|| pc.getEdit() || pc.getDelete()
										|| pc.getActivate()) {
									tempGroup.addRootPermission(
											es.getElementName(), pc);

									if (StringUtils.equals(pc.getField(),
											es.getElementName() + "/project")) {
										// inherit project permissions to shared
										// project permissions
										if ((wasSet != null && wasSet
												.equals("1")) || pc.getRead()) {

											final PermissionCriteria share = new PermissionCriteria();
											share.setField(es.getElementName()
													+ "/sharing/share/project");
											share.setFieldValue(ug.getTag());
											share.setRead(pc.getRead());
											share.setComparisonType("equals");
											tempGroup.addRootPermission(
													es.getElementName(), share);
										}
									}
								}
							}
						...
						// verify that the user isn't trying to gain access to
						// other projects.
						for (XdatElementAccess ea : tempGroup
								.getElementAccess()) {
							for (XdatFieldMappingSet set : ea
						...
						// because we've checked the values in the permissions,
						// we can use the pre-authorized save method
						SaveItemHelper.authorizedSave(
								tempGroup,
								user,
								false,
								true,
					...
					if (props.containsKey("src")) {
						...
						this.getResponse().redirectSeeOther(
								XFT.GetSiteURL() + "/data/projects/"
										+ group.getTag() + "?format=html");
					} else {
						returnDefaultRepresentation();
					}
				} else {
	...
	/**
	 * This checks for hacking attempts. The user cannot add permissions to this
	 * group for other projects.
	 * 
	 * @param set
	 * @param tag
	 * @throws InvalidValueException
	 */
	private void verifyGroupPermissions(XdatFieldMappingSet set, String tag)
			throws InvalidValueException {
			...
			if (group == null) {
				...
				try {
					StringBuffer query = new StringBuffer(
							"SELECT ug.id, ug.displayname,ug.tag,ug.xdat_usergroup_id, COUNT(map.groups_groupid_xdat_user_xdat_user_id) AS users FROM xdat_userGroup ug LEFT JOIN xdat_user_groupid map ON ug.id=map.groupid WHERE tag='")
							.append(proj.getId()).append("' ");
					...
					return this.representTable(table, overrideVariant(variant),
							params);
				...
				return this.representItem(group.getItem(),
