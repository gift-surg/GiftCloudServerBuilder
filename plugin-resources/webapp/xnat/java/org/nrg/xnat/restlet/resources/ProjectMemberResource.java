...
public class ProjectMemberResource extends SecureResource {
	...
	public ProjectMemberResource(Context context, Request request,
			Response response) {
		...
		try {
			...
			for (int i = 0; i < ids.length; i++) {
				...
				if (xdat_user_id == null) {
					...
					if (newUser == null) {
						// by email
						ArrayList<ItemI> items = ItemSearch.GetItems(
								"xdat:user/email", uID, user, false).items();
						if (items.size() > 0) {
					...
					XdatUser tempUser = XdatUser.getXdatUsersByXdatUserId(
							xdat_user_id, user, false);
					if (tempUser != null) {
		...
		displayHiddenUsers = Boolean.parseBoolean((String) getParameter(
				request, "DISPLAY_HIDDEN_USERS"));
			...
			try {
				if (user.canDelete(proj)) {
					if (unknown.size() > 0) {
						...
						try {
							for (String uID : unknown) {
								...
								context.put("server",
										TurbineUtils.GetFullServerPath(request));
								context.put("process",
								...
								final PersistentWorkflowI wrk = PersistentWorkflowUtils
										.getOrCreateWorkflowData(
												...
												newEventInstance(
														EventUtils.CATEGORY.PROJECT_ACCESS,
														EventUtils.INVITE_USER_TO_PROJECT
																+ " ("
																+ uID
																+ ")"));
								try {
									ProjectAccessRequest
											.InviteUser(
													...
													user.getFirstname()
															...
															+ DisplayManager
																	.GetInstance()
																	.getSingularDisplayNameForProject()
																	.toLowerCase()
															+ ".");
									WorkflowUtils.complete(wrk,
											wrk.buildEvent());
								} catch (Exception e) {
					...
					if (newUsers.size() > 0) {
						...
						String email = (this.isQueryVariableTrue("sendemail")) ? "true"
								: "false";
						...
						for (XDATUser newUser : newUsers) {
							final PersistentWorkflowI wrk = PersistentWorkflowUtils
									.getOrCreateWorkflowData(
											...
											user,
											newUser.getXSIType(),
											newUser.getXdatUserId().toString(),
											proj.getId(),
											newEventInstance(
													EventUtils.CATEGORY.PROJECT_ACCESS,
							...
							try {
								proj.addGroupMember(
										...
										newUser,
										user,
										WorkflowUtils.setStep(wrk, "Add "
												+ newUser.getLogin()), true);
								...
								if (sendmail) {
									try {
										...
										context.put("user", user);
										context.put("server", TurbineUtils
												.GetFullServerPath(request));
										context.put("process",
												"Transfer to the archive.");
										context.put("system",
												TurbineUtils.GetSystemName());
