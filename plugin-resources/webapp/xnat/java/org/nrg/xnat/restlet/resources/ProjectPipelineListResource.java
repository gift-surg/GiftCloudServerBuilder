...
public class ProjectPipelineListResource extends SecureResource {
	...
	public ProjectPipelineListResource(Context context, Request request,
			Response response) {
	...
	public void handleDelete() {
		// Remove the Pipeline identified by the path for the project and the
		// datatype
		if (proj != null) {
			...
			if (pathToPipeline != null && datatype != null) {
				...
				if (isUserAuthorized) {
					try {
						ArcProject arcProject = ArcSpecManager
								.GetFreshInstance().getProjectArc(proj.getId());
						boolean success = PipelineRepositoryManager
								.GetInstance().delete(arcProject,
										pathToPipeline, datatype, user);
						if (!success) {
							getLogger().log(
									getLogger().getLevel(),
							...
							getResponse().setEntity(
									getRepresentation(getVariants().get(0)));
							Representation selectedRepresentation = getResponse()
									.getEntity();
							if (getRequest().getConditions().hasSome()) {
								final Status status = getRequest()
										.getConditions().getStatus(
							...
							// getResponse().setStatus(Status.SUCCESS_OK,"Pipeline has been removed from project "
							// + proj.getId());
						}
					} catch (Exception e) {
						e.printStackTrace();
					...
					getResponse()
							.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
									"User unauthroized to remove pipeline from project");
				}
			}
		} else {
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (isUserAuthorized) {
			...
			// If yes, then return that entry. If not then construct a new
			// ArcProject element and insert an attribute to say that it's an
			// already existing
			// entry or not
			try {
				if (arcProject == null) { // No Project pipelines set in the
											// archive specification
					if (additional) {
						arcProject = PipelineRepositoryManager.GetInstance()
								.createNewArcProject(proj);
						comment = "new";
					} else {
					...
					if (additional) { // Return all the pipelines that are
										// applicable to the project but not
										// selected
						arcProject = PipelineRepositoryManager.GetInstance()
								.getAdditionalPipelines(proj);
						comment = "additional";
					} else {
			...
			if (mt.equals(MediaType.TEXT_XML)) {
				return representItem(arcProject.getItem(), mt, null, false,
						true);
			} else if (mt.equals(MediaType.APPLICATION_JSON)) {
				XFTTable table = PipelineRepositoryManager.GetInstance()
						.toTable(arcProject);
