...
public class ProjectResource extends ItemResource {
	...
	public ProjectResource(Context context, Request request, Response response)
		...
		// This was part of a fix for XNAT-3453, but it breaks other
		// non-standard REST ways of setting project properties.
		...
		fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XMLPathShortcuts.PROJECT_DATA, false));
	}
					...
					if (user.canDelete(proj)) {
						final PersistentWorkflowI workflow = WorkflowUtils
								.getOrCreateWorkflowData(
										...
										newEventInstance(
												...
												EventUtils.getDeleteAction(proj
														.getXSIType())));
						...
						try {
							...
							String query = "UPDATE xnat_projectdata_history SET id = '"
									...
									+ timestamp
									+ "' where id = '" + proj.getId() + "';";
							PoolDBUtils.ExecuteNonSelectQuery(query, proj
									.getItem().getDBName(), user.getLogin());
						} catch (Exception e) {
							logger.error("", e);
						...
						getResponse()
								.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
										"User account doesn't have permission to delete this project.");
					}
			...
			try {
				if (proj == null || user.canEdit(proj)) {
					...
					if (getQueryVariable("allowDataDeletion") != null
							&& getQueryVariable("allowDataDeletion")
					...
					if (item.instanceOf("xnat:projectData")) {
						...
						if (filepath != null && !filepath.equals("")) {
							...
							if (filepath.startsWith("quarantine_code/")) {
										...
										if (qc.equals("true")) {
											...
											getResponse()
													.setStatus(
															...
															"Prearchive code must be an integer.");
											return;
										}
									}
								...
								if (!qc.equals("")) {
									...
									create(project,
											...
											newEventInstance(
													EventUtils.CATEGORY.PROJECT_ADMIN,
													"Configured current arc"));
									ArcSpecManager.Reset();
								}
							} else if (filepath
									.startsWith("scan_type_mapping/")) {
							...
							if (!StringUtils.IsAlphaNumericUnderscore(project
									.getId())
									&& !isQueryVariableTrue("testHyphen")) {
								getResponse().setStatus(
										Status.CLIENT_ERROR_EXPECTATION_FAILED,
		...
		if (proj != null) {
			...
			try {
				for (FilteredResourceHandlerI filter : getHandlers(
						"org.nrg.xnat.restlet.projectResource.extensions",
						_defaultHandlers)) {
		...
		public Representation handle(SecureResource resource, Variant variant)
			...
			if (resource.filepath != null && !resource.filepath.equals("")) {
				if (resource.filepath.equals("quarantine_code")) {
					try {
						return new StringRepresentation(projResource.proj
								.getArcSpecification().getQuarantineCode()
								.toString(), mt);
					} catch (Throwable e) {
						logger.error("", e);
						projResource.getResponse().setStatus(
					...
					try {
						return new StringRepresentation(projResource.proj
								.getArcSpecification().getCurrentArc(), mt);
					} catch (Throwable e) {
						logger.error("", e);
				...
				return projResource.representItem(projResource.proj.getItem(),
						mt);
			}
		}
