...
public class ProjectSearchResource extends ItemResource {
	...
	public ProjectSearchResource(Context context, Request request,
			Response response) {
		...
		if (xss != null) {
			if (mt.equals(MediaType.TEXT_XML)) {
				ItemXMLRepresentation rep = new ItemXMLRepresentation(
						xss.getItem(), MediaType.TEXT_XML, true,
						!this.isQueryVariableTrue("concealHiddenFields"));
				if (sID.startsWith("@")) {
			...
			// return new
			// JSONTableRepresentation(item,params,MediaType.APPLICATION_JSON);
			// }else{
			// return new
			// HTMLTableRepresentation(item,params,MediaType.TEXT_HTML);
			// }
		}
