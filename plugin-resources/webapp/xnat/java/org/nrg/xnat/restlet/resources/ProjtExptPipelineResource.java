...
public class ProjtExptPipelineResource extends SecureResource {
	...
	public ProjtExptPipelineResource(Context context, Request request,
			Response response) {
		...
		if (proj != null && step != null) {
			...
			try {
				if (expt == null) { // Look for Project level pipeline
					arcPipeline = (ArcPipelinedata) arcProject
							.getPipeline(step);
				} else { // Look for experiment level pipeline
					arcPipeline = (ArcPipelinedata) arcProject
							.getPipelineForDescendant(expt.getXSIType(), step);
				}
				MediaType mt = overrideVariant(variant);
				if (mt.equals(MediaType.TEXT_XML)) {
					return representItem(arcPipeline.getItem(), mt, null,
							false, true);
				} else {
					return null;
				}
			...
			try {
				if (step.equals(XNATRestConstants.TRIGGER_PIPELINES)) {
					if (user.canEdit(expt)) {
						...
						.equals(XNATRestConstants.PULL_DATA_FROM_HEADERS)
				...
				} else if (step.equals(XNATRestConstants.FIX_SCAN_TYPES)
						&& expt instanceof XnatImagesessiondata) {
					if (user.canEdit(expt)) {
					...
					if (StringUtils.isNotBlank(XMLbody)) {
						ParametersDocument doc = ParametersDocument.Factory
								.parse(XMLbody);
						for (ParameterData param : doc.getParameters()
								.getParameterArray()) {
							Values values = param.getValues();
							if (values.isSetUnique()) {
								xmlDocumentParams.put(param.getName(),
										values.getUnique());
							} else {
								String listCSV = "["
										+ StringUtils.join(
					...
					// If we have passed in "legacy" as a query param, if
					// "legacy=true" we are in legacy mode, else not
					// Else, if we have ANY params in the form body, xml
					// document, or query params we are not in legacy mode
					boolean legacy = true;
					if (queryParams.containsKey("legacy")) {
						legacy = queryParams.get("legacy").equalsIgnoreCase(
								"true");
						queryParams.remove("legacy");
					} else if (!bodyParams.keySet().isEmpty()
							|| !xmlDocumentParams.keySet().isEmpty()
							|| !queryParams.keySet().isEmpty()) {
						legacy = false;
					}
					...
					try {
						ArrayList<ArcPipelinedataI> arcPipelines = arcProject
								.getPipelinesForDescendant(expt.getXSIType(),
										step, match);
						for (ArcPipelinedataI arcPipeline : arcPipelines) {

							logger.info("Launching pipeline at step "
									+ arcPipeline.getLocation()
									+ File.separator + arcPipeline.getName());
							if (legacy) {
						...
						e.printStackTrace();
						logger.error("Pipeline step " + step + " for project "
								+ proj.getId() + " does not exist");
						// getResponse().setStatus(Status.SERVER_ERROR_INTERNAL);
					}
				}
			} catch (Exception e) {
	...
	private void launch(ArcPipelinedataI arcPipeline,
		...
		String buildDir = PipelineFileUtils
				.getBuildDir(expt.getProject(), true);
		buildDir += "restlaunch";
		...
		for (String paramName : paramsMap.keySet()) {
			...
			if (paramVal.length() > 2 && paramVal.startsWith("[")
					&& paramVal.endsWith("]")) {
				String[] paramArray = StringUtils.substringBetween(paramVal,
						"[", "]").split(",");
		...
		String paramFileName = expt.getLabel() + "_" + arcPipeline.getName()
				+ "_params_" + dateSuffix + ".xml";
		String paramFilePath = saveParameters(
				buildDir + File.separator + expt.getLabel(), paramFileName,
				parameters);
	...
	protected String saveParameters(String rootpath, String fileName,
