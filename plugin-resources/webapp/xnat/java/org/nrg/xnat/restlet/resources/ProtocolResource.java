...
public class ProtocolResource extends ItemResource {
	...
	public void handlePut() {
		try {
			...
			XFTItem item = this.loadItem("xnat:datatypeProtocol", true,
					template);
			...
			if (item.instanceOf("xnat:datatypeProtocol")) {
				...
				if (this.proj == null && protocol.getProject() != null) {
					proj = XnatProjectdata.getXnatProjectdatasById(
							protocol.getProject(), user, false);
				}

				if (this.proj != null) {
					if (protocol.getProject() == null
							|| protocol.getProject().equals("")) {
						protocol.setProperty("xnat_projectdata_id",
								proj.getId());
					}
				} else {
					this.getResponse()
				...
				PersistentWorkflowI wrk = PersistentWorkflowUtils
						.getOrCreateWorkflowData(null, user, proj.getItem(),
								this.newEventInstance(
										EventUtils.CATEGORY.PROJECT_ADMIN,
										"Modified event data-type protocol."));
				try {
					if (SaveItemHelper.authorizedSave(protocol, user, false,
							true, wrk.buildEvent())) {
	...
	public void handleDelete() {
		...
		try {
			...
			if (!user.canEdit(proj)) {
				this.getResponse()
						.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
								"User account doesn't have permission to delete this subject.");
				return;
			}
				...
				if (protocol != null) {
					PersistentWorkflowI wrk = PersistentWorkflowUtils
							.getOrCreateWorkflowData(
									...
									this.newEventInstance(
											EventUtils.CATEGORY.PROJECT_ADMIN,
											"Deleted event data-type protocol."));
					try {
						SaveItemHelper.authorizedDelete(protocol.getItem()
								.getCurrentDBVersion(), user, wrk.buildEvent());
						PersistentWorkflowUtils.complete(wrk, wrk.buildEvent());
					} catch (Exception e1) {
						PersistentWorkflowUtils.fail(wrk, wrk.buildEvent());
						throw e1;
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (protocol != null) {
			...
			if (this.getQueryVariable("dataType") != null && proj != null) {
				...
				XnatDatatypeprotocol temp = (XnatDatatypeprotocol) proj
						.getProtocolByDataType(dataType);

				try {
					ElementSecurity ess = ElementSecurity
							.GetElementSecurity(dataType);
					...
					if (temp == null && ess != null) {
						...
						if (temp.getXSIType().equals("xnat:datatypeProtocol")) {
							temp.setProperty(
									...
									temp.getProperty("data-type"));
							temp.setProperty(
									"xnat:datatypeProtocol/definitions/definition[ID=default]/project-specific",
									"false");
						}
						PersistentWorkflowI wrk = PersistentWorkflowUtils
								.getOrCreateWorkflowData(null, user, proj
										.getItem(), this.newEventInstance(
										EventUtils.CATEGORY.PROJECT_ADMIN,
										"Modified event data-type protocol."));
						try {
							SaveItemHelper.authorizedSave(temp, user, false,
									false, wrk.buildEvent());
							PersistentWorkflowUtils.complete(wrk,
									wrk.buildEvent());
						} catch (Exception e1) {
							PersistentWorkflowUtils.fail(wrk, wrk.buildEvent());
							throw e1;
				...
				if (temp == null) {
					this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
							"Unable to find the specified protocol.");
					return null;
				} else {
