...
public abstract class QueryOrganizerResource extends SecureResource {
	...
	public QueryOrganizerResource(Context context, Request _request,
			Response _response) {
	...
	public CriteriaCollection processStringQuery(String xmlPath, String values) {
		ArrayList<String> al = StringUtils
				.CommaDelimitedStringToArrayList(values);
	...
	public CriteriaCollection processDateQuery(String column, String dates) {
		ArrayList<String> al = StringUtils
				.CommaDelimitedStringToArrayList(dates);
		CriteriaCollection cc = new CriteriaCollection("OR");
		for (String date : al) {
			if (date.indexOf("-") > -1) {
				String date1 = null;
				try {
					date1 = DateUtils.parseDate(
							date.substring(0, date.indexOf("-"))).toString();
				...
				String date2 = null;
				try {
					date2 = DateUtils.parseDate(
							date.substring(date.indexOf("-") + 1)).toString();
				} catch (ParseException e) {
	...
	public CriteriaCollection processQueryCriteria(String xPath, String values) {
		CriteriaCollection cc = new CriteriaCollection("OR");
		try {
			GenericWrapperField gwf = GenericWrapperElement
					.GetFieldForXMLPath(xPath);
	...
	public void populateQuery(QueryOrganizer qo) {
		if (hasQueryVariable("columns")
				&& !getQueryVariable("columns").equals("DEFAULT")) {
			try {
				columns = StringUtils
						.CommaDelimitedStringToArrayList(URLDecoder.decode(
								getQueryVariable("columns"), "UTF-8"));
		...
		if (this.fieldMapping.size() > 0) {
			for (String key : fieldMapping.keySet()) {
				if (!key.equals("xsiType") && hasQueryVariable(key)) {
					cc.add(this.processQueryCriteria(
							this.fieldMapping.get(key), getQueryVariable(key)));
				}
			}
		}
		...
		if (isQueryVariable("req_format", "form", false)) {
			...
			if (hasBodyVariable("columns")) {
				columns = StringUtils
						.CommaDelimitedStringToArrayList(getBodyVariable("columns"));
	...
	public XFTTable formatHeaders(XFTTable table, QueryOrganizer qo,
			String idpath, String URIpath) {
	...
	public String getRootElementName() {
		try {
			GenericWrapperElement rootElementName = GenericWrapperElement
					.GetElement(this.getDefaultElementName());
			if (this.getQueryVariable("xsiType") != null
					&& this.getQueryVariable("xsiType").indexOf(",") == -1) {
			...
			for (String key : getQueryVariableKeys()) {
				if (key.indexOf("/") > -1) {
					...
					for (String col : StringUtils
							.CommaDelimitedStringToArrayList(getQueryVariable("columns"))) {
						if (col.indexOf("/") > -1) {
							fields.add(col);
						} else if (this.fieldMapping.containsKey(col)) {
			...
			for (String field : fields) {
				try {
					GenericWrapperElement ge = StringUtils
							.GetRootElement(field);
					if (!ge.getXSIType().equals(rootElementName.getXSIType())
							&& ge.isExtensionOf(rootElementName)) {
						rootElementName = ge;
					}
				} catch (ElementNotFoundException e) {
					logger.error("", e);
	...
	public Representation getRepresentation(Variant variant) {
		try {
			if (this.getQueryVariable("fields") != null) {
				...
				return this.representTable(table,
						this.overrideVariant(variant), null);
			} else {
				return null;
			}
		} catch (Exception e) {
