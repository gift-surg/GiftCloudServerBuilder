...
public class ReconList extends QueryOrganizerResource {
	...
	public ReconList(Context context, Request request, Response response) {
		...
		this.fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XMLPathShortcuts.RECON_DATA, true));
			...
			if (item.instanceOf("xnat:reconstructedImageData")) {
				...
				if (existing == null) {
					...
					this.getResponse().setStatus(Status.CLIENT_ERROR_CONFLICT,
							"Specified reconstruction already exists.");
					return;
					// MATCHED
				}
				...
				this.getResponse().setStatus(
						...
						"Only Scan documents can be PUT to this address.");
			}
	...
	public Representation getRepresentation(Variant variant) {
		if (recon != null) {
			return new ItemXMLRepresentation(recon.getItem(),
					MediaType.TEXT_XML);
		} else {
			...
			try {
				...
				table = formatHeaders(table, qo,
						"xnat:reconstructedImageData/ID", String.format(
								"/data/experiments/%s/reconstructions/",
								session.getId()));
			} catch (Exception e) {
				e.printStackTrace();
