...
public class ReconResource extends ItemResource {
	...
	public ReconResource(Context context, Request request, Response response) {
		...
		this.fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XMLPathShortcuts.RECON_DATA, false));
	}
	...
	public void handlePut() {
		...
		try {
			...
			if (item.instanceOf("xnat:reconstructedImageData")) {
				recon = (XnatReconstructedimagedata) BaseElement
				...
				if (this.session != null) {
					...
					if (recon.getImageSessionId() != null
							&& !recon.getImageSessionId().equals("")) {
						this.session = (XnatImagesessiondata) XnatExperimentdata
								.getXnatExperimentdatasById(
										recon.getImageSessionId(), user, false);

						if (this.session == null && this.proj != null) {
							this.session = (XnatImagesessiondata) XnatExperimentdata
									.GetExptByProjectIdentifier(
											this.proj.getId(),
											recon.getImageSessionId(), user,
											false);
						}
						if (this.session != null) {
							recon.setImageSessionId(this.session.getId());
				...
				if (recon.getImageSessionId() == null) {
					this.getResponse()
							.setStatus(Status.CLIENT_ERROR_EXPECTATION_FAILED,
									"Specified reconstruction must reference a valid image session.");
					return;
				}
				...
				if (recon.getXnatReconstructedimagedataId() != null) {
					existing = (XnatReconstructedimagedata) XnatReconstructedimagedata
							.getXnatReconstructedimagedatasByXnatReconstructedimagedataId(
				...
				if (recon.getId() != null) {
					existing = (XnatReconstructedimagedata) XnatReconstructedimagedata
							.getXnatReconstructedimagedatasById(recon.getId(),
									user, completeDocument);
				}
				...
				if (existing == null) {
					...
					if (recon.getId() == null || recon.getId().equals("")) {
						...
						try {
							int i = 1;
							Long idCOUNT = (Long) PoolDBUtils
									.ReturnStatisticQuery(
											query + this.session.getId()
													+ "_RECON_" + i + "';",
											"id_count", user.getDBName(), login);
							while (idCOUNT > 0) {
								i++;
								idCOUNT = (Long) PoolDBUtils
										.ReturnStatisticQuery(query
												+ this.session.getId()
												+ "_RECON_" + i + "';",
												"id_count", user.getDBName(),
												login);
							}
				...
				create(session,
						...
						newEventInstance(
								...
								EventUtils.getAddModifyAction(
										recon.getXSIType(), recon == null)));
			} else {
				this.getResponse()
						.setStatus(Status.CLIENT_ERROR_UNPROCESSABLE_ENTITY,
								"Only reconstruction documents can be PUT to this address.");
			}
	...
	public void handleDelete() {
		if (recon == null && exptID != null) {
			recon = (XnatReconstructedimagedata) XnatReconstructedimagedata
					.getXnatReconstructedimagedatasById(exptID, user,
							completeDocument);
		}
		...
		try {
			...
			final PersistentWorkflowI workflow = WorkflowUtils
					.getOrCreateWorkflowData(
							...
							session.getXSIType(),
							session.getId(),
							(proj == null) ? session.getProject() : proj
									.getId(),
							newEventInstance(EventUtils.CATEGORY.DATA,
									EventUtils.getDeleteAction(recon
											.getXSIType())));
				...
				if (removeFiles != null) {
					for (XnatAbstractresourceI om : recon.getOut_file()) {
						...
						resourceA.deleteWithBackup(
								session.getArchiveRootPath(), user, ci);
					}
				}
				SaveItemHelper.authorizedDelete(recon.getItem()
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (recon != null) {
			return this
					.representItem(recon.getItem(), overrideVariant(variant));
