...
public class ScanList extends QueryOrganizerResource {
	...
	public ScanList(Context context, Request request, Response response) {
		...
		this.fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XnatImagescandata.SCHEMA_ELEMENT_NAME, true));
		...
		try {
			...
			if (item.instanceOf("xnat:imageScanData")) {
				...
				if (scan.getXnatImagescandataId() != null) {
					existing = (XnatImagescandata) XnatImagescandata
				...
				create(session,
						...
						newEventInstance(EventUtils.CATEGORY.DATA, EventUtils
								.getAddModifyAction(scan.getXSIType(),
										scan == null)));
		...
		if (scan != null) {
			...
			try {
				...
				table = formatHeaders(
						...
						String.format("/data/experiments/%s/scans/",
								session.getId()));
			} catch (Exception e) {
				logger.error("", e);
