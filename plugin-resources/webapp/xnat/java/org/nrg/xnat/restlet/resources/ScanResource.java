	...
	public ScanResource(Context context, Request request, Response response) {
		...
		if (assessedID != null) {
			...
			if (session == null && proj != null) {
				session = (XnatImagesessiondata) XnatExperimentdata
						.GetExptByProjectIdentifier(proj.getId(), assessedID,
								user, false);
			}
			...
			fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
					XnatImagescandata.SCHEMA_ELEMENT_NAME, false));
		} else {
			response.setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
					"You must specify a session ID for this request.");
		}
	}
		...
		try {
			...
			if (!item.instanceOf("xnat:imageScanData")) {
				getResponse()
						.setStatus(
								...
								"Only Scan documents can be PUT to this address. Expected: xnat:imageScanData Received: "
										+ item.getXSIType());
				return;
			}
			...
			if (item.getXSIType().equals("xnat:imageScanData")) {
				getResponse()
						.setStatus(
								Status.CLIENT_ERROR_BAD_REQUEST,
								"Multiple scan modalities can be associated here.  Please retry with the specification of a particular modality (i.e. ?xsiType=xnat:mrScanData).");
				return;
			}
			...
			if (scan.getImageSessionId() == null) {
				getResponse().setStatus(Status.CLIENT_ERROR_EXPECTATION_FAILED,
						"Specified scan must reference a valid image session.");
				return;
			}

			if (session == null) {
				getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
						"Specified image session doesn't exist.");
				return;
			}
			...
			if (scan.getXnatImagescandataId() != null) {
				existing = XnatImagescandata
						.getXnatImagescandatasByXnatImagescandataId(
								scan.getXnatImagescandataId(), user,
								completeDocument);
			}
			...
			if (existing == null) {
				if (!user.canEdit(session)) {
					getResponse()
							.setStatus(
									...
									"Specified user account has insufficient create privileges for sessions in this project.");
					return;
				}
				// IS NEW
				if (scan.getId() == null || scan.getId().equals("")) {
					String query = "SELECT count(id) AS id_count FROM xnat_imageScanData WHERE image_session_id='"
							+ session.getId() + "' AND id='";

					String login = user.getUsername();
					try {
						int i = 1;
						Long idCOUNT = (Long) PoolDBUtils.ReturnStatisticQuery(
								query + i + "';", "id_count", user.getDBName(),
								login);
						while (idCOUNT > 0) {
							i++;
							idCOUNT = (Long) PoolDBUtils.ReturnStatisticQuery(
									query + i + "';", "id_count",
									user.getDBName(), login);
						}
						scan.setId("" + i);
					} catch (Exception e) {
						logger.error("", e);
				...
				if (!user.canEdit(session)) {
					getResponse()
							.setStatus(
									Status.CLIENT_ERROR_FORBIDDEN,
									"Specified user account has insufficient edit privileges for sessions in this project.");
					return;
				}
				// MATCHED
			}
			...
			if (getQueryVariable("allowDataDeletion") != null
			...
			create(session,
					...
					newEventInstance(EventUtils.CATEGORY.DATA,
							EventUtils.getAddModifyAction(scan.getXSIType(),
									scan == null)));

			if (isQueryVariableTrue(XNATRestConstants.PULL_DATA_FROM_HEADERS)
				...
				try {
					PullScanDataFromHeaders pull = new PullScanDataFromHeaders(
							scan, user, allowDataDeletion, false, c);
					pull.call();
	...
	public void handleDelete() {
		...
		try {
			...
			delete(session,
					scan,
					newEventInstance(EventUtils.CATEGORY.DATA,
							EventUtils.getDeleteAction(scan.getXSIType())));
