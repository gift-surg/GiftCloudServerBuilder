...
public class ScriptResource extends AutomationResource {
	...
	public ScriptResource(Context context, Request request, Response response)
		...
		_runnerService = XDAT.getContextService().getBean(
				ScriptRunnerService.class);
		...
		if (request.getMethod().equals(Method.DELETE)
				&& StringUtils.isBlank(_scriptId)) {
			response.setStatus(Status.CLIENT_ERROR_METHOD_NOT_ALLOWED,
					"You must specify a specific script ID to delete a script.");
	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		if (StringUtils.isNotBlank(_scriptId)) {
			try {
				...
				// have to check if it's null, or else it will return a
				// StringRepresentation containing the word null instead of a
				// 404
				if (script != null) {
					return new StringRepresentation(
							MAPPER.writeValueAsString(script), mediaType);
				} else {
					return null;
				}
				...
				throw new ResourceException(
						...
						"An error occurred marshalling the script data to JSON",
						e);
			} catch (IOException e) {
				throw new ResourceException(
						Status.SERVER_ERROR_INTERNAL,
		...
		try {
			if (StringUtils.isNotBlank(_scriptId)) {
				...
				throw new ClientException(
						...
						"You must specify a script ID on the REST URL to PUT a script to the server.");
			}
		} catch (ClientException e) {
			getResponse().setStatus(e.getStatus(), e.getMessage());
		} catch (ServerException e) {
			_log.error(
					"Server error occurred trying to store a script resource",
					e);
			getResponse().setStatus(e.getStatus(), e.getMessage());
		}
	}
	...
	public void handleDelete() {
		try {
			if (_log.isDebugEnabled()) {
				_log.debug("Preparing to delete script: " + _scriptId
			...
			getResponse()
					.setStatus(Status.SERVER_ERROR_INTERNAL, e,
							"A service exception occurred trying to delete (disable) script");
		}
	}
	...
	private Representation listScripts(final MediaType mediaType) {
		...
		final List<Script> scripts = getScope() == Scope.Site ? _runnerService
				.getScripts() : _runnerService.getScripts(getScope(),
				getProjectDataInfo());
		for (final Script script : scripts) {
			table.insertRowItems(script.getScriptId(), script.getLanguage(),
					script.getLanguageVersion(), script.getDescription());
		}
	...
	private void putScript() throws ClientException, ServerException {
		...
		if (entity.getSize() == 0) {
			...
			getResponse().setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
					"Unable to find script parameters: no data sent?");
			return;
		}
		...
		if (getScope() == Scope.Project) {
			...
			// Only create site association when this is a new script. Otherwise
			// site association must be made
			// explicitly.
		...
		try {
			...
			getResponse().setStatus(Status.CLIENT_ERROR_UNPROCESSABLE_ENTITY,
					e, "An error occurred saving the script " + _scriptId);
		}
	}

	private static final Logger _log = LoggerFactory
			.getLogger(ScriptResource.class);
