...
public class ScriptTriggerResource extends AutomationResource {
    ...
    public ScriptTriggerResource(Context context, Request request, Response response) throws ResourceException {
        ...
        if (request.getMethod().equals(Method.DELETE) && StringUtils.isBlank(_event)) {
            throw new ResourceException(Status.CLIENT_ERROR_METHOD_NOT_ALLOWED, "You must specify a specific scope and event to delete a script trigger.");
        }
    ...
    public Representation represent(Variant variant) throws ResourceException {
        ...
        if (StringUtils.isNotBlank(_event)) {
            try {
                ...
                if (trigger == null) {
                    throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND, "Didn't find a script trigger for the indicated " + formatScopeEntityIdAndEvent());
                }
                return new StringRepresentation(MAPPER.writeValueAsString(trigger), mediaType);
    ...
    public void handlePut() {
        try {
            if (StringUtils.isNotBlank(_event)) {
                ...
                throw new ClientException(Status.CLIENT_ERROR_METHOD_NOT_ALLOWED, "You must specify an event on the REST URL to PUT a script trigger to the server.");
            }
    ...
    public void handleDelete() {
        try {
            ...
            final ScriptTrigger trigger = _service.getByAssociationAndEvent(getAssociation(), _event);
            if (trigger == null) {
                throw new ClientException(Status.CLIENT_ERROR_NOT_FOUND, "Didn't find a trigger to delete associated with " + formatScopeEntityIdAndEvent());
            }
    ...
    private Representation listScriptTriggers(final MediaType mediaType) {
        ...
        final List<ScriptTrigger> triggers = getScope() == Scope.Site ? _service.getSiteTriggers() : _service.getByScope(getScope(), getProjectDataInfo());
    ...
    private ScriptTrigger getScriptTrigger() {
        return _service.getByScopeEntityAndEvent(getScope(), getProjectDataInfo(), _event);
    }
    ...
    private void putScriptTrigger() throws ClientException, ServerException {
        ...
        ScriptTrigger trigger = _service.getByScopeEntityAndEvent(getScope(), getProjectDataInfo(), _event);
        if (trigger == null) {
            ...
            final String triggerId = properties.getProperty("triggerId", _service.getDefaultTriggerName(scriptId, getScope(), getProjectDataInfo(), event));
            final String description = properties.getProperty("description", null);
            trigger = _service.newEntity(triggerId, description, scriptId, getAssociation(), event);
            ...
            if (StringUtils.isNotBlank(scriptId) && !scriptId.equals(trigger.getScriptId())) {
                trigger.setScriptId(scriptId);
                isDirty = true;
            }
            if (StringUtils.isNotBlank(event) && !event.equals(trigger.getEvent())) {
                trigger.setEvent(event);
                isDirty = true;
            }
            if (StringUtils.isNotBlank(triggerId) && !triggerId.equals(trigger.getTriggerId())) {
                trigger.setTriggerId(triggerId);
                isDirty = true;
            }
            // Description is a little different because you could specify an empty description.
            if (description != null && !description.equals(trigger.getDescription())) {
