...
public class ScriptTriggerTemplateResource extends AutomationResource {
	...
	private static final Logger _log = LoggerFactory
			.getLogger(ScriptTriggerTemplateResource.class);
	...
	public ScriptTriggerTemplateResource(Context context, Request request,
		...
		if (getScope() == Scope.Site) {
			if (!user.isSiteAdmin()) {
				...
				throw new ResourceException(
						...
						"Only site admins can view or update script trigger templates for the entire site.");
			}
		} else {
			if (!user.isSiteAdmin() || user.isOwner(getProjectId())) {
				...
				throw new ResourceException(
						Status.CLIENT_ERROR_FORBIDDEN,
						"Only site admins and project owners can view or update script trigger templates for the project "
								+ getProjectId() + ".");
			}
		}

		_triggerService = XDAT.getContextService().getBean(
				ScriptTriggerService.class);
		_templateService = XDAT.getContextService().getBean(
				ScriptTriggerTemplateService.class);

		// A bit much for now, but built to handle more scopes, e.g. subjects,
		// users, etc.
		switch (getScope()) {
		case Project:
			XnatProjectdata project = XnatProjectdata.getProjectByIDorAlias(
					getProjectId(), user, false);
			if (project == null) {
				throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
						"Couldn't find the project with ID or alias "
								+ getProjectId());
			}
			break;
		...
		default:
			throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
					"Unknown scope " + getScope());
		}

		if (_log.isDebugEnabled()) {
			_log.debug(getRequestContext("Servicing script request for user "
					+ user.getLogin()));
		}
	}
	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		if (StringUtils.isBlank(_templateId)) {
			// List the available templates (that method figures out if it's
			// site-wide or for a project.
			return listTemplates(mediaType);
		}
		// If there was a template ID, then list the triggers for that template
		// ID.
		try {
		...
		try {
			if (StringUtils.isNotBlank(_templateId)) {
				...
				throw new ClientException(
						...
						"You must specify a template ID on the REST URL to PUT a template to the server.");
			}
		} catch (ClientException e) {
		...
		try {
			if (StringUtils.isBlank(_templateId)) {
				throw new ClientException(
						...
						"You must specify a template ID on the REST URL to DELETE a template from the server.");
			}
			ScriptTriggerTemplate template = _templateService
					.getByName(_templateId);
			if (template == null) {
				throw new ClientException(Status.CLIENT_ERROR_NOT_FOUND,
						"The template ID " + _templateId
	 ...
	 *
	 * @return A representation of the scripts available at the specified scope
	 *         and entity ID (if specified).
	 */
	private Representation listTemplates(MediaType mediaType) {
		...
		final List<ScriptTriggerTemplate> templates = StringUtils
				.isBlank(getProjectId()) ? _templateService.getAll()
				: _templateService.getTemplatesForEntity(Long
						.parseLong(getProjectId()));
		for (final ScriptTriggerTemplate template : templates) {
			table.insertRowItems(template.getTemplateId(),
	...
	private Representation listTriggers(MediaType mediaType)
		...
		if (template == null) {
			throw new ClientException(Status.CLIENT_ERROR_NOT_FOUND,
					"Couldn't find a template with ID " + _templateId);
		}

		for (final ScriptTrigger trigger : template.getTriggers()) {
			table.insertRowItems(trigger.getTriggerId(), trigger.getScriptId(),
					trigger.getAssociation(), trigger.getEvent(),
	...
	private void putTemplate() throws ClientException {
		...
		if (entity.getSize() == 0) {
			throw new ClientException(Status.CLIENT_ERROR_BAD_REQUEST,
					"Unable to find template parameters: no data sent?");
		}
		...
		if (mediaType.equals(MediaType.APPLICATION_WWW_FORM)) {
			try {
				...
				throw new ClientException(
						...
						"An error occurred trying to handle the incoming form data.",
						e);
			}
			...
			try {
				...
				throw new ClientException(Status.SERVER_ERROR_INTERNAL,
						"An error occurred processing the script properties", e);
			}
		} else {
			throw new ClientException(
					Status.CLIENT_ERROR_UNSUPPORTED_MEDIA_TYPE,
					"This function currently only supports the media type "
							+ MediaType.APPLICATION_JSON);
		}
		assert found != null;
		saveTemplate(found);
	...
	private void saveTemplate(final ScriptTriggerTemplate found) {
		final ScriptTriggerTemplate existing = _templateService.getByName(found
				.getTemplateId());
		if (existing == null) {
			final Set<ScriptTrigger> triggers = saveTriggers(found
					.getTriggers());
			if (triggers != null) {
			...
			final Set<Long> currentAssociates = new TreeSet<Long>(
					existing.getAssociatedEntities());
			final Set<Long> proposedAssociates = new TreeSet<Long>(
					found.getAssociatedEntities());
			if (currentAssociates.size() != proposedAssociates.size()
					|| !currentAssociates.equals(proposedAssociates)) {
				existing.setAssociatedEntities(proposedAssociates);
				isDirty = true;
			}
			final Set<ScriptTrigger> currentTriggers = new TreeSet<ScriptTrigger>(
					existing.getTriggers());
			final Set<ScriptTrigger> proposedTriggers = new TreeSet<ScriptTrigger>(
					found.getTriggers());
			if (currentTriggers.size() != proposedTriggers.size()
					|| !currentTriggers.equals(proposedTriggers)) {
				existing.setTriggers(proposedTriggers);
				isDirty = true;
			}
			if (isDirty) {
				final Set<ScriptTrigger> triggers = saveTriggers(existing
						.getTriggers());
				if (triggers != null) {
	...
	private Set<ScriptTrigger> saveTriggers(final Set<ScriptTrigger> triggers) {
		final Set<ScriptTrigger> persisted = new HashSet<ScriptTrigger>(
				triggers.size());
		for (final ScriptTrigger trigger : triggers) {
			final ScriptTrigger existing = _triggerService
					.getByTriggerId(trigger.getTriggerId());
			if (existing == null) {
				...
				if (!existing.getAssociation().equals(trigger.getAssociation())) {
					existing.setAssociation(Scope.encode(Scope.Project,
							trigger.getAssociation()));
				}
