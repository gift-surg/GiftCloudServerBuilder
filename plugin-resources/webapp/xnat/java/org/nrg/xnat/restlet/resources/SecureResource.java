...
public abstract class SecureResource extends Resource {
	...
	public static List<Variant> STANDARD_VARIANTS = Arrays.asList(new Variant(
			MediaType.APPLICATION_JSON), new Variant(MediaType.TEXT_HTML),
			new Variant(MediaType.TEXT_XML));

	public Hashtable<String, String> fieldMapping = new Hashtable<String, String>();

	// TODO: these should be proper extension types: application/x-xList,
	// application/x-xcat+xml, application/x-xar
	...
	public static final MediaType TEXT_CSV = MediaType.register("text/csv",
			"CSV");
	...
	public static String getUrlEncodedParameter(Request request, String key) {
		try {
			...
			return param == null ? null : TurbineUtils.escapeParam(URLDecoder
					.decode(param, "UTF-8"));
	...
	public void logAccess() {
		...
		if (!(Method.GET.equals(getRequest().getMethod()) && url
				.contains("resources/SNAPSHOTS"))) {
			String login = "";
			if (user != null) {
				login = user.getLogin();
			}

			AccessLogger.LogServiceAccess(login, getRequest().getClientInfo()
					.getAddress(), getRequest().getMethod() + " " + url, "");
		}
	}
	...
	public static boolean isZIPRequest(MediaType mt) {
		return !(mt == null || !(mt.equals(MediaType.APPLICATION_ZIP)
				|| mt.equals(MediaType.APPLICATION_TAR) || mt
					.equals(MediaType.APPLICATION_GNU_TAR)));
	}

	private Form f = null;

	/**
	 * This method is used internally to get the Query form. It should remain
	 * private so that all access to parameters are guaranteed to be properly
	 * escaped.
	 *
	...
	/**
	 * This method is used internally to get the Body form. It should remain
	 * private so that all access to parameters are guaranteed to be properly
	 * escaped.
	 *
	...
	private Form getBodyAsForm() {
		if (_body == null) {
			...
			if (entity != null && RequestUtil.isMultiPartFormData(entity)
					&& entity.getSize() > 0) {
	...
	/**
	 * This creates an instance of the specified class and populates it from the
	 * mapped form data. Each key in the map is treated as a property name. If a
	 * setter method for any of the specified properties is not found, an
	 * exception will be thrown.
	 * 
	 * @param clazz
	 *            The type of the class you want to create.
	 * @return The initialized instance of the specified class type.
	 */
	public <T> T createObjectFromFormData(final Class<T> clazz)
	...
	public boolean isQueryVariable(String key, String value,
			boolean caseSensitive) {
		if (getQueryVariable(key) != null) {
			if ((caseSensitive && getQueryVariable(key).equals(value))
					|| (!caseSensitive && getQueryVariable(key)
	...
	public Representation representTable(XFTTable table, MediaType mt,
	...
	public Representation representTable(XFTTable table, MediaType mt,
		...
		if (table != null) {
			if (getQueryVariable("sortBy") != null) {
				...
				if (isQueryVariable("sortOrder", "DESC", false)
						&& !mt.equals(APPLICATION_XLIST)) {
					table.reverse();
				}
			}
			...
			if (clazz != null) {
				try {
					Class[] parameterTypes = { XFTTable.class, Map.class,
							Hashtable.class, MediaType.class };
					Object[] parameters = { table, cp, params, mt };
					Constructor<OutputRepresentation> rep = clazz
			...
			if (mt.equals(MediaType.TEXT_XML)) {
				return new XMLTableRepresentation(table, cp, params,
						MediaType.TEXT_XML);
			} else if (mt.equals(MediaType.APPLICATION_JSON)) {
				return new JSONTableRepresentation(table, cp, params,
				...
				Representation rep = new HTMLTableRepresentation(table, cp,
						params, MediaType.TEXT_HTML, false);
				rep.setMediaType(MediaType.TEXT_HTML);
				return rep;
			} else {
				if (mt.equals(MediaType.TEXT_HTML)
						&& hasQueryVariable("requested_screen")) {
					try {
						for (String key : this.getQueryVariableKeys()) {
							params.put(key, this.getQueryVariable(key));
						}
						params.put("table", table);
						params.put("hideTopBar",
								isQueryVariableTrue("hideTopBar"));
						return new StandardTurbineScreen(MediaType.TEXT_HTML,
								getRequest(), user,
								getQueryVariable("requested_screen"), params);
					} catch (TurbineException e) {
						logger.error("", e);
						return new HTMLTableRepresentation(table, cp, params,
								MediaType.TEXT_HTML, true);
					}
				} else {
					return new HTMLTableRepresentation(table, cp, params,
							MediaType.TEXT_HTML, true);
	...
	public Representation representItem(XFTItem item, MediaType mt,
		...
		if (item != null) {
			if (mt.equals(MediaType.TEXT_XML)) {
				return new XMLXFTItemRepresentation(item, MediaType.TEXT_XML,
						metaFields, allowDBAccess, allowSchemaLocation);
			} else {
				return null;
			}
		} else {
	...
	public Representation representItem(XFTItem item, MediaType mt) {
		...
		if (mt.equals(MediaType.TEXT_HTML)) {
			try {
				return new ItemHTMLRepresentation(item, MediaType.TEXT_HTML,
						getRequest(), user,
						getQueryVariable("requested_screen"),
			...
			try {
				FlattenedItemA.HistoryConfigI history = (isQueryVariableTrue("includeHistory")) ? FlattenedItemA.GET_ALL
						: new FlattenedItemA.HistoryConfigI() {
							@Override
				...
				return new JSONObjectRepresentation(MediaType.APPLICATION_JSON,
						(new ItemJSONBuilder()).call(item, history,
								isQueryVariableTrue("includeHeaders")));
			} catch (Exception e) {
				getResponse().setStatus(Status.SERVER_ERROR_INTERNAL, e);
				return null;
			}
		} else {
			return new ItemXMLRepresentation(item, MediaType.TEXT_XML, true,
					!isQueryVariableTrue("concealHiddenFields"));
		}
	}
	...
	public MediaType buildMediaType(MediaType mt, String fName) {
		if (fName.endsWith(".txt")) {
			...
			if ((mt != null && mt.equals(MediaType.TEXT_XML))
					&& !fName.endsWith(".xml")) {
				mt = MediaType.ALL;
			} else {
	...
	public FileRepresentation representFile(File f, MediaType mt) {
		...
		if (mt.getName().startsWith("APPLICATION")
				|| !XFT.getBooleanProperty(
						"security.allow-HTML-resource-rendering", true)) {
	...
	public boolean allowDataDeletion() {
		return getQueryVariable("allowDataDeletion") != null
				&& getQueryVariable("allowDataDeletion").equals("true");
	}

	public boolean populateFromDB() {
		return getQueryVariable("populateFromDB") == null
				|| isQueryVariableTrue("populateFromDB");
	}

	protected boolean completeDocument = false;

	public XFTItem loadItem(String dataType, boolean parseFileItems)
			throws ClientException, ServerException {
		return loadItem(dataType, parseFileItems, null);
	}

	/**
	 * Attempts to generate a XFTItem based on the contents of the submitted
	 * request If the content-type is multi-part form data, the form entries
	 * will be reviewed for xml files that can be parsed. If the content-type is
	 * text/xml OR req_format=xml OR inbody=true, then the body of the message
	 * will be parsed as an xml document. If the req_format=form OR content-type
	 * is application_www_form, or multi-part_all, then the individual
	 * parameters of the submitted form will be reviewed as individual
	 * parameters for the XFTItem. No matter which format is submitted, the
	 * query string parameters will be parsed to add individual parameters to
	 * the generated XFTItem. These will override values from the previous
	 * methods.
	 * 
	 * @param dataType
	 *            - xsi:type of object to be created.
	 * @param parseFileItems
	 *            - set to false if you are expecting something else to be in
	 *            the body of the message, and don't want it parsed.
	 * @param template
	 *            - item to add parameters to.
	 ...
	 */
	public XFTItem loadItem(String dataType, boolean parseFileItems,
			XFTItem template) throws ClientException, ServerException {
		XFTItem item = null;
		...
		if (parseFileItems) {
			if ((RequestUtil.hasContent(entity) && RequestUtil
					.compareMediaType(entity, MediaType.MULTIPART_FORM_DATA))
					&& !req_format.equals("form")) {
				// handle multi part form data (where xml is being submitted as
				// a field in a multi part form)
				// req_format is checked to allow the body parsing to use the
				// form method rather then file fields.
				try {
					org.apache.commons.fileupload.DefaultFileItemFactory factory = new DefaultFileItemFactory();
					org.restlet.ext.fileupload.RestletFileUpload upload = new RestletFileUpload(
							factory);
			...
			} else if (RequestUtil.hasContent(entity)
					&& (RequestUtil.compareMediaType(entity,
							MediaType.TEXT_XML, MediaType.APPLICATION_XML)
							|| req_format.equals("xml") || isQueryVariableTrue("inbody"))) {
				...
				|| RequestUtil.isMultiPartFormData(entity)) {
			try {
				...
				if (dataType != null) {
					PopulateItem populater = item != null ? PopulateItem
							.Populate(params, user, dataType, true, item)
							: PopulateItem.Populate(params, user, dataType,
									true);
	...
	protected static boolean isQueryVariableTrueHelper(Object queryVariableObj) {
		if (queryVariableObj != null && queryVariableObj instanceof String) {
			...
			return !(queryVariable.equalsIgnoreCase("false") || queryVariable
					.equalsIgnoreCase("0"));
		} else {
			return false;
		}
	...
	private static boolean isQueryVariableFalseHelper(String queryVariable) {
		return queryVariable != null
				&& (queryVariable.equalsIgnoreCase("false") || queryVariable
						.equalsIgnoreCase("0"));
	}
	...
	/**
	 * Sets the Content-Disposition response header. The filename parameter
	 * indicates the name of the content. This method specifies the content as
	 * an attachment. If you need to specify inline content (e.g. for MIME
	 * content in email or embedded content situations), use
	 * {@link #setContentDisposition(String, boolean)}.
	 * <p/>
	 * <b>Note:</b> This differs from the
	 * {@link TurbineUtils#setContentDisposition(HttpServletResponse, String)}
	 * version of this method in that it performs the header set in a
	 * "restlet-y" way. Both methods use the same
	 * {@link TurbineUtils#createContentDispositionValue(String, boolean)}
	 * method to create the actual value set for the response header.
	 *
	 * @param filename
	 *            The suggested filename for downloaded content.
	 */
	 ...
	 * Sets the Content-Disposition response header. The filename parameter
	 * indicates the name of the content. This method specifies the content as
	 * an attachment when the <b>isAttachment</b> parameter is set to true, and
	 * as inline content when the <b>isAttachment</b> parameter is set to false.
	 * You can specify the content as an attachment by default by calling
	 * {@link #setContentDisposition(String)}.
	 * <p/>
	 * <b>Note:</b> This differs from the
	 * {@link TurbineUtils#setContentDisposition(HttpServletResponse, String, boolean)}
	 * version of this method in that it performs the header set in a
	 * "restlet-y" way. Both methods use the same
	 * {@link TurbineUtils#createContentDispositionValue(String, boolean)}
	 * method to create the actual value set for the response header.
	 ...
	 *            The suggested filename for downloaded content.
	 * @param isAttachment
	 *            Indicates whether the content is an attachment or inline.
	 */
	@SuppressWarnings("unchecked")
	public void setContentDisposition(String filename, boolean isAttachment) {
		...
		if (attributes.containsKey(CONTENT_DISPOSITION)) {
			throw new IllegalStateException(
					"A content disposition header has already been added to this response.");
		}
		...
		headers.add(new Parameter(CONTENT_DISPOSITION, TurbineUtils
				.createContentDispositionValue(filename, isAttachment)));
		attributes.put(HttpConstants.ATTRIBUTE_HEADERS, headers);
	}

	/**
	 * Return the list of query string parameters value with the name 'action'.
	 * List is created on first access, and cached for later access.
	 *
	...
	public List<FileWriterWrapperI> getFileWriters()
	...
	/**
	 * Gets file writers and load parameters from the request entity. When
	 * <b>useFileFieldName</b> is <b>true</b>, this uses the field name in the
	 * form as the name in the {@link FileWriterWrapperI} object. Otherwise, it
	 * uses the filename as the name of the {@link FileWriterWrapperI}
	 * parameter. When form fields are encountered, the
	 * {@link #handleParam(String, Object)} method is called to cache all of the
	 * standard form fields.
	 *
	 * @param entity
	 *            The request entity.
	 * @param useFileFieldName
	 *            Indicates whether the form field name should be used to
	 *            identify the extracted files.
	 * @return A list of any {@link FileWriterWrapperI} objects found in the
	 *         request.
	 * @throws FileUploadException
	 * @throws ClientException
	 */
	public List<FileWriterWrapperI> getFileWritersAndLoadParams(
			final Representation entity, boolean useFileFieldName)
			throws FileUploadException, ClientException {
			...
			if (entity != null
				...
				getResponse()
						.setStatus(
								...
								"In-body File posts must include the file directly as the body of the message (not as part of multi-part form data).");
				return null;
			} else {
				// NOTE: modified driveFileName here to return a name when
				// content-type is null
				final String fileName = (filepath == null || filepath
						.equals("")) ? RequestUtil.deriveFileName("upload",
						entity, false) : filepath;

				if (fileName == null) {
					throw new FileUploadException(
							"In-body File posts must include the file directly as the body of the message.");
				}
			...
			for (final FileItem item : items) {
				if (item.isFormField()) {
					...
					if (fieldName.equals("reference")) {
						throw new FileUploadException(
								"multi-part form posts may not be used to upload files via reference.");
					} else {
				...
				if (item.getName() == null) {
					throw new FileUploadException(
							"multi-part form posts must contain the file name of the uploaded file.");
				}
				...
				wrappers.add(new FileWriterWrapper(item,
						useFileFieldName ? item.getFieldName() : fileName));
			}
		} else {
	...
	public String getContextPath() {
		if (CONTEXT_PATH == null) {
			CONTEXT_PATH = TurbineUtils
					.GetRelativePath(getHttpServletRequest());
		}
		return CONTEXT_PATH;
	}
	...
	public Integer identifyCompression(Integer defaultCompression)
	...
	public Representation buildChangesets(XFTItem item, String key, MediaType mt)
			throws Exception {
		String files = getQueryVariable("includeFiles");
		String details = getQueryVariable("includeDetails");
		final boolean includeFiles = (StringUtils.isEmpty(files)) ? false
				: Boolean.valueOf(files);
		final boolean includeDetails = (StringUtils.isEmpty(details)) ? false
				: Boolean.valueOf(details);

		return new JSONObjectRepresentation(MediaType.APPLICATION_JSON,
				(new WorkflowBasedHistoryBuilder(item, key, user, includeFiles,
						includeDetails)).toJSON(getQueryVariable("dateFormat")));
	}
	...
	public EventDetails newEventInstance(EventUtils.CATEGORY cat) {
		return EventUtils.newEventInstance(cat, getEventType(), getAction(),
				getReason(), getComment());
	}

	public EventDetails newEventInstance(EventUtils.CATEGORY cat, String action) {
		return EventUtils.newEventInstance(cat, getEventType(),
				(getAction() != null) ? getAction() : action, getReason(),
				getComment());
	}

	public void delete(ArchivableItem item, EventDetails event)
			throws Exception {
		final PersistentWorkflowI workflow = WorkflowUtils
				.getOrCreateWorkflowData(getEventId(), user, item.getXSIType(),
						item.getId(), item.getProject(), event);
		final EventMetaI ci = workflow.buildEvent();

		try {
			String removeFiles = getQueryVariable("removeFiles");
			if (removeFiles != null) {
				final List<XFTItem> hash = item.getItem().getChildrenOfType(
				...
				for (XFTItem resource : hash) {
					...
					if (om instanceof XnatAbstractresource) {
						...
						resourceA.deleteWithBackup(item.getArchiveRootPath(),
								user, ci);
					}
				}
			}
	...
	public void delete(ArchivableItem parent, ItemI item, EventDetails event)
			throws Exception {
		final PersistentWorkflowI workflow = WorkflowUtils
				.getOrCreateWorkflowData(getEventId(), user,
						parent.getXSIType(), parent.getId(),
						parent.getProject(), event);
		final EventMetaI ci = workflow.buildEvent();
		...
		try {
			...
			if (removeFiles != null) {
				...
				for (XFTItem resource : hash) {
					...
					if (om instanceof XnatAbstractresource) {
						XnatAbstractresource resourceA = (XnatAbstractresource) om;
						resourceA.deleteWithBackup(parent.getArchiveRootPath(),
								user, ci);
					}
				}
			}
	...
	@SuppressWarnings("unused")
	public void create(ArchivableItem parent, ItemI sub,
			boolean overwriteSecurity, boolean allowDataDeletion,
			EventDetails event) throws Exception {
		PersistentWorkflowI wrk = WorkflowUtils.getOrCreateWorkflowData(
				getEventId(), user, parent.getItem(), event);
	...
	public void create(ArchivableItem sub, boolean overwriteSecurity,
			boolean allowDataDeletion, EventDetails event) throws Exception {
		PersistentWorkflowI wrk = WorkflowUtils.getOrCreateWorkflowData(
				getEventId(), user, sub.getItem(), event);
		EventMetaI c = wrk.buildEvent();

		try {
			if (SaveItemHelper.authorizedSave(sub, user, overwriteSecurity,
					allowDataDeletion, c)) {
				WorkflowUtils.complete(wrk, c);
				MaterializedView.DeleteByUser(user);
			}
	...
	protected void postSaveManageStatus(ItemI i) throws ActionException {
		try {
				...
				if (user.canActivate(i.getItem())) {
					PersistentWorkflowI wrk = PersistentWorkflowUtils
							.getOrCreateWorkflowData(
									...
									user,
									i.getItem(),
									newEventInstance(EventUtils.CATEGORY.DATA,
											"Activated"));
					try {
						i.activate(user);
			...
			logger.error("", e);
			throw new org.nrg.action.ServerException("Error modifying status",
					e);
		}
	}
	...
	/**
	 * This method walks the
	 * <b>org.nrg.xnat.restlet.extensions.table.extensions</b> package and
	 * attempts to find extensions for the set of available REST table
	 * representations.
	 */
	...
	private synchronized Map<String, Class<?>> getExtensionTableRepresentations() {
		if (map == null) {
			...
			try {
				classes = Reflection
						.getClassesForPackage("org.nrg.xnat.restlet.representations.table.extensions");
			...
			for (Class<?> clazz : classes) {
				if (clazz.isAnnotationPresent(XnatTableRepresentation.class)) {
					XnatTableRepresentation annotation = clazz
							.getAnnotation(XnatTableRepresentation.class);
					boolean required = annotation.required();
					if (!OutputRepresentation.class.isAssignableFrom(clazz)) {
						String message = "You can only apply the XnatTableRepresentation annotation to classes that subclass the org.restlet.resource.Resource class: "
								+ clazz.getName();
						if (required) {
						...
						if (MediaType.valueOf(annotation.mediaType()) != null) {
							MediaType.register(annotation.mediaType(),
									annotation.mediaTypeDescription());
						}
	...
	private boolean checkWhitelist(Integer projectId) {
		...
		String config = projectId != null ? configService.getConfigContents(
				"user-resource-whitelist", "whitelist.json",
				Long.valueOf(projectId)) : configService.getConfigContents(
				"user-resource-whitelist", "users/whitelist.json");
		...
		try {
			List<String> userResourceWhitelist = OBJECT_MAPPER.readValue(
					config, TYPE_REFERENCE_LIST_STRING);
			if (userResourceWhitelist != null) {
				return userResourceWhitelist.contains(user.getUsername());
			}
		} catch (IOException e) {
			String message = "Error retrieving user list"
					+ (projectId == null ? "" : " for project " + projectId);
	...
	private static Map<String, List<FilteredResourceHandlerI>> handlers = Maps
			.newConcurrentMap();

	/**
	 * Get a list of the possible handlers. This allows additional handlers to
	 * be injected at a later date or via a module.
	 * 
	 * @return
	 * @throws InstantiationException
	 * @throws IllegalAccessException
	 */
	public static List<FilteredResourceHandlerI> getHandlers(String _package,
			List<FilteredResourceHandlerI> _defaultHandlers)
		...
		if (handlers.get(_package) == null) {
			...
			for (Class<?> clazz : classes) {
				if (FilteredResourceHandlerI.class.isAssignableFrom(clazz)) {
					handlers.get(_package).add(
							(FilteredResourceHandlerI) clazz.newInstance());
				}
			}
		}
	...
	/**
	 * Checks whether the resource URL is "clean", that is, has no remaining
	 * part (any string past the part of the URL that matched the restlet URL)
	 * that is not a query string or URL fragment. For example, for the project
	 * resource restlet, you can go to /data/projects/projectId/foo/bar. The
	 * "/projects/projectId" part of the URL will match the mapped URL for that
	 * restlet and the remaining part is then "/foo/bar". If there is any
	 * remaining part, it's then checked to see if that part is actually the
	 * beginning of query-string parameters by checking whether it starts with
	 * the '?' character or if it's part of a URI fragment by checking whether
	 * it starts with the '#' character. If the remaining part contains neither
	 * query-string parameters or a URI fragment, this method then sets the
	 * request status code to 400 (bad request) and throws an exception.
	 *
	 * @param request
	 *            The restlet request.
	 * @param response
