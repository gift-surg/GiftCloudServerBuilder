...
public class SubjAssessmentResource extends SubjAssessmentAbst {
	...
	public SubjAssessmentResource(Context context, Request request,
			Response response) {
	...
	private XnatSubjectdata getExistingSubject(XnatProjectdata proj,
			String subjectId) {
		// First check if the subject is associated with the project,
		// if that fails check the global pool.
		XnatSubjectdata s = XnatSubjectdata.GetSubjectByProjectIdentifier(
				proj.getId(), subjectId, user, false);
		if (s == null) {
	...
	private XnatSubjectassessordata getExistingExperiment(
			XnatSubjectassessordata currExp) {
		XnatSubjectassessordata retExp = null;
		if (currExp.getId() != null) {
			retExp = (XnatSubjectassessordata) XnatExperimentdata
					.getXnatExperimentdatasById(currExp.getId(), null,
							completeDocument);
		}

		if (retExp == null && currExp.getProject() != null
				&& currExp.getLabel() != null) {
			retExp = (XnatSubjectassessordata) XnatExperimentdata
					.GetExptByProjectIdentifier(currExp.getProject(),
							currExp.getLabel(), user, completeDocument);
		}

		if (retExp == null) {
			for (XnatExperimentdataShareI pp : currExp.getSharing_share()) {
				retExp = (XnatSubjectassessordata) XnatExperimentdata
						.GetExptByProjectIdentifier(pp.getProject(),
	...
	public void handlePut() {
		...
		try {
			...
			if (item == null) {
				if (proj != null) {
					XnatSubjectassessordata om = (XnatSubjectassessordata) XnatSubjectassessordata
							.GetExptByProjectIdentifier(proj.getId(),
									this.exptID, user, false);
					if (om != null) {
						item = om.getItem();
					}
				}
				...
				if (item == null) {
					XnatSubjectassessordata om = (XnatSubjectassessordata) XnatSubjectassessordata
							.getXnatExperimentdatasById(this.exptID, null,
									false);
					if (om != null) {
						item = om.getItem();
					}
			...
			if (item.instanceOf("xnat:subjectAssessorData")) {
				expt = (XnatSubjectassessordata) BaseElement
						.GetGeneratedItem(item);

				if (filepath != null && !filepath.equals("")) {
					if (filepath.startsWith("projects/")) {
						String newProjectS = filepath.substring(9);
						XnatProjectdata newProject = XnatProjectdata
								.getXnatProjectdatasById(newProjectS, user,
										false);
						...
						if (newProject != null) {
							...
							if (!user.canRead(expt)) {
								this.getResponse()
										.setStatus(
												...
												"Specified user account has insufficient privileges for experiments in this project.");
								return;
							}
							...
							for (XnatExperimentdataShareI pp : expt
								...
								if (pp.getProject().equals(newProject.getId())) {
									...
									if (newLabel != null
											&& !pp.getLabel().equals(newLabel)) {
										((XnatExperimentdataShare) pp)
												.setLabel(newLabel);
										BaseXnatExperimentdata
												.SaveSharedProject(
														(XnatExperimentdataShare) pp,
														expt,
														user,
														newEventInstance(
																...
																EventUtils.RENAME_IN_SHARED_PROJECT));
									}
									break;
								}
								index++;
							...
							if (this.getQueryVariable("primary") != null
									&& this.getQueryVariable("primary").equals(
											"true")) {
								...
								XnatExperimentdata match = XnatExperimentdata
										.GetExptByProjectIdentifier(
												newProject.getId(), newLabel,
												user, false);
								if (match != null) {
									this.getResponse()
											.setStatus(
													Status.CLIENT_ERROR_CONFLICT,
													"Specified label is already in use.");
									return;
								}

								List<String> assessorList = null;
								if (this.getQueryVariable("moveAssessors") != null) {
									String moveAssessors = this
											.getQueryVariable("moveAssessors");
									assessorList = Arrays.asList(moveAssessors
											.split(","));
								}

								EventMetaI c = BaseXnatExperimentdata
										.ChangePrimaryProject(
												user,
												expt,
												newProject,
												newLabel,
												newEventInstance(
														EventUtils.CATEGORY.DATA,
														EventUtils.MODIFY_PROJECT),
												assessorList);

								if (matched != null) {
									SaveItemHelper
											.authorizedRemoveChild(
													expt.getItem(),
													"xnat:experimentData/sharing/share",
													matched.getItem(), user, c);
								...
								if (matched == null) {
									...
									if (newLabel != null) {
										XnatExperimentdata temp = XnatExperimentdata
												.GetExptByProjectIdentifier(
														newProject.getId(),
														newLabel, null, false);
										if (temp != null) {
									...
									if (user.canCreate(expt.getXSIType()
											+ "/project", newProject.getId())) {
										XnatExperimentdataShare pp = new XnatExperimentdataShare(
												(UserI) user);
										...
										BaseXnatExperimentdata
												.SaveSharedProject(
														...
														newEventInstance(
																EventUtils.CATEGORY.DATA,
																EventUtils.CONFIGURED_PROJECT_SHARING));
									} else {
										this.getResponse()
												.setStatus(
														Status.CLIENT_ERROR_FORBIDDEN,
														"Specified user account has insufficient create privileges for experiments in the "
																+ newProject
																		.getId()
																+ " project.");
										return;
					...
					if (this.proj == null && expt.getProject() != null) {
						proj = XnatProjectdata.getXnatProjectdatasById(
								expt.getProject(), user, false);
					}

					if (this.proj != null) {
						if (expt.getProject() == null
								|| expt.getProject().equals("")) {
						...
						this.getResponse()
								.setStatus(
										...
										"Submitted experiment record must include the project attribute.");
						return;
					}
					...
					if (this.subject != null) {
						...
						if (StringUtils.IsEmpty(expt.getSubjectId())
								&& org.apache.commons.lang.StringUtils
										.isNotEmpty(subID)) {
							expt.setSubjectId(subID);
						}

						if (expt.getSubjectId() != null
								&& !expt.getSubjectId().equals("")) {
							this.subject = XnatSubjectdata
									.getXnatSubjectdatasById(
											expt.getSubjectId(), user, false);

							if (this.subject == null
									&& expt.getProject() != null
									&& expt.getLabel() != null) {
								this.subject = XnatSubjectdata
										.GetSubjectByProjectIdentifier(
												expt.getProject(),
												expt.getSubjectId(), user,
												false);
							}
							...
							if (this.subject == null) {
								...
								this.subject.setId(XnatSubjectdata
										.CreateNewID());
								...
								BaseXnatSubjectdata
										.save(this.subject,
												...
												newEventInstance(
														EventUtils.CATEGORY.DATA,
														EventUtils.AUTO_CREATE_SUBJECT));
								expt.setSubjectId(this.subject.getId());
							}
						}
					}
					...
					if (existing == null) {
						if (!user.canCreate(expt)) {
							this.getResponse()
									.setStatus(
											Status.CLIENT_ERROR_FORBIDDEN,
											"Specified user account has insufficient create privileges for experiments in this project.");
							return;
						}
						// IS NEW
						...
						if (!existing.getProject().equals(expt.getProject())) {
							this.getResponse()
									.setStatus(Status.CLIENT_ERROR_CONFLICT,
											"Project must be modified through separate URI.");
							return;
						}
						...
						if (!user.canEdit(expt)) {
							this.getResponse()
									.setStatus(
											Status.CLIENT_ERROR_FORBIDDEN,
											"Specified user account has insufficient edit privileges for experiments in this project.");
							return;
						}

						if (this.getQueryVariable("subject_ID") != null
								&& !this.getQueryVariable("subject_ID").equals(
										"")) {
							if (!expt.getSubjectId().equals(
									this.getQueryVariable("subject_ID"))) {
								XnatSubjectdata s = this.getExistingSubject(
										proj,
										this.getQueryVariable("subject_ID"));
								if (s != null) {
									// \"subject_ID\" can be overloaded on both
									// the subject's label
									// and XNAT unique subject identifier
									if (!expt.getSubjectId().equals(s.getId())) {
										// only accept subjects that are
									...
									try {
										this.subject = new XnatSubjectdata(
												(UserI) user);
										this.subject.setProject(this.proj
												.getId());
										this.subject
												.setLabel(this
														.getQueryVariable("subject_ID"));
										this.subject.setId(XnatSubjectdata
												.CreateNewID());
										if (!user.canCreate(this.subject)) {
											this.getResponse()
						...
						if (this.getQueryVariable("label") != null
								&& !this.getQueryVariable("label").equals("")) {
							...
							if (!label.equals(existing.getLabel())) {
								...
								Rename renamer = new Rename(proj, existing,
										label, user, getReason(),
										getEventType());
								try {
									renamer.call();
									...
									this.getResponse().setStatus(
											Status.CLIENT_ERROR_CONFLICT,
											"Specified session is being processed ("
													+ e.getPipeline_name()
													+ ").");
									return;
									...
									this.getResponse()
											.setStatus(
													Status.CLIENT_ERROR_CONFLICT,
													"File system destination contains pre-existing files");
									return;
									...
									this.getResponse()
											.setStatus(
													...
													"Non-standard archive structure in existing experiment directory.");
									return;
					...
					if (this.isQueryVariableTrue(XNATRestConstants.FIX_SCAN_TYPES)
							|| this.containsAction(XNATRestConstants.FIX_SCAN_TYPES)) {
						if (expt instanceof XnatImagesessiondata) {
							FixScanTypes fst = new FixScanTypes(expt, user,
									proj, false, c);
							fst.call();
						}
					}

					if (!StringUtils.IsEmpty(expt.getLabel())
							&& !StringUtils.IsAlphaNumericUnderscore(expt
									.getId())) {
						this.getResponse().setStatus(
								Status.CLIENT_ERROR_EXPECTATION_FAILED,
								"Invalid character in experiment label.");
						return;
					}
					...
					try {
						// Preserve the previous version of the experiment
						// before we save it.
						XnatSubjectassessordata previous = getExistingExperiment(expt);

						// check for unexpected modifications of ID, Project and
						// label
						if (existing != null
								&& !org.apache.commons.lang.StringUtils.equals(
										existing.getId(), expt.getId())) {
							this.getResponse().setStatus(
									Status.CLIENT_ERROR_BAD_REQUEST,
						...
						if (existing != null
								&& !org.apache.commons.lang.StringUtils.equals(
										existing.getLabel(), expt.getLabel())) {
							this.getResponse()
									.setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
											"Label must be modified through separate URI.");
							return;
						}

						if (SaveItemHelper.authorizedSave(expt, user, false,
								allowDataDeletion, c)) {
							WorkflowUtils.complete(wrk, c);
							user.clearLocalCache();
							MaterializedView.DeleteByUser(user);

							if (this.proj.getArcSpecification()
									.getQuarantineCode() != null
									&& this.proj.getArcSpecification()
											.getQuarantineCode().equals(1)) {
								expt.quarantine(user);
							}

							if (previous != null
									&& expt != null
									&& expt.getSubjectId() != null
									&& !expt.getSubjectId().equals(
											previous.getSubjectId())) {
								try {
									// re-apply this project's edit script
									expt.applyAnonymizationScript(new ProjectAnonymizer(
											(XnatImagesessiondata) expt, expt
													.getProject(), expt
													.getArchiveRootPath()));
					...
					if (user.canEdit(expt.getItem())) {
						if ((this
								.isQueryVariableTrue(XNATRestConstants.PULL_DATA_FROM_HEADERS) || this
								.containsAction(XNATRestConstants.PULL_DATA_FROM_HEADERS))
								&& expt instanceof XnatImagesessiondata) {
							try {
								wrk = PersistentWorkflowUtils
										.buildOpenWorkflow(
												...
												newEventInstance(
														EventUtils.CATEGORY.DATA,
														EventUtils.DICOM_PULL));
								c = wrk.buildEvent();
								try {
									PullSessionDataFromHeaders pull = new PullSessionDataFromHeaders(
											(XnatImagesessiondata) expt,
											user,
											this.allowDataDeletion(),
											this.isQueryVariableTrue("overwrite"),
											false, c);
									pull.call();
									WorkflowUtils.complete(wrk, c);
								} catch (Exception e) {
						...
						if (this.isQueryVariableTrue(XNATRestConstants.TRIGGER_PIPELINES)
								|| this.containsAction(XNATRestConstants.TRIGGER_PIPELINES)) {
							TriggerPipelines tp = new TriggerPipelines(
									expt,
									this.isQueryVariableTrue(XNATRestConstants.SUPRESS_EMAIL),
									user);
							tp.call();
						}
					}
				...
				this.returnString(expt.getId(),
						(existing == null) ? Status.SUCCESS_CREATED
								: Status.SUCCESS_OK);
			} else {
				this.getResponse()
	...
	public void handleDelete() {
		...
		if (expt == null && exptID != null) {
			expt = (XnatSubjectassessordata) XnatExperimentdata
					.getXnatExperimentdatasById(exptID, user, false);

			if (expt == null && this.proj != null) {
				expt = (XnatSubjectassessordata) XnatExperimentdata
						.GetExptByProjectIdentifier(this.proj.getId(), exptID,
								user, false);
			}
		}
		...
		if (filepath != null && !filepath.equals("")) {
			if (filepath.startsWith("projects/")) {
				String newProjectS = filepath.substring(9);
				newProject = XnatProjectdata.getXnatProjectdatasById(
						newProjectS, user, false);
				if (newProject == null) {
					this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
							"Unable to identify project: " + newProjectS);
					return;
				}
			} else {
				this.getResponse().setStatus(Status.CLIENT_ERROR_BAD_REQUEST);
		...
		try {
			wrk = WorkflowUtils.buildOpenWorkflow(
					user,
					expt.getItem(),
					newEventInstance(EventUtils.CATEGORY.DATA,
							EventUtils.getDeleteAction(expt.getXSIType())));
			EventMetaI c = wrk.buildEvent();

			try {
				String msg = expt.delete((newProject != null) ? newProject
						: proj, user, this.isQueryVariableTrue("removeFiles"),
						c);
				if (msg != null) {
					WorkflowUtils.fail(wrk, c);
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (expt != null) {
			if (filepath != null && !filepath.equals("")
					&& filepath.equals("status")) {

				return returnStatus(expt, mt);
			} else if (filepath != null && !filepath.equals("")
					&& filepath.equals("history")) {
				try {
					return buildChangesets(expt.getItem(),
							expt.getStringProperty("ID"), mt);
				} catch (Exception e) {
					logger.error("", e);
					this.getResponse().setStatus(Status.SERVER_ERROR_INTERNAL,
							e.getMessage());
				...
				for (Map.Entry<XnatProjectdataI, String> entry : expt
						.getProjectDatas().entrySet()) {
					row = new Object[4];
