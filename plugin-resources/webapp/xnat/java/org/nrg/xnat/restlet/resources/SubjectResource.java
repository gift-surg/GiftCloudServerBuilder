...
public class SubjectResource extends ItemResource {
	...
	public SubjectResource(Context context, Request request, Response response) {
		...
		if (existing == null) {
			existing = XnatSubjectdata.getXnatSubjectdatasById(subID, user,
					false);
			if (existing != null
		...
		this.fieldMapping.putAll(XMLPathShortcuts.getInstance().getShortcuts(
				XMLPathShortcuts.SUBJECT_DATA, false));
	}
	...
	public void handlePut() {
		try {
			...
			XFTItem item = this.loadItem("xnat:subjectData", true, (this
					.isQueryVariableFalse("loadExisting")) ? null : template);
				...
				if (filepath != null && !filepath.equals("")) {
					if (filepath.startsWith("projects/")) {
						if (!user.canRead(sub)) {
							this.getResponse()
									.setStatus(
											...
											"Specified user account has insufficient privileges for subjects in this project.");
							return;
						}
						...
						if (newProject != null) {
								...
								if (pp.getProject().equals(newProject.getId())) {
									...
									if (newLabel != null
											&& (pp.getLabel() == null || (!pp
													.getLabel()
													.equals(newLabel)))) {
										XnatSubjectdata temp = XnatSubjectdata
							...
							if (newLabel != null) {
								XnatSubjectdata existing = XnatSubjectdata
										.getXnatSubjectdatasById(sub.getId(),
												user, false);
							...
							if (this.isQueryVariableTrue(PRIMARY)) {
								...
								EventMetaI c = BaseXnatSubjectdata
								...
								if (matched == null) {
									...
									if (user.canCreate(sub.getXSIType()
											+ "/project", newProject.getId())) {
										XnatProjectparticipant pp = new XnatProjectparticipant(
												(UserI) user);
										...
										this.getResponse()
												.setStatus(
														Status.CLIENT_ERROR_FORBIDDEN,
														"Specified user account has insufficient create privileges for subjects in the "
																+ newProject
																		.getId()
																+ " project.");
										return;
					...
					if (this.getQueryVariable("gender") != null) {
						sub.setProperty(
								...
								this.getQueryVariable("gender"));
					}

					if (!StringUtils.IsEmpty(sub.getLabel())
							&& !StringUtils.IsAlphaNumericUnderscore(sub
									.getId())) {
						this.getResponse().setStatus(
					...
					try {
						// If the label was changed, re apply the anonymization
						// script on all the subject's imaging sessions.
						boolean applyAnonScript = (null != existing && existing
								.getLabel().equals(sub.getLabel()));

						// check for unexpected modifications of ID and Project
						if (existing != null
								&& !org.apache.commons.lang.StringUtils.equals(
										existing.getId(), sub.getId())) {
							this.getResponse().setStatus(
									Status.CLIENT_ERROR_BAD_REQUEST,
						...
						// Save the experiment.
						if (SaveItemHelper.authorizedSave(sub, user, false,
							...
							if (applyAnonScript) {
								for (final XnatSubjectassessordata expt : sub
										.getExperiments_experiment("xnat:imageSessionData")) {
									try {
	...
	public void handleDelete() {
		...
		try {
			wrk = WorkflowUtils.buildOpenWorkflow(
					...
					sub.getItem(),
					newEventInstance(EventUtils.CATEGORY.DATA,
							EventUtils.getDeleteAction(sub.getXSIType())));
			EventMetaI c = wrk.buildEvent();

			try {
				String msg = sub.delete((newProject != null) ? newProject
	...
	public Representation represent(Variant variant) {
		...
		if (sub != null) {
			...
			if (filepath != null && filepath.equals("status")) {
				...
				for (Map.Entry<XnatProjectdataI, String> entry : sub
			...
			if (proj == null) {
				...
				message.append(subID).append(
						" is not a known subject accession ID.");
			} else {
				message.append("The project ")
						.append(proj.getId())
						.append(" does not contain a subject identifiable by the ID or label ")
						.append(subID).append(".");
			}
			this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
					message.toString());
	...
	public Representation representItem(XFTItem item, MediaType mt) {
		...
		if (representation != null && proj != null
				&& representation instanceof TurbineScreenRepresentation
				&& StringUtils.HasContent(proj.getId())) {
			// provides appropriate rendering if the caller is querying this
			// subject in the context of a shared project
			((TurbineScreenRepresentation) representation).setRunDataParameter(
					"project", proj.getId());
		}
