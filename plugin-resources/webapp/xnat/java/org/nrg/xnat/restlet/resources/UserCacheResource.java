...
public class UserCacheResource extends SecureResource {
	...
	static final String[] zipExtensions = { ".zip", ".jar", ".rar", ".ear",
			".gar", ".xar" };
	...
	public void handlePost() {
		...
		if (pXNAME == null && pFILE == null) {
			...
			if (this.isQueryVariableTrue("inbody")) {
				fail(Status.CLIENT_ERROR_BAD_REQUEST,
						"Please use HTTP PUT request to specify a file name in the URL.");
	...
	public void fail(Status status, String msg) {
		...
		if (_return != null) {
			...
			if (_return != null) {
				getResponse().setEntity(
						new StringRepresentation(_return, MediaType.TEXT_HTML));
			}
		}
	}
	...
	public void handlePut() {
		...
		try {
			...
			if (pXNAME == null && pFILE == null) {
				...
				// // PUT Specification wants to enable a GET request on the
				// same URL. Wouldn't want to put extracted files without the
				// // original archive file, or would't want to create and
				// delete it. Could possibly enable extraction by create, then
	...
	private void returnFileList(String userPath, String pXNAME) {
		...
		if (dir.exists() && dir.isDirectory()) {
			...
			// Implement a sorting comparator on file list: Unnecessary, it is
			// sorted by the representTable method.
			ArrayList<String> columns = new ArrayList<String>();
			columns.add("Name");
			columns.add("Size");
			columns.add("URI");
			...
			while (i.hasNext()) {
				...
				Object[] oarray = new Object[] { path.substring(1), f.length(),
						constructURI(path) };
				table.insertRow(oarray);
			}
			...
			this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
					"User directory not found or is not a directory.");
		}
	...
	private void returnFile(String userPath, String pXNAME, String pFILE) {
		...
		File reqFile = new File(new File(userPath, pXNAME), pFILE
	...
	private void deleteUserFiles(String userPath, String pXNAME, String pFILE) {
		...
		if (fileString.contains(",")) {
			...
			for (String s : fileArr) {
				...
				if (f.exists()) {
					...
					this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
							"One or more specified files do not exist.");
					return;
				}
			}
		} else {
		...
		if (fileList.size() > 0) {
			...
			if (deleteOK) {
				...
				this.getResponse().setStatus(Status.SERVER_ERROR_INTERNAL,
						"Problem deleting one or more server files.");
			}
		} else {
			this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
	...
	private boolean uploadUserFile(String userPath, String pXNAME, String pFILE) {
		...
		// Upload to non-existing resources (auto-create) or fail? Should
		// auto-create.
		if (!dir.exists()) {
			if (!dir.mkdirs()) {
				this.getResponse().setStatus(Status.SERVER_ERROR_INTERNAL,
	...
	private boolean handleInbodyUserFileUpload(String userPath,
			String dirString, String fileName) {

		try {

			// This is probably redundant due to current doPut/doPost coding,
			// but including it anyway.
			...
			(new FileWriterWrapper(this.getRequest().getEntity(), fileName))
					.write(ouf);
	...
	private boolean handleAttachedUserFileUpload(String userPath,
			String dirString, String requestedName) {
			...
			for (FileItem fi : fileItems) {
				...
				if (extract != null && extract.equalsIgnoreCase("true")) {
					...
					if (method != CompressionMethod.NONE) {
						if (!extractCompressedFile(fi.getInputStream(),
								dirString, fileName, method)) {
							this.getResponse().setStatus(
									Status.SERVER_ERROR_INTERNAL,
									"Error extracting file.");
							return false;
						} else {
							// If successfully extracted, don't create
							// unextracted file
							continue;
						}
					}
	...
	private boolean extractCompressedFile(InputStream is, String dirString,
			String fileName, CompressionMethod method) throws IOException {
	...
	private String constructURI(String path) {
		...
		String requestPart = this.getHttpServletRequest().getServletPath()
				+ this.getHttpServletRequest().getPathInfo();
		if (requestPart.endsWith("/resources/files")
				|| !requestPart.endsWith("/files")) {
			requestPart += "/files";
		}
	...
	public static String constructPath(File f) {
		String filePart = f.getAbsolutePath().replace(
				ArcSpecManager.GetInstance().getGlobalCachePath(), "");
	...
	private void sendZippedFiles(String userPath, String pXNAME,
			String fileName, ArrayList<File> fileList) throws ActionException {

		ZipRepresentation zRep;
		if (getRequestedMediaType() != null
				&& getRequestedMediaType()
						.equals(MediaType.APPLICATION_GNU_TAR)) {
			zRep = new ZipRepresentation(MediaType.APPLICATION_GNU_TAR,
					userPath, ZipOutputStream.DEFLATED);
			this.setContentDisposition(String.format("%s.tar.gz", fileName));
		} else if (getRequestedMediaType() != null
				&& getRequestedMediaType().equals(MediaType.APPLICATION_TAR)) {
			zRep = new ZipRepresentation(MediaType.APPLICATION_TAR, userPath,
		...
		zRep.addAllAtRelativeDirectory(userPath + File.separator + pXNAME,
				fileList);
