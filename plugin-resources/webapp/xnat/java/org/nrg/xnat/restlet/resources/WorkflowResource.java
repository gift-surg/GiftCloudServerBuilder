...
public class WorkflowResource extends ItemResource {
	...
	public void handlePut() {
		...
		try {
			...
			if (workflowId != null && !workflowId.isEmpty()) {
				...
				workflow = (WrkWorkflowdata) WorkflowUtils.getUniqueWorkflow(
						user, workflowId);
				if (workflow != null) {
					// If the workflow exists, set the workflow id on the new
					// item.
					item.setProperty("wrk_workflowData_id", workflowId);
				} else {
					// If we couldn't find the workflow, 404
					getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
							"Unable to find the specified workflow.");
					return;
				}
			} else {
				// Lookup the workflow by pipeline_name, launch_time, and ID
				workflow = (WrkWorkflowdata) WorkflowUtils.getUniqueWorkflow(
						user, pipeline_name, id, launch_time);
			}

			// If the workflow exists, Make sure the user has permission to edit
			// an existing workflow.
			if (workflow != null && !canUserEditWorkflow(user, workflow)) {
				// If the user is not allow to modify this workflow, 403
				getResponse()
						.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
								"You are not allowed to make changes to this workflow.");
				return;
			}

			// Id, launch_time, data_type, and pipeline_name are all required in
			// order to save a new workflow
			if (workflow == null
					&& (StringUtils.isEmpty(id)
							|| StringUtils.isEmpty(item
									.getStringProperty("launch_time"))
							|| StringUtils.isEmpty(pipeline_name) || StringUtils
								.isEmpty(item.getStringProperty("data_type")))) {
				getResponse()
						.setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
								"Id, launch_time, data_type and pipeline_name are all required.");
				return;
			}
	...
	public Representation represent(Variant variant) {
		...
		if (workflowId != null && !workflowId.isEmpty()) {
			// Lookup the workflow by the ID provided by the user.
			workflow = (WrkWorkflowdata) WorkflowUtils.getUniqueWorkflow(user,
					workflowId);
		} else {
			try {
				// Lookup the workflow by pipeline_name, launch_time, and ID
		...
		if (workflow != null) {
			// If we found the workflow, represent it with the requested media
			// type
			MediaType mt = this.getRequestedMediaType();
			return this.representItem(workflow.getItem(),
					(mt == null) ? MediaType.TEXT_XML : mt);
		} else {
			// If we couldn't find the workflow, 404
