...
public class CatalogResource extends XNATCatalogTemplate {
	...
	public CatalogResource(Context context, Request request, Response response) {
		...
		checkForNonEmptyFilePath(this.getRequest().getResourceRef()
				.getRemainingPart());
			...
			if (catalogs != null && catalogs.size() > 0) {
				for (Object[] row : catalogs.rows()) {
					...
					for (String resourceID : this.resource_ids) {
						if (id.toString().equals(resourceID)
								|| (label != null && label.equals(resourceID))) {
							resources
									.add(XnatAbstractresource
	...
	private void checkForNonEmptyFilePath(String remainingUrlPart) {
		...
		filePathIsEmpty = org.apache.commons.lang.StringUtils
				.isBlank(remainingUrlPart)
				|| remainingUrlPart.matches("^/+")
				|| remainingUrlPart.matches("^/*\\?.*");
	}

	/**
	 * See XNAT-1674. If the client mistakenly passes a file path to us, slap
	 * the wrist. This is better than discarding the file path and naively
	 * processing the request (and say, deleting the whole catalog instead of
	 * the individual file delete that was desired).
	 */
		...
		if (filePathIsEmpty) {
			...
			this.getResponse()
					.setStatus(
							...
							"This resource works at the catalog level only and does not accept file paths.  To work with the resource files, append '/files' after the resource ID.");
			return true;
		}
	}
			...
			try {
				if (user.canEdit(this.security)) {
					...
					if (item.instanceOf("xnat:resourceCatalog")) {
						XnatResourcecatalog catResource = (XnatResourcecatalog) BaseElement
								.GetGeneratedItem(item);

						if (catResource.getXnatAbstractresourceId() != null) {
							XnatAbstractresource existing = XnatAbstractresource
									.getXnatAbstractresourcesByXnatAbstractresourceId(
											catResource
							...
							if (existing != null) {
								this.getResponse().setStatus(
										Status.CLIENT_ERROR_CONFLICT,
										"Specified catalog already exists.");
								// MATCHED
								return;
							} else {
								this.getResponse()
										.setStatus(
												...
												"Contains erroneous generated fields (xnat_abstractresource_id).");
								// MATCHED
								return;
							}
						}
						...
						if (this.getQueryVariable("description") != null) {
							catResource.setDescription(this
									.getQueryVariable("description"));
						}
						if (this.getQueryVariable("format") != null) {
							catResource.setFormat(this
									.getQueryVariable("format"));
						}
						if (this.getQueryVariable("content") != null) {
							catResource.setContent(this
									.getQueryVariable("content"));
						}
							...
							for (String tag : tags) {
								...
								if (!tag.equals("")) {
									for (String s : StringUtils
											.CommaDelimitedStringToArrayList(tag)) {
										s = s.trim();
										if (!s.equals("")) {
											XnatAbstractresourceTag t = new XnatAbstractresourceTag(
													(UserI) user);
											if (s.indexOf("=") > -1) {
												t.setName(s.substring(0,
														s.indexOf("=")));
												t.setTag(s.substring(s
														.indexOf("=") + 1));
											} else {
												if (s.indexOf(":") > -1) {
													t.setName(s.substring(0,
						...
						if (wrk == null
								&& "SNAPSHOTS".equals(catResource.getLabel())) {
							if (getSecurityItem() instanceof XnatExperimentdata) {
								...
								if (wrks != null && wrks.size() == 1) {
									wrk = (WrkWorkflowdata) CollectionUtils
											.get(wrks, 0);
						...
						this.getResponse()
								.setStatus(
										Status.CLIENT_ERROR_UNPROCESSABLE_ENTITY,
										"Only ResourceCatalog documents can be PUT to this address.");
					}
				} else {
					this.getResponse()
	...
	public void handleDelete() {
		...
		if (resources.size() > 0 && this.parent != null
				&& this.security != null) {
					...
					if (user.canDelete(this.security)) {
						...
						if (parent.getItem().instanceOf("xnat:experimentData")) {
							...
							if (proj == null)
								...
								"xnat:experimentData")) {
							securityId = ((XnatExperimentdata) security)
									.getId();
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (resources.size() == 0) {
			this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
					"Unable to find the specified catalog.");
			...
			try {
				...
				if (resource.getItem().instanceOf("xnat:resourceCatalog")) {
					boolean includeRoot = this
							.isQueryVariableTrue("includeRootPath");
					...
					if (cat != null)
						...
						this.getResponse().setStatus(
								...
								"Unable to find catalog file.");
					}
				} else {
					return new ItemXMLRepresentation(resource.getItem(),
							MediaType.TEXT_XML);
				}
