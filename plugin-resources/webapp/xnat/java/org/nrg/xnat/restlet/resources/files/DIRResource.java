...
public class DIRResource extends SecureResource {
		...
		if (expt instanceof XnatSubjectassessordata) {
			...
			try {
				...
				if (src.size() == 0) {
					...
					// TODO: Need to add XAR output here? (Probably not for
					// single-file zipping)
					if (isZIPRequest(mt)) {
						...
						rep.addEntry(f);
						this.setContentDisposition(String.format("%s.zip",
								f.getName()));
						return rep;
					} else {
					...
					if ((isZIPRequest(mt) || (mt.equals(APPLICATION_XAR)))) {
						...
						if (mt.equals(APPLICATION_XAR)) {
							try {
								String userPath = ArcSpecManager.GetInstance()
										.getGlobalCachePath()
										+ "USERS"
										+ File.separator + user.getXdatUserId();
								File outF = new File(userPath, "expt_"
										+ (new Date()).getTime());
								outF.deleteOnExit();
								...
								writer.setLocation(TurbineUtils
										.GetRelativePath(this
												.getHttpServletRequest())
										+ "/" + "schemas/");
								writer.setRelativizePath(((XnatSubjectassessordata) expt)
										.getArchiveDirectoryName() + "/");
								writer.write(expt.getItem());
								rep.addEntry(
										((XnatSubjectassessordata) expt)
												.getId() + ".xml", outF);
							} catch (Exception e) {
						...
						if (this.getQueryVariable("format") != null) {
							if (qsParams.equals(""))
								...
								qsParams += "&";
							qsParams += String.format("format=%s",
									this.getQueryVariable("format"));
						}
						...
						for (final FileSet fs : dest) {
							...
							for (final File f : fs.getMatches()) {
								...
								row[0] = ((parent.toURI().relativize(f.toURI()))
										.getPath());
								row[1] = f.isDirectory();
								row[2] = (f.length());

								final String rel = (session_dir.toURI()
										.relativize(f.toURI())).getPath();
								final String qs = (f.isDirectory()) ? qsParams
										: "";
								row[3] = String.format(
										"/data/experiments/%1s/DIR/%2s%3s",
										new Object[] { expt.getId(), rel, qs });
						...
						String rootPath = this.getRequest().getRootRef()
								.getPath();
						if (rootPath.endsWith("/data")) {
							rootPath = rootPath.substring(0,
									rootPath.indexOf("/data"));
						}
						if (rootPath.endsWith("/REST")) {
							rootPath = rootPath.substring(0,
									rootPath.indexOf("/REST"));
						}
				...
				this.getResponse().setStatus(
						...
						String.format(
								"'%s' is not allowed in this resource URI.",
								e.characters));
				return null;
			}
		} else {
	...
	public static List<File> getFiles(File dir, String path, boolean recursive)
		...
		if (slash > -1) {
			...
			final GlobFilenameFilter glob = new GlobFilenameFilter(
					(path.equals("")) ? "*" : path);
			final String[] children = dir.list(glob);
			for (final String child : children) {
