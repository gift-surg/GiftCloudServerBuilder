...
public class FileList extends XNATCatalogTemplate {
	...
	private final boolean listContents = isQueryVariableTrueHelper(this
			.getQueryVariable("listContents"));
	private static String[] zipExtensions = null;

	static {
		final String defaultExtensions = "zip,jar,rar,ear,gar,mrb";
		try {
			// Try to get the files.zip_extensions configuration property. Use
			// default values if it isn't set.
			zipExtensions = XDAT.getSiteConfigurationProperty(
					"files.zip_extensions", defaultExtensions).split(",");
		} catch (Throwable t) {
			// Something went wrong with the above call to the config service,
			// so set zipExtensions to the default values.
			logger.error(
					"Error occurred while initializing FileList service. Unable to read files.zip_extensions property.",
					t);
	...
	public FileList(Context context, Request request, Response response) {
		...
		notifyList = isQueryVariableTrue("notify", request) ? getQueryVariable(
				"notify").split(",") : new String[0];
		try {
			if (resource_ids != null) {
					...
					for (Object[] row : catalogs.rows()) {
						...
						for (String resourceID : resource_ids) {
							if (!alreadyAdded.contains(id)
									&& (id.toString().equals(resourceID) || (label != null && label
											.equals(resourceID)))) {
								XnatAbstractresource res = XnatAbstractresource
										.getXnatAbstractresourcesByXnatAbstractresourceId(
												id, user, false);
								if (row.length == 7)
									res.setBaseURI((String) row[6]);
								resources.add(res);
								alreadyAdded.add(id);
							}
						}
				...
				// if caller is asking for the files directly by resource ID
				// (e.g. /experiments/{EXPT_ID}/resources/{RESOURCE_ID}/files),
				// the catalog will not be found by the superclass
				// (unless caller passes all=true, which seems clunky to require
				// given that they are passing in the resource PK).
				// So here we provide an alternate path finding the resource
				// added check to make sure it's an number. You can also
				// reference resource labels here (not just pks).
				for (String resourceID : resource_ids) {
					try {
			...
			logger.error("Error occurred while initializing FileList service",
					e);
	...
	public Representation represent(Variant variant) {
		...
		try {
			if (proj == null) {
				...
				// this only works because the absolute paths are stored in the
				// database for each resource, so the actual project path isn't
				// used.
				if (parent != null
						&& parent.getItem().instanceOf("xnat:experimentData")) {
					proj = ((XnatExperimentdata) parent)
							.getPrimaryProject(false);
					// Per FogBugz 4746, prevent NPE when user doesn't have
					// access to resource (MRH)
					// Check access through shared project when user doesn't
					// have access to primary project
					if (proj == null) {
						proj = (XnatProjectdata) ((XnatExperimentdata) parent)
								.getFirstProject();
					}
				} else if (security != null
						&& security.getItem().instanceOf("xnat:experimentData")) {
					proj = ((XnatExperimentdata) security)
							.getPrimaryProject(false);
					// Per FogBugz 4746, ....
					if (proj == null) {
						proj = (XnatProjectdata) ((XnatExperimentdata) security)
								.getFirstProject();
					}
				} else if (security != null
						&& security.getItem().instanceOf("xnat:subjectData")) {
					proj = ((XnatSubjectdata) security)
							.getPrimaryProject(false);
					// Per FogBugz 4746, ....
					if (proj == null) {
						proj = (XnatProjectdata) ((XnatSubjectdata) security)
								.getFirstProject();
					}
				} else if (security != null
						&& security.getItem().instanceOf("xnat:projectData")) {
			...
			if (resources.size() == 1 && !(isZIPRequest(mt))) {
				...
				for (Hashtable<String, Object> rowHash : catalogs.rowHashs()) {
					...
					if (rowHash.containsKey("resource_path"))
						res.setBaseURI((String) rowHash.get("resource_path"));
					resources.add(res);
				}
				...
				if (user.canEdit(security)) {
					if (proj == null) {
						if (parent.getItem().instanceOf("xnat:experimentData")) {
							proj = ((XnatExperimentdata) parent)
									.getPrimaryProject(false);
						} else if (security.getItem().instanceOf(
								"xnat:experimentData")) {
							proj = ((XnatExperimentdata) security)
									.getPrimaryProject(false);
						} else if (parent.getItem().instanceOf(
								"xnat:subjectData")) {
							proj = ((XnatSubjectdata) parent)
									.getPrimaryProject(false);
						} else if (security.getItem().instanceOf(
								"xnat:subjectData")) {
							proj = ((XnatSubjectdata) security)
					...
					PersistentWorkflowI wrk = PersistentWorkflowUtils
							.getWorkflowByEventId(user, getEventId());
					if (wrk == null && resource != null
							&& "SNAPSHOTS".equals(resource.getLabel())) {
						if (getSecurityItem() instanceof XnatExperimentdata) {
							Collection<? extends PersistentWorkflowI> workflows = PersistentWorkflowUtils
									.getOpenWorkflows(user,
											((ArchivableItem) security).getId());
							if (workflows != null && workflows.size() == 1) {
								wrk = (WrkWorkflowdata) CollectionUtils.get(
										workflows, 0);
								if (!"xnat_tools/AutoRun.xml".equals(wrk
										.getPipelineName())) {
									wrk = null;
								}
							}
					...
					try {
						...
						if (writers != null) {
							if (writers.size() > 0) {
								if (StringUtils.isEmpty(reference) || !async) {
									List<String> duplicates = buildResourceModifier(
											overwrite, um).addFile(writers,
											resourceIdentifier, type, filepath,
											buildResourceInfo(um), extract);
									...
									MoveStoredFileRequest request = new MoveStoredFileRequest(
											buildResourceModifier(overwrite, um),
											resourceIdentifier, writers, user,
											wrk.getWorkflowId(), delete,
											notifyList, type, filepath,
											buildResourceInfo(um), extract);
									...
									getResponse().setEntity(
											new JSONObjectRepresentation(
													MediaType.TEXT_HTML, json));
								}
							} else {
								final String method = getRequest().getMethod()
										.toString();
								final long size = getRequest().getEntity()
										.getAvailableSize();
								if (size == 0) {
									getResponse()
											.setStatus(
													...
													"You tried to "
															+ method
															+ " to this service, but didn't provide any data (found request entity size of 0). Please check the format of your service request.");
								} else {
									getResponse()
											.setStatus(
													Status.CLIENT_ERROR_BAD_REQUEST,
													"You tried to "
															...
															+ CatalogUtils
																	.formatSize(size)
															+ " to this service, but didn't provide any data. If you think you sent data to upload, you can try to "
															+ method
						...
						logger.error(
								"Error occurred while trying to POST file", e);
						throw e;
					}
		...
		if (resource != null && parent != null && security != null) {
			try {
				if (user.canDelete(security)) {
					if (!((security).getItem().isActive() || (security)
							.getItem().isQuarantine())) {
						// cannot modify it if it isn't active
						throw new ClientException(
								Status.CLIENT_ERROR_FORBIDDEN, new Exception());
					}
					...
					if (resource instanceof XnatResourcecatalog) {
						...
						final File catFile = catResource.getCatalogFile(proj
								.getRootArchivePath());
						final String parentPath = catFile.getParent();
						final CatCatalogBean cat = catResource.getCleanCatalog(
								proj.getRootArchivePath(), false, null, null);

						CatEntryBean e = (CatEntryBean) CatalogUtils
								.getEntryByURI(cat, filepath);
						if (e != null) {
							entries.add(e);
						}
						if (entries.size() == 0) {
							e = (CatEntryBean) CatalogUtils.getEntryById(cat,
									filepath);
							if (e != null) {
								entries.add(e);
							}
						...
						if (entries.size() == 0 && filepath.endsWith("/")) {
							...
							entries.addAll(CatalogUtils.getEntriesByFilter(cat,
									folderFilter));
						}
						...
						if (entries.size() == 0 && filepath.endsWith("*")) {
							...
							entries.addAll(CatalogUtils.getEntriesByRegex(cat,
									regex.toString()));
						}
						...
						for (CatEntryI entry : entries) {
							...
							if (f.exists()) {
								PersistentWorkflowI work = WorkflowUtils
								...
								CatalogUtils.moveToHistory(catFile, f,
										(CatEntryBean) entry, ci);

								if (!isQueryVariableFalse("removeFiles")
										&& !f.delete()) {
									logger.warn("Error attempting to delete physical file for deleted resource: "
											+ f.getAbsolutePath());
								}

								// if parent folder is empty, then delete folder
								if (FileUtils.CountFiles(f.getParentFile(),
										true) == 0) {
									FileUtils.DeleteFile(f.getParentFile());
								}

								CatalogUtils.populateStats(catResource,
										proj.getRootArchivePath());
								SaveItemHelper.authorizedSave(catResource,
										user, false, false, ci);
	...
	public Representation representTable(XFTTable table, MediaType mt,
		...
		if (mt.equals(SecureResource.APPLICATION_XCAT)) {
			...
			for (Object[] row : table.rows()) {
				...
				String relative = RestFileUtils.getRelativePath(uri,
						session_mapping);
				...
				relative = RestFileUtils.replaceResourceLabel(relative,
						row[cat_IDIndex], (String) row[collectionIndex]);

				for (Map.Entry<String, String> e : valuesToReplace.entrySet()) {
					relative = RestFileUtils.replaceInPath(relative,
							e.getKey(), e.getValue());
				}
			...
			try {
				rep = new ZipRepresentation(mt, getSessionIds(),
			...
			// Refactored on 3/24 to allow the returning of the old file
			// structure. This was to support Mohana's legacy pipelines.
			...
			if (structure.equalsIgnoreCase("legacy")
					|| structure.equalsIgnoreCase("simplified")) {
				valuesToReplace = new Hashtable<String, String>();
			} else {
				valuesToReplace = getReMaps();
			}

			// TODO: This should all be rewritten. The implementation of the
			// path relativization should be injectable, particularly to support
			...
			for (final Object[] row : table.rows()) {
				...
				if (child != null && child.exists()) {
					...
					if (structure.equals("improved")) {
						...
						relative = RestFileUtils
								.buildRelativePath(pathForZip, session_mapping,
										valuesToReplace, row[cat_IDIndex],
										(String) row[collectionIndex])
								.replace("/resources", "")
								.replace("/files", "");
					} else {
						relative = RestFileUtils
	...
	private String getImprovedPath(String fileUri, Object catNumber,
		...
		for (Object[] row : rows) { // iterate through the rows of the catalog
									// to find
			if (row[0].equals(catNumber)) { // the catalog entry matching the
											// current object
				root = row[3].toString() + "/"; // resource type, e.g. scans,
												// resources, assessors
				if (row[4] != null && !row[4].equals("")) { // folder name,
															// usually scan
															// number_scan type
					root += row[4].toString();
					// extend the folder name with scan type as long as it's not
					// a tar (tar's have a 100 character limit)
					if (!mt.equals(MediaType.APPLICATION_GNU_TAR)
							&& !mt.equals(MediaType.APPLICATION_TAR)
							&& row[5] != null && !row[5].equals("")) {
						// session types can have special characters that
						// interfere with filepath creation, so those should be
				...
				if (row[1] != null && !row[1].equals("")) {
					root += row[1].toString() + "/"; // data subfolder, most
														// commonly DICOM
				} else {
					root += row[0].toString() + "/"; // if no subfolder name,
														// use resource id
				}
			}
		}
	...
	public CatEntryFilterI buildFilter() {
		...
		if ((file_content != null && file_content.length > 0)
				|| (file_format != null && file_format.length > 0)) {
			return new CatEntryFilterI() {
				public boolean accept(CatEntryI entry) {
					if (file_format != null && file_format.length > 0) {
						if (entry.getFormat() == null) {
							if (!ArrayUtils.contains(file_format, "NULL"))
								return false;
						} else {
							if (!ArrayUtils.contains(file_format,
									entry.getFormat()))
								return false;
						}
					}
					...
					if (file_content != null && file_content.length > 0) {
						if (entry.getContent() == null) {
							if (!ArrayUtils.contains(file_content, "NULL"))
								return false;
						} else {
							if (!ArrayUtils.contains(file_content,
									entry.getContent()))
								return false;
						}
					}
	...
	private List<FileWriterWrapperI> getReferenceWrapper(String value)
			throws FileUploadException {
		File file = new File(value);
		if (!file.exists()) {
			throw new FileUploadException(
					"The resource referenced does not exist: " + value);
		}
		List<FileWriterWrapperI> files = new ArrayList<FileWriterWrapperI>();
		if (file.isFile()) {
			files.add(new StoredFile(file, true, "", true));
		} else {
			// TODO: This is a simple recursive find of all files underneath the
			// specified root. It'd be nice to support manifest files containing
			// ant path specifiers or something similar to that.
			Collection found = org.apache.commons.io.FileUtils.listFiles(file,
			...
			for (Object foundObject : found) {
				...
				if (foundFile.isFile()) {
					files.add(new StoredFile(foundFile, true, file.toURI()
							.relativize(foundFile.getParentFile().toURI())
							.getPath(), true));
				}
			}
		}
	...
	protected Representation handleMultipleCatalogs(MediaType mt)
		...
		if (!isZip && getQueryVariable("locator") != null) {
			...
			} else if (getQueryVariable("locator").equalsIgnoreCase(
					"projectPath")) {
				locator = "projectPath";
				headers[ArrayUtils.indexOf(headers, "URI")] = locator;
			}
		...
		final Integer index = (containsQueryVariable("index")) ? Integer
				.parseInt(getQueryVariable("index")) : null;
		...
		for (final XnatAbstractresource temp : resources) {
			if (temp.getItem().instanceOf("xnat:resourceCatalog")) {
				...
				final CatCatalogBean cat = catResource.getCleanCatalog(
						proj.getRootArchivePath(), includeRoot, null, null);
				final String parentPath = catResource.getCatalogFile(
						proj.getRootArchivePath()).getParent();

				if (cat != null) {
					if (filepath == null || filepath.equals("")) {
						table.rows()
								.addAll(CatalogUtils.getEntryDetails(
										cat,
										parentPath,
										(catResource.getBaseURI() != null) ? catResource
												.getBaseURI() + "/files"
												: baseURI
														+ "/resources/"
														+ catResource
																.getXnatAbstractresourceId()
														+ "/files",
										catResource, isZip || (index != null),
										entryFilter, proj, locator));
					} else {
						...
						if (entries.size() == 0 && filepath.endsWith("/")) {
							...
							final boolean recursive = !(this
									.isQueryVariableFalse("recursive"));
														...
														.contains(
																entry.getUri()
																		.substring(
																				dir.length() + 1),
																"/")) {
											return (entryFilter == null || entryFilter
													.accept(entry));
										}
									}
									return false;
						...
						if (entries.size() == 1) {
							if (FileUtils.IsAbsolutePath(entries.get(0)
									.getUri())) {
								f = new File(entries.get(0).getUri());
							} else {
								f = new File(parentPath, entries.get(0)
										.getUri());
							}
				...
				// not catalog
				if (entryFilter == null) {
					ArrayList<File> files = temp.getCorrespondingFiles(proj
							.getRootArchivePath());
					for (File subFile : files) {
						...
						if (locator.equalsIgnoreCase("URI")) {
							row[2] = (temp.getBaseURI() != null) ? temp
									.getBaseURI()
									+ "/files/"
									+ subFile.getName() : baseURI
									+ "/resources/"
									+ temp.getXnatAbstractresourceId()
									+ "/files/" + subFile.getName();
		...
		String downloadName;
		if (security != null) {
			downloadName = ((ArchivableItem) security)
		...
		if (StringUtils.isEmpty(filepath) && index == null) {
			...
			// return file
			if (fileList.size() > 0) {
				if ((mt.equals(MediaType.APPLICATION_ZIP) && !name
						.toLowerCase().endsWith(".zip"))
						|| (mt.equals(MediaType.APPLICATION_GNU_TAR) && !name
								.toLowerCase().endsWith(".tar.gz"))
						|| (mt.equals(MediaType.APPLICATION_TAR) && !name
								.toLowerCase().endsWith(".tar"))) {
					final ZipRepresentation rep;
					try {
						rep = new ZipRepresentation(mt,
								((ArchivableItem) security)
										.getArchiveDirectoryName(),
								identifyCompression(null));
					} catch (ActionException e) {
						logger.error("", e);
	...
	protected Representation handleSingleCatalog(MediaType mt)
			throws ElementNotFoundException {
		File f = null;
		...
		if (resource.getItem().instanceOf("xnat:resourceCatalog")) {
			...
			CatCatalogBean cat = catResource.getCleanCatalog(
					proj.getRootArchivePath(), includeRoot, null, null);
			String parentPath = catResource.getCatalogFile(
					proj.getRootArchivePath()).getParent();
			...
			if (StringUtils.isEmpty(filepath) && index == null) {
				...
				CatEntryI entry;
				if (index != null) {
					entry = CatalogUtils.getEntryByFilter(cat,
							new CatEntryFilterI() {
								private int count = 0;
					...
					for (String s : zipExtensions) {
						...
						if (lowercase.contains(s + "!")
								|| lowercase.contains(s + "/")) {
							zipEntry = filepath.substring(lowercase.indexOf(s)
									+ s.length());
							filepath = filepath.substring(0,
									lowercase.indexOf(s) + s.length());
				...
				if (entry == null && filepath.endsWith("/")) {
					...
					final CatalogUtils.CatEntryFilterI folderFilter = new CatalogUtils.CatEntryFilterI() {
						...
						public boolean accept(CatEntryI entry) {
							if (entry.getUri().startsWith(dir)) {
								if (recursive
										|| StringUtils.contains(entry.getUri()
					...
					// If there are no matching entries, I'm not sure if this
					// should throw a 404, or return an empty list.
								...
								.addAll(CatalogUtils.getEntryDetails(
										...
										baseURI
												...
												+ catResource
														.getXnatAbstractresourceId()
												+ "/files", catResource, false,
										folderFilter, proj, locator));
					} else {
						getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
								"Unable to find catalog entry for given uri.");
						return new StringRepresentation("");
					}
				} else if (entry == null) {
					getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
					...
					if (f.exists()) {
						...
						try {
							// If the user is requesting a file within the zip
							// archive
							if (zipEntry != null) {
								// Get the zip entry requested
								ZipFile zF = new ZipFile(f);
								ZipEntry zE = zF.getEntry(URLDecoder.decode(
										zipEntry, "UTF-8"));
								if (zE == null) {
									getResponse().setStatus(
											Status.CLIENT_ERROR_NOT_FOUND,
									...
									return new InputRepresentation(
											...
											buildMediaType(mt, fName));
								}
								// If the user is requesting a list of the
								// contents within the zip file
							} else if (listContents && isFileZipArchive(fName)) {
								// Get the contents of the zip file
								ZipFile zF = new ZipFile(f);
								Enumeration<? extends ZipEntry> entries = zF
										.entries();

								// Create a new XFTTable with File Name and Size
								// columns
								XFTTable t = new XFTTable();
								t.initTable(new String[] { "File Name", "Size" });

								// Populate table rows and add the row to the
								// table
								while (entries.hasMoreElements()) {
									ZipEntry zE = entries.nextElement();
									t.rows().add(
											new Object[] { zE.getName(),
													zE.getSize() });
								}
								zF.close();
								...
								if (t.rows().size() != 0) {
									table = t; // table gets passed into
												// representTable() below
								}
							} else {
								// Return the requested file
								return getFileRepresentation(f,
										buildMediaType(mt, fName));
							}
			...
			if (filepath == null || filepath.equals("")) {
				String baseURI = getBaseURI();
				if (entryFilter == null) {
					ArrayList<File> files = resource.getCorrespondingFiles(proj
							.getRootArchivePath());
					for (File subFile : files) {
	...
	/**
	 * Function determines if the given file is a zip archive by checking
	 * whether the fileName contains a zip extension
	 *
	 * @param f
	 *            - the file name
	...
	private Map<String, String> getSessionMaps() {
		...
		if (assesseds.size() > 0) {
			// Check if the session containing the assessor has an "ASSESSORS"
			// directory.
			// This signifies that the directory structure is based on a
			// "modern" version of XNAT.
			if (new File(assesseds.get(0).getSessionDir(), "ASSESSORS")
					.isDirectory() && expts.size() > 0) {
				for (XnatExperimentdata session : expts) {
					session_ids.put(session.getId(),
							session.getArchiveDirectoryName());
				}
			} else {
	...
	private FileRepresentation getFileRepresentation(File f, MediaType mt) {
		...
		try {
			...
			getResponse().setStatus(Status.SERVER_ERROR_INTERNAL,
					"Unable to return file as " + mt.getName());
		}
		return fr;
	}
