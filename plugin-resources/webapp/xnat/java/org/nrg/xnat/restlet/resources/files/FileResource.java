...
public class FileResource extends ItemResource {
	...
	public FileResource(Context context, Request request, Response response) {
		...
		if (assessid != null) {
			assessed = XnatImagesessiondata.getXnatImagesessiondatasById(
					assessid, user, false);

			if (assessed == null) {
				assessed = (XnatImagesessiondata) XnatImagesessiondata
						.GetExptByProjectIdentifier(proj.getId(), assessid,
								user, false);
			}
		}
		...
		if (exptID != null) {
			expt = XnatImagesessiondata.getXnatImagesessiondatasById(exptID,
					user, false);

			if (expt == null) {
				expt = (XnatImagesessiondata) XnatImagesessiondata
		...
		if (scanID != null && this.assessed != null) {
			...
			ArrayList<XnatImagescandata> scans = XnatImagescandata
					.getXnatImagescandatasByField(cc, user, completeDocument);
			if (scans.size() > 0) {
				scan = scans.get(0);
			}
		}
		...
		if (reconID != null) {
			...
			cc.addClause("xnat:reconstructedImageData/image_session_ID",
					assessed.getId());
			ArrayList<XnatReconstructedimagedata> scans = XnatReconstructedimagedata
		...
		try {
			...
			AdminUtils.sendAdminEmail(user, "Possible SQL Injection attempt.",
					"User passed " + resourceID + " as a resource identifier.");
		...
		if (recon != null) {
			...
			if (type != null) {
				if (type.equals("in")) {
					...
					query += " WHERE xnat_reconstructedimagedata_xnat_reconstructedimagedata_id="
							+ recon.getXnatReconstructedimagedataId();
					query += " AND map.xnat_abstractresource_xnat_abstractresource_id="
							+ resourceID;
				} else {
			...
			query += " FROM xnat_abstractresource abst"
					+ " LEFT JOIN xnat_resource res ON abst.xnat_abstractresource_id=res.xnat_abstractresource_id";
			query += " WHERE xnat_imagescandata_xnat_imagescandata_id="
					+ scan.getXnatImagescandataId() + "";
					...
					if (type != null) {
						if (type.equals("in")) {
							query += " FROM img_assessor_in_resource map "
									+ " LEFT JOIN xnat_abstractresource abst ON map.xnat_abstractresource_xnat_abstractresource_id=abst.xnat_abstractresource_id"
									+ " LEFT JOIN xnat_resource res ON abst.xnat_abstractresource_id=res.xnat_abstractresource_id";
							query += " WHERE xnat_imageassessordata_id='"
									+ expt.getId() + "'";
							query += " AND map.xnat_abstractresource_xnat_abstractresource_id="
									+ resourceID;
						} else {
						...
						query += " FROM xnat_experimentdata_resource map "
								+ " LEFT JOIN xnat_abstractresource abst ON map.xnat_abstractresource_xnat_abstractresource_id=abst.xnat_abstractresource_id"
								+ " LEFT JOIN xnat_resource res ON abst.xnat_abstractresource_id=res.xnat_abstractresource_id";
						query += " WHERE xnat_experimentdata_id='"
								+ expt.getId() + "'";
						query += " AND map.xnat_abstractresource_xnat_abstractresource_id="
								+ resourceID;
					}
		...
		try {
			XFTTable table = XFTTable
					.Execute(query, user.getDBName(), userName);
			if (table.size() > 0) {
				resource = XnatAbstractresource
						.getXnatAbstractresourcesByXnatAbstractresourceId(
								resourceID, user, false);
		...
		if (resource != null && this.parent != null && this.security != null) {
			try {
				if (user.canEdit(this.security)) {
					...
					File catFile = catResource.getCatalogFile(proj
							.getRootArchivePath());
					...
					if (entry == null) {
						this.getResponse().setStatus(
								...
								"Unable to identify specified file.");
						return;
					}
					...
					if (f.exists()) {
						...
						this.getResponse().setStatus(
								Status.CLIENT_ERROR_NOT_FOUND, "File missing");
						return;
					}
				} else {
					this.getResponse()
	...
	public Representation getRepresentation(Variant variant) {
		...
		if (resource != null) {
			try {
				...
				CatCatalogI cat = catResource.getCleanCatalog(
						proj.getRootArchivePath(), false, null, null);
