...
public final class PrearcSessionResource extends SecureResource {
	...
	private final Logger logger = LoggerFactory
			.getLogger(PrearcSessionResource.class);
	...
	public void handlePost() {
		...
		if (POST_ACTION_BUILD.equals(action)) {
			try {
				if (PrearcDatabase.setStatus(session, timestamp, project,
						PrearcUtils.PrearcStatus.BUILDING)) {
					PrearcDatabase.buildSession(sessionDir, session, timestamp,
							project, (String) params.get(VISIT),
							(String) params.get(PROTOCOL),
							(String) params.get(TIMEZONE),
							(String) params.get(SOURCE));
					PrearcUtils.resetStatus(user, project, timestamp, session,
							true);
					returnString(wrapPartialDataURI(PrearcUtils.buildURI(
							project, timestamp, session)),
							MediaType.TEXT_URI_LIST, Status.SUCCESS_OK);
				} else {
					this.getResponse().setStatus(Status.CLIENT_ERROR_CONFLICT,
							"session document locked");
				}
			} catch (InvalidPermissionException e) {
				logger.error("", e);
			...
			try {
				...
				PrearcUtils.resetStatus(user, project, timestamp, session, tag,
						true);
				returnString(wrapPartialDataURI(PrearcUtils.buildURI(project,
						timestamp, session)), MediaType.TEXT_URI_LIST,
						Status.SUCCESS_OK);
			} catch (InvalidPermissionException e) {
				logger.error("", e);
				PrearcUtils.log(project, timestamp, session, e);
				...
				if (!PrearcUtils.canModify(user, newProj)) {
					this.getResponse()
							.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
									"Unable to modify session data for destination project.");
					return;
				}
				if (PrearcDatabase.setStatus(session, timestamp, project,
						PrearcStatus.MOVING)) {
					PrearcDatabase
							.moveToProject(session, timestamp,
									(project == null) ? "Unassigned" : project,
									newProj);
					returnString(wrapPartialDataURI(PrearcUtils.buildURI(
							newProj, timestamp, session)),
			...
			try {
				if (PrearcDatabase.setStatus(session, timestamp, project,
						PrearcUtils.PrearcStatus.BUILDING)) {
					final SessionData sd = PrearcDatabase.getSession(session,
							timestamp, project);
					if (null == sd.getAutoArchive()
							&& !Strings.isNullOrEmpty(project)) {
						final XnatProjectdata p = XnatProjectdata
								.getProjectByIDorAlias(project, user, false);
						PrearcDatabase.setAutoArchive(session, timestamp,
								project, PrearchiveCode.code(p
										.getArcSpecification()
										.getPrearchiveCode()));
					}
					PrearcDatabase.buildSession(sessionDir, session, timestamp,
					...
					final FinishImageUpload uploader = new FinishImageUpload(
							null, user, new PrearcSession(project, timestamp,
									session, params, user), null, false, true,
							false);
					try {
						if (uploader.isAutoArchive()) {
							returnString(wrapPartialDataURI(uploader.call()),
									Status.REDIRECTION_PERMANENT);
						} else {
							returnString(wrapPartialDataURI(uploader.call()),
									MediaType.TEXT_URI_LIST, Status.SUCCESS_OK);
				...
				logger.error("", e);
				if (e.getCause() != null
						&& e.getCause() instanceof ActionException) {
			...
			try {
				PrearcDatabase.setStatus(session, timestamp, project,
						(String) this.params.get("status"));
			} catch (Exception e) {
	...
	public Representation getRepresentation(final Variant variant) {
		...
		String popup = StringUtils.equalsIgnoreCase(
				this.getQueryVariable("popup"), "true") ? "true" : "false";
		...
		if (StringUtils.isNotEmpty(filepath)) {
			if (filepath.startsWith("logs/") && filepath.length() > 5) {
				...
				if (logId.equals("last")) {
					contents = PrearcUtils.getLastLog(project, timestamp,
							session);
				} else {
					contents = PrearcUtils.getLog(project, timestamp, session,
							logId);
				}
				...
				if (this.getQueryVariable("template") == null
						|| this.getQueryVariable("template").equals("details")) {
					tb.initTable(new String[] { "id", "date", "entry" });

					try {
						for (File log : PrearcUtils.getLogs(project, timestamp,
								session)) {
							Date timestamp = new Date(log.lastModified());
							String id = log.getName().substring(0,
									log.getName().indexOf(".log"));
							tb.insertRow(new Object[] { id, timestamp,
					...
					for (String id : PrearcUtils.getLogIds(project, timestamp,
							session)) {
		...
		if (MediaType.TEXT_HTML.equals(mt) || StringUtils.isNotEmpty(screen)) {
			...
			if (StringUtils.isEmpty(screen)) {
				screen = "XDATScreen_brief_xnat_imageSessionData.vm";
					...
					.equals("XDATScreen_uploaded_xnat_imageSessionData.vm")) {
				if (project == null) {
					this.getResponse()
							.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
									"Projects in the unassigned folder cannot be archived.");
					return null;
				}
				getResponse()
						.redirectSeeOther(
								getContextPath()
										+ String.format(
												"/app/action/LoadImageData/project/%s/timestamp/%s/folder/%s/popup/%s",
												project, timestamp, session,
												popup));
				return null;
			}
			...
			if (!sessionXML.isFile()) {
				this.getResponse()
						.setStatus(
								...
								"The named session exists, but its XNAT session document is not available. The session is likely invalid or incomplete.");
				return null;
			}
			...
			l.add(new SessionDataTriple().setFolderName(sessionDir.getName())
					.setProject(project).setTimestamp(timestamp));
			XFTTable table = null;
			try {
				table = PrearcUtils.convertArrayLtoTable(PrearcDatabase
						.buildRows(l));
			} catch (Exception e) {
				this.getResponse().setStatus(Status.SERVER_ERROR_INTERNAL,
						e.getMessage());
			}
			return this.representTable(table, MediaType.APPLICATION_JSON,
					new Hashtable<String, Object>());
		} else if (MediaType.APPLICATION_GNU_ZIP.equals(mt)
				|| MediaType.APPLICATION_ZIP.equals(mt)) {
			final ZipRepresentation zip;
			try {
				zip = new ZipRepresentation(mt, sessionDir.getName(),
