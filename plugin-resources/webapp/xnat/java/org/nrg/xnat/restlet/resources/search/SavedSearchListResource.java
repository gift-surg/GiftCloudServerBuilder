...
public class SavedSearchListResource extends SecureResource {
	static org.apache.log4j.Logger logger = Logger
			.getLogger(SavedSearchListResource.class);

	public SavedSearchListResource(Context context, Request request,
			Response response) {
	...
	public List<String> retrieveAllTags() {
		try {
			return (List<String>) (XFTTable.Execute(
					"SELECT tag from xdat_stored_search", user.getDBName(),
					user.getLogin()).convertColumnToArrayList("tag"));
			...
			if (includeTagged != null) {
				if (includeTagged.equals("true")) {
					...
					if (retrieveAllTags().contains(includeTagged)) {
						...
						logger.error("", new Exception("Unknown tag: "
								+ includeTagged));
