...
public class SearchElementListResource extends SecureResource {
	public SearchElementListResource(Context context, Request request,
			Response response) {
	...
	public Representation getRepresentation(Variant variant) {
		...
		fields.initTable(new String[] { "ELEMENT_NAME", "SINGULAR", "PLURAL",
				"SECURED", "COUNT" });

		try {
			Hashtable<String, ElementSecurity> allES = (Hashtable<String, ElementSecurity>) ElementSecurity
					.GetElementSecurities().clone();

			// remove security elements
			for (ElementSecurity es : ((Hashtable<String, ElementSecurity>) allES
					.clone()).values()) {
