...
public class SearchFieldListResource extends SecureResource {
	...
	public SearchFieldListResource(Context context, Request request,
			Response response) {
	...
	private void setBooleanProperty(XFTItem found, String field,
			boolean _default) {
		try {
	...
	private void setAction(XFTItem found, int count, String action_name,
			String display_name, String img, String secureAccess, String popup) {
		try {
			found.setProperty(
					"xdat:element_security.element_actions.element_action__"
							+ count + ".element_action_name", action_name);
			found.setProperty(
					"xdat:element_security.element_actions.element_action__"
							+ count + ".display_name", display_name);
			found.setProperty(
					"xdat:element_security.element_actions.element_action__"
							+ count + ".sequence", new Integer(count));
			if (img != null)
				found.setProperty(
						"xdat:element_security.element_actions.element_action__"
								+ count + ".image", img);
			if (secureAccess != null)
				found.setProperty(
						"xdat:element_security.element_actions.element_action__"
								+ count + ".secureAccess", secureAccess);
			if (popup != null)
				found.setProperty(
						"xdat:element_security.element_actions.element_action__"
								+ count + ".popup", popup);
		...
		try {
			if (XFTTool.ValidateElementName(elementName)) {
				try {
					...
					if ((!this.isQueryVariableFalse("secure"))
							&& se.hasField(se.getFullXMLName() + "/project")
							&& se.hasField(se.getFullXMLName()
									+ "/sharing/share/project")) {
						found.setProperty("secure", Boolean.TRUE);
						found.setProperty(
								"primary_security_fields.primary_security_field__0",
								se.getFullXMLName() + "/project");
						found.setProperty(
								"primary_security_fields.primary_security_field__1",
					...
					if (this.getQueryVariable("singular") != null) {
						found.setProperty("singular",
								this.getQueryVariable("singular"));
					}

					if (this.getQueryVariable("plural") != null) {
						found.setProperty("plural",
								this.getQueryVariable("plural"));
					}
					...
					this.setAction(found, count++, "edit", "Edit", "e.gif",
							"edit", null);

					this.setAction(found, count++, "xml", "View XML", "r.gif",
							null, null);

					this.setAction(found, count++, "xml_file", "Download XML",
							"save.gif", null, null);

					this.setAction(found, count++, "email_report", "Email",
							"right2.gif", null, "always");
	...
	public Representation getRepresentation(Variant variant) {
		...
		fields.initTable(new String[] { "FIELD_ID", "HEADER", "SUMMARY",
				"TYPE", "REQUIRES_VALUE", "DESC", "ELEMENT_NAME", "SRC" });

		ArrayList<String> elementNames = StringUtils
				.CommaDelimitedStringToArrayList(elementName);
			...
			try {
				...
				while (iter.hasNext()) {
					...
					if (df.isSearchable()) {
						...
						Boolean requiresValue = (df instanceof SQLQueryField) ? true
								: false;
						...
						sub[3] = type;
						sub[4] = requiresValue;
						sub[5] = (df.getDescription() == null) ? (df
								.getHeader() == null) ? df.getId() : df
								.getHeader() : df.getDescription();
				...
				try {
					ArrayList<List> custom_fields = user
					...
					try {
						if (GenericWrapperElement.GetFieldForXMLPath(se
								.getFullXMLName() + "/project") != null) {
							try {
								List<Object> av = user.getAllowedValues(se,
										se.getFullXMLName() + "/project",
										"read");
								for (Object o : av) {
									Object[] sub = new Object[8];
									sub[0] = pi.getId() + "=" + o;
									sub[1] = o;
									sub[2] = "Label within the " + o
											+ " project.";
									sub[3] = "string";
									sub[4] = false;
									sub[5] = "Label within the " + o
											+ " project.";
									sub[6] = se.getFullXMLName();
									sub[7] = 2;
