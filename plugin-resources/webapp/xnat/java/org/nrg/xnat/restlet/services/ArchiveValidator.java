	...
	public void handlePost() {
		...
		try {
			...
			if (sessions.size() == 1) {
				...
				PrearcSessionValidator validator = new PrearcSessionValidator(
						session, user, additionalValues);
				List<PrearcSessionValidator.Notice> validation = validator
						.validate();
				...
				for (PrearcSessionValidator.Notice notice : validation) {
					t.rows().add(
							new Object[] { notice.getCode(), notice.getType(),
									notice.getMessage() });
				}
				...
				throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST,
						"Cannot validate multiple sessions in one request.");
			}
