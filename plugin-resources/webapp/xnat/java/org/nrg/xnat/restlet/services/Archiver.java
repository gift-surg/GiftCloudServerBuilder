...
public class Archiver extends BatchPrearchiveActionsA {
	...
	public void handlePost() {
		...
		try {
			...
			if (overwriteFILES != null
					&& overwriteFILES.toString().equalsIgnoreCase(
							RequestUtil.TRUE)) {
			...
			if ((project_id == null || timestamp == null || sessionFolder == null)
					&& (srcs == null)) {
				this.getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND,
						"Unknown prearchive session.");
				return;
			} else if (srcs != null) {
				for (final String src : srcs) {
					...
					try {
						sessions.add(new PrearcSession(
								(URIManager.PrearchiveURI) data,
								additionalValues, user));
					} catch (InvalidPermissionException e) {
						throw new ResourceException(
								Status.CLIENT_ERROR_FORBIDDEN, data.getUri());
					} catch (Exception e) {
						throw new ResourceException(
								Status.SERVER_ERROR_INTERNAL, data.getUri()
										+ " invalid.");
					}
				}
			} else if (dest != null) {
				...
				for (final String s : sessionFolder) {
					try {
						sessions.add(new PrearcSession(project_id, timestamp,
								s, additionalValues, user));
					} catch (InvalidPermissionException e) {
						throw new ResourceException(
								Status.CLIENT_ERROR_FORBIDDEN,
								String.format(
										"/prearchive/projects/%s/%s/%s not found.",
										project_id, timestamp, s));
					} catch (Exception e) {
						throw new ResourceException(
								Status.SERVER_ERROR_INTERNAL,
								String.format(
										"/prearchive/projects/%s/%s/%s invalid.",
										project_id, timestamp, s));
					}
				}
			}
			...
			for (final PrearcSession map : sessions) {
				if (map.getProject() == null) {
					throw new ResourceException(
							Status.CLIENT_ERROR_BAD_REQUEST,
							"Cannot archive sessions from the Unassigned folder.");
				}

				if (!map.getSessionDir().exists()) {
					throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
							map.getUrl() + " not found.");
				}
			}
			...
			if (sessions.size() == 1) {
				...
				if (PrearcDatabase.setStatus(session.getFolderName(),
						...
						PrearcStatus.ARCHIVING)) {
					FinishImageUpload.setArchiveReason(session, false);
					_return = "/data"
							+ PrearcDatabase.archive(session,
									allowDataDeletion, overwrite,
									overwrite_files, user, listeners);
				} else {
					this.getResponse()
							.setStatus(Status.CLIENT_ERROR_FORBIDDEN,
									"Operation already in progress on this prearchive entry.");
					return;
				}
				...
				if (!StringUtils.isEmpty(redirect)
					...
					getResponse().setEntity(_return + CRLF,
				...
				for (PrearcSession ps : sessions) {
					if (!ps.getAdditionalValues().containsKey(
							EventUtils.EVENT_REASON))
						ps.getAdditionalValues().put(EventUtils.EVENT_REASON,
								"Batch archive");
				}

				m = PrearcDatabase.archive(sessions, allowDataDeletion,
						overwrite, overwrite_files, user, listeners);

				getResponse().setEntity(
						updatedStatusRepresentation(m.keySet(),
			...
			if (e.cause != null && e.cause instanceof ActionException) {
				...
				this.getResponse().setStatus(
						((ActionException) e.cause).getStatus(),
						e.cause.getMessage());
				return;
			} else {
				logger.error("", e);
	...
	public static PrearcSessionArchiver buildArchiver(
			final PrearcSession session, final Boolean overrideExceptions,
			final Boolean allowSessionMerge, final Boolean overwriteFiles,
			final XDATUser user, final boolean waitFor) throws IOException,
			SAXException {
		final PrearcSessionArchiver archiver;

		archiver = new PrearcSessionArchiver(session, user,
				session.getAdditionalValues(), overrideExceptions,
