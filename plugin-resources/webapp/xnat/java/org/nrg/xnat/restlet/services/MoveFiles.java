...
public class MoveFiles extends SecureResource {
	...
	public URIManager.UserCacheURI convertKey(final String key)
			throws ClientException {
		try {
		...
		try {
			...
			if (moves.size() == 0) {
				this.getResponse().setStatus(Status.CLIENT_ERROR_BAD_REQUEST,
						"Missing src and dest path");
				return;
			}

			EventMetaI ci;
			PersistentWorkflowI work = PersistentWorkflowUtils
			...
			for (Map.Entry<URIManager.UserCacheURI, ResourceURII> entry : moves
					.entrySet()) {
