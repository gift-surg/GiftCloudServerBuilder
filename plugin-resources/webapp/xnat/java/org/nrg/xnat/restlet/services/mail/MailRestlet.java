...
public class MailRestlet extends SecureResource {
	...
	public void handlePost() {
		...
		try {
			...
			if (!extractParameters(entity)) {
				_log.warn("I was unable to properly parse an incoming request, check the logs for more information, returning: "
			...
			// When receiving email send requests through the REST service, the
			// from address is always the admin, with the mail sent on behalf of
			// the validating user.
			...
			if (tos == null || tos.length == 0) {
				throw new ClientException(
						"No valid user IDs were provided for the to address.");
			}
			message.setTos(tos);
			...
			respondToException(exception,
					(exception.status != null) ? exception.status
							: Status.CLIENT_ERROR_BAD_REQUEST);
		...
		if (_issues.size() > 0) {
			...
			StringBuilder issues = new StringBuilder(
					"Found a number of issues sending email:\n\n");
	...
	private boolean extractParameters(Representation entity) throws Exception {
		...
		if (MediaType.APPLICATION_WWW_FORM.equals(mediaType, true)) {
			...
			List<FileWriterWrapperI> files = getFileWritersAndLoadParams(
					entity, true);
			for (FileWriterWrapperI file : files) {
				...
				// TODO: I don't like having to write to a temporary file here,
				// but this is a restriction coming from JavaMail where
				// attachments may need to be opened multiple times.
		...
		if (!_parameters.containsKey(key)) {
			if (_log.isDebugEnabled()) {
				_log.debug(String.format(
						"Creating new value list for parameter: %s", key));
			}
			_parameters.put(key, new ArrayList<String>());
		}

		if (_log.isDebugEnabled()) {
			_log.debug(String.format("Adding value for parameter %s: [%s]",
					key, value));
		}

		// For now assume that value is a string until we can support other
		// types.
	...
	private String[] getAddresses(String type) {
		...
		if (_parameters.containsKey(type)) {
			...
			for (String id : ids) {
				try {
					// Try to parse this as an integer. If it's a user ID, it'll
					// parse and we'll use the get by ID function.
					XdatUser requested = XDATUser.getXdatUsersByXdatUserId(
							Integer.parseInt(id), user, true);
					if (requested == null) {
						addIssue(String
								.format("The user ID %s was not found in the system and was not included on the email.",
										id));
					} else {
						addresses.add(requested.getEmail());
					}
				} catch (NumberFormatException exception) {
					// If not an integer, we'll try it as an email address. It
					// has to match an existing email address in the system!
					ArrayList<XdatUser> users = XDATUser.getXdatUsersByField(
							"xdat:user/email", id, user, true);
					if (users.size() == 0) {
						addIssue(String
								.format("The user email %s was not found in the system and was not included on the email.",
										id));
					} else {
						addresses.add(id);
					}
	...
	private String getParameter(String parameter) {
		return _parameters.containsKey(parameter) ? _parameters.get(parameter)
				.get(0) : null;
	}
	...
	private boolean validateParameters() {
		...
		if (!_parameters.containsKey(PARAM_TO)) {
			validationErrors
					.add("You must specify at least one recipient for your message.");
		}
		if (!_parameters.containsKey(PARAM_SUBJECT)) {
			validationErrors
					.add("Emails sent through this service must have a subject.");
		} else if (_parameters.get(PARAM_SUBJECT).size() > 1) {
			validationErrors
					.add("You can only provide a single value for the email subject.");
		}
		if (!_parameters.containsKey(PARAM_TEXT)
				&& !_parameters.containsKey(PARAM_HTML)) {
			validationErrors
					.add("Emails sent through this service must have a message body in either the HTML or text parameters.");
		} else {
			if (_parameters.containsKey(PARAM_TEXT)
					&& _parameters.get(PARAM_TEXT).size() > 1) {
				validationErrors
						.add("You can only provide a single value for the email text body.");
			}
			if (_parameters.containsKey(PARAM_HTML)
					&& _parameters.get(PARAM_HTML).size() > 1) {
				validationErrors
						.add("You can only provide a single value for the email HTML body.");
			}
		}
	...
	private String formatErrorMessage(List<String> errors) {
		StringBuilder message = new StringBuilder(
				"The following problems were found with your submission:\n\n");
