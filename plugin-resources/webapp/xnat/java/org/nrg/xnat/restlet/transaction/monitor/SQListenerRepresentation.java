...
public class SQListenerRepresentation extends SecureResource {
	...
	static org.apache.log4j.Logger logger = Logger
			.getLogger(SQListenerRepresentation.class);

	public SQListenerRepresentation(Context context, Request request,
			Response response) {
	...
	private StatusMessage buildMessage(final Status s, final String msg)
	...
	public void acceptRepresentation(Representation entity)
		...
		try {
			final StatusMessage msg = buildMessage(
					buildStatus(TurbineUtils.escapeParam(form
							.getFirstValue("status"))),
					TurbineUtils.escapeParam(form.getFirstValue("message")));
	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		try {
			...
			if (mt.equals(MediaType.APPLICATION_JSON)) {
				...
				return new StringRepresentation(sq.toString(),
			...
			throw new ResourceException(
					org.restlet.data.Status.CLIENT_ERROR_NOT_FOUND, e);
		} catch (JSONException e) {
			throw new ResourceException(e);
		}
	}

	private JSONObject buildJSONObject(final StatusList sq)
	...
	class HTMLStatusListRepresentation extends OutputRepresentation {
		...
		public HTMLStatusListRepresentation(MediaType mediaType,
				final StatusList o) {
			super(mediaType);
			this.o = o;
		}
