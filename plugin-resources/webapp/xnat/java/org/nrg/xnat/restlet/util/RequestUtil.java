...
public class RequestUtil {
	...
	// method with boolean flag to allow curl processing where media types are
	// not specified
	public static String deriveFileName(final String prefix,
			final Representation entity, boolean returnNullForUnsupported) {
		if (!(entity == null || entity.getMediaType() == null)) {
			if (supported_upload_types.containsKey(entity.getMediaType())) {
				return prefix
						+ supported_upload_types.get(entity.getMediaType());
			}
		}
	...
	// original deriveFileName method returned null filename for
	// null/unsupported types
	public static String deriveFileName(final String prefix,
	...
	public static <T extends MediaType> boolean compareMediaType(
			Representation entity, T... mts) {
		if (entity == null || entity.getMediaType() == null) {
			return false;
		}
	...
	public static boolean isMultiPartFormData(Representation entity) {
		...
		if (mt.equals(MediaType.MULTIPART_ALL)) {
			...
			return true;
			// check string values of main/subtypes where doesn't resolve to one
			// of the above media types.
			// Happens for some Curl requests
		} else if (mt.getMainType().equalsIgnoreCase("multipart")
				&& mt.getSubType().equalsIgnoreCase("form-data")) {
			return true;
		} else {
			return false;
		}
