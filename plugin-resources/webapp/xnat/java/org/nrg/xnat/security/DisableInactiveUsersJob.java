...
public class DisableInactiveUsersJob implements JobInterface {
	...
	public void execute() throws JobExecutionException {
		final int secondsBeforeLockout = Integer.parseInt((String) _map
				.get("inactivityBeforeLockout"));
			...
			try {
				XFTTable t = XFTTable
						.Execute(
								"SELECT xdat_user.login FROM xdat_user INNER JOIN "
										...
										+ ") y "
										+ // get last login times for each user
										...
										+ ") x "
										+ // get dates that each non-admin user
											// was created
										"ON x.login=xdat_user.login AND ((x.activation_date<(now()- INTERVAL '"
										+ secondsBeforeLockout
										+ " seconds')) AND ((x.last_login IS NULL) OR x.last_login<(now()- INTERVAL '"
										+ secondsBeforeLockout
										+ " seconds'))) AND xdat_user.enabled=1",
								null, null);
				...
				while (t.hasMoreRows()) {
					...
					try {
						...
						// Fixes XNAT-2407. Only disable user if they have not
						// been recently modified (enabled).
						if (!hasUserBeenModified(u, secondsBeforeLockout)) {
							u.setEnabled("0");
							u.setVerified("0");
							XDATUser.ModifyUser(u, u, EventUtils
									.newEventInstance(
							...
							System.out.println("Locked out " + u.getLogin()
							...
							AdminUtils
									.sendAdminEmail(
											...
											"User "
													+ u.getLogin()
													+ " has been automatically disabled due to inactivity.");
						}
					} catch (Exception e) {
						logger.error("", e);
					}
	...
	/**
	 * Function determines if the user has been modified in the past amount of
	 * seconds. Fixes XNAT-2407. This keeps the job from disabling a user if the
	 * admin has just enabled (modified) them.
	 * 
	 * @param u
	 *            - the user we are interested in.
	 * @param seconds
	 *            - Has the user been modified in the past amount of seconds.
	...
	private boolean hasUserBeenModified(XDATUser u, int seconds) {
		...
		// If the time is before the last modified date, the user has been
		// modified.
	...
	private Logger logger = LoggerFactory
			.getLogger(DisableInactiveUsersJob.class);
}
