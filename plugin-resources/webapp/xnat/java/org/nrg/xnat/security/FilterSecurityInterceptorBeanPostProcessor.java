...
public class FilterSecurityInterceptorBeanPostProcessor implements
		BeanPostProcessor {
	...
	public ExpressionBasedFilterInvocationSecurityMetadataSource getMetadataSource(
		...
		for (String openUrl : _openUrls) {
			...
			map.put(new RequestKey(openUrl),
					SecurityConfig.createList(PERMIT_ALL));
		}
		...
		for (String adminUrl : _adminUrls) {
			...
			map.put(new RequestKey(adminUrl),
					SecurityConfig.createList(ADMIN_EXPRESSION));
		}

		final String nonopen = requiredLogin ? DEFAULT_EXPRESSION : PERMIT_ALL;
		if (_log.isDebugEnabled()) {
			_log.debug("Setting " + nonopen + " on the default pattern: "
					+ DEFAULT_PATTERN);
		}
		map.put(new RequestKey(DEFAULT_PATTERN),
				SecurityConfig.createList(nonopen));
		return new ExpressionBasedFilterInvocationSecurityMetadataSource(
				urlMatcher, map, handler);
	}
	...
	private boolean isRequiredLogin() {
		...
		// If it's STILL null, then arcSpec hasn't been initialized in the
		// database, so just say false.
		if (_arcSpec == null || !ArcSpecManager.HasPersisted()) {
			return false;
		}
	...
	private void displayMetadataSource(
			final SecurityMetadataSource metadataSource) {
		if (metadataSource != null) {
			_log.debug("Found metadata source configuration, now iterating.");
			for (ConfigAttribute attribute : metadataSource
	...
	private static final Log _log = LogFactory
			.getLog(FilterSecurityInterceptorBeanPostProcessor.class);
