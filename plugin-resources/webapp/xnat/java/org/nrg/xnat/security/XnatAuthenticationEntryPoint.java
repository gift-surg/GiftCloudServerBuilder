...
public class XnatAuthenticationEntryPoint extends
		LoginUrlAuthenticationEntryPoint {
	/**
	 * Overrides
	 * {@link LoginUrlAuthenticationEntryPoint#commence(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, org.springframework.security.core.AuthenticationException)}
	 * to test for data path and user agent. If this request is for a data path
	 * by an non-interactive agent, the response status is set to HTTP 302, i.e.
	 * unauthorized. Otherwise the base implementation is used, which redirects
	 ...
	 * @param authException
	 *            An authentication exception that may have redirected the agent
	 *            to re-authenticate.
	 * @throws IOException
	...
	public void commence(final HttpServletRequest request,
			...
			ServletException {
		final String strippedUri = request.getRequestURI().substring(
				request.getContextPath().length());
		final String userAgent = request.getHeader("User-Agent");

		if (_log.isDebugEnabled()) {
			_log.debug("Evaluating data path request: " + strippedUri
					+ ", user agent: " + userAgent);
		}

		if (strippedUri != null
				&& strippedUri.contains("/action/AcceptProjectAccess/par/")) {
	...
	/**
	 * Sets the data paths, i.e. those paths which require a user-agent
	 * interactivity test to determine whether the user should be denied as
	 * unauthorized or redirected to the login page. Each data path should be a
	 * valid Ant-style pattern matching the URL(s) to be designated as data
	 * paths.
	 *
	 * @param dataPaths
	 *            A list of strings in Ant-style patterns indicating data paths.
	 */
	...
	/**
	 * Sets the list of interactive agents to redirect to the login page even on
	 * data paths.
	 *
	 * @param interactiveAgents
	 *            A list of interactive agents to be directed to the login page
	 *            even on data paths.
	 */
	public void setInteractiveAgents(final List<String> interactiveAgents) {
		for (final String interactiveAgent : interactiveAgents) {
			if (_log.isDebugEnabled()) {
				_log.debug("Adding interactive agent specifier: "
						+ interactiveAgent);
			}
	...
	private boolean isInteractiveAgent(final String userAgent) {
		...
		if (!StringUtils.isBlank(userAgent)) {
			for (Pattern interactiveAgent : _agentPatterns) {
				if (interactiveAgent.matcher(userAgent).matches()) {
					if (_log.isDebugEnabled()) {
						_log.debug("User agent "
								+ userAgent
								+ " is interactive, matched simple regex pattern: "
								+ interactiveAgent);
					}
					return true;
				}
		...
		if (_log.isDebugEnabled()) {
			_log.debug("User agent " + userAgent
					+ " is not interactive, failed to match any patterns");
		}
		return false;
	}

	private static final Log _log = LogFactory
			.getLog(XnatAuthenticationEntryPoint.class);
