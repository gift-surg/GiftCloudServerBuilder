...
public class XnatAuthenticationFilter extends
	...
	public Authentication attemptAuthentication(HttpServletRequest request,
			...
			if (!StringUtils.isBlank(header) && header.startsWith("Basic ")) {
				...
				try {
					...
					if (_log.isDebugEnabled()) {
						_log.debug("Basic Authentication Authorization header found for user '"
								+ username + "'");
					}
				} catch (UnsupportedEncodingException exception) {
					_log.error("Encoding exception on authentication attempt",
							exception);
				}
			}
		}
		...
		if (StringUtils.isEmpty(providerName) && !StringUtils.isEmpty(username)) {
			...
			if (StringUtils.isEmpty(auth_method)) {
				throw new BadCredentialsException(
						"Missing login_method parameter.");
			} else {
				authRequest = buildUPTokenForAuthMethod(auth_method, username,
						password);
			}
		} else {
			authRequest = buildUPTokenForProviderName(providerName, username,
					password);
		}
		...
		try {
			Authentication auth = super.getAuthenticationManager()
					.authenticate(authRequest);
			AccessLogger.LogServiceAccess(username,
					AccessLogger.GetRequestIp(request), "Authentication",
					"SUCCESS");
			return auth;
	...
	public static void logFailedAttempt(String username, HttpServletRequest req) {
		if (!StringUtils.isBlank(username)) {
			...
			if (uid != null) {
				try {
					...
					item.setProperty("xdat:user_login.ip_address",
							AccessLogger.GetRequestIp(req));
					item.setProperty("xdat:user_login.login_date", Calendar
			...
			AccessLogger.LogServiceAccess(username,
					AccessLogger.GetRequestIp(req), "Authentication", "FAILED");
		}
	}
	...
	public static UsernamePasswordAuthenticationToken buildUPTokenForAuthMethod(
	...
	public static UsernamePasswordAuthenticationToken buildUPTokenForProviderName(
			String providerName, String username, String password) {
	...
	public static String retrieveAuthMethod(final String username) {
		...
		if (auth == null) {
			List<XdatUserAuth> userAuths = XDAT.getXdatUserAuthService()
					.getUsersByName(username);
			if (userAuths.size() == 1) {
				auth = userAuths.get(0).getAuthMethod();
				cached_methods.put(username.intern(), auth.intern());
				// The list may contain localdb auth method even when password
				// is empty and LDAP authentication is used (MRH)
			} else if (userAuths.size() > 1) {
				for (XdatUserAuth userAuth : userAuths) {
					if (!userAuth.getAuthMethod().equalsIgnoreCase(
	...
	private static XnatAuthenticationProvider findAuthenticationProviderByAuthMethod(
	...
	private static XnatAuthenticationProvider findAuthenticationProviderByProviderName(
	...
	private static XnatAuthenticationProvider findAuthenticationProvider(
			XnatAuthenticationProviderMatcher matcher) {
		List<AuthenticationProvider> prov = XDAT.getContextService()
				.getBean("customAuthenticationManager", ProviderManager.class)
				.getProviders();
	...
	private static UsernamePasswordAuthenticationToken buildUPToken(
			XnatAuthenticationProvider provider, String username,
			String password) {
		if (provider instanceof XnatLdapAuthenticationProvider) {
			return new XnatLdapUsernamePasswordAuthenticationToken(username,
					password, provider.getProviderId());
		} else if (provider instanceof AliasTokenAuthenticationProvider) {
			return new AliasTokenAuthenticationToken(username,
					Long.parseLong(password));
		} else {
			return new XnatDatabaseUsernamePasswordAuthenticationToken(
					username, password);
		}
	}

	private static Map<String, String> cached_methods = Maps.newConcurrentMap();// this
																				// will
																				// prevent
																				// 20,000
																				// curl
	...
	private static final Log _log = LogFactory
			.getLog(XnatAuthenticationFilter.class);
