...
public class XnatBasicAuthenticationFilter extends BasicAuthenticationFilter {
	...
	private boolean authenticationIsRequired(String username) {
		...
		// already present
		// This shouldn't happen very often, as BasicProcessingFilter is meant
		...
		if ((header != null) && header.startsWith("Basic ")) {
			...
			if (authenticationIsRequired(username)) {
				UsernamePasswordAuthenticationToken authRequest = XnatAuthenticationFilter
						.buildUPTokenForAuthMethod(XnatAuthenticationFilter
								.retrieveAuthMethod(username), username,
								password);
				...
				try {
					...
					sessionStrategy.onAuthentication(authResult, request,
							response);
					...
					XnatAuthenticationFilter
							.logFailedAttempt(username, request);// originally I
																	// put this
																	// in the
					...
					response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
							AdminUtils.GetLoginFailureMessage());
	...
	@Override
	// XNAT-2186 requested that REST logins also leave records of last login
	// date
