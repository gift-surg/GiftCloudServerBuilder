...
public class XnatLdapUserDetailsMapper extends LdapUserDetailsMapper {
	...
	public XnatLdapUserDetailsMapper(final String authMethodId,
			final Map<String, String> properties) {
		super();
		Assert.hasText(authMethodId,
				"You must provide an authentication method ID.");
		Assert.notEmpty(properties,
				"You must provide the authentication provider properties.");
		if (_log.isInfoEnabled()) {
			_log.info("Creating user details mapper with the auth method ID ["
					+ authMethodId
					+ "] and "
					+ (properties != null && properties.size() > 0 ? "mapping properties: "
							+ properties.toString()
	...
	public XDATUserDetails mapUserFromContext(DirContextOperations ctx,
		...
		String email = (String) ctx.getObjectAttribute(_properties
				.get(PROPERTY_EMAIL));
		String firstname = (String) ctx.getObjectAttribute(_properties
				.get(PROPERTY_FIRST));
		String lastname = (String) ctx.getObjectAttribute(_properties
				.get(PROPERTY_LAST));

		XDATUserDetails userDetails = XDAT.getXdatUserAuthService()
				.getUserDetailsByNameAndAuth(user.getUsername(),
						XdatUserAuthService.LDAP, _authMethodId, email,
						lastname, firstname);

		try {
			XDATUser xdatUser = new XDATUser(userDetails.getUsername());
			if (((!(XDAT.verificationOn())) || xdatUser.isVerified())
					&& userDetails.getAuthorization().isEnabled()) {
				return userDetails;
			} else {
	...
	private static final Log _log = LogFactory
			.getLog(XnatLdapUserDetailsMapper.class);
