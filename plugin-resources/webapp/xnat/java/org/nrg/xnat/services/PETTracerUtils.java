...
public class PETTracerUtils {
	public PETTracerUtils() throws Exception {
		if (_instance != null) {
			throw new Exception(
					"The PETTracerUtils service is already initialized, try calling getInstance() instead.");
		}
		_instance = this;
	}
	...
	public Configuration getTracerList(String path, Long project) {
		if (logger.isDebugEnabled()) {
			logger.debug("Retrieving tracer list for {}, {} for project: {}",
					new Object[] { TOOL_NAME, path, project });
		}

		return _configService.getConfig(TOOL_NAME, path, project);
	}

	public void setProjectTracerList(String login, String path,
			String tracerList, Long project) throws ConfigServiceException {
		if (logger.isDebugEnabled()) {
			logger.debug("Setting tracer list for {}, {} for project: {}",
					new Object[] { TOOL_NAME, path, project });
		}
	...
	public void setSiteWideTracerList(String login, String path,
			String tracerList) throws ConfigServiceException {
		_configService.replaceConfig(login, "", TOOL_NAME, path, tracerList);
	}
	...
	public static File getDefaultTracerList() throws FileNotFoundException {
		...
		if (def.exists()) {
			...
			throw new FileNotFoundException("Default tracer list: "
					+ DEFAULT_TRACER_LIST + " not found in " + XFT.GetConfDir());
		}
	}
	...
	private static final Logger logger = LoggerFactory
			.getLogger(PETTracerUtils.class);
