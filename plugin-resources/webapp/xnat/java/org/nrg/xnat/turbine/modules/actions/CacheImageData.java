...
public class CacheImageData extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		final String folder = (String) TurbineUtils.GetPassedParameter(
				"folder", data);
		final String investigator = (String) TurbineUtils.GetPassedParameter(
				"investigator", data);
		...
		if (null != folder && null != root) {
			...
			if (data.getParameters().containsKey("project")) {
				final String project = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("project", data));
				String path = null;
				try {
					path = ArcSpecManager.GetInstance()
			...
			if (root.equals("NONE")) {
				...
				if (dir.exists()) {
					...
					for (int i = 0; i < folders.length; i++) {
						if (folders[i].equals(folder)) {
							mvFolder = prearchive_path + root + File.separator
									+ folders[i];
							break;
						}
					}
			...
			if (mvFolder == null) {
				...
				if (f.exists()) {
					try {
						FileUtils.MoveDir(f, new File(cache_path + root
								+ File.separator + folder), true);
						data.setMessage("Folder Removed: " + folder);

						mvFolder += ".xml";
						File prearcXML = new File(mvFolder);
						if (prearcXML.exists()) {
							String cachePath = cache_path + root
									+ File.separator + folder + File.separator
									+ folder + ".xml";

							File cacheXML = new File(cachePath);
							org.nrg.xft.utils.FileUtils.MoveFile(prearcXML,
									cacheXML, true);
						}
						...
						data.getParameters().setString("investigator",
								investigator);
						data.setScreenTemplate(PREARC_PAGE);
						return;
					} catch (Exception e) {
					...
					data.setMessage("Unknown folder: " + folder);
					data.getParameters()
							.setString("investigator", investigator);
					data.setScreenTemplate(PREARC_PAGE);
					return;
				}
			}
