...
public class CompleteUpload extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		if (params.containsKey("project")) {
			...
			try {
				rootPath = ArcSpecManager.GetInstance()
						.getPrearchivePathForProject(p);
			} catch (Throwable e) {
				logger.error("", e);
			}
		}

		String uploadsPath = cachepath + "uploads" + File.separator
		...
		UploadManager um = new UploadManager(user, image_type, cachepath
				+ "batch" + File.separator + user.getLogin() + File.separator
				+ image_type + File.separator + sb.toString() + File.separator,
