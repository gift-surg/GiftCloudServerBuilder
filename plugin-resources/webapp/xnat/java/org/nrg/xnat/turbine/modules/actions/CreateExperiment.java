...
public class CreateExperiment extends SecureAction {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(CreateExperiment.class);
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		if (TurbineUtils.HasPassedParameter("parent_expt_id", data)) {
			parent_expt_id = (String) TurbineUtils.GetPassedParameter(
					"parent_expt_id", data);
		...
		if (TurbineUtils.HasPassedParameter("subtype", data)) {
			...
			// if coming to this page from certain other pages, 'subtype' may be
			// termed as 'protocol'
			if (subtype == null)
				subtype = (String) TurbineUtils.GetPassedParameter("protocol",
						data);
			context.put("subtype", subtype);
		}

		if (TurbineUtils.HasPassedParameter("visit_name", data)) {
			visit_name = (String) TurbineUtils.GetPassedParameter("visit_name",
					data);
			context.put("visit_name", visit_name);
		}

		String dataType = (String) TurbineUtils.GetPassedParameter("data_type",
				data);
		SchemaElement se = SchemaElement.GetElement(dataType);
		data.setScreenTemplate("XDATScreen_edit_" + se.getFormattedName()
				+ ".vm");
	}
