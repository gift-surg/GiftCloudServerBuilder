...
public class DownloadSessionsAction2 extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		String[] session_ids = ((String[]) TurbineUtils.GetPassedObjects(
				"sessions", data));

		String[] requestScanTypes = ((String[]) TurbineUtils.GetPassedObjects(
				"scan_type", data));
		String[] scanFormats = ((String[]) TurbineUtils.GetPassedObjects(
				"scan_format", data));
		String[] recons = ((String[]) TurbineUtils.GetPassedObjects("recon",
				data));
		String[] assessors = ((String[]) TurbineUtils.GetPassedObjects(
				"assessors", data));
		String[] resources = ((String[]) TurbineUtils.GetPassedObjects(
				"resources", data));

		// BEGIN:IOWA customization: to allow project and subject included in
		// path
		boolean projectIncludedInPath = "true"
				.equalsIgnoreCase((String) TurbineUtils.GetPassedParameter(
						"projectIncludedInPath", data));
		boolean subjectIncludedInPath = "true"
				.equalsIgnoreCase((String) TurbineUtils.GetPassedParameter(
						"subjectIncludedInPath", data));
		boolean simplified = "true".equalsIgnoreCase((String) TurbineUtils
				.GetPassedParameter("simplified", data));
		...
		for (String session : session_ids) {
			...
			// narrow down the range of scan types to only the ones relevant to
			// this session
			if (requestScanTypes != null && requestScanTypes.length > 0) {
				String query = "SELECT id FROM xnat_imagescandata WHERE image_session_id = '"
						+ session + "' AND " + getTypeClause(requestScanTypes);
				XFTTable table = XFTTable.Execute(query,
						TurbineUtils.getUser(data).getDBName(), TurbineUtils
								.getUser(data).getLogin());
				List<String> sessionScans = table
						.convertColumnToArrayList("id");
						...
						if (scanFormats != null && scanFormats.length > 0) {
							for (String scanFormat : scanFormats) {
								...
								String uri = server
										...
										+ "/resources/"
										+ URLEncoder
												.encode(scanFormat, "UTF-8")
										+ "/files?format=zip" + extraParam;
								entry.setUri(uri);
								l.add(uri);
							...
							CatEntryBean entry = new CatEntryBean();
							entry.setFormat("ZIP");
							String uri = server + "data/experiments/" + session
									+ "/scans/" + scan + "/files?format=zip"
									+ extraParam;
							entry.setUri(uri);
							l.add(uri);
							scansCatalog.addEntries_entry(entry);
			...
			if (resources != null && resources.length > 0) {
				...
				for (final String res : resources) {
					...
					entry.setFormat("ZIP");
					String uri = server + "data/experiments/" + session
							+ "/resources/" + URLEncoder.encode(res, "UTF-8")
							+ "/files?format=zip" + extraParam;
					entry.setUri(uri);
					l.add(uri);
					scansCatalog.addEntries_entry(entry);
			...
			if (recons != null && recons.length > 0) {
				...
				for (String scanType : recons) {
					...
					String uri = server + "data/experiments/" + session
							+ "/reconstructions/"
							+ URLEncoder.encode(scanType, "UTF-8")
							+ "/files?format=zip" + extraParam;
					entry.setUri(uri);
					l.add(uri);
					scansCatalog.addEntries_entry(entry);
		...
		if (TurbineUtils.GetPassedParameter("download_option", data).equals(
				"applet")) {
	...
	private String getTypeClause(final String[] requestScanTypes) {
		...
		if (length == 0 && !foundNull) {
			throw new RuntimeException(
					"Bad state found: no scan types specified, not even NULL!");
		}
		if (length > 0) {
