...
public class EditSubjectAction extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		try {
			final EditScreenA screen = (EditScreenA) ScreenLoader.getInstance()
					.getInstance("XDATScreen_edit_xnat_subjectData");

			final XFTItem newItem = (XFTItem) screen.getEmptyItem(data);

			// TurbineUtils.OutputDataParameters(data);

			final PopulateItem populater = PopulateItem.Populate(data,
					"xnat:subjectData", true, newItem);
			...
			if (hasError) {
				...
				if (!message.endsWith("/addID : Required Field")) {
					...
					if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
							.GetPassedParameter("edit_screen", data)) != null) {
						data.setScreenTemplate(((String) org.nrg.xdat.turbine.utils.TurbineUtils
								.GetPassedParameter("edit_screen", data)));
					}
					if (TurbineUtils.HasPassedParameter("destination", data))
						data.getParameters().add(
								"destination",
								(String) TurbineUtils.GetPassedParameter(
										"destination", data));
					return;
				}
			}

			final Iterator addIDS = found.getChildItems(
					"xnat:subjectData/addID").iterator();
			...
			final Iterator addFields = found.getChildItems(
					"xnat:subjectData/fields/field").iterator();
			...
			// System.out.println("EditSubjectAction: PRE DOB:" +
			// found.getProperty("dob"));
			// System.out.println("EditSubjectAction: PRE YOB:" +
			// found.getProperty("yob"));

			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("dob_estimated", data)) != null) {
				final Date year = (Date) found.getProperty("demographics/dob");
				found.setProperty("demographics/yob",
						new Integer(year.getYear() + 1900));
				found.setProperty("demographics/dob", null);
			}

			if ((found.getProperty("demographics/yob") != null && !found
					.getProperty("demographics/yob").equals("NULL"))
					|| (found.getProperty("demographics/age") != null && !found
							.getProperty("demographics/age").equals("NULL"))) {
				found.setProperty("demographics/dob", null);
			}

			// System.out.println("EditSubjectAction: POST DOB:" +
			// found.getProperty("dob"));
			// System.out.println("EditSubjectAction: POST YOB:" +
			// found.getProperty("yob"));
			...
			if (subject.getLabel() != null && subject.getProject() != null) {
				...
				subcc.addClause("xnat:subjectData/project",
						subject.getProject());
			...
			if (subject.getAddid().size() > 0) {
				for (final XnatSubjectdataAddidI addID : subject.getAddid()) {
					final CriteriaCollection subCC = new CriteriaCollection(
							"AND");
					subCC.addClause("xnat:subjectData/addID/name",
							addID.getName());
					subCC.addClause("xnat:subjectData/addID/addID",
							addID.getAddid());
					cc.add(subCC);
				}
			}
			...
			if (!subject.getSharing_share().isEmpty()) {
				for (final XnatProjectparticipantI addID : subject
						.getSharing_share()) {
					final CriteriaCollection subCC = new CriteriaCollection(
							"AND");
					if (addID.getLabel() != null) {
						subCC.addClause(
								"xnat:subjectData/sharing/share/project",
								addID.getProject());
						subCC.addClause("xnat:subjectData/sharing/share/label",
								addID.getLabel());
						cc.add(subCC);
					}
				}
			...
			if (cc.size() > 0) {
				final ItemCollection items = ItemSearch.GetItems(
						"xnat:subjectData", cc, TurbineUtils.getUser(data),
						false);

				if (subject.getId() == null) {
					// new item
					if (items.size() > 0) {
						final ArrayList matches = BaseElement.WrapItems(items
								.getItems());
						...
						if (TurbineUtils
								.HasPassedParameter("destination", data))
							data.getParameters().add(
									"destination",
									(String) TurbineUtils.GetPassedParameter(
											"destination", data));
					...
					if (items.size() > 0) {
						if (items.size() > 1) {
							final ArrayList matches = BaseElement
									.WrapItems(items.getItems());
							context.put("matches", matches);
							TurbineUtils.SetEditItem(found, data);
							if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
									.GetPassedParameter("edit_screen", data)) != null) {
							...
							if (TurbineUtils.HasPassedParameter("destination",
									data))
								data.getParameters().add(
										"destination",
										(String) TurbineUtils
							...
							if (!match
									.getStringProperty("xnat:subjectData.ID")
									.equalsIgnoreCase(
											found.getStringProperty("xnat:subjectData.ID"))) {
								...
								if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
										.GetPassedParameter("edit_screen", data)) != null) {
									data.setScreenTemplate(((String) org.nrg.xdat.turbine.utils.TurbineUtils
											.GetPassedParameter("edit_screen",
													data)));
								}
								if (TurbineUtils.HasPassedParameter(
										"destination", data))
									data.getParameters()
											.add("destination",
													(String) TurbineUtils
			...
			PersistentWorkflowI wrk = WorkflowUtils.getOrCreateWorkflowData(
					...
					subject.getId(),
					subject.getProject(),
					newEventInstance(data, EventUtils.CATEGORY.DATA, EventUtils
							.getAddModifyAction(subject.getXSIType(), false)));
			...
			for (int i = 0; i < keysArray.length; i++) {
				...
				if (key.toLowerCase().startsWith("remove_")) {
					...
					final Object value = org.nrg.xdat.turbine.utils.TurbineUtils
							.GetPassedParameter(key, data);
					logger.debug("FOUND REMOVE: " + field + " " + value);
					final ItemCollection items = ItemSearch.GetItems(field,
							value, TurbineUtils.getUser(data), false);
					if (items.size() > 0) {
						...
						try {
							...
							SaveItemHelper.unauthorizedRemoveChild(
									first.getItem(), null, toRemove.getItem(),
									TurbineUtils.getUser(data), ci);
			...
			if (vr != null && !vr.isValid()) {
				...
				try {
					...
					if (!user.canEdit(sub)) {
						error(new InvalidPermissionException(
								"Unable to save subject " + sub.getId()), data);
					}

					this.preSave(user, sub,
							TurbineUtils.GetDataParameterHash(data),
							wrk.buildEvent());

					SaveItemHelper.authorizedSave(sub,
							TurbineUtils.getUser(data), false, false, ci);
					PersistentWorkflowUtils.complete(wrk, ci);
				...
				final SchemaElement se = SchemaElement.GetElement(first
						.getXSIType());
				...
				if (TurbineUtils.HasPassedParameter("source", data)) {
					data.setScreenTemplate((String) TurbineUtils
							.GetPassedParameter("source", data));
					...
					this.redirectToReportScreen((String) TurbineUtils
							.GetPassedParameter("destination", data), first,
							data);
				} else {
			...
			if (TurbineUtils.HasPassedParameter("destination", data))
				data.getParameters().add(
						"destination",
						(String) TurbineUtils.GetPassedParameter("destination",
								data));
			TurbineUtils.SetEditItem(found, data);
			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("edit_screen", data)) != null) {
	...
	public interface PreSaveAction {
		public void execute(XDATUser user, XnatSubjectdata src,
				Map<String, String> params, EventMetaI event) throws Exception;
	}

	private void preSave(XDATUser user, XnatSubjectdata src,
			Map<String, String> params, EventMetaI event) throws Exception {
		List<Class<?>> classes = Reflection
				.getClassesForPackage("org.nrg.xnat.actions.subjectEdit.preSave");
