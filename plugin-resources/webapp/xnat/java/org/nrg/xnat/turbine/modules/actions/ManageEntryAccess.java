...
public class ManageEntryAccess extends ModifyItem {
	...
	public void save(XFTItem first, RunData data, Context context)
		...
		try {
			for (XFTItem i : al) {
				if (first.getGenericSchemaElement().instanceOf(
						"xnat:subjectData")) {
					i.setProperty("subject_id", first.getProperty("ID"));
				} else {
					i.setProperty("sharing_share_xnat_experimentda_id",
							first.getProperty("ID"));
				}

				Authorizer.getInstance().authorizeSave(first,
						TurbineUtils.getUser(data));

				SaveItemHelper.authorizedSave(i, TurbineUtils.getUser(data),
						false, false, c);
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		try {
			...
			while (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter(header + counter, data)) != null) {
				String elementToLoad = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter(header + counter++, data));
				Integer numberOfInstances = ((Integer) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedInteger(elementToLoad, data));
				if (numberOfInstances != null
						&& numberOfInstances.intValue() != 0) {
					int subCount = 0;
			...
			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					...
					&& !((String) org.nrg.xdat.turbine.utils.TurbineUtils
							...
							.equals("")) {
				screenName = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("edit_screen", data)).substring(0,
						((String) org.nrg.xdat.turbine.utils.TurbineUtils
								.GetPassedParameter("edit_screen", data))
								.lastIndexOf(".vm"));
			}
			...
			while (keys.hasMoreElements()) {
				...
				if (screenName == null) {
					...
					if (screenName.equals("XDATScreen_edit_"
							+ e.getFormattedName())) {
						EditScreenA screen = (EditScreenA) ScreenLoader
								.getInstance().getInstance(screenName);
			...
			// if
			// (((String)org.nrg.xdat.turbine.utils.TurbineUtils.GetPassedParameter("edit_screen",data))
			// !=null)
			// {
			...
			if (vr != null) {
				data.getSession().setAttribute(
						this.getReturnEditItemIdentifier(), first);
				context.put("vr", vr);
				if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("edit_screen", data)) != null) {
