...
public class ManagePipeline extends SecureAction {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ManagePipeline.class);

	public void doPerform(RunData data, Context context) throws Exception {
		String task = ((String) TurbineUtils.GetPassedParameter("task", data));
		if (task != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("Got a request to perform a pipeline management task: "
						+ task);
			}
	...
	private void doDeletefromproject(RunData data) {
		...
		String projectId = ((String) TurbineUtils.GetPassedParameter("project",
				data));
		try {
			...
			if (pline != null) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(
							"The user {} has requested removal of the pipeline {} from the project {}.",
							user.getLogin(), pipeline.getPath(), projectId);
				}
				data.setMessage("Item deleted");
				PipelineRepositoryManager.RemoveReferenceToPipelineFromProject(
						pipeline, user, projectId, getEventType(data),
						getReason(data));
				PipelineRepositoryManager.Reset();
			}
		} catch (Exception e) {
			logger.error(
					"Error deleting "
							+ TurbineUtils.GetPassedParameter("search_value",
									data), e);
		...
		try {
			...
			if (pipeline != null) {
				PersistentWorkflowI wrk = PersistentWorkflowUtils
						.buildOpenWorkflow(user, pipeline.getXSIType(),
								pipeline.getPKValueString(),
								...
								EventUtils.newEventInstance(
										...
										"Deleted registered pipeline"));
				PipePipelinerepository pipelines = PipelineRepositoryManager
						.GetInstance();
				pipelinePath = pipeline.getStringProperty("path");
				try {
					logger.info("Attempting to delete the pipeline {}",
							pipelinePath);
					SaveItemHelper.authorizedRemoveChild(
							pipelines.getCurrentDBVersion(), null,
							pipeline.getCurrentDBVersion(), user,
							wrk.buildEvent());
					logger.info("Deleted {}", pipelinePath);
					data.setMessage("Pipeline removed from site repository: "
							+ pipelinePath);
					PipelineRepositoryManager
							.RemoveReferenceToPipelineFromProjects(
									pipelinePath, user, wrk.buildEvent());
					PipelineRepositoryManager.Reset();
					...
					logger.error(
							"Error occurred deleting the indicated pipeline: "
									+ pipelinePath, e);
			...
			logger.error(
					"Error deleting "
							+ TurbineUtils.GetPassedParameter("search_value",
									data), e);
			data.setMessage("Error deleting requested pipeline: "
					+ (pipelinePath == null ? "Couldn't find the pipeline object"
							: pipelinePath));
		}
	...
	public void doRedirect(RunData data, Context context) throws Exception {
		try {
			String projectId = ((String) TurbineUtils.GetPassedParameter(
					"project", data));
			String pipelinePath = ((String) TurbineUtils.GetPassedParameter(
					"pipeline", data));
			String schemaType = ((String) TurbineUtils.GetPassedParameter(
					"schema_type", data));
			...
			try {
				...
				// project
				PipePipelinedetails pipelineDetails = PipelineRepositoryManager
						.GetInstance().getPipeline(pipelinePath);
			...
			logger.error(
					"Error occurred trying to redirect to the pipeline launch page.",
					e);
		}

	}

	private String getStepId(String templateSuppliedStepId,
			boolean launchedAtAutoArchive, String nextStepId, String displayText) {
		if (templateSuppliedStepId != null) {
			if (launchedAtAutoArchive) {
				if (templateSuppliedStepId
						.startsWith(PipelineUtils.AUTO_ARCHIVE)) {
					return templateSuppliedStepId;
				}
				return nextStepId;
			} else if (!templateSuppliedStepId
					.startsWith(PipelineUtils.AUTO_ARCHIVE)) {
				return templateSuppliedStepId;
			}
			return displayText;
	...
	public void doAddprojectpipeline(RunData data, Context context)
			throws Exception {
		XDATUser user = TurbineUtils.getUser(data);
		XFTItem found;
		try {
			...
			String pipelinePath = ((String) TurbineUtils.GetPassedParameter(
					"pipeline_path", data));
			String dataType = ((String) TurbineUtils.GetPassedParameter(
					"dataType", data));
			String schemaType = ((String) TurbineUtils.GetPassedParameter(
					"schemaType", data));
			...
			XFTItem newItem = XFTItem.NewItem(schemaType,
			...
			PopulateItem populater = PopulateItem.Populate(data, schemaType,
					true, newItem);
			found = populater.getItem();

			boolean launchedAtAutoArchive = data.getParameters().getBoolean(
					"auto_archive");

			// boolean launchedAtAutoArchive =
			// ((Boolean)org.nrg.xdat.turbine.utils.TurbineUtils.GetPassedParameter("auto_archive",data));
			// Boolean autoArchive = (Boolean)
			// org.nrg.xdat.turbine.utils.TurbineUtils.GetPassedParameter("auto_archive",data);
			// boolean launchedAtAutoArchive = autoArchive == null ? false :
			// autoArchive.booleanValue();
			...
			PersistentWorkflowI wrk = WorkflowUtils.buildOpenWorkflow(
					...
					projectId,
					projectId,
					newEventInstance(data, EventUtils.CATEGORY.PROJECT_ADMIN,
							EventUtils.MODIFY_CONFIGURED_PIPELINE));
			...
			if (dataType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) { // Its a
				...
				if (edit) {
					ArcProjectPipeline existing = arcProject
							.getPipelineEltByPath(pipelinePath);
					copy(data, existing);
					if (existing.getStepid().startsWith(
							PipelineUtils.AUTO_ARCHIVE)
							&& !launchedAtAutoArchive) {
						existing.setStepid(existing.getDisplaytext());
					}
					saved = SaveItemHelper.authorizedSave(existing, user,
							false, false, c);
				} else {
					String stepId = getStepId(templateSuppliedStepId,
							launchedAtAutoArchive,
							PipelineUtils.getNextAutoArchiveStepId(arcProject),
							newPipeline.getDisplaytext());
					newPipeline.setStepid(stepId);
				}
				arcProject.setPipelines_pipeline(newPipeline.getItem());
			} else {
				ArcProjectDescendant existingDesc = arcProject
						.getDescendant(dataType);
				...
				if (existingDesc == null) {
					...
					String stepId = getStepId(
							templateSuppliedStepId,
							launchedAtAutoArchive,
							PipelineUtils
									.getNextAutoArchiveStepId((ArcProjectDescendant) null),
							newPipeline.getDisplaytext());
					newPipeline.setStepid(stepId);
					newDesc.setPipeline(newPipeline.getItem());
					arcProject.setPipelines_descendants_descendant(newDesc
							.getItem());
				} else {
					if (edit) {
						ArcProjectDescendantPipeline existingPipeline = existingDesc
								.getPipeline(pipelinePath);
						copy(data, existingPipeline);
						if (existingPipeline.getStepid().startsWith(
								PipelineUtils.AUTO_ARCHIVE)
								&& !launchedAtAutoArchive) {
							existingPipeline.setStepid(existingPipeline
									.getDisplaytext());
						}
						saved = SaveItemHelper.authorizedSave(existingPipeline,
								user, false, false, c);
					} else {
						String stepId = getStepId(
								templateSuppliedStepId,
								launchedAtAutoArchive,
								PipelineUtils
										.getNextAutoArchiveStepId(existingDesc),
								newPipeline.getDisplaytext());
						newPipeline.setStepid(stepId);
			...
			if (!edit) {
				saved = SaveItemHelper.authorizedSave(arcProject, user, false,
						false, c);
			}
	...
	private void copy(RunData data,
			ArcProjectDescendantPipeline existingPipeline)
			throws XFTInitException, ElementNotFoundException,
			FieldNotFoundException, InvalidValueException {
		Hashtable hash = (Hashtable) TurbineUtils.GetDataParameterHash(data);
		existingPipeline.getItem().setProperties(hash, true);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void copy(RunData data, ArcProjectPipeline existingPipeline)
			throws XFTInitException, ElementNotFoundException,
			FieldNotFoundException, InvalidValueException {
		Hashtable hash = (Hashtable) TurbineUtils.GetDataParameterHash(data);
		existingPipeline.getItem().setProperties(hash, true);
		...
		try {
			...
			if (vr != null) {
				...
				if (TurbineUtils.GetPassedParameter("edit_screen", data) != null) {
					data.setScreenTemplate(((String) TurbineUtils
							.GetPassedParameter("edit_screen", data)));
				}
			} else {
				try {
					...
					SaveItemHelper.authorizedSave(pipelineRepository, user,
							false, true, EventUtils.newEventInstance(
									EventUtils.CATEGORY.SIDE_ADMIN,
									EventUtils.TYPE.WEB_FORM,
									"Added registered pipeline"));
					PipelineRepositoryManager.Reset();
					data.setMessage("Pipeline " + pipelineDetails.getPath()
	...
	@SuppressWarnings("unused")
	public void doLaunchpipeline(RunData data, Context context)
			throws Exception {
		XDATUser user = TurbineUtils.getUser(data);
		XFTItem item = TurbineUtils.GetItemBySearch(data);
		String pipeline_path = ((String) TurbineUtils.GetPassedParameter(
				"pipeline_path", data));
		...
		boolean runPipelineInProcess = data.getParameters().containsKey(
				"run_pipeline_in_process") ? data.getParameters().getBoolean(
				"run_pipeline_in_process")
				: XnatPipelineLauncher.DEFAULT_RUN_PIPELINE_IN_PROCESS;
		...
		// isn't selected.
		boolean recordWorkflowEntries = data.getParameters().containsKey(
				"suppress_workflow_entries") ? !data.getParameters()
				.getBoolean("suppress_workflow_entries")
				: XnatPipelineLauncher.DEFAULT_RECORD_WORKFLOW_ENTRIES;
		...
		String paramFilePath = saveParameters(buildDir + File.separator
				+ exptLabel, paramFileName, parameters);
		...
		// TODO: We need to get status back for in-process pipeline launching
		// and use that for when runPipelineInProcess is true.
		data.setMessage(runPipelineInProcess ? "<p><b>The requested pipeline has completed.</b></p>"
				: "<p><b>The pipeline has been scheduled.  Status email will be sent upon its completion.</b></p>");
	...
	private Parameters extractParameters(RunData data) {
		...
		for (int i = 0; i < totalParams; i++) {
			String name = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("param[" + i + "].name", data));
			int rowcount = TurbineUtils.GetPassedInteger("param[" + i
					+ "].name.rowcount", data);
			...
			for (int j = 0; j < rowcount; j++) {
				...
				if (TurbineUtils.HasPassedParameter(formfieldname, data)) // formvalues.add(data.getParameters().get(formfieldname));
					formvalues
							.add(((String) org.nrg.xdat.turbine.utils.TurbineUtils
									.GetPassedParameter(formfieldname, data)));
			}
	...
	private String saveParameters(String rootpath, String fileName,
		...
		paramDoc.save(paramFile, new XmlOptions().setSavePrettyPrint()
