...
public class ManageProjectPermissions extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		// (XnatProjectdata)XnatProjectdata.getXnatProjectdatasById(projectID,
		// null, false);
		...
		//
		// XFTTable users =
		// XFTTable.Execute("SELECT login, xdat_user_id FROM xdat_user ORDER BY lastname || ', ' || firstname;",
		// project.getDBName(), null);
		//
		...
		// {
		// XdatUser tempUSER =
		// (XdatUser)XdatUser.getXdatUsersByXdatUserId(userID, user, false);
		//
		// PersistentWorkflowI
		// wrk=PersistentWorkflowUtils.getOrCreateWorkflowData(null, user, user,
		// EventUtils.newEventInstance(EventUtils.CATEGORY.SIDE_ADMIN,
		// EventUtils.TYPE.WEB_FORM, "Modified user permissions");
		//
		//
		// String query =
		// "SELECT element_name, xdat_field_mapping_set_id FROM xdat_element_access ea LEFT JOIN xdat_field_mapping_set fms ON ea.xdat_element_access_id=fms.permissions_allow_set_xdat_elem_xdat_element_access_id"
		// +
		// " WHERE xdat_user_xdat_user_id=" + userID + ";";
		// XFTTable permissions = XFTTable.Execute(query, project.getDBName(),
		// null);
		//
		...
		//
		// Object create =
		// ((String)org.nrg.xdat.turbine.utils.TurbineUtils.GetPassedParameter(tempUSER.getLogin()
		// + "_" + protocol +"_c", data));
		// Object read =
		// ((String)org.nrg.xdat.turbine.utils.TurbineUtils.GetPassedParameter(tempUSER.getLogin()
		// + "_" + protocol +"_r", data));
		...
		// fm.setProperty("xdat_field_mapping_set_xdat_field_mapping_set_id",
		// fieldMappingSetID);
		//
