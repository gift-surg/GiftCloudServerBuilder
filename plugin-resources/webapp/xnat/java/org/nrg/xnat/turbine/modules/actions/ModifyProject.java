...
public class ModifyProject extends SecureAction {
		...
		try {
			...
			if (!user.canEdit(project)) {
				error(new InvalidPermissionException(
						"User cannot modify project " + project.getId()), data);
				return;
			}
			...
			if (TurbineUtils.HasPassedParameter("destination", data)) {
				this.redirectToReportScreen(
						(String) TurbineUtils.GetPassedParameter("destination",
								data, "AddStep2.vm"), postSave, data);
			} else {
				this.redirectToReportScreen(
	...
	/**
	 * Inelegant solution to the need to be able to remove investigators from a
	 * project.
	 * 
	 * @param project
	 * @param user
	 * @throws Exception
	 */
	private void removeExcessInvestigators(XnatProjectdata project,
		...
		for (XnatInvestigatordataI investigator : project
				.getInvestigators_investigator()) {
			if (investigator.getXnatInvestigatordataId() != null)
				investigatorIds.add(investigator.getXnatInvestigatordataId());
		}

		// if there are investigators, we don't want to delete them, so create a
		// statement to exclude them from the delete
		...
		if (!investigatorIds.isEmpty()) {
			...
			supplementaryClause = " AND xnat_investigatordata_xnat_investigatordata_id NOT IN ("
					+ sb.toString() + ")";
		}
		String query = "DELETE FROM xnat_projectdata_investigator WHERE xnat_projectdata_id = '"
				+ project.getId() + "'" + supplementaryClause + ";";
	...
	/**
	 * Inelegant solution to the need to be able to remove aliases from a
	 * project.
	 * 
	 * @param project
	 * @param user
	 * @throws Exception
	 */
	private void removeExcessAliases(XnatProjectdata project, XDATUser user)
			throws Exception {
		...
		// if there are aliases, we don't want to delete them, so create a
		// statement to exclude them from the delete
		String supplementaryClause = "";
		if (!aliases.isEmpty()) {
			StringBuilder sb = null;
		...
		String query = "DELETE FROM xnat_projectdata_alias WHERE aliases_alias_xnat_projectdata_id = '"
				+ project.getId() + "'" + supplementaryClause + ";";
		PoolDBUtils.ExecuteNonSelectQuery(query, user.getDBName(),
				user.getLogin());
