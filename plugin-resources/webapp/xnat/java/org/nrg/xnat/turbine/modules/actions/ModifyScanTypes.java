...
public class ModifyScanTypes extends ModifyItem {
	private final Logger logger = LoggerFactory
			.getLogger(ModifyScanTypes.class);
	...
	public void doPerform(final RunData data, final Context context)
		...
		try {
			...
			final XnatProjectdata p = XnatProjectdata.getProjectByIDorAlias(
					project, user, false);
			final int numRows = Integer.parseInt((String) TurbineUtils
					.GetPassedParameter("numRows", data));
			...
			for (final XnatExperimentdata exp : p.getExperiments()) {
				if (exp instanceof XnatImagesessiondata) {
					...
					if (user.canEdit(imageSession)) {
						final List<XnatImagescandata> scans = imageSession
								.getScans_scan();
						if (scans != null) {
							final PersistentWorkflowI wrk = WorkflowUtils
							...
							int count = 0;
							while (count <= numRows) {
								final int frames = Integer
										.parseInt((String) TurbineUtils
												.GetPassedParameter("frames_"
														+ count, data));
								final String series_description = standardizeFormat((String) TurbineUtils
										.GetPassedParameter(
												"series_description_" + count,
												data));
								final String imagetype = standardizeFormat((String) TurbineUtils
										.GetPassedParameter(
												"parameters_imagetype_" + count,
												data));

								if ((data.getParameters().getString(
										"change_" + count) != null)
										&& ((String) TurbineUtils
												.GetPassedParameter("change_"
														+ count, data))
												.equals("doChange")) {
									for (final XnatImagescandata scan : scans) {
										if (Integer.valueOf(frames).equals(
												scan.getFrames())
												&& equalsStandardized(
														series_description,
												...
												&& equalsStandardized(
														imagetype,
														(String) scan
																.getProperty("parameters_imagetype"))) {
											scan.setType((String) TurbineUtils
													.GetPassedParameter("type_"
															+ count, data));
											SaveItemHelper.authorizedSave(scan,
													user, false, false, ci);
										}
									}
								}
			...
			this.redirectToReportScreen(
					"XDATScreen_report_xnat_projectData.vm", (ItemI) p, data);
	...
	private static boolean equalsStandardized(final String standardized,
			final String s) {
