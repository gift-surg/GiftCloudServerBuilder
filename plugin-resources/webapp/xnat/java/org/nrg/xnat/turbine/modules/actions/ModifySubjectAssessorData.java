...
public class ModifySubjectAssessorData extends ModifyItem {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		XFTItem found = null;
		try {
			String element0 = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
			...
			if (element0 == null) {
				this.handleException(
						...
						null,
						new Exception(
								"Configuration Exception.<br><br> Please create an &lt;input&gt; with name 'ELEMENT_0' and value 'SOME XSI:TYPE' in your form.  This will tell the Submit action, which data type it is looking for."));
			}

			PopulateItem populater = PopulateItem.Populate(data, element0,
					false);
			...
			if (dbVersion == null) {
				if (StringUtils.isNotEmpty(found.getStringProperty("project"))
						&& StringUtils.isNotEmpty(found
								.getStringProperty("label"))) {
					// check for match by label
					XnatExperimentdata expt = XnatExperimentdata
							.GetExptByProjectIdentifier(
									found.getStringProperty("project"),
									found.getStringProperty("label"),
									TurbineUtils.getUser(data), false);
					if (expt != null) {
						logger.error("Duplicate experiment with label "
								+ found.getStringProperty("label"));
						data.setMessage("Please use a unique session ID.  "
								+ found.getStringProperty("label")
								+ " is already in use.");
				...
				wrk = PersistentWorkflowUtils
						.getOrCreateWorkflowData(
								...
								newEventInstance(data,
										EventUtils.CATEGORY.DATA, EventUtils
												.getAddModifyAction(
														found.getXSIType(),
														dbVersion == null)));
			...
			XnatSubjectassessordata sa = (XnatSubjectassessordata) BaseElement
					.GetGeneratedItem(found);
			...
			if (vr.isValid()) {
				try {
					if (!TurbineUtils.getUser(data).canEdit(sa)) {
						error(new InvalidPermissionException(
								"Unable to modify experient " + sa.getId()),
								data);
						return;
					}
					...
					try {
						dynamicPreSave(TurbineUtils.getUser(data), sa,
								TurbineUtils.GetDataParameterHash(data), wrk);
					} catch (CriticalException e) {
						throw e;
					} catch (RuntimeException e) {
						logger.error("", e);
					...
					try {
						SaveItemHelper.authorizedSave(found,
								TurbineUtils.getUser(data), false,
								allowDataDeletion(), c);
					} catch (Exception e1) {
						WorkflowUtils.fail(wrk, c);
					...
					try {
						MaterializedView.DeleteByUser(TurbineUtils
								.getUser(data));
					...
					dynamicPostSave(TurbineUtils.getUser(data), sa,
							TurbineUtils.GetDataParameterHash(data), wrk);

					SchemaElement se = SchemaElement.GetElement(found
							.getXSIType());
					if (TurbineUtils.HasPassedParameter("destination", data)) {
						super.redirectToReportScreen((String) TurbineUtils
								.GetPassedParameter("destination", data),
								found, data);
					} else {
						redirectToReportScreen(
								DisplayItemAction.GetReportScreen(se), found,
								data);
					}
				} catch (Exception e) {
				...
				TurbineUtils.OutputPassedParameters(data, context, this
						.getClass().getName());
	...
	public interface CompareAction {
		public void execute(XDATUser user, XFTItem from, XFTItem to)
				throws Exception;
	}

	private void dynamicCompare(XDATUser user, XFTItem from, XFTItem to)
			throws Exception {
		List<Class<?>> classes = Reflection
				.getClassesForPackage("org.nrg.xnat.actions.sessionEdit.compare");
	...
	public interface PreSaveAction {
		public void execute(XDATUser user, XnatSubjectassessordata src,
				Map<String, String> params, PersistentWorkflowI wrk)
				throws Exception;
	}

	private void dynamicPreSave(XDATUser user, XnatSubjectassessordata src,
			Map<String, String> params, PersistentWorkflowI wrk)
			throws Exception {
		List<Class<?>> classes = Reflection
				.getClassesForPackage("org.nrg.xnat.actions.sessionEdit.preSave");
	...
	private void dynamicPostSave(XDATUser user, XnatSubjectassessordata src,
			Map<String, String> params, PersistentWorkflowI wrk)
			throws Exception {
		List<Class<?>> classes = Reflection
				.getClassesForPackage("org.nrg.xnat.actions.sessionEdit.postSave");

		if (classes != null && classes.size() > 0) {
			for (Class<?> clazz : classes) {
				if (PostSaveAction.class.isAssignableFrom(clazz)) {
					PostSaveAction action = (PostSaveAction) clazz
							.newInstance();
					action.execute(user, src, params, wrk);
				}
			}
	 ...
	 * 
	 * @see
	 * org.nrg.xdat.turbine.modules.actions.ModifyItem#preProcess(org.nrg.xft
	 * .XFTItem, org.apache.turbine.util.RunData,
	 ...
	 * 
	 * @see
	 * org.nrg.xdat.turbine.modules.actions.ModifyItem#handleException(org.apache
	 * .turbine.util.RunData, org.nrg.xft.XFTItem, java.lang.Throwable)
	 */
	@Override
	public void handleException(RunData data, XFTItem first, Throwable error) {
		...
		try {
			if (first != null) {
				String part_id = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("part_id", data));
				if (part_id == null) {
				...
				if (part_id != null) {
					ItemI part = ItemSearch.GetItems("xnat:subjectData.ID",
							part_id, TurbineUtils.getUser(data), false)
							.getFirst();
			...
			if (first != null)
				data.setScreenTemplate("XDATScreen_edit_"
						+ first.getGenericSchemaElement().getFormattedName()
						+ ".vm");
			else
