...
public class PipelineActions extends SecureAction {
	static org.apache.log4j.Logger logger = Logger
			.getLogger(PipelineActions.class);
	...
	public void doLaunch(RunData data, Context context) throws Exception {
		String project = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("project", data));
		String step = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("pipelineStep", data));
		boolean isDescendant = ((Boolean) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedBoolean("isdescendant", data));
		ItemI data_item = TurbineUtils.GetItemBySearch(data);
		XnatPipelineLauncher xnatPipelineLauncher = getGenericCommonParameters(
				data, context, project, step, data_item);
		LinkedHashMap<ArcPipelineparameterdataI, ArrayList> paramHash = null;
		// String launcherPrefix =
		// ((String)org.nrg.xdat.turbine.utils.TurbineUtils.GetPassedParameter("launcherPrefix",data));
		...
		if (isDescendant) {
			String pipelineXml = PipelineManager.getPathToPipelineForProject(
					project, step, data_item.getXSIType());
			String pipelineName = PipelineManager.getPipelineNameForProject(
					project, step, data_item.getXSIType());
			xnatPipelineLauncher.setPipelineName(pipelineXml);
			cc.addClause("wrk:workflowData.pipeline_name", pipelineName);
			paramHash = PipelineManager
					.getResolvedParametersForDescendantPipeline(step, project,
							data_item);
		} else {
			String pipelineXml = PipelineManager.getPathToPipelineForProject(
					project, step);
			...
			xnatPipelineLauncher.setPipelineName(pipelineXml);
			cc.addClause("wrk:workflowData.pipeline_name", pipelineName);
			paramHash = PipelineManager.getResolvedParametersForPipeline(step,
					project, data_item);
		}
		...
		if (paramHash != null) {
			...
			while (paramIter.hasNext()) {
				ArcPipelineparameterdataI aParameter = (ArcPipelineparameterdataI) paramIter
						.next();
				String parameterTrueName = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter(paramStr + aParameter.getName()
								+ ":truename", data));
					...
					if (TurbineUtils.HasPassedParameter(dataParam, data)) {
						xnatPipelineLauncher
								.setParameter(
										parameterTrueName,
										((String) org.nrg.xdat.turbine.utils.TurbineUtils
												.GetPassedParameter(dataParam,
														data)));
					}
				}
			}
		}
		// Remove all preexisting workflow entries for the given item and the
		// pipeline
		ArrayList<WrkWorkflowdata> workflows = WrkWorkflowdata
				.getWrkWorkflowdatasByField(cc, TurbineUtils.getUser(data),
						false);
		if (workflows != null && workflows.size() > 0) {
			WrkWorkflowdata workFlow = workflows.get(0);
			if (workFlow.getStatus().equals(
					org.nrg.xdat.om.base.BaseWrkWorkflowdata.AWAITING_ACTION)) {
	...
	private XnatPipelineLauncher getGenericCommonParameters(RunData data,
			Context context, String projectId, String step, ItemI item)
			throws Exception {
		...
		xnatPipelineLauncher.setParameter("adminemail", ArcSpecManager
				.GetInstance().getSiteAdminEmail());
		xnatPipelineLauncher.setParameter("xnatserver", ArcSpecManager
				.GetInstance().getSiteId());
		xnatPipelineLauncher.setParameter("mailhost", ArcSpecManager
				.GetInstance().getSmtpHost());

		String emailsStr = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
	...
	public void doBuild(RunData data, Context context) throws Exception {
		String projectId = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("projectId", data));
		int totalSessionsToBuild = ((Integer) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedInteger("param:control:total", data));
		String step = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("step", data));
		data.getParameters().remove("step");
		try {
			int selectedCount = 0;
			for (int i = 1; i <= totalSessionsToBuild; i++) {
				String sessionParamCode = ":session" + i;
				if (TurbineUtils.HasPassedParameter("param" + sessionParamCode
						+ ":sessionId", data)) {
					selectedCount++;
				}
			}
			...
			for (int i = 1; i <= totalSessionsToBuild; i++) {
				XnatPipelineLauncher xnatPipelineLauncher = getCommonParameters(
						data, context, projectId, pipelineXml, step);
				if (xnatPipelineLauncher == null)
					throw new Exception(
							"Unable to construct the Xnat Pipeline Launcher");
				String sessionParamCode = ":session" + i;
				if (TurbineUtils.HasPassedParameter("param" + sessionParamCode
						+ ":sessionId", data)) {
					Hashtable<String, String> sessionParams = getParametersForKey(
							data, context, "param" + sessionParamCode + ":",
							sessionParamCode);
					String sessionId = sessionParams.get("param:sessionid");
					String xnat_sessionId = sessionParams
							.get("param:xnat_sessionid");
					xnatPipelineLauncher.setId(sessionId);
					if (sessionParams.keySet().size() > 0) {
						xnatPipelineLauncher.setParameter("xnat_sessionId",
								xnat_sessionId);
						setCommandLineArguments(data, sessionParams, projectId,
								step, xnatPipelineLauncher);
					}
					selectedCountLast++;
					if (selectedCountLast == selectedCount)
						xnatPipelineLauncher.setParameter("isLast", "1");
					xnatPipelineLauncher.setParameter("projectId", projectId);
					xnatPipelineLauncher.launch();
				}
			}
			String destinationPage = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("destinationpage", data));
			System.out
					.println("BuildPipelineActions::doBuild Destination page is "
							+ destinationPage);
			System.out
					.println(((String) org.nrg.xdat.turbine.utils.TurbineUtils
			...
			if (destinationPage != null) {
				data.setRedirectURI(TurbineUtils.GetRelativeServerPath(data)
						+ "/app/template/"
						+ destinationPage
						+ "/search_field/"
						+ ((String) org.nrg.xdat.turbine.utils.TurbineUtils
			...
			logger.error("", e);
			data.setMessage("<p><img src=\"/fcon/images/error.gif\">The build process failed to launch. Please contact the <a href=\"mailto:"
					+ AdminUtils.getAdminEmailId()
					+ "?subject=Failed to launch build \">NRG techdesk</a>");
			data.setScreenTemplate("Error.vm");
		}
	}

	private XnatPipelineLauncher getCommonParameters(RunData data,
			Context context, String projectId, String pipelineName, String step)
			throws Exception {
		XnatPipelineLauncher xnatPipelineLauncher = new XnatPipelineLauncher(
				data, context);
		xnatPipelineLauncher.setAdmin_email(AdminUtils.getAdminEmailId());
	...
	private Hashtable<String, String> setCommandLineArguments(RunData data,
			Hashtable<String, String> paramNameValue, String projectId,
			String step, XnatPipelineLauncher xnatPipelineLauncher) {
		Hashtable<String, String> trueNameValues = new Hashtable<String, String>();
		try {
			List parameters = PipelineManager.getParametersForPipeline(
					projectId, step);
			if (parameters != null) {
				LinkedHashMap<String, String> parametersHash = new LinkedHashMap<String, String>();
				// TrueName is required as the data parameter names are case
				// insensitive
				...
				Collections.sort(paramKeys, new Comparator<Object>() {
					public int compare(Object o1, Object o2) {
						...
						if (o1_parts != null && o1_parts.length > 1
								&& o2_parts != null && o2_parts.length > 1) {
							...
							if (o1_parts[1].equals(o2_parts[1])
									&& (o1_parts.length == 3 && o2_parts.length == 3)) {
				...
				for (int i = 0; i < paramKeys.size(); i++) {
					...
					if (parts != null && parts.length > 1
							&& parts[0].equals("param")) {
						String paramName = parts[1];
						if (!trueName.containsKey(paramName)) {
							ArcPipelineparameterdataI aParameter = PipelineManager
									.getParameterByName(projectId, step,
											paramName, true);
							if (aParameter != null)
						...
						if (parametersHash.containsKey(paramName)) {
							parametersHash.put(
									paramName,
									((String) parametersHash.get(paramName))
											+ ","
											+ paramNameValue
													.get((String) paramKeys
															.get(i)));
						} else {
							parametersHash.put(paramName, paramNameValue
									.get((String) paramKeys.get(i)));
						}
					}
				}
				...
				while (iter.hasNext()) {
					...
					if (paramValues.endsWith(","))
						paramValues = paramValues.substring(0,
								paramValues.length() - 1);
					// System.out.println("Adding parameter values for " +
					// trueParamName + " " + paramValues);
					xnatPipelineLauncher.setParameter(trueParamName,
							paramValues);
					trueNameValues.put(trueParamName, paramValues);
				}
			}
		} catch (Exception e) {
			logger.debug("Unable to construct the build parameters for step "
					+ step + " " + e.getMessage() + " " + e.getCause(), e);
			// AdminUtils.sendErrorNotification(data,"Unable to construct the Build statement for "
			// + " Step " + step);
		}
		return trueNameValues;
	}

	private Hashtable<String, String> getParametersForKey(RunData data,
			Context context, String pattern, String replace) {
		...
		while (keys.hasNext()) {
			...
			if (key.startsWith(pattern)) {
				if (replace != null)
					rtn.put(org.apache.commons.lang.StringUtils.replace(key,
							replace, ""),
							((String) org.nrg.xdat.turbine.utils.TurbineUtils
									.GetPassedParameter(key, data)));
				else
					rtn.put(key,
							((String) org.nrg.xdat.turbine.utils.TurbineUtils
									.GetPassedParameter(key, data)));
