...
public class ProcessAccessRequest extends SecureAction {
	...
	public void doDenial(RunData data, Context context) throws Exception {
		...
		XdatUser other = XdatUser.getXdatUsersByXdatUserId(id,
		...
		final PersistentWorkflowI wrk = PersistentWorkflowUtils
				.getOrCreateWorkflowData(
						null,
						user,
						AutoXnatProjectdata.SCHEMA_ELEMENT_NAME,
						project.getId(),
						project.getId(),
						newEventInstance(data,
								EventUtils.CATEGORY.PROJECT_ACCESS,
								EventUtils.REJECT_PROJECT_REQUEST));
		EventMetaI c = wrk.buildEvent();
		WorkflowUtils.save(wrk, c);

		if (other != null) {
			if (!user.canDelete(project)) {
				error(new InvalidPermissionException("Invalid permissions"),
						data);
				return;
			}
			...
			try {
				...
				for (Map.Entry<String, UserGroup> entry : otherU.getGroups()
						.entrySet()) {
				...
				ProjectAccessRequest par = ProjectAccessRequest
						.RequestPARByUserProject(otherU.getXdatUserId(),
								project.getId(), user);
			...
			Template template = Velocity
					.getTemplate("/screens/RequestProjectAccessDenialEmail.vm");
			template.merge(context, sw);
			String message = sw.toString();

			String from = AdminUtils.getAdminEmailId();
			String subject = TurbineUtils.GetSystemName()
					+ " Access Request for " + project.getName() + " Denied";

			try {
				XDAT.getMailService().sendHtmlMessage(from, otherU.getEmail(),
						user.getEmail(), AdminUtils.getAdminEmailId(), subject,
						message);
			} catch (Exception e) {
	...
	public void doApprove(RunData data, Context context) throws Exception {
		...
		String p = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("project", data));
		String access_level = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("access_level", data));
		if (StringUtils.isEmpty(access_level)) {
			access_level = "member";
		} else {
			if (!(access_level
					.equalsIgnoreCase(BaseXnatProjectdata.MEMBER_GROUP)
					|| access_level
							.equalsIgnoreCase(BaseXnatProjectdata.OWNER_GROUP) || access_level
						.equalsIgnoreCase(BaseXnatProjectdata.COLLABORATOR_GROUP))) {
				error(new Exception("Unknown Access level:" + access_level),
						data);
				return;
			}
		}
		...
		final PersistentWorkflowI wrk = PersistentWorkflowUtils
				.getOrCreateWorkflowData(
						...
						newEventInstance(data,
								EventUtils.CATEGORY.PROJECT_ACCESS,
								EventUtils.APPROVE_PROJECT_REQUEST));
		EventMetaI c = wrk.buildEvent();
		WorkflowUtils.save(wrk, c);
		...
		if (other != null) {
			...
			try {
				...
				project.addGroupMember(
						project.getId() + "_" + access_level.toLowerCase(),
						otherU, user, c, true);
			...
			String[] projectOwnerEmails = ownerEmails
					.toArray(new String[ownerEmails.size()]);
			SendAccessApprovalEmail(context, AdminUtils.getAdminEmailId(),
					new String[] { otherU.getEmail() }, projectOwnerEmails,
					new String[] { AdminUtils.getAdminEmailId() },
					TurbineUtils.GetSystemName() + " Access Request for "
							+ project.getName() + " Approved");
		}
		// data.setScreenTemplate("XDATScreen_manage_xnat_projectData.vm");
		// data.setScreenTemplate("/xnat_projectData/xnat_projectData_summary_management.vm");
		TurbineUtils.SetSearchProperties(data, project);
	...
	public static void SendAccessApprovalEmail(Context context,
			String otherEmail, XDATUser user, String subject) throws Exception {
		String admin = AdminUtils.getAdminEmailId();
		SendAccessApprovalEmail(context, admin, new String[] { otherEmail },
				new String[] { user.getEmail() }, new String[] { admin },
				subject);
	}

	public static void SendAccessApprovalEmail(Context context, String from,
			String[] to, String[] cc, String[] bcc, String subject)
			throws Exception {
		StringWriter sw = new StringWriter();
		Template template = Velocity
				.getTemplate("/screens/RequestProjectAccessApprovalEmail.vm");
		template.merge(context, sw);
		String message = sw.toString();

		try {
			XDAT.getMailService().sendHtmlMessage(from, to, cc, bcc, subject,
					message);
		} catch (Exception e) {
			logger.error("Unable to send mail", e);
			throw e;
