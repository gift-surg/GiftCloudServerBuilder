...
public class QDECAction extends ListingAction {
	...
	// is sent to the QDEC Analysis with the QDEC parameters selected by the
	// user
	public void doQdec(RunData data, Context context) throws Exception {
		...
		search.execute(
				new org.nrg.xdat.presentation.HTMLNoTagsAllFieldsPresenter(),
				TurbineUtils.getUser(data).getLogin());
		XFTTableI table = search.getPresentedTable();

		String analysis_name = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("analysis_name", data));
		...
		param.addNewValues().setUnique(
				userFolderPath + File.separator + analysis_name + ".qdec");

		param = analysisParameters.addNewParameter();
		param.setName("output-relative-path");
		param.addNewValues().setUnique(
				id + File.separator + analysis_name + ".qdec");

		// param = analysisParameters.addNewParameter();
		// param.setName("subjects-dir");
		// param.addNewValues().setUnique(userFolderPath
		// +File.separator+"SUBJECTS_DIR");

		// Launch the job
		String pipelineName = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("hdn_pipelinename", data));
		...
		try {
			String paramFilePath = saveParameters(userFolderPath,
					analysisParameters);
			...
			data.setMessage("<p><b>The QDEC Analysis process could not be launched.  Please contact <A HREF=\"mailto:"
					+ AdminUtils.getAdminEmailId()
					+ "?subject=Error: Performing QDEC Group Analysis"
					+ "\">Report Error to"
	...
	private Parameters createParameters(RunData data, Context context,
			XFTTableI table) {
		...
		String morph_measure = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("morph_measure", data));
		ParameterData param = parameters.addNewParameter();
		param.setName("measurement");
		param.addNewValues().setUnique(morph_measure);

		String morph_hemisphere = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("morph_hemisphere", data));
		param = parameters.addNewParameter();
		param.setName("hemisphere");
		param.addNewValues().setUnique(morph_hemisphere);

		String morph_fwhm = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("morph_fwhm", data));
		param = parameters.addNewParameter();
	...
	private String saveParameters(String rootpath, Parameters parameters)
			throws Exception {
	...
	private String createQdecTableDatFile(RunData data, XFTTableI table,
			String folderName) throws Exception {
		String userFolderPath = getAnalysisFolder(data, folderName);
		File userFolder = new File(userFolderPath);
		if (!userFolder.exists())
			userFolder.mkdirs();
		File qdecTableFile = new File(userFolder.getAbsolutePath()
				+ File.separator + "qdec.table.dat");
	...
	private ArrayList<String> getContinousVariables(RunData data) {
		ArrayList<String> rtn = new ArrayList<String>();
		String continuousVar1 = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("continuousVar1", data));
		String continuousVar2 = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("continuousVar2", data));
	...
	private ArrayList<String> getDiscreteVariables(RunData data) {
		ArrayList<String> rtn = new ArrayList<String>();
		String discreteVar1 = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("discreteVar1", data));
		String discreteVar2 = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("discreteVar2", data));
	...
	private String getSessionIdColumnName(RunData data) {
		String hdn_id = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("hdn_id_col", data));
		return hdn_id;
	}
	...
	private ArrayList<String> getQdecFileColumns(RunData data) {
		...
		if (continous.size() > 0)
			rtn.addAll(continous);
		return rtn;
	}

	private String createLevelsFile(RunData data, Context context,
			String discreteVariable, XFTTableI table, String folderName)
			throws Exception {
		String rtn = null;
		if (discreteVariable != null
				&& !discreteVariable.equalsIgnoreCase("BAD")) {
			// Create the .levels file
			String userFolderPath = getAnalysisFolder(data, folderName);
			Hashtable distinctValues = getDistinctValues(discreteVariable,
					table);
			if (distinctValues != null && distinctValues.size() > 0) {
				File distinctValuesFile = new File(userFolderPath
						+ File.separator + discreteVariable + ".levels");
				PrintWriter output = null;
				try {
					output = new PrintWriter(new BufferedWriter(new FileWriter(
							distinctValuesFile)));
	...
	private String getAnalysisFolder(RunData data, String subFolder) {
		return org.nrg.xnat.turbine.utils.ArcSpecManager.GetInstance()
				.getGlobalCachePath()
				+ TurbineUtils.getUser(data).getLogin()
	...
	private Hashtable getDistinctValues(String columnHeader, XFTTableI table) {
		...
		while (table.hasMoreRows()) {
			...
			if (rowColumnValue != null) {
				String colValue = StringUtils.deleteWhitespace(rowColumnValue
						.toString());
