...
public class QuickSearchAction extends SecureAction {
		...
		if (s == null || s.equalsIgnoreCase("")) {
			...
			if (PoolDBUtils.HackCheck(s)) {
				AdminUtils.sendAdminEmail(user,
						"Possible SQL Injection Attempt", "VALUE:" + s);
				this.error(new Exception("Illegal Search Value(" + s + ")"),
						data);
				return;
			}
			...
			try {
				String projectQ = "SELECT DISTINCT id FROM xnat_projectData prj LEFT JOIN xnat_projectdata_alias ali ON prj.id=ali.aliases_alias_xnat_projectdata_id WHERE lower(id)='"
						+ s
						+ "' OR  lower(secondary_id)='"
						+ s
						+ "' OR  lower(name)='"
						+ s
						+ "' OR  lower(alias)='"
						+ s + "' OR lower(keywords) LIKE '%" + s + "%'";
				XFTTable table = TableSearch.Execute(projectQ, TurbineUtils
						.getUser(data).getDBName(), TurbineUtils.getUser(data)
						.getLogin());
				secureTable(user, table, new String[] { "id" },
						"xnat:projectData/ID");
				if (table.size() > 0) {
					if (table.size() == 1) {
						Hashtable row = table.rowHashs().get(0);
						String projectId = (String) row.get("id");
						data.getParameters().setString("search_value",
								projectId);
						data.getParameters().setString("search_element",
								"xnat:projectData");
						data.getParameters().setString("search_field",
								"xnat:projectData.ID");
						// Here we're just bouncing the user straight to the
						// subjectData page for this one result
						String rdirurl = String
								.format("%s/app/action/DisplayItemAction/search_value/%s/search_element/xnat:projectData/search_field/xnat:projectData.ID",
										data.getContextPath(), projectId);
						...
						cc.addClause("xnat:projectData/aliases/alias/alias",
								"=", s);
				...
				table = TableSearch.Execute(query, TurbineUtils.getUser(data)
						.getDBName(), TurbineUtils.getUser(data).getLogin());
				secureTable(user, table,
						new String[] { "project", "projects" },
				...
				if (table.size() > 0) {
					if (table.size() == 1) {
						...
						// subjectData page for this one result
						String rdirurl = String
								.format("%s/app/action/DisplayItemAction/search_value/%s/search_element/%s/search_field/%s.ID",
										data.getContextPath(), elementId,
										elementName, elementName);
						data.getResponse().sendRedirect(rdirurl);
						return;
					}
				} else {
					query = "SELECT DISTINCT s.ID, 'xnat:subjectData' AS element_name, ids,s.project AS project,projects,s.label FROM xnat_subjectData s LEFT JOIN xnat_subjectdata_addid addid ON s.id=addid.xnat_subjectdata_id LEFT JOIN xnat_projectparticipant pp ON s.id=pp.subject_id LEFT JOIN (SELECT project_group.subject_id AS id, btrim(xs_a_concat(project_group.ids || ', '::text), ', '::text) AS ids, btrim(xs_a_concat(('<'::text || project_group.projects) || '>, '::text), ', '::text) AS projects  FROM ( SELECT ((COALESCE(xnat_projectparticipant.label, xnat_projectparticipant.subject_id)::text || ' ('::text) || btrim(xs_a_concat(xnat_projectparticipant.project::text || ', '::text), ', '::text)) || ')'::text AS ids, btrim(xs_a_concat(('<'::text || xnat_projectparticipant.project::text) || '>, '::text), ', '::text) AS projects, xnat_projectparticipant.subject_id FROM xnat_projectparticipant GROUP BY COALESCE(xnat_projectparticipant.label, xnat_projectparticipant.subject_id), xnat_projectparticipant.subject_id) project_group GROUP BY project_group.subject_id) xnat_projs ON s.id=xnat_projs.id WHERE LOWER(s.ID) LIKE '%"
							+ s
							+ "%' OR  LOWER(addid) LIKE '%"
							+ s
							+ "%' OR LOWER(s.label) LIKE '%"
							+ s
							+ "%' OR LOWER(pp.label) LIKE '%"
							+ s
							+ "%' UNION SELECT DISTINCT expt.ID,element_name,ids,expt.project,projects,expt.label FROM xnat_experimentData expt LEFT JOIN xnat_experimentData_share proj ON expt.id=proj.sharing_share_xnat_experimentda_id LEFT JOIN xdat_meta_element me ON expt.extension=me.xdat_meta_element_id LEFT JOIN ( SELECT project_group.sharing_share_xnat_experimentda_id AS id, btrim(xs_a_concat(project_group.ids || ', '::text), ', '::text) AS ids, btrim(xs_a_concat(project_group.projects || ', '::text), ', '::text) AS projects   FROM ( SELECT ((COALESCE(proj.label, sharing_share_xnat_experimentda_id)::text || ' ('::text) || btrim(xs_a_concat(proj.project::text || ', '::text), ', '::text)) || ')'::text AS ids, btrim(xs_a_concat(('<'::text || proj.project::text) || '>, '::text), ','::text) AS projects, sharing_share_xnat_experimentda_id           FROM xnat_experimentdata_share proj      LEFT JOIN xnat_experimentdata expt ON proj.sharing_share_xnat_experimentda_id::text = expt.id::text  GROUP BY COALESCE(proj.label, sharing_share_xnat_experimentda_id), sharing_share_xnat_experimentda_id) project_group  GROUP BY project_group.sharing_share_xnat_experimentda_id) xnat_projects ON expt.id=xnat_projects.id WHERE LOWER(expt.ID) LIKE '%"
							+ s
							+ "%' OR LOWER(expt.label) LIKE '%"
							+ s
							+ "%' OR LOWER(proj.label) LIKE '%"
							+ s
							+ "%' ORDER BY element_name;";

					table = TableSearch.Execute(query,
							TurbineUtils.getUser(data).getDBName(),
							TurbineUtils.getUser(data).getLogin());
					secureTable(user, table, new String[] { "project",
							"projects" }, "xnat:subjectData/project");
				}
				...
				if (table.size() > 0) {
					if (table.size() == 1) {
						...
						while (table.hasMoreRows()) {
							...
							if (project != null) {
								i.setProperty(element_name + "/project",
										project);
							}
							if (label != null) {
							...
							if (ids != null)
								while (ids.indexOf(")") != -1) {
									String idSet = ids.substring(0,
											ids.indexOf(")"));
									if (ids.length() > (ids.indexOf(")") + 2)) {
										ids = ids
												.substring(ids.indexOf(")") + 2);
									} else {
										ids = "";
									}

									String identifier = idSet.substring(0,
											idSet.indexOf(" ("));
									idSet = idSet
											.substring(idSet.indexOf(" (") + 2);
									ArrayList<String> ps = StringUtils
											.CommaDelimitedStringToArrayList(
													idSet, true);

									for (String p : ps) {
										if (user.canRead(element_name
												+ "/sharing/share/project", p)) {
											i.setProperty(element_name
													+ "/sharing/share["
													+ projectCounter
													+ "]/project", p);
											localProjects.add(p);
							...
							// i.setProperty(element_name + "/sharing/share[" +
							// j +"]/identifier", "*****");
							// }
							// }
							...
							if (user.canRead(i)) {
								if (!hash.containsKey(element_name)) {
									elementNames
											.put(element_name,
													ElementSecurity
															.GetPluralDescription(element_name));
									hash.put(
											element_name,
											new Hashtable<String, ArrayList<ItemI>>());
								}

								if (!typeProjectMapping
										.containsKey(element_name)) {
									typeProjectMapping.put(element_name,
											new ArrayList<String>());
								}

								for (String p : localProjects) {
									if (!typeProjectMapping.get(element_name)
											.contains(p)) {
										typeProjectMapping.get(element_name)
												.add(p);
									}
									if (hash.get(element_name).get(p) == null) {
										hash.get(element_name).put(p,
												new ArrayList<ItemI>());
										totalCount++;
									}
									hash.get(element_name)
											.get(p)
											.add(BaseElement
													.GetGeneratedItem(i));
								}
								...
								if (hash.get(element_name).get(project) == null) {
									hash.get(element_name).put(project,
											new ArrayList<ItemI>());
									totalCount++;
								}
								hash.get(element_name).get(project)
										.add(BaseElement.GetGeneratedItem(i));

							}
						}

						Hashtable pMappings = user.getCachedItemValuesHash(
								"xnat:projectData", null, false,
								"xnat:projectData/ID",
	 ...
	 * @param xsiPath
	 */
	private void secureTable(XDATUser user, XFTTable t, String[] projColumns,
			String xsiPath) {
