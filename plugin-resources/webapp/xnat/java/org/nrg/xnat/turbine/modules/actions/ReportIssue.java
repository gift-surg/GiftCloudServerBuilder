...
public class ReportIssue extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		String subject = TurbineUtils.GetSystemName() + " Issue Report from "
				+ user.getLogin();
		// TODO: Need to figure out how to handle attachments in notifications.
		Map<String, File> attachments = getAttachmentMap(data.getSession()
				.getId(), parameters);

		// XDAT.getMailService().sendHtmlMessage(adminEmail, new String[] {
		// adminEmail }, null, null, subject, body, null, attachments);
		...
		XDAT.getNotificationService().createNotification(
				NotificationType.Issue.toString(), properties);
	}

	private Map<String, File> getAttachmentMap(String sessionId,
		...
		if (fi != null) {
			final String cachePath = location(ArcSpecManager.GetInstance()
					.getGlobalCachePath(), "issuereports", sessionId);
			...
			try {
				...
				logger.warn("Could not attach file, " + file.getAbsolutePath(),
						exception);
			}
		}
	...
	private String emailBody(XDATUser user, ParameterParser parameters,
			RunData data, Context context, boolean html) throws Exception {
		if (html) {
			context.put("html", "html");
			context.put("htmlDescription", parameters.get("description")
					.replaceAll("\n", "<br/>"));
		} else {
		...
		context.put("postgres_version", PoolDBUtils.ReturnStatisticQuery(
				"SELECT version();", "version", user.getDBName(),
				user.getLogin()));

		if (html) {
			context.put("html", "html");
			return AdminUtils.populateVmTemplate(context,
					"/screens/email/html_issue_report.vm");
		} else {
			return AdminUtils.populateVmTemplate(context,
					"/screens/email/issue_report.vm");
		}
	}
