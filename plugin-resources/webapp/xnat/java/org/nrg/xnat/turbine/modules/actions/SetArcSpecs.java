...
public class SetArcSpecs extends AdminAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		ArcSpecManager.save(
				...
				newEventInstance(data, EventUtils.CATEGORY.SIDE_ADMIN,
						"Modified archive specifications."));

		Channel channel = XDAT.getHtmlMailChannel();

		List<ArcArchivespecificationNotificationTypeI> types = arcSpec
				.getNotificationTypes_notificationType();
		for (ArcArchivespecificationNotificationTypeI type : types) {
			List<Subscriber> subscribers = getSubscribersFromAddresses(type
					.getEmailAddresses());
			Definition definition = getDefinitionForEvent(type
					.getNotificationType());
	 ...
	 *
	 * @param addresses
	 *            The comma-separated list of usernames, email addresses, and
	 *            combined IDs.
	...
	private List<Subscriber> getSubscribersFromAddresses(String addresses) {
		...
		for (String address : addresses.split("[\\s]*,[\\s]*")) {
			...
			if (userMatcher.matches()) {
				...
				username = address;
				XdatUser user = XDATUser.getXdatUsersByLogin(username, null,
						true);
				if (user != null) {
					email = user.getEmail();
				} else {
					// TODO: Need to add users that aren't located to a list of
					// error messages.
					continue;
				}
			} else {
				...
				if (emailMatcher.matches()) {
					...
					List<XdatUser> users = XDATUser.getXdatUsersByField(
							"xdat:user/email", email = address, null, true);
					if (users == null || users.size() == 0) {
						// If we didn't find the user, do something.
						// TODO: Need to add users that aren't located to a list
						// of error messages.
						continue;
					} else {
					...
					if (combinedMatcher.matches()) {
						...
						// capture, email second capture (0 capture in regex is
						// full expression).
			...
			if (subscriber == null) {
				try {
					subscriber = getNotificationService()
							.getSubscriberService().createSubscriber(username,
									email);
