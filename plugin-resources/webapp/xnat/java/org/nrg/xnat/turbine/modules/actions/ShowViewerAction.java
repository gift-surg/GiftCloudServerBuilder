...
public class ShowViewerAction extends SecureAction {
	static org.apache.log4j.Logger logger = Logger
			.getLogger(ShowViewerAction.class);
	...
	public void doPerform(RunData data, Context context) throws Exception {
		if (data.getParameters().containsKey("searchValue")) {
			...
			if (s == null || s.equalsIgnoreCase("")) {
				...
				try {
					org.nrg.xft.XFTTable table = TableSearch.Execute(
							"SELECT ID from xnat_subjectdata WHERE LOWER(ID) LIKE '%"
									+ s + "%';", TurbineUtils.getUser(data)
									.getDBName(), TurbineUtils.getUser(data)
									.getLogin());
					...
					if (table.size() > 0) {
						if (table.size() == 1) {
							...
							data.getParameters().setString("ELEMENT_0",
									"xnat:subjectData");
							data.getParameters()
									.setString(
											"xnat:subjectData.COMBO0_FIELDS",
											"xnat:subjectData.SUBJECTID_equals,xnat:subjectData/label_equals,xnat:subjectData/sharing/share/label_equals");
							data.getParameters().setString(
									"xnat:subjectData.COMBO0", s);
							dsa.doPerform(data, context);
							return;
						}
					} else {
						table = TableSearch.Execute(
								"SELECT ID FROM xnat_mrSessionData WHERE LOWER(ID) LIKE '%"
										+ s + "%';", TurbineUtils.getUser(data)
										.getDBName(), TurbineUtils
						...
						if (table.size() > 0) {
							if (table.size() == 1) {
								...
								data.getParameters().setString("ELEMENT_0",
										"xnat:mrSessionData");
								data.getParameters().setString(
										"xnat:mrSessionData.SESSION_ID_equals",
										s);
								dsa.doPerform(data, context);
								return;
							}
		...
		if (data.getParameters().containsKey("skipq")) {
			...
			context.put("id", ((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("id", data)));
			data.getParameters().remove("skipq");
		} else {
			String sessionId = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("session", data));
			context.put("sessionId", sessionId);
			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("startDisplayWith", data)) != null
					&& !((String) org.nrg.xdat.turbine.utils.TurbineUtils
							.GetPassedParameter("startDisplayWith", data))
							.equalsIgnoreCase(""))
				context.put("startDisplayWith",
						((String) org.nrg.xdat.turbine.utils.TurbineUtils
								.GetPassedParameter("startDisplayWith", data)));
		}
