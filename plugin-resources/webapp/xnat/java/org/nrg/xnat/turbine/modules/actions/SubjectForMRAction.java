...
public class SubjectForMRAction extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		...
		String destination = (String) TurbineUtils.GetPassedParameter(
				"destination", data, "XDATScreen_edit_xnat_mrSessionData.vm");
		...
		ItemCollection items = ItemSearch.GetItems(cc,
		...
		if (items.size() > 0) {
			...
			if (!confirmed) {
				...
				data.getParameters().add("part_id",
			...
			if (s != null) {
				...
				vr.addResult(null, "Invalid Subject Id", "part_id",
						(String) null);
				context.put("vr", vr);
