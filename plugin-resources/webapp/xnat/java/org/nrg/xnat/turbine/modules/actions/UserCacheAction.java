...
public class UserCacheAction extends SecureAction {
	...
	public void doPerform(RunData data, Context context) throws Exception {
		String folder = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("folder", data));
		Boolean delete = new Boolean(
				((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("delete", data)));
		if (folder != null) {
	...
	public void doDownload(RunData data, Context context) throws Exception {
		...
		if (folder != null) {
			...
			String globalCachePath = ArcSpecManager.GetInstance()
					.getGlobalCachePath() + user.getLogin();
			...
			if (folderFile.exists()) {
				...
				Object xarDownload = TurbineUtils.GetPassedParameter(
						"download_type", data);
				...
				try {
					 ...
					 * else { URL fileDir = null; URLConnection urlConn = null;
					 * BufferedInputStream buf = null; try { fileDir =
					 * folderFile.toURL(); urlConn = fileDir.openConnection();
					 * response.setContentLength((int)
					 * urlConn.getContentLength()); buf = new
					 * BufferedInputStream(urlConn.getInputStream()); int
					 * readBytes = 0; //read from the file; write to the
					 * ServletOutputStream while ((readBytes = buf.read()) !=
					 * -1) outStream.write(readBytes); }finally{ if (outStream
					 * != null) outStream.close(); if (buf != null) buf.close();
					 * } }
					 */
				} catch (Exception e) {
					logger.error("", e);
