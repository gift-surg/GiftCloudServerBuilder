...
public class ArcPut extends RawScreen {
	...
	protected void doOutput(RunData data) throws Exception {
		String session = (String) TurbineUtils.GetPassedParameter("session",
				data);
		String mr_session_id = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("mr_session_id", data));
		...
		try {
			...
			XnatMrsessiondata tempMR = XnatMrsessiondata
					.getXnatMrsessiondatasById(mr_session_id, user, false);
			if (tempMR == null) {
				ArrayList<XnatMrsessiondata> al = XnatMrsessiondata
						.getXnatMrsessiondatasByField(
								"xnat:mrSessionData/label", mr_session_id,
								user, false);
				if (al.size() > 0) {
					tempMR = al.get(0);
				}
			...
			if (tempMR == null) {
				...
				pw.println("<message>Unknown MRSession ID: " + mr_session_id
						+ "</message>");
				pw.println("</UploadResponse>");
				return;
			}
			...
			if (fi != null) {
				...
				if (normalized.endsWith(".tar")) {
					...
					compressionMethod = filename.substring(filename
							.lastIndexOf("."));
				}
				...
				if (counter > 0) {
					PersistentWorkflowI workflow = WorkflowUtils
							.getOrCreateWorkflowData(null, user, tempMR
									.getXSIType(), tempMR.getId(), tempMR
									.getProject(), SecureAction
					...
					try {
						SaveItemHelper.authorizedSave(tempMR, user, false,
								false, ci);
