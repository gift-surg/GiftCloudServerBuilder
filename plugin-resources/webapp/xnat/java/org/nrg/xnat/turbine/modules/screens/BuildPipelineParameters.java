...
public class BuildPipelineParameters extends SecureReport {
	...
	public void finalProcessing(RunData data, Context context) {
		...
		String pipelineName = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("pipelineName", data));
		...
		try {
			/*
			 * LinkedHashMap parametersHash =
			 * BuildSpecification.GetInstance().getResolvedParametersForPipeline
			 * (pipelineName,mr); context.put("parametersHash",parametersHash);
			...
			String errorString = "<img src=\"/cnda1/images/error.gif\"> Error in the Build Spec file document for the pipeline "
					+ context.get("pipelineName");
			errorString += "<p>Please contact the <a href=\"mailto:"
					+ XFT.GetAdminEmail()
					+ "?subject=Error in Build Spec file for "
					+ mr.getSessionType() + " pipeline " + pipelineName
					+ "\">CNL techdesk</a> to resolve the error.</p>";
