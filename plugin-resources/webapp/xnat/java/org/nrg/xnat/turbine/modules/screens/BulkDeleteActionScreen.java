...
public class BulkDeleteActionScreen extends SecureScreen {
	...
	// Enumeration to determine what type of item is stored in an ItemContainer
	// object.
	private enum ITEM_TYPE {
		SUBJECT, SUBJ_ASSESSOR, IMG_ASSESSOR
	};
	...
	protected void doBuildTemplate(RunData data, Context context)
		...
		XFTTable table = (org.nrg.xft.XFTTable) search.execute(null,
				TurbineUtils.getUser(data).getLogin());

		XFTTable experiments = null;
		XDATUser user = TurbineUtils.getUser(data);

		// Build the query to retrieve information about the items we want to
		// delete.
		...
		// Build the WHERE Clause based on what type of query we are performing.
		// (Subject, Subject Assessor, or Image Assessor)
		// Also, set the searchType, we need this in the velocity to determine
		// if a user is allowed to delete an item. (see
		// ItemContainer.canDelete() below)
		if (search.getRootElement().getFullXMLName().equals("xnat:subjectData")) {
			...
			query.append("WHERE subj.id IN ('")
					.append(buildWhereClause(table
							.convertColumnToArrayList("subjectid")))
					.append("');");
			// XNAT-2829: If the search was for imaging sessions
		} else if (search.getRootElement().getGenericXFTElement()
				.instanceOf("xnat:imageSessionData")) {
			context.put("searchType", "subject_assessor"); // Set the searchType
			query.append("WHERE session_expt.id IN ('")
					.append(buildWhereClause(table
							.convertColumnToArrayList("session_id")))
					.append("');");
			// XNAT-2829: If the search was for subject assessors (non imaging)
		} else if (search.getRootElement().getGenericXFTElement()
				.instanceOf("xnat:subjectAssessorData")) {
			context.put("searchType", "subject_assessor"); // Set the searchType
			query.append("WHERE session_expt.id IN ('")
					.append(buildWhereClause(table
							.convertColumnToArrayList("expt_id")))
					.append("');");
		} else if (search.getRootElement().getGenericXFTElement()
				.instanceOf("xnat:imageAssessorData")) {
		...
		experiments = XFTTable.Execute(query.toString(), user.getDBName(),
				user.getUsername());
	...
	/**
	 * Function takes a XFTtable and converts it into a Hashtable of
	 * ItemContainers.
	 * 
	 * @param t
	 *            - XFTTable
	 ...
	 * @throws Exception
	 *             - if the Hashtable is null or empty after function executes.
	 */
	private Hashtable<String, ItemContainer> getItems(XFTTable t)
			throws Exception {
		...
		for (Hashtable exp : t.rowHashs()) {
			...
			if (null != subj_id) {
				...
				if (null == subj) {
					...
					subj = new ItemContainer(ITEM_TYPE.SUBJECT, subj_id,
							(String) exp.get("subj_project"),
							(String) exp.get("subj_label"));
				}
				...
				if (null != session_id) {
					...
					Hashtable<String, ItemContainer> subjAssessors = subj
							.getAssessors();
					ItemContainer experiment = subjAssessors.get(session_id);

					if (null == experiment) {
						// If the experiment doesn't exist create it.
						experiment = new ItemContainer(ITEM_TYPE.SUBJ_ASSESSOR,
								session_id, (String) exp.get("session_label"),
								(String) exp.get("session_project"),
								(Date) exp.get("session_date"),
								(String) exp.get("session_xsi"));
					}

					// Get a list of image assessors for the experiment
					Hashtable<String, ItemContainer> imgAssessors = experiment
							.getAssessors();
					String assess_id = (String) exp.get("assess_id");

					if (assess_id != null && !imgAssessors.contains(assess_id)) {
						// If the assessor doesn't exist, create it, add it to
						// the list of assessors and attach it to the
						// experiment.
						imgAssessors.put(
								assess_id,
								new ItemContainer(ITEM_TYPE.IMG_ASSESSOR,
										assess_id, (String) exp
												.get("assess_label"),
										(String) exp.get("assess_project"),
										(Date) exp.get("assess_date"),
										(String) exp.get("assess_xsi")));
						experiment.addAssessors(imgAssessors);
					}

					// Update the list of subject Assessors and attach it to the
					// subject
	...
	/**
	 * Function builds a comma delimited string of id's to be inserted in the
	 ...
	 * It has helper methods to make it easier to access the information from
	 * the velocity.
	 */
	public class ItemContainer {
		 ...
		 * @param xsiType
		 */
		public ItemContainer(ITEM_TYPE itemType, String id, String label,
				String project, Date date, String xsiType) {
			this.id = id;
			this.label = label;
		 ...
		 * @param label
		 */
		public ItemContainer(ITEM_TYPE itemType, String id, String project,
				String label) {
			this.id = id;
			this.project = project;
			this.label = label;
		...
		public Hashtable<String, ItemContainer> getAssessors() {
			return this.assessors;
		}

		public void addAssessors(Hashtable<String, ItemContainer> assessors) {
		...
		/**
		 * Function will determine if the given user is allowed to delete this
		 * item. Also, some items should not be delete based on the search type.
		 * (e.g.) If user searches for Image Assessors, they should not be
		 * allowed to delete the entire subject.
		 * 
		 * @param u
		 *            - the XDATUser
		 * @param searchType
		 *            - subject, subject_assessor, image_assessor (this is set
		 *            on lines 49, 54, and 59 above)
		 * @return
		 */
		public String canDelete(XDATUser u, String searchType) {
			try {
				// Is the user allowed to delete this item
				boolean canDelete = u.canAction(this.xsi + "/project",
						this.project, "delete");

				// The search type determines which items a user is allowed to
				// delete.
				if (searchType.equals("subject")) {
					// If the user searched for subjects, they should be allowed
					// to delete subjects, subject assessors, and image
					// assessors.
					return String.valueOf(canDelete);
				} else if (searchType.equals("subject_assessor")) {
					// If the user searched for subject assessors, we should
					// allow them to delete subject assessors and image
					// assessors.
					return String
							.valueOf((this.itemType == ITEM_TYPE.SUBJ_ASSESSOR || this.itemType == ITEM_TYPE.IMG_ASSESSOR)
									&& canDelete);
				} else if (searchType.equals("image_assessor")) {
					// If the user searched for image assessors, we should allow
					// them to delete image assessors only.
					return String
							.valueOf((this.itemType == ITEM_TYPE.IMG_ASSESSOR)
									&& canDelete);
				} else {
					// If the search type is anything else, something went wrong
					// so we return false.
					return "false";
				}
			} catch (Exception e) {
