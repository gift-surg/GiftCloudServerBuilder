...
public abstract class DefaultPipelineScreen extends SecureReport {
	...
	protected void setHasDicomFiles(XnatImagesessiondata mr,
			String mprageScanType, Context context) {
		if (mr instanceof XnatMrsessiondata || mr instanceof XnatPetsessiondata) {
			boolean rtn = false;
			String[] types = mprageScanType.split(",");
			for (int j = 0; j < types.length; j++) {
				ArrayList<XnatImagescandata> scans = mr.getScansByType(types[j]
						.trim());
				if (scans != null && scans.size() > 0) {
					List files = scans.get(0).getFile();
					if (files.size() > 0) {
						for (int i = 0; i < files.size(); i++) {
							XnatAbstractresource absFile = (XnatAbstractresource) files
									.get(i);
	...
	protected void setHasFreesurfer(XnatMrsessiondata mr, Context context) {
		...
		ArcProject arcProject = ArcSpecManager.GetFreshInstance()
				.getProjectArc(project);
		List<ArcProjectDescendantPipelineI> descPipelines = arcProject
				.getPipelinesForDescendant(mr.SCHEMA_ELEMENT_NAME);
		for (int i = 0; i < descPipelines.size(); i++) {
			ArcProjectDescendantPipeline aPipeline = (ArcProjectDescendantPipeline) descPipelines
					.get(i);
			if (aPipeline.getLocation().endsWith(
					File.separator + "StdFreeSurferBuild.xml"))
				;
			{
				hasFreesurfer = 1;
				break;
	...
	protected void setWorkflows(RunData data, Context context) {
		...
		try {
			...
			org.nrg.xft.collections.ItemCollection items = org.nrg.xft.search.ItemSearch
					.GetItems(cc, TurbineUtils.getUser(data), false);
			// Sort by Launch Time
	...
	public void doBuildTemplate(RunData data, Context context) {
		...
		if (item == null) {
			...
			try {
				if (XFT.VERBOSE)
					System.out.println("Creating report: " + getClass());
				;
				context.put("item", item.getItem());
				if (XFT.VERBOSE)
					System.out
							.println("Loaded item object (org.nrg.xft.ItemI) as context parameter 'item'.");
				context.put("user", TurbineUtils.getUser(data));
				if (XFT.VERBOSE)
					System.out
							.println("Loaded user object (org.nrg.xdat.security.XDATUser) as context parameter 'user'.");
				context.put("element", org.nrg.xdat.schema.SchemaElement
						.GetElement(item.getXSIType()));
				...
				project = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
	...
	protected boolean isAnyQueuedOrRunning(ArrayList<WrkWorkflowdata> workflows) {
		...
		try {
			for (int i = 0; i < workflows.size(); i++) {
				...
				if (wrkFlow.getStatus().toUpperCase()
						.equals(BaseWrkWorkflowdata.QUEUED)
						|| wrkFlow.getStatus().toUpperCase()
								.equals(BaseWrkWorkflowdata.RUNNING)) {
					rtn = true;
					break;
				}
			}
	...
	protected boolean hasBeenCompletedInThePast(String pipelinePath,
			ArrayList<WrkWorkflowdata> workflows) {
		boolean rtn = false;
		for (int i = 0; i < workflows.size(); i++) {
			WrkWorkflowdata wrkFlow = workflows.get(i);
			String matchPipelineName = wrkFlow.getPipelineName();
			if (matchPipelineName.equals(pipelinePath)
					|| pipelinePath.contains(matchPipelineName)) {
				if (wrkFlow.getStatus().equalsIgnoreCase(
						BaseWrkWorkflowdata.COMPLETE)) {
					rtn = true;
	...
	protected ArcPipelineparameterdata getProjectPipelineSetting(
			String parameterName) throws Exception {
		return projectParameters.get(parameterName);
	}

	protected ArcPipelineparameterdata getParameter(ArcProject arcProject,
			String parameterName) throws PipelineNotFoundException {
		ArcPipelineparameterdata rtn = null;
		ArcPipelinedataI pipelineData = arcProject
				.getPipelineByPath(pipelinePath);
		List<ArcPipelineparameterdataI> params = pipelineData
				.getParameters_parameter();
		for (int i = 0; i < params.size(); i++) {
			ArcPipelineparameterdata aParam = (ArcPipelineparameterdata) params
					.get(i);
	...
	protected void setParameters(String pipeline)
		...
		if (arcProject == null) { // Project pipeline hasnt been set
			PipePipelinedetails pipelineDetails = PipelineRepositoryManager
					.GetInstance().getPipeline(pipeline);
			List<PipePipelinedetailsParameterI> params = pipelineDetails
					.getParameters_parameter();
			for (int i = 0; i < params.size(); i++) {
				ArcPipelineparameterdata aParam = (ArcPipelineparameterdata) PipelineRepositoryManager
						.GetInstance().convertToArcPipelineParameter(
								(PipePipelinedetailsParameter) params.get(i));
			...
			if (om.getXSIType().equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) {
				...
				pipelineData = arcProject.getPipelineForDescendantByPath(
						om.getXSIType(), pipeline);
			}
			List<ArcPipelineparameterdataI> params = pipelineData
					.getParameters_parameter();
			for (int i = 0; i < params.size(); i++) {
	...
	protected void setParameters(ArcPipelinedataI arcPipeline, Context context)
			throws Exception {
		List<ArcPipelineparameterdataI> pipelineParameters = arcPipeline
				.getParameters_parameter();
