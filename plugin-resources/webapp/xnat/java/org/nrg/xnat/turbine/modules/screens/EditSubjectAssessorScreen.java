...
public abstract class EditSubjectAssessorScreen extends EditScreenA {
	private final Logger logger = LoggerFactory
			.getLogger(EditSubjectAssessorScreen.class);
	...
	public void finalProcessing(final RunData data, final Context context) {
		try {
			...
			context.put(
					"qualityLabels",
					ScanQualityUtils.getQualityLabels(project,
							TurbineUtils.getUser(data)));
			context.put("petTracerList",
					PetTracerListUtils.getPetTracerList(project));
		} catch (Throwable t) {
	...
	private void dynamicContextExpansion(RunData data, Context context,
			ItemI item) throws Exception {
		List<Class<?>> classes = Reflection
				.getClassesForPackage("org.nrg.xnat.screens.sessionEdit.context");
