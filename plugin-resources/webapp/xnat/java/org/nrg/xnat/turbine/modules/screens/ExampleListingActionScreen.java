...
public class ExampleListingActionScreen extends SecureScreen {
	...
	protected void doBuildTemplate(RunData data, Context context)
		...
		org.nrg.xft.XFTTable table = (org.nrg.xft.XFTTable) search.execute(
				null, TurbineUtils.getUser(data).getLogin());
		...
		// The 'session_id' value is specified as the DisplayField ID for the
		// xnat:mrSessionData/ID field in the Display docs.
		// This value should match the value at the header of the session id
		// column in the previous ExampleListingActionScreen implementation.
		String sessionIDHeader = "session_id";

		// Build a comma-delimited list from the passed search table to use in
		// our WHERE clause.
		...
		QueryOrganizer qo = new QueryOrganizer("xnat:mrScanData", user,
				ViewManager.ALL);
		...
		XFTTable scans = XFTTable.Execute(query,
				SchemaElement.GetElement("xnat:mrScanData").getDbName(),
				user.getLogin());

		// HERE you will probably iterate through the rows in this table,
		// reorganize them as need be, and then pass them to whatever is
		...
		// for the example, we will just pass this new table to the VM template,
		// as we previously passed the search.
