	...
	public void finalProcessing(RunData data, Context context) {
		try {
			...
			if (TurbineUtils.HasPassedParameter("as_of_date", data)) {
				date = DateUtils.parseDateTime(TurbineUtils.GetPassedParameter(
						"as_of_date", data).toString());
					...
					public boolean accept(FlattenedItemI i) {
						if (i.getStartDate() == null && i.getEndDate() == null) {
							...
							if (i.getStartDate() != null
									&& DateUtils.isOnOrAfter(date,
											i.getStartDate())) {
								return true;
							} else if (i.getEndDate() != null
									&& DateUtils.isOnOrBefore(date,
											i.getEndDate())) {
								return true;
							} else {
								return false;
							...
							boolean after = DateUtils.isOnOrAfter(date,
									i.getStartDate());
							boolean before = DateUtils.isOnOrBefore(date,
									i.getEndDate());
							if (after && before) {
								return true;
							} else {
			...
			context.put(
					"built_html",
					ItemHtmlBuilder.build(
							date,
							Arrays.asList(ItemMerger.merge(ItemFilterer.filter(
									ItemPropBuilder
											.build(item.getItem(),
