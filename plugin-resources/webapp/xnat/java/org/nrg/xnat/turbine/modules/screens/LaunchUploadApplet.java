...
public class LaunchUploadApplet extends UploadAppletScreen {
	...
	public void doBuildTemplate(RunData data, Context context) {
		...
		if (StringUtils.trimToEmpty(
				(String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("search_field", data)).equals(
				"xnat:subjectData.ID")) {
			context.put(
					"subject",
					StringUtils
							.trimToEmpty((String) org.nrg.xdat.turbine.utils.TurbineUtils
				...
				try {
					...
					if (jsonParams.getLaunch() != null) {
						for (String key : jsonParams.getLaunch().keySet()) {
							// put EVERYTHING in the context so your VM can use
							// it.
							// remember it is all string (no booleans) so your
							// VM has to test for string equality.
	...
	private void dynamicContextExpansion(RunData data, Context context)
			throws Exception {
		List<Class<?>> classes = Reflection
				.getClassesForPackage("org.nrg.xnat.screens.uploadApplet.context");
