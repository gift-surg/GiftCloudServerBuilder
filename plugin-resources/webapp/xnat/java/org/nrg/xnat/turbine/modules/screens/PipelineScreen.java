...
public abstract class PipelineScreen extends SecureReport {
	...
	protected void populateSkipToList(RunData data, Context context) {
		...
		if (om instanceof XnatProjectdata) {
			skipToList = PipelineManager.getIndependentPipelinesForProject(
					projectId, true);
		} else {
			skipToList = PipelineManager
					.getIndependentPipelinesForProjectDescendant(projectId,
							om.getXSIType(), true);
		}
	}
	...
	protected void setUpContext(RunData data, Context context) {
		...
		context.put("BaseWrkWorkflowdata", new FieldMethodizer(
	...
	protected <A extends ArcProjectPipelineI> A getCurrentPipeline(
			ArrayList<A> pipelines, ArrayList<WrkWorkflowdata> workflows,
		...
		if (pipeline_step != null) { // User wants to jump to a particular
										// pipeline
			return getRequestedPipeline(pipelines, pipeline_step, skipToList);
		}
		if (workflows == null || workflows.size() == 0) { // No workflows are
															// available
			rtn = pipelines.get(0);
			...
			if (latestWrkFlow.getStatus().equalsIgnoreCase(
					BaseWrkWorkflowdata.QUEUED)
					|| latestWrkFlow.getStatus().equalsIgnoreCase(
							BaseWrkWorkflowdata.RUNNING)) {
				message = "A pipeline is currently "
						+ latestWrkFlow.getStatus()
	...
	protected <A extends ArcProjectPipelineI> A getRequestedPipeline(
			ArrayList<A> pipelines, String pipeline_step, ArrayList skipToList) {
		A rtn = null;
		...
		if (rtn == null) {
			message = "Pipeline with step id "
					+ pipeline_step
					+ " is not defined. Please contact your site administrator <a href=\"mailto:"
					+ XFT.GetAdminEmail() + "?subject=Invalid Pipeline Step "
					+ pipeline_step + " for " + item.getXSIType()
					+ "\">Report problem</A>";
		}
		return rtn;
	}

	protected <A extends ArcProjectPipelineI> A getNextPipelineFromWorkflow(
			ArrayList<A> pipelines, ArrayList<WrkWorkflowdata> workflows) {
		A rtn = null;
		WrkWorkflowdata latestWrkFlow = workflows.get(0);
		boolean found = false;
		for (int i = 0; i < pipelines.size(); i++) {
			A pipeline = pipelines.get(i);
			WrkAbstractexecutionenvironmentI absEnv = latestWrkFlow
			...
			// matchPipelineName =
			// ((WrkXnatexecutionenvironment)absEnv).getPipeline();
			// }else {
			matchPipelineName = latestWrkFlow.getPipelineName();
			// }
			if (pipelinePath.equals(matchPipelineName)
					&& (latestWrkFlow.getStatus().equalsIgnoreCase(
							BaseWrkWorkflowdata.FAILED) || latestWrkFlow
							.getStatus().equalsIgnoreCase(
									BaseWrkWorkflowdata.AWAITING_ACTION))) {
				rtn = pipeline;
				found = true;
				break;
			} else if (pipelinePath.equals(matchPipelineName)
					&& latestWrkFlow.getStatus().equalsIgnoreCase(
							BaseWrkWorkflowdata.COMPLETE)) {
				skipToList.add(pipeline);
				if (i != (pipelines.size() - 1)) { // All pipelines are not
													// complete
		...
		// If no matching pipelines were found from the list of existing
		// workflows for this entity, it will be the first pipeline which will
		// be launched.
		if (!found) {
			rtn = pipelines.get(0);
		}
