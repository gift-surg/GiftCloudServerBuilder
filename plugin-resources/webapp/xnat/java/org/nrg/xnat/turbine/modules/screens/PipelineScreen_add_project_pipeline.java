...
public class PipelineScreen_add_project_pipeline extends SecureReport {
	public static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(PipelineScreen_add_project_pipeline.class);
	...
	public void doBuildTemplate(RunData data, Context context) {
		...
		String pipelinePath = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("pipeline_path", data));
		String dataType = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("dataType", data));
		boolean edit = ((Boolean) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedBoolean("edit", data));
		...
		try {
			...
			if (edit) {
				...
				if (dataType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) { // Its
																			// a
																			// project
																			// level
																			// pipeline
					ArcProjectPipeline newPipeline = arcProject
					...
					ArcProjectDescendantPipeline newPipeline = arcProject
							.getPipelineForDescendantEltByPath(dataType,
									pipelinePath);
					templateFile = newPipeline.getCustomwebpage();
					context.put("newpipeline", newPipeline);
					context.put("isAutoArchive", isAutoArchive(newPipeline));
				...
				// Set the pipeline for the data-type and send it to the screen
				// for the user to add parameters
				...
				if (dataType.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME)) { // Its
					...
					List<PipePipelinedetailsParameterI> parameter = pipeline
							.getParameters_parameter();
					for (int i = 0; i < parameter.size(); i++) {
						ArcPipelineparameterdata arcParam = extractArcPipelineParameter((PipePipelinedetailsParameter) parameter
								.get(i));
						newPipeline.setParameters_parameter(arcParam.getItem());
					}
					if (pipeline.getCustomwebpage() != null) {
						newPipeline.setCustomwebpage(pipeline
								.getCustomwebpage());
						templateFile = newPipeline.getCustomwebpage();
					}
					context.put("newpipeline", newPipeline);
	...
	private boolean isAutoArchive(ArcProjectDescendantPipeline newPipeline) {
		...
		if (newPipeline == null)
			return rtn;
		if (newPipeline.getStepid() != null
				&& newPipeline.getStepid().startsWith(
	...
	private void redirect(RunData data, String templateFile) throws Exception {
		if (templateFile != null) {
			...
			logger.debug("PipelineScreen_add_pipeline looking for: "
					+ templateName);
	...
	private ArcPipelineparameterdata extractArcPipelineParameter(
			PipePipelinedetailsParameter pipeParameter) {
