...
public class PipelineScreen_details extends XDATScreen_pdf {
	static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(PipelineScreen_details.class);
	...
	protected ByteArrayOutputStream buildPdf(RunData data) throws Exception {
		...
		try {
			PipePipelinerepository pipelineRepository = PipelineRepositoryManager
					.GetInstance();
			PipePipelinedetails pipeline = (PipePipelinedetails) BaseElement
			...
			HeaderFooter footer = new HeaderFooter(new Phrase(
					"Document Production Date: " + dateFormat.format(now),
					new Font(Font.TIMES_ROMAN, 9, Font.BOLD)), false);
			footer.setAlignment(Element.ALIGN_RIGHT);
			document.setFooter(footer);
			document.open();

			Font titleFont = new Font(Font.HELVETICA, 18, Font.BOLD, new Color(
					0, 0, 128));

			Font textFont = new Font(Font.TIMES_ROMAN, 10, Font.BOLD);

			Paragraph title = new Paragraph("Pipeline "
					+ pipelineDescriptorPath, titleFont);
			...
			Paragraph p = new Paragraph(
					"Runs on: "
							+ pipelineRepository.getDisplayName(pipeline
									.getAppliesto()), textFont);
			p.setAlignment(Element.ALIGN_LEFT);
			document.add(p);

			p = new Paragraph("Generates: "
					+ pipelineRepository.getElementsGeneratedBy(pipeline),
					textFont);
			p.setAlignment(Element.ALIGN_LEFT);
			document.add(p);

			p = new Paragraph("Description: " + pipeline.getDescription(),
					textFont);
			p.setAlignment(Element.ALIGN_LEFT);
			document.add(p);
			...
			if (pipelineDoc.getPipeline().isSetResourceRequirements()) {
				...
				Property[] properties = pipelineDoc.getPipeline()
						.getResourceRequirements().getPropertyArray();
				...
				if (resourceRequirements.endsWith(", ")) {
					...
					resourceRequirements = resourceRequirements.substring(0,
							index);
				}
				p = new Paragraph("Resource Requirements: "
						+ resourceRequirements, textFont);
				p.setAlignment(Element.ALIGN_LEFT);
				document.add(p);
			}

			if (pipelineDoc.getPipeline().isSetDocumentation()) {
				Documentation doc = pipelineDoc.getPipeline()
						.getDocumentation();
				if (doc.isSetWebsite()) {
					p = new Paragraph("More Information: " + doc.getWebsite(),
							textFont);
					p.setAlignment(Element.ALIGN_LEFT);
					document.add(p);
				}
				if (doc.isSetPublications()) {
					...
					for (int i = 0; i < pubs.length; i++)
						table.addCell(makeCell(pubs[i], new Font(
				...
				if (doc.isSetAuthors()) {
					...
					table.addCell(makeCell("Name", new Font(Font.TIMES_ROMAN,
							10, Font.BOLD)));
					table.addCell(makeCell("Email", new Font(Font.TIMES_ROMAN,
							10, Font.BOLD)));
					table.addCell(makeCell("Phone", new Font(Font.TIMES_ROMAN,
							10, Font.BOLD)));
					for (int i = 0; i < authors.length; i++) {
						Author aAuthor = authors[i];
						table.addCell(makeCell(aAuthor.getFirstname() + " "
								+ aAuthor.getLastname(), new Font(
								Font.TIMES_ROMAN, 10, Font.NORMAL)));
						if (aAuthor.isSetContact()) {
							Contact contact = aAuthor.getContact();
							if (contact.isSetEmail()) {
								table.addCell(makeCell(contact.getEmail(),
										new Font(Font.TIMES_ROMAN, 10,
												Font.NORMAL)));
							} else {
								table.addCell(makeCell(" ", new Font(
										Font.TIMES_ROMAN, 10, Font.NORMAL)));
							}
							if (contact.isSetPhone()) {
								table.addCell(makeCell(contact.getPhone(),
										new Font(Font.TIMES_ROMAN, 10,
												Font.NORMAL)));
							} else {
								table.addCell(makeCell(" ", new Font(
				...
				if (doc.isSetInputParameters()) {
					...
					table.addCell(makeCell("Name", new Font(Font.TIMES_ROMAN,
							10, Font.BOLD)));
					table.addCell(makeCell("Description", new Font(
							Font.TIMES_ROMAN, 10, Font.BOLD)));
					table.addCell(makeCell("CSVValue", new Font(
							Font.TIMES_ROMAN, 10, Font.BOLD)));
					table.addCell(makeCell("Schema Link", new Font(
							Font.TIMES_ROMAN, 10, Font.BOLD)));
					for (int i = 0; i < params.length; i++) {
						Parameter aParam = params[i];
						table.addCell(makeCell(aParam.getName(), new Font(
								Font.TIMES_ROMAN, 10, Font.NORMAL)));
						table.addCell(makeCell(aParam.getDescription(),
								new Font(Font.TIMES_ROMAN, 10, Font.NORMAL)));
						if (aParam.isSetValues()) {
							if (aParam.getValues().isSetCsv()) {
								table.addCell(makeCell(aParam.getValues()
										.getCsv(), new Font(Font.TIMES_ROMAN,
										10, Font.NORMAL)));
							} else {
								table.addCell(makeCell("", new Font(
										Font.TIMES_ROMAN, 10, Font.NORMAL)));
			...
			for (int i = 0; i < steps.length; i++) {
				Step aStep = steps[i];
				p = new Paragraph("Step " + aStep.getId() + ":"
						+ aStep.getDescription(), textFont);
				p.setAlignment(Element.ALIGN_LEFT);
				document.add(p);
			}
			document.close();
	...
	private PipelineDocument getDocument(String pathToPipelineXmlFile)
			throws Exception {
