...
public class PipelineScreen_launch_pipeline extends SecureReport {
	...
	public void finalProcessing(RunData data, Context context) {
		// Given the XFTItem look for the pipelines that are defined for that
		// Schema Element and fill in the list
		try {
			...
			try { // Is this an experiment belonging to the project
				...
				context.put("isQueuedOrRunning",
						isQueuedOrRunning(experiment.getId(), data));
				...
				try {
					...
					context.put("isQueuedOrRunning",
							isQueuedOrRunning(project.getId(), data));
			...
			if (projectId != null && schema_element_name != null) {
				...
				if (schema_element_name
						.equals(XnatProjectdata.SCHEMA_ELEMENT_NAME))
					pipelines = arcProject.getPipelines_pipeline();
				else
					pipelines = arcProject
							.getPipelinesForDescendant(schema_element_name);
				context.put("pipelines", pipelines);
				// ArrayList additionalPipelines =
				// PipelineRepositoryManager.GetInstance().getAdditionalPipelinesForDatatype(project,
				// schema_element_name);
	...
	private boolean isQueuedOrRunning(String id, RunData data) throws Exception {
		...
		// Sort by Launch Time
		ArrayList workitems = items.getItems("wrk:workflowData.launch_time",
				"DESC");
		if (workitems != null && workitems.size() > 0) {
			WrkWorkflowdata wrk = new WrkWorkflowdata(
					(XFTItem) workitems.get(0));
			if (wrk.getStatus().equalsIgnoreCase("QUEUED")
					|| wrk.getStatus().equalsIgnoreCase("RUNNING")
					|| wrk.getStatus().equalsIgnoreCase("HOLD")
					|| wrk.getStatus().equalsIgnoreCase("AWAITING ACTION"))
				rtn = true;
		}
		return rtn;
	}
