...
public class QualityControl extends FileScreen {
	...
	public File getDownloadFile(RunData data, Context context) {
		String extension = (String) TurbineUtils.GetPassedParameter(
				"extension", data);

		if (extension == null || extension.equals("")
				|| !extension.toLowerCase().endsWith(".gif")) {
			logger.error("Invalid file access prevented. Extension '"
					+ extension + "' not allowed.");
			return null;
		}

		String session_label = (String) TurbineUtils.GetPassedParameter(
				"session_label", data);
		String session_id = (String) TurbineUtils.GetPassedParameter(
				"session_id", data);
		...
		if (!img.exists()) {
			String prearcP = ArcSpecManager.GetInstance()
			...
			if (prearc.exists()) {
				for (File f : prearc.listFiles()) {
					if (f.isDirectory()) {
						img = new File(f, session_label + "/RAW/" + scan + "/"
								+ labelImg);
						if (img.exists()) {
							break;
						}
						...
						img = new File(f, session_label + "/SCANS/" + scan
								+ "/" + labelImg);
						if (img.exists()) {
							break;
						}
