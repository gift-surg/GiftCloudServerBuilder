	...
	public void doOutput(RunData data) {
		...
		try {
			...
			if (fi != null) {
				...
				String cachepath = ArcSpecManager.GetInstance()
						.getGlobalCachePath();
				...
				cachepath += "user_uploads/" + user.getXdatUserId() + "/"
						+ uploadID + "/";
				...
				if (itemsByType.size() == 0) {
					...
					// ONLY ONE DOCUMENT TYPE... so files can all be moved
					// together
					...
					if (itemsByType.containsKey("SESSION")) {
						...
						for (XFTItem resource : session.getItem()
								.getChildrenOfType("xnat:abstractResource")) {
							XnatAbstractresource res = (XnatAbstractresource) org.nrg.xdat.base.BaseElement
									.GetGeneratedItem(resource);
							res.prependPathsWith(FileUtils
									.AppendSlash(dest_path));
						}
						...
						for (ItemI om : items) {
							...
							for (XFTItem resource : scan.getItem()
									.getChildrenOfType("xnat:abstractResource")) {
								XnatAbstractresource res = (XnatAbstractresource) org.nrg.xdat.base.BaseElement
										.GetGeneratedItem(resource);
								res.prependPathsWith(FileUtils
					...
					if (multiSession) {
							...
							if (children != null) {
								for (File child : children) {
									if (child.isDirectory())
										...
										FileUtils.MoveFile(child, new File(
												dest, child.getName()), true);
								}
							}
						} else {
						...
						for (ItemI item : items) {
							...
							if (user.canEdit(item)) {
								...
								try {
									SaveItemHelper.unauthorizedSave(item, user,
											false, true, c);
					...
					// MULTIPLE DOCUMENT TYPEs... so files must be moved
					// separately
