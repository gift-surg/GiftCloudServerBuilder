...
public class UserCacheFileAssignment extends SecureReport {
	...
	public void finalProcessing(RunData data, Context context) {
		...
		String userPath = UserUtils.getUserCacheUploadsPath(TurbineUtils
				.getUser(data));
		...
		try {
			context.put("destFiles",
					convertOMtoJSON(expt, expt.getPrimaryProject(false)));
		} catch (JSONException e) {
			logger.error("", e);
		}
		...
		context.put(
				"user_path",
				String.format("/data/user/cache/resources/%s/files",
						expt.getId()));
	...
	public JSONObject convertOMtoJSON(XnatExperimentdata expt,
			XnatProjectdata proj) throws JSONException {
		final JSONObject dest = createTextNode(expt.getIdentifier(expt
				.getProject()));
		...
		if (!res.isEmpty()) {
			...
			for (final XnatAbstractresource abst : res) {
				...
				r.put("dest", String.format(
						"/data/archive/experiments/%s/resources/%s/files",
						expt.getId(), abst.getXnatAbstractresourceId()));
				r.put("labelStyle", "icon-of");
				addNode(r1, r);
				final List<File> files = abst.getCorrespondingFiles(proj
						.getRootArchivePath());
				if (files.size() > 0) {
				...
				for (final XnatImagescandataI scan : scans) {
					...
					for (final XnatAbstractresource abst : s_ress) {
						...
						s_res.put(
								"dest",
								String.format(
										"/data/archive/experiments/%s/scans/%s/resources/%s/files",
										expt.getId(), scan.getId(),
										abst.getXnatAbstractresourceId()));
						addNode(s, s_res);

						s_res.put("labelStyle", "icon-of");

						final List<File> files = abst
								.getCorrespondingFiles(proj
										.getRootArchivePath());
						if (files.size() > 0) {
	...
	public static void addNode(JSONObject parent, JSONObject child)
