...
public class Viewer extends SecureReport {
	...
	public void noItemError(RunData data, Context context) {
		if (context.containsKey("skipq")) {
			try {
				XnatMrsessiondata mr = XNATUtils.getLastSessionForParticipant(
						(String) context.get("id"), TurbineUtils.getUser(data));
				...
				if (mr == null) {
					...
					context.put("om", mr);
					context.put("appletPath",
							TurbineUtils.GetRelativeServerPath(data)
									+ "/applet");
					context.put("jsessionid", XnatHttpUtils.getJSESSIONID(data));
				}
			} catch (Exception e) {
