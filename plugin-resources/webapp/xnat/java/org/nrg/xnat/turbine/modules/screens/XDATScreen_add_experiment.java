...
public class XDATScreen_add_experiment extends SecureScreen {
	...
	protected void doBuildTemplate(RunData data, Context context)
		...
		// Iterator editable_Displays =
		// TurbineUtils.getUser(data).getEditableElementDisplays().iterator();
		...
		// if
		// (se.getGenericXFTElement().getExtendedElements().contains("xnat:subjectAssessorData"))
		// {
		...
		if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("confirmed", data)) != null) {
			context.put("confirmed",
					((String) org.nrg.xdat.turbine.utils.TurbineUtils
							.GetPassedParameter("confirmed", data)));
		}
		...
		if (TurbineUtils.HasPassedParameter("visit_id", data)) {
			context.put("visit_id",
					TurbineUtils.GetPassedParameter("visit_id", data));
		}
		...
		if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
				.GetPassedParameter("search_field", data)) != null) {
			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("search_field", data))
					.equalsIgnoreCase("xnat:subjectData.ID")) {
				if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
						.GetPassedParameter("search_value", data)) != null) {
					part_id = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
		...
		if (StringUtils.isEmpty(part_id)) {
			if (TurbineUtils.HasPassedParameter("subject_id", data)) {
				part_id = (String) TurbineUtils.GetPassedParameter(
						"subject_id", data);
			}
			if (TurbineUtils.HasPassedParameter("part_id", data)) {
				part_id = (String) TurbineUtils.GetPassedParameter("part_id",
						data);
			}
		}
