...
public class XDATScreen_data_management extends SecureScreen {
	static org.apache.log4j.Logger logger = Logger
			.getLogger(XDATScreen_data_management.class);
	...
	protected void doBuildTemplate(RunData data, Context context)
			throws Exception {
		try {
			String project = (String) TurbineUtils.GetPassedParameter(
					"project", data);
			String prearchive_path = null;
			if (project == null) {
				prearchive_path = ArcSpecManager.GetInstance()
			...
			DisplaySearch search = TurbineUtils.getUser(data).getSearch(
					"wrk:workflowData", "listing");
			search.addCriteria("wrk:workflowData.pipeline_name", "=",
					"Transfer");
