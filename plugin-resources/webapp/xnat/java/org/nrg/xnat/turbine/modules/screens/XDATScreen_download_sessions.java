...
public class XDATScreen_download_sessions extends SecureScreen {
	...
	protected void doBuildTemplate(RunData data, Context context)
		...
		String[] sessions = ((String[]) TurbineUtils.GetPassedObjects(
				"sessions", data));
		if (sessions == null) {
			// If the sessions aren't directly embedded in the data, check for a
			// search element.
			String element = (String) TurbineUtils.GetPassedParameter(
					"search_element", data);
			String field = (String) TurbineUtils.GetPassedParameter(
					"search_field", data);
			String value = (String) TurbineUtils.GetPassedParameter(
					"search_value", data);

			// TODO: For now, hard-limit this to MR sessions.
			if (!StringUtils.IsEmpty(element) && !StringUtils.IsEmpty(field)
					&& !StringUtils.IsEmpty(value)) {
				SchemaElement se = SchemaElement.GetElement(element);
				if (se.getGenericXFTElement().instanceOf(
			...
			// Add the targeted flag so that the page can display based on the
			// single session-targeted action.
			context.put("targeted", true);
		} else {
			// Add the targeted flag so that the page can display based on the
			// multiple session-targeted action.
		...
		if (sessionList != null) {
			...
			if (project == null) {
				...
				if (!retrieveAllTags(TurbineUtils.getUser(data)).contains(
						project)) {
				...
				query = "SELECT expt.id,COALESCE(pp.label,expt.label,expt.id) AS IDS,modality"
						+ " FROM xnat_imageSessionData isd"
						+ " LEFT JOIN xnat_experimentData expt ON expt.id=isd.id"
						+ " LEFT JOIN xnat_experimentData_share pp ON expt.id=pp.sharing_share_xnat_experimentda_id AND pp.project='"
						+ project
						+ "'"
						+ " WHERE isd.ID IN ("
						+ sessionString
			...
			XFTTable table = XFTTable.Execute(query, TurbineUtils.getUser(data)
			...
			query = "SELECT type,COUNT(*) FROM xnat_imagescandata "
					+ " WHERE xnat_imagescandata.image_session_id IN ("
					+ sessionString + ") GROUP BY type ORDER BY type;";

			table = XFTTable.Execute(query, TurbineUtils.getUser(data)
					.getDBName(), TurbineUtils.getUser(data).getLogin());
			...
			query = "SELECT type,COUNT(*) FROM xnat_reconstructedimagedata "
					+ " WHERE xnat_reconstructedimagedata.image_session_id IN ("
					+ sessionString + ") GROUP BY type ORDER BY type;";
			...
			for (List assessor : scans) {
				...
				sub.addAll(assessor);
				sub.add(ElementSecurity.GetPluralDescription((String) assessor
						.get(0)));
				assessors.add(sub);
			}
			...
			query = "SELECT label,COUNT(*) FROM xnat_experimentData_resource JOIN xnat_abstractResource ON xnat_experimentData_resource.xnat_abstractresource_xnat_abstractresource_id=xnat_abstractResource.xnat_abstractresource_id "
					+ " WHERE xnat_experimentData_resource.xnat_experimentdata_id IN ("
					+ sessionString + ") GROUP BY label;";
	...
	public List<String> retrieveAllTags(final XDATUser user) {
		try {
			return (List<String>) (XFTTable.Execute(
					"SELECT DISTINCT id from xnat_projectData;",
