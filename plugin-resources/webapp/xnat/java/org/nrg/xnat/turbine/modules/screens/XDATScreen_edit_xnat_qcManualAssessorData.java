...
public class XDATScreen_edit_xnat_qcManualAssessorData
	...
	public ItemI getEmptyItem(RunData data) throws Exception {
		...
		final XnatQcmanualassessordata qcAccessor = new XnatQcmanualassessordata(
		...
		if (!StringUtils.IsEmpty(search_element)) {
			...
			if (se.instanceOf(XnatImagesessiondata.SCHEMA_ELEMENT_NAME)) {
				...
				if (!StringUtils.IsEmpty(search_value)) {
					...
					qcAccessor.setLabel(imageSession.getLabel() + "_"
					...
					for (XnatImagescandataI imageScan : imageSession
							.getScans_scan()) {
						XnatQcscandata scan;
						if (XnatPetscandata.SCHEMA_ELEMENT_NAME
								.equals(imageScan.getXSIType())) {
							scan = new XnatPetqcscandata(user);
						} else if (XnatMrscandata.SCHEMA_ELEMENT_NAME
								.equals(imageScan.getXSIType())) {
							scan = new XnatMrqcscandata(user);
						} else {
							// do not create anything but PET and MR QCs for now
							// (e.g. PET/CT, only do QC on PET)
							continue;
						}
