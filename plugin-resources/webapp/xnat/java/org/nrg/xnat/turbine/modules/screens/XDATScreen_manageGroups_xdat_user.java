		...
		try {
			...
			if (item == null) {
				...
				try {
					...
					XFTTable groups = XFTTable
							.Execute(
									"SELECT id,displayname,tag FROM xdat_usergroup ORDER BY tag,id",
									tempUser.getDBName(), null);
					...
					ArrayList<XnatProjectdata> allprojects = XnatProjectdata
							.getAllXnatProjectdatas(null, false);

					Collections.sort(allprojects,
							XnatProjectdata.GetComparator());

					for (XnatProjectdata proj : allprojects) {
						ArrayList<List> pGroups = proj.getGroupIDs();

						for (List<String> row : pGroups) {
							ArrayList<ArrayList<Object>> projects = projectGroups
									.get(proj.getSecondaryId());
							if (projects == null) {
								projects = new ArrayList<ArrayList<Object>>();
								projectGroups.put(proj.getSecondaryId(),
										projects);
							}
							String id = row.get(0);
					...
					while (groups.hasMoreRows()) {
						...
						if (row.get("id") != null) {
							...
							if (tag != null) {
								// Hashtable<Object,Object> projects =
								// projectGroups.get(tag);
								// if (projects==null){
