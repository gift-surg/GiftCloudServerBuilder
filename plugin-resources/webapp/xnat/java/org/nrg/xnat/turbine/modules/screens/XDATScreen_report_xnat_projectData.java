...
public class XDATScreen_report_xnat_projectData extends SecureReport {
	...
	public void finalProcessing(RunData data, Context context) {
		...
		try {
			// org.nrg.xft.search.CriteriaCollection cc = new
			// CriteriaCollection("AND");
			// cc.addClause("wrk:workflowData.ID",project.getId());
			// org.nrg.xft.collections.ItemCollection items =
			// org.nrg.xft.search.ItemSearch.GetItems(cc,TurbineUtils.getUser(data),false);
			// //Sort by Launch Time
			// ArrayList workitems =
			// items.getItems("wrk:workflowData.launch_time","DESC");
			...
			if (ProjectAccessRequest.CREATED_PAR_TABLE) {
				Integer parcount = (Integer) PoolDBUtils.ReturnStatisticQuery(
						"SELECT COUNT(par_id)::int4 AS count FROM xs_par_table WHERE proj_id='"
								+ project.getId() + "'", "count",
			...
			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("topTab", data)) != null) {
				context.put("topTab",
						((String) org.nrg.xdat.turbine.utils.TurbineUtils
								.GetPassedParameter("topTab", data)));
			}

			if (((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("bottomTab", data)) != null) {
				context.put("bottomTab",
						((String) org.nrg.xdat.turbine.utils.TurbineUtils
								.GetPassedParameter("bottomTab", data)));
			}

			setDefaultTabs("xnat_projectData_summary_details",
					"xnat_projectData_summary_management",
					"xnat_projectData_summary_manage",
					"xnat_projectData_summary_pipeline",
					"xnat_projectData_summary_history");
