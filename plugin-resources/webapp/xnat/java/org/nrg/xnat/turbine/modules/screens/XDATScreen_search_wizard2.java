...
public class XDATScreen_search_wizard2 extends SecureScreen {
	...
	protected void doBuildTemplate(RunData data, Context context)
		...
		context.put("ELEMENT_0",
				TurbineUtils.GetPassedParameter("ELEMENT_0", data));
		for (ElementDisplay ed : TurbineUtils.getUser(data)
				.getSearchableElementDisplaysByPluralDesc()) {
			if (TurbineUtils.HasPassedParameter("super_" + ed.getElementName(),
					data))
				additional.put(
						ed.getElementName(),
						TurbineUtils.GetPassedParameter(
								"super_" + ed.getElementName(), data));
		}
