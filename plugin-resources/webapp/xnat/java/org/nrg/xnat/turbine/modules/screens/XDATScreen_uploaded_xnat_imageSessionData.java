...
public final class XDATScreen_uploaded_xnat_imageSessionData extends
	...
	final org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(XDATScreen_uploaded_xnat_imageSessionData.class);
	...
	public void finalProcessing(final RunData data, final Context context) {
		...
		XnatImagesessiondata session = (XnatImagesessiondata) BaseElement
				.GetGeneratedItem(item);
		context.put("notes", session.getNote());
		final ItemI part = TurbineUtils.GetParticipantItem(data);
		if (part != null) {
			final XnatSubjectdata xsd = (part instanceof XnatSubjectdata) ? (XnatSubjectdata) part
					: new XnatSubjectdata(part);
		...
		if (data.getParameters().containsKey("tag")) {
			tag = ((String) org.nrg.xdat.turbine.utils.TurbineUtils
					.GetPassedParameter("tag", data));
		} else {
		...
		// XNAT-2916 - Set the subject label for the session if one exists in
		// the system.
		if (!StringUtils.isEmpty(session.getSubjectId())
				&& !StringUtils.isEmpty(session.getProject())) {
			BaseXnatSubjectdata subj = BaseXnatSubjectdata
					.GetSubjectByProjectIdentifierCaseInsensitive(
							session.getProject(), session.getSubjectId(), null,
							false);
			if (null != subj) {
				if (!StringUtils.equals(session.getSubjectId(), subj.getId())
						&& !StringUtils.equals(session.getSubjectId(),
								subj.getLabel())) {
		...
		context.put("edit_screen",
				"XDATScreen_uploaded_xnat_imageSessionData.vm");
		context.put("petTracerList",
				PetTracerListUtils.getPetTracerList(session.getProject()));
	}
