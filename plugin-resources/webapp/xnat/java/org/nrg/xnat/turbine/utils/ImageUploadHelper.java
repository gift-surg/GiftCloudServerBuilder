...
public class ImageUploadHelper extends StatusProducer implements
		Callable<List<File>> {
	...
	public ImageUploadHelper(final Object uID, final XDATUser user,
			final String project, final File src, final File dest,
	...
	public List<File> call() throws ClientException, ServerException {
		final PrearcImporter pw = PrearcImporterFactory.getFactory()
				.getPrearcImporter(project, dest, src);
			...
			if (f.isDirectory()) {
				...
				if (xml.exists()) {
					if (this.additionalValues != null
							&& this.additionalValues.size() > 0) {
						processing("Setting additional values for '"
								+ f.getName() + "'");
						final SAXReader reader = new SAXReader(null);
						try {
							final org.nrg.xft.XFTItem item = reader.parse(s);
							for (Map.Entry<String, Object> entry : additionalValues
									.entrySet()) {
								try {
									...
									logger.error("", e);
									failed("failed to set appropriate field for '"
											+ f.getName()
							...
							logger.error("", e);
							failed("failed to set appropriate field(s) for '"
									+ f.getName()
									+ "'.  Data may be publicly accessible until archived.");
						}
					}
				} else {
					failed("failed to load generated xml file ('"
							+ xml.getName()
							+ "').  Data may be publicly accessible until archived.");
				}
			}
