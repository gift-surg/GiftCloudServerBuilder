...
public class ProjectAccessRequest {
	...
	public static final Map<String, Pattern> PAR_PATTERNS = new HashMap<String, Pattern>() {
		{
			...
			put("ID %s for project %s", Pattern.compile(String.format(
					PAR_BY_USER_AND_PROJ_ID, "(.*?)", "(.*?)")));
			put("Email %s",
					Pattern.compile(String.format(PAR_BY_EMAIL, "(.*?)")));
		}
	};

	public ProjectAccessRequest(final String query, final XDATUser user)
			throws XNATException, SQLException, DBPoolException {
		XFTTable table = runInitQuery(query, user);
		table.resetRowCursor();
		int numRows = table.getNumRows();
		if (numRows == 0) {
			throw new XFTItemNotFoundException(
					"Found no results for project access request search, should return one and only one.",
					user) {
				{
					setQuery(query);
				}
			...
			if (logger.isInfoEnabled()) {
				logger.info("Found "
						+ numRows
						+ " results for project access request search, should return one and only one for: "
						+ getIdentifierFromQuery(query) + "\n" + query);
			}
		}
		initializeFromDataRow(table.nextRow());
	}

	public ProjectAccessRequest(Object[] row) throws SQLException,
			DBPoolException {
	 ...
	 *            The user ID to set for the request approver
	 */
	 ...
	 * 
	 * @param email
	 *            A string containing the email associated with this request.
	 */
	 ...
	 * 
	 * @param guid
	 *            A string containing the GUID for this request.
	 */
	...
	/**
	 * Save the project access request with the <b>user</b> as the request
	 * approver.
	 * 
	 * @param user
	 *            The user to specify as the request approver.
	 * @throws Exception
	 */
	public void save(XDATUser user) throws Exception {
		...
		if (_requestId == null) {
			...
			StringBuilder query = new StringBuilder(
					"INSERT INTO xs_par_table (proj_id, level, user_id");
			if (_approved != null) {
				query.append(", approval_date, approved, approver_id");
			}
			query.append(")");
			query.append(" VALUES ('").append(_projectId).append("', '")
					.append(_level).append("', ");
			query.append(_userId != null ? _userId : "NULL");
			if (_approved != null) {
				if (_approvalDate == null) {
					query.append(", NOW()");
				} else {
					query.append(", '").append(_approvalDate.toString())
							.append("'");
				}
				query.append(",").append(_approved).append(",")
						.append(user.getXdatUserId());
			}
			query.append(");");
			PoolDBUtils.ExecuteNonSelectQuery(query.toString(),
					user.getDBName(), user.getLogin());
		} else {
			// Existing request
			StringBuilder query = new StringBuilder(
					"UPDATE xs_par_table SET proj_id='");
			query.append(_projectId).append("', level='").append(_level)
					.append("'");
			...
			if (_approved != null) {
				if (_approvalDate == null) {
					...
					query.append(", approval_date='")
							.append(_approvalDate.toString()).append("'");
				}
	...
	/**
	 * Processes the project access request, setting the <b>user</b> as the
	 * accepting or declining user. Note that this will also process all other
	 * requests associated with the same email as this request. This does
	 * <i>not</i> find all requests with the same email as the specified user,
	 * just those with the same email as the initial request. However, all
	 * processed requests are set to the email of the actual user object as
	 * they're processed.
	 * 
	 * @param user
	 *            The user who accepts or declines the request.
	 * @param accept
	 *            Whether the request was accepted or declined.
	 * @param eventType
	 *            A system event for tracking requests.
	 * @param reason
	 *            The reason for the operation.
	 * @param comment
	 *            Any related comments.
	 * @return A list of all of the project access requests associated with the
	 *         original email and accepted or declined.
	 * @throws Exception
	 */
	public List<String> process(XDATUser user, boolean accept,
			EventUtils.TYPE eventType, String reason, String comment)
			throws Exception {
	...
	public static ProjectAccessRequest RequestPARByGUID(String guid,
			XDATUser user) {
		return RequestPAR(String.format(PAR_BY_GUID, guid), user);
	}

	public static ProjectAccessRequest RequestPARByUserProject(Integer userId,
			String projectId, XDATUser user) {
		return RequestPAR(String.format(PAR_BY_USER_AND_PROJ_ID,
				userId.toString(), projectId), user);
	}

	public static ArrayList<ProjectAccessRequest> RequestPARsByUserEmail(
			String email, XDATUser user) {
	...
	public static ProjectAccessRequest RequestPAR(String where, XDATUser user) {
		try {
			...
			_logger.error(
					"Error occurred while requesting project access request for user ["
							+ user.getUsername() + "]: " + where, exception);
			return null;
		}
	}

	public static ArrayList<ProjectAccessRequest> RequestPARs(String where,
			XDATUser user) {
		...
		try {
			...
			final String identifier = user != null ? user.getUsername()
					: getIdentifierFromWhere(where);
			_logger.error(
					"Error occurred while requesting project access requests for user ["
							+ identifier + "]: " + where, exception);
		}
		return PARs;
	}
	...
	public static void CreatePAR(String pID, String level, XDATUser user) {
		try {
			...
			String query = String
					.format("INSERT INTO xs_par_table (proj_id,user_id,level) VALUES ('%s', %d, '%s');",
							pID, user.getID(),
							StringUtils.RemoveChar(level, '\''));
			PoolDBUtils.ExecuteNonSelectQuery(query, user.getDBName(),
					user.getLogin());
		...
		try {
			if (!CREATED_PAR_TABLE) {
				...
				String exists = (String) PoolDBUtils.ReturnStatisticQuery(
						query, "relname", PoolDBUtils.getDefaultDBName(), null);
				...
				if (exists != null) {
					...
					XFTTable t = con
							.executeSelectQuery(
									"select LOWER(attname) as col_name,typname, attnotnull from pg_attribute, pg_class,pg_type where attrelid = pg_class.oid AND atttypid=pg_type.oid AND attnum>0 and LOWER(relname) = 'xs_par_table';",
									PoolDBUtils.getDefaultDBName(), null);
					while (t.hasMoreRows()) {
						t.nextRow();
					...
					if (!containsEmail) {
						...
						PoolDBUtils.ExecuteNonSelectQuery(query,
								PoolDBUtils.getDefaultDBName(), null);
					}
					if (!containsGuid) {
	...
	public static void InviteUser(Context context, String invitee,
			XDATUser user, String subject) throws Exception {
		...
		try {
			...
			StringBuilder query = new StringBuilder(
					"INSERT INTO xs_par_table (email, guid, proj_id, approver_id, level) VALUES ('");
			query.append(invitee).append("', '").append(guid).append("', '")
					.append(project.getId()).append("', ").append(user.getID())
					.append(", '").append(context.get("access_level"))
					.append("');");
			...
			request = RequestPAR(
					"xs_par_table.email = '" + invitee + "' AND guid = '"
							+ guid + "' AND proj_id = '" + project.getId()
							+ "' AND approved IS NULL", user);
		} catch (SQLException exception) {
			_logger.error("Error occurred while running SQL query", exception);
		} catch (Exception exception) {
			_logger.error("General error occurred when inviting user "
					+ invitee, exception);
		}
		...
		Template template = Velocity
				.getTemplate("/screens/InviteProjectAccessEmail.vm");
		...
		try {
			XDAT.getMailService().sendHtmlMessage(from, invitee,
					user.getEmail(), bcc, subject, writer.toString());
	...
	private List<String> process(final XDATUser user, final boolean accept,
			final EventUtils.TYPE eventType, final String reason,
			final String comment, final boolean processRelated)
			throws Exception {
		...
		if (!org.apache.commons.lang.StringUtils.equalsIgnoreCase(parEmail,
				user.getEmail())) {
		...
		if (accept) {
			...
			XnatProjectdata project = getXnatProjectdatasById(_projectId, null,
					false);

			PersistentWorkflowI workflow = WorkflowUtils
					.getOrCreateWorkflowData(null, user, SCHEMA_ELEMENT_NAME,
							_projectId, _projectId, EventUtils
									.newEventInstance(
			...
			try {
				...
				user.addGroup(project.addGroupMember(_level, user, user,
						eventInfo, true));

				WorkflowUtils.complete(workflow, eventInfo);

				try {
					ItemAccessHistory.LogAccess(user, project.getItem(),
							"report");
				} catch (Throwable e) {
					_logger.error("", e);
				}
		...
		List<String> processedProjects = new ArrayList<String>() {
			{
				add(getProjectByIDorAlias(getProjectId(), user, false)
						.getDisplayName());
			}
		};
		if (processRelated) {
			processedProjects.addAll(processRelatedPARs(parEmail, user, accept,
					eventType, reason, comment));
		}
		return processedProjects;
	}

	private static XFTTable runInitQuery(final String query, final XDATUser user)
			throws SQLException, DBPoolException {
		if (!CREATED_PAR_TABLE) {
			CreatePARTable();
		}

		final String dbName = user != null ? user.getDBName() : PoolDBUtils
				.getDefaultDBName();
	...
	private List<String> processRelatedPARs(String parEmail, XDATUser user,
			boolean accept, EventUtils.TYPE eventType, String reason,
			String comment) {
		List<String> processedProjects = new ArrayList<String>();
		List<ProjectAccessRequest> parsForEmail = RequestPARsByUserEmail(
				parEmail, user);
		for (ProjectAccessRequest par : parsForEmail) {
			try {
				processedProjects.addAll(par.process(user, accept, eventType,
						reason, comment, false));
			} catch (Exception exception) {
				_logger.error("Error occurred trying to process project access request "
						+ par.getRequestId());
			}
		}
		return processedProjects;
	...
	private static final Logger _logger = Logger
			.getLogger(ProjectAccessRequest.class);
