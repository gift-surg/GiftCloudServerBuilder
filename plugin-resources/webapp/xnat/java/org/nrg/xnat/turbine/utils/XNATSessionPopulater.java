...
public class XNATSessionPopulater {
	...
	/**
	 * Use this object to populate a Session object from the prearchive. The xml
	 * File can point to the xml or the session directory.
	 * 
	 ...
	 */
	public XNATSessionPopulater(XDATUser user, File xml, String project,
			boolean nullifySubject) {
		this.user = user;
		this.xml = (xml.getPath().endsWith(".xml")) ? xml : new File(
				xml.getPath() + ".xml");
	...
	public XnatImagesessiondata populate() throws IOException, SAXException {
		...
		XnatImagesessiondata imageSessionData = (XnatImagesessiondata) BaseElement
				.GetGeneratedItem(item);
		imageSessionData.fixScanTypes();

		final List<String> scanQualityLabels = ScanQualityUtils
				.getQualityLabels(project, user);
		...
		if (imageSessionData.getSubjectId() != null
				&& imageSessionData.getSubjectId().startsWith("INVALID:")) {
			imageSessionData.setSubjectId(imageSessionData.getSubjectId()
					.substring(8).trim());
		}

		if (XNATUtils.hasValue(imageSessionData.getSubjectId())) {
			imageSessionData.setSubjectId(XnatSubjectdata
					.cleanValue(imageSessionData.getSubjectId()));
		...
		if (!XNATUtils.hasValue(imageSessionData.getSubjectId())) {
			if (XNATUtils.hasValue(imageSessionData.getDcmpatientname())) {
				imageSessionData.setSubjectId(XnatSubjectdata
						.cleanValue(imageSessionData.getDcmpatientname()));
