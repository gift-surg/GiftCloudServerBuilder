...
public class XNATUtils {
	...
	public static Hashtable getInvestigatorsForRead(String elementName,
			RunData data) {
	...
	public static Hashtable getInvestigatorsForRead(String elementName,
		...
		try {
			...
			_return = ElementSecurity.GetDistinctIdValuesFor(
					"xnat:investigatorData", "default", login);
		} catch (Exception e) {
			logger.error("", e);
		}
	...
	public static Hashtable getInvestigatorsForCreate(String elementName,
			RunData data) {
		XDATUser tempUser = TurbineUtils.getUser(data);
	...
	public static Hashtable getProjectsForCreate(String elementName,
			RunData data) {
		XDATUser tempUser = TurbineUtils.getUser(data);
	...
	public static Hashtable getProjectsForAction(String elementName,
			XDATUser user, String action) {
		Hashtable _return = new Hashtable();
		try {
			String login = null;
			if (user != null) {
			...
			if (ElementSecurity.IsSecureElement(elementName, action)) {
				List<Object> permisionItems = user.getAllowedValues(
						SchemaElement.GetElement(elementName), elementName
								+ "/project", action);

				Hashtable temp = ElementSecurity.GetDistinctIdValuesFor(
						"xnat:projectData", "default", login);
	...
	public static ArrayList<XnatAbstractprotocol> getProtocolsForCreate(
			String elementName, RunData data) {
		XDATUser tempUser = TurbineUtils.getUser(data);
	...
	public static ArrayList<XnatAbstractprotocol> GetAllProtocols() {
		if (ALL_PROTOCOLS == null) {
			ALL_PROTOCOLS = XnatAbstractprotocol.getAllXnatAbstractprotocols(
					null, false);
		}
	...
	public static ArrayList<XnatAbstractprotocol> getProtocolsForCreate(
			String elementName, XDATUser user) {
		...
		try {
			...
			if (ElementSecurity.IsSecureElement(elementName,
				...
				while (iter.hasNext()) {
					...
					for (XnatAbstractprotocol protocol : allProtocols) {
						if (protocol.getXnatAbstractprotocolId().toString()
								.equals(o.toString())) {
	...
	public static String getLastSessionIdForParticipant(String id, XDATUser user) {
		...
		String query = "SELECT mr.id FROM xnat_mrSessionData mr LEFT JOIN xnat_subjectAssessorData sad ON mr.ID=sad.ID LEFT JOIN xnat_experimentData ed ON sad.ID=ed.ID WHERE subject_id='"
				+ id + "' ORDER BY date DESC LIMIT 1";
		try {
			XFTTable table = TableSearch
					.Execute(query, user.getDBName(), login);
			if (table.size() > 0) {
	...
	public static XnatMrsessiondata getLastSessionForParticipant(String id,
			XDATUser user) {
		try {
			...
			ItemI mr = ItemSearch.GetItem("xnat:mrSessionData.ID", mr_id, user,
					false);
			if (mr == null) {
				return null;
			}
	...
	//
	// public String getCurrentArchiveFolder() throws
	// org.nrg.xnat.exceptions.UndefinedArchive,org.nrg.xnat.exceptions.InvalidArchiveStructure,IOException{
	...
	// if (index ==-1 )
	// {
	// throw new org.nrg.xnat.exceptions.InvalidArchiveStructure(f.getName() +
	// " does not exist in " + curA);
	// }else{
	...
	// }
	//
	// public static String GetCurrentArchiveFolder() throws
	// org.nrg.xnat.exceptions.UndefinedArchive,org.nrg.xnat.exceptions.InvalidArchiveStructure,IOException{
	// return GetInstance().getCurrentArchiveFolder();
	// }

	public static void populateCatalogBean(CatCatalogBean cat, String header,
			File f) {
		if (f.isDirectory()) {
	...
	public static CatalogSet getCatalogBean(RunData data, ItemI input) {
		...
		final String url = server
				+ "/app/template/GetFile.vm/search_element/"
		...
		if (project != null) {
			for (XFTItem resource : hash) {
				...
				if (om instanceof XnatAbstractresource) {
					...
					ArrayList<File> files = resourceA
							.getCorrespondingFiles(project.getRootArchivePath());
					for (int i = 0; i < files.size(); i++) {
						File f = files.get(i);
						// String xPath= item.getXSIType() + "[" +
						// ((XFTItem)item).getPKString() + "]/" + key;
						// xPath = xPath.replace('/', '.');
					...
					if (om instanceof XnatResourcecatalog) {
						File f = ((XnatResourcecatalog) om)
								.getCatalogFile(project.getRootArchivePath());
						CatEntryBean entry = new CatEntryBean();
