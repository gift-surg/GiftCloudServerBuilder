...
public class CatalogUtils {
	...
	public final static String[] FILE_HEADERS = { "Name", "Size", "URI",
			"collection", "file_tags", "file_format", "file_content", "cat_ID" };
	public final static String[] FILE_HEADERS_W_FILE = { "Name", "Size", "URI",
			"collection", "file_tags", "file_format", "file_content", "cat_ID",
			"file" };

	public static boolean getChecksumConfiguration(final XnatProjectdata project)
			throws ConfigServiceException {
		Integer projectdataInfo = (Integer) project.getItem().getProps()
				.get("projectdata_info");
		Configuration configuration = XDAT.getConfigService().getConfig(
				"checksums", "checksums", (long) projectdataInfo);

		if (configuration != null) {
			String checksumProperty = XDAT
					.getSiteConfigurationProperty("checksums");
	...
	public static Boolean getChecksumConfiguration()
			throws ConfigServiceException {
	...
	/**
	 * This sets the cached value for the checksum configuration. Note that this
	 * does <i>not</i> set the persisted configuration value for the checksum
	 * configuration. This is used by the
	 * {@link org.nrg.xnat.utils.ChecksumsSiteConfigurationListener} to update
	 * the cached value whenever the database value is changed elsewhere.
	 * 
	 * @param checksumConfig
	 *            The value to set for the cached checksum configuration
	 *            setting.
	...
	public static void calculateResourceChecksums(final CatCatalogI cat,
			final File f) {
	 ...
	 *
	 * @param entry
	 *            CatEntryI for operation
	 * @param path
	 *            Path to catalog (used for relative paths)
	 * @return true if entry was modified, false if not.
	 */
	private static boolean setChecksum(final CatEntryI entry, final String path) {
		if (StringUtils.isBlank(entry.getDigest())) {// this should only occur
														// if the MD5 isn't
														// already there.
			final File file = CatalogUtils.getFile(entry, path);// this will
																// allow
																// absolute
																// paths to be
																// functional.
																...
																// XNAT would
																// make them.
			if (file != null && file.exists()) {// fail safe to missing files,
												// maybe the files haven't been
												// put in place yet...
				try {
					...
					if (logger.isWarnEnabled()) {
						logger.warn(
								"Error occurred calculating checksum for file: "
	...
	public static List<Object[]> getEntryDetails(CatCatalogI cat,
			String parentPath, String uriPath, XnatResource _resource,
			boolean includeFile, final CatEntryFilterI filter,
			XnatProjectdata proj, String locator) {
		final ArrayList<Object[]> al = new ArrayList<Object[]>();
		for (final CatCatalogI subset : cat.getSets_entryset()) {
			al.addAll(getEntryDetails(subset, parentPath, uriPath, _resource,
					includeFile, filter, proj, locator));
		}
		...
		for (final CatEntryI entry : cat.getEntries_entry()) {
			if (filter == null || filter.accept(entry)) {
				...
				final String entryPath = StringUtils.replace(
						FileUtils.AppendRootPath(parentPath, entry.getUri()),
						"\\", "/");
				...
				if (locator.equalsIgnoreCase("URI")) {
					...
					row[2] = entryPath.substring(proj
							.getRootArchivePath()
							.substring(
									0,
									proj.getRootArchivePath().lastIndexOf(
											proj.getId())).length());
				}
	...
	/**
	 * Takes a size of a file or heap of memory in the form of a long and
	 * returns a formatted readable version in the form of byte units. For
	 * example, 46 would become 46B, 1,024 would become 1KB, 1,048,576 would
	 * become 1MB, etc.
	 *
	 * @param size
	...
	public static String formatSize(long size) {
		...
		return String.format("%.1f %sB", size / Math.pow(1024, exp),
				"KMGTPE".charAt(exp - 1));
	}
	 ...
	 *
	 * @param label
	 *            The label of the object (session, scan, resource, etc.)
	 * @param fileCount
	 *            The number of files that compose the object.
	 * @param rawSize
	 *            The size of the files that compose the object.
	 * @return A formatted display of the file statistics.
	 */
	public static String formatFileStats(final String label,
			final long fileCount, final Object rawSize) {
		long size = 0;
		if (rawSize != null) {
		...
		return String.format("%s: %s in %s files", label, formatSize(size),
				fileCount);
	}
	...
	public static CatEntryI getEntryByFilter(final CatCatalogI cat,
			final CatEntryFilterI filter) {
		CatEntryI e;
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			try {
				...
				logger.error(
						"Error occurred filtering catalog entry: " + entry,
						exception);
			}
		}
	...
	public static Collection<CatEntryI> getEntriesByFilter(
			final CatCatalogI cat, final CatEntryFilterI filter) {
	...
	public static List<File> getFiles(CatCatalogI cat, String parentPath) {
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			String entryPath = StringUtils.replace(
					FileUtils.AppendRootPath(parentPath, entry.getUri()), "\\",
					"/");
	...
	/**
	 * Gets file from file system. This method supports relative or absolute
	 * paths in the CatEntryI. It also supports files that are gzipped on the
	 * file system, but don't include .gz in the catalog URI (this used to be
	 * very common).
	 * 
	 * @param entry
	 *            Catalog Entry for file to be retrieved
	 * @param parentPath
	 *            Path to catalog file directory
	 * @return File object represented by CatEntryI
	 */
	public static File getFile(CatEntryI entry, String parentPath) {
		String entryPath = StringUtils
				.replace(FileUtils.AppendRootPath(parentPath, entry.getUri()),
						"\\", "/");
	...
	public static Collection<CatEntryI> getEntriesByRegex(
			final CatCatalogI cat, String regex) {
		List<CatEntryI> entries = new ArrayList<CatEntryI>();
		for (CatCatalogI subset : cat.getSets_entryset()) {
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			try {
				...
				logger.error("Error occurred testing catalog entry: " + entry,
						exception);
			}
		}
		return entries;
	...
	public static CatDcmentryI getDCMEntryByUID(CatCatalogI cat, String uid) {
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			if (entry instanceof CatDcmentryI
					&& ((CatDcmentryI) entry).getUid().equals(uid)) {
	...
	public static CatDcmentryI getDCMEntryByInstanceNumber(CatCatalogI cat,
			Integer num) {
		CatDcmentryI e;
		for (CatCatalogI subset : cat.getSets_entryset()) {
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			if (entry instanceof CatDcmentryI
					&& ((CatDcmentryI) entry).getInstancenumber().equals(num)) {
				return (CatDcmentryI) entry;
			}
		}
	...
	public static void configureEntry(final CatEntryBean newEntry,
			final XnatResourceInfo info, boolean modified) {
		...
		if (info.getMeta().size() > 0) {
			for (final Map.Entry<String, String> entry : info.getMeta()
					.entrySet()) {
		...
		if (modified) {
			...
			if (info.getEvent_id() != null
					&& newEntry.getModifiedeventid() == null) {
			...
			if (info.getEvent_id() != null
					&& newEntry.getCreatedeventid() == null) {
	...
	public static void configureEntry(final XnatResource newEntry,
			final XnatResourceInfo info, final XDATUser user) throws Exception {
		if (info.getDescription() != null) {
			newEntry.setDescription(info.getDescription());
		}
		if (info.getFormat() != null) {
		...
		if (info.getTags().size() > 0) {
			for (final String entry : info.getTags()) {
				final XnatAbstractresourceTag t = new XnatAbstractresourceTag(
						(UserI) user);
				t.setTag(entry);
	...
	public static List<String> storeCatalogEntry(
			final List<? extends FileWriterWrapperI> fileWriters,
			final String destination, final XnatResourcecatalog catResource,
			final XnatProjectdata proj, final boolean extract,
			final XnatResourceInfo info, final boolean overwrite,
		...
		for (FileWriterWrapperI fileWriter : fileWriters) {
			...
			if (extract
					&& (compression_method.equalsIgnoreCase(".tar")
							|| compression_method.equalsIgnoreCase(".gz")
							|| compression_method.equalsIgnoreCase(".zip") || compression_method
								.equalsIgnoreCase(".zar"))) {
				...
				final List<File> files = zipper.extract(is,
						destinationDir.getAbsolutePath(), overwrite, ci);

				for (final File f : files) {
					if (!f.isDirectory()) {
						final String relative = destinationDir.toURI()
				...
				if (saveTo.exists() && !overwrite) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Saving filename " + filename
								+ " to file " + saveTo.getAbsolutePath());
					}
					...
					if (saveTo.isDirectory()) {
						if (logger.isDebugEnabled()) {
							logger.debug("Found a directory: "
									+ saveTo.getAbsolutePath());
						}

						@SuppressWarnings("unchecked")
						final Iterator<File> iterator = org.apache.commons.io.FileUtils
								.iterateFiles(saveTo, null, true);
						while (iterator.hasNext()) {
							final File movedF = iterator.next();

							String relativePath = FileUtils.RelativizePath(
									saveTo, movedF).replace('\\', '/');
							if (instance != null) {
						...
						if (logger.isDebugEnabled()) {
							logger.debug("Updating catalog entry for file "
									+ saveTo.getAbsolutePath());
						}
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Writing catalog file " + catFile.getAbsolutePath()
					+ " with " + cat.getEntries_entry().size()
					+ " total entries");
		}
	...
	public static Map<String, Map<String, Integer>> retrieveAuditySummary(
			CatCatalogI cat) {
		if (cat == null)
	...
	public static void addAuditEntry(Map<String, Map<String, Integer>> summary,
			String key, String action, Integer i) {
	...
	public static void addAuditEntry(Map<String, Map<String, Integer>> summary,
			Integer eventId, Object d, String action, Integer i) {
		String key = eventId + ":" + d;
		addAuditEntry(summary, key, action, i);
	}

	public static void writeCatalogToFile(CatCatalogI xml, File dest)
			throws Exception {
		try {
			writeCatalogToFile(xml, dest, getChecksumConfiguration());
		} catch (ConfigServiceException exception) {
			throw new Exception(
					"Error attempting to retrieve checksum configuration",
					exception);
		}
	}

	public static void writeCatalogToFile(CatCatalogI xml, File dest,
			boolean calculateChecksums) throws Exception {
	...
	public static File getCatalogFile(final String rootPath,
		...
		if (!f.exists()) {
			...
			try {
				...
				logger.error("Error writing to the folder: "
						+ f.getParentFile().getAbsolutePath(), exception);
			} catch (Exception exception) {
	...
	public static CatCatalogBean getCatalog(File catalogFile) {
		...
		try {
			...
			logger.error("Error occurred reading file: " + catalogFile,
					exception);
		} catch (SAXException exception) {
			logger.error("Error processing XML in file: " + catalogFile,
					exception);
		}
	...
	/**
	 * Parses catalog xml for resource and returns the Bean object. Returns null
	 * if not found.
	 *
	 * @param rootPath
	 *            The root path for the catalog.
	 * @param resource
	 *            The resource catalog.
	 * @return The initialized catalog bean.
	 */
	public static CatCatalogBean getCatalog(String rootPath,
		...
		try {
			...
			if (catalogFile != null) {
				...
				logger.error("Couldn't locate a file based on root path: "
						+ rootPath, exception);
			}
		} catch (IOException exception) {
			if (catalogFile != null) {
			...
			logger.error("Unknown exception reading file at: " + rootPath,
					exception);
		}
	...
	public static CatCatalogBean getCleanCatalog(String rootPath,
			XnatResourcecatalogI resource, boolean includeFullPaths) {
		return getCleanCatalog(rootPath, resource, includeFullPaths, null, null);
	}

	public static CatCatalogBean getCleanCatalog(String rootPath,
			XnatResourcecatalogI resource, boolean includeFullPaths,
			UserI user, EventMetaI c) {
		File catalogFile = null;
		try {
			...
			logger.error(
					"Couldn't find file "
							+ (catalogFile != null ? "indicated by "
									+ catalogFile.getAbsolutePath()
									: "of unknown location"), exception);
		} catch (SAXException exception) {
			logger.error(
					"Couldn't parse file "
							+ (catalogFile != null ? "indicated by "
			...
			logger.error(
					"Couldn't parse or unzip file "
							+ (catalogFile != null ? "indicated by "
									+ catalogFile.getAbsolutePath()
									: "of unknown location"), exception);
		} catch (Exception exception) {
			logger.error(
					"Unknown error handling file "
							+ (catalogFile != null ? "indicated by "
									+ catalogFile.getAbsolutePath()
									: "of unknown location"), exception);
		}
	 ...
	 * 
	 * @param cat
	 *            Catalog entry to be cleaned
	 * @param catPath
	 *            Path to catalog file (used to access files with relative
	 *            paths).
	 * @param user
	 *            User in operation
	 * @param now
	 ...
	 */
	public static boolean formalizeCatalog(final CatCatalogI cat,
			final String catPath, UserI user, EventMetaI now) {
		return formalizeCatalog(cat, catPath, user, now, false, false);
		// default to false for checksums for now. Maybe it should use the
		// default setting for the server. But, this runs everytime a catalog
		// xml is loaded. So, it will get re-run over and over. Not sure we want
		// to add that amount of processing.
	}
	 ...
	 * @param now
	 *            Corresponding event
	 * @param createChecksums
	 *            Boolean whether or not to generate checksums (if missing)
	 * @param removeMissingFiles
	 *            Boolean whether or not to delete references to missing files
	 * @return true if catalog was modified, otherwise false
	 */
	public static boolean formalizeCatalog(final CatCatalogI cat,
			final String catPath, UserI user, EventMetaI now,
			boolean createChecksums, boolean removeMissingFiles) {
		return formalizeCatalog(cat, catPath, cat.getId(), user, now,
				createChecksums, removeMissingFiles);
	}

	public static String getFullPath(String rootPath,
			XnatResourcecatalogI resource) {

		String fullPath = StringUtils.replace(
				FileUtils.AppendRootPath(rootPath, resource.getUri()), "\\",
				"/");
	...
	public boolean modifyEntry(CatCatalogI cat, CatEntryI oldEntry,
			CatEntryI newEntry) {
	...
	public static Boolean maintainFileHistory() {
		if (_maintainFileHistory == null) {
			_maintainFileHistory = XDAT.getBoolSiteConfigurationProperty(
					"audit.maintain-file-history", false);
		}
		return _maintainFileHistory;
	}

	public static void moveToHistory(File catFile, File f, CatEntryBean entry,
			EventMetaI ci) throws Exception {
		// move existing file to audit trail
		if (CatalogUtils.maintainFileHistory()) {
			final File newFile = FileUtils.MoveToHistory(f,
					EventUtils.getTimestamp(ci));
			addCatHistoryEntry(catFile, newFile.getAbsolutePath(), entry, ci);
		}
	}

	public static void addCatHistoryEntry(File catFile, String f,
			CatEntryBean entry, EventMetaI ci) throws Exception {
		// move existing file to audit trail
		...
		File newCatFile = FileUtils.BuildHistoryFile(catFile,
				EventUtils.getTimestamp(ci));
	...
	public static XFTTable populateTable(XFTTable table, XDATUser user,
			XnatProjectdata proj, boolean cacheFileStats) {
		XFTTable newTable = new XFTTable();
		String[] fields = { "xnat_abstractresource_id", "label",
				"element_name", "category", "cat_id", "cat_desc", "file_count",
				"file_size", "tags", "content", "format" };
		...
		while (table.hasMoreRows()) {
			...
			XnatAbstractresource res = XnatAbstractresource
					.getXnatAbstractresourcesByXnatAbstractresourceId(old[0],
							user, false);
			...
			if (cacheFileStats) {
				...
				try {
					...
					if (res instanceof XnatResourcecatalog) {
						logger.error(
								"Failed to save updates to resource catalog: "
										+ res.getLabel(), exception);
					} else {
						logger.error(
								"Failed to save updates to abstract resource: "
	...
	public static boolean populateStats(XnatAbstractresource abstractResource,
			String rootPath) {
	...
	private static void updateEntry(CatCatalogBean cat, String dest, File f,
			XnatResourceInfo info, EventMetaI ci) {
	...
	private static String convertAuditToString(
	...
	private static Map<String, Map<String, Integer>> convertAuditToMap(
			String audit) {
	...
	private static Map<String, Map<String, Integer>> buildAuditSummary(
			CatCatalogI cat) {
		Map<String, Map<String, Integer>> summary = new HashMap<String, Map<String, Integer>>();
	...
	private static void buildAuditSummary(CatCatalogI cat,
			Map<String, Map<String, Integer>> summary) {
		for (CatCatalogI subSet : cat.getSets_entryset()) {
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			addAuditEntry(summary, entry.getCreatedeventid(),
					entry.getCreatedtime(), ChangeSummaryBuilderA.ADDED, 1);

			if (entry.getModifiedtime() != null) {
				addAuditEntry(summary, entry.getModifiedeventid(),
						entry.getModifiedtime(), ChangeSummaryBuilderA.REMOVED,
						1);
			}
		}
	}

	private static File handleCatalogFile(final String rootPath,
			final XnatResourcecatalogI resource) throws Exception {
		...
		if (catalog.getName().endsWith(".gz")) {
			try {
				...
				logger.error("Error occurred reading file: " + catalog,
						exception);
			}
		}
		return catalog;
	...
	/**
	 * Reviews the catalog directory and adds any files that aren't already
	 * referenced in the catalog.
	 *
	 * @param catFile
	 *            path to catalog xml file
	 * @param cat
	 *            content of catalog xml file
	 * @param user
	 *            user for transaction
	 * @param event_id
	 *            event id for transaction
	 * @return true if the cat was modified (and needs to be saved).
	 */
	@SuppressWarnings("unchecked")
	public static boolean addUnreferencedFiles(final File catFile,
			final CatCatalogI cat, XDATUser user, Number event_id) {
		// list of all files in the catalog folder
		final Collection<File> files = org.apache.commons.io.FileUtils
				.listFiles(catFile.getParentFile(), null, true);
		...
		// URI object for the catalog folder (used to generate relative file
		// paths)
		...
		for (final File f : files) {
			if (!f.equals(catFile)) {// don't add the catalog xml to its own
										// list
				// relative path is used to compare to existing catalog entries,
				// and add it if its missing. entry paths are relative to the
				// location of the catalog file.
				final String relative = catFolderURI.relativize(f.toURI())
						.getPath();
				...
				if (e == null) {
					...
					// create basic resource info to specify file properties at
					// creation.
					final XnatResourceInfo info = XnatResourceInfo
							.buildResourceInfo(null, null, null, null, user,
									now, now, event_id);
					configureEntry(newEntry, info, false);
					...
					try {
						...
						logger.error(
								"Something very weird occurred when adding catalog entries",
								exception);
					}
				}
	...
	private static boolean isCatalogFile(File f) {
		...
		try {
			if (org.apache.commons.io.FileUtils.readFileToString(f,
					Charset.defaultCharset()).indexOf("<cat:Catalog") >= 0) {
				return true;
			}
		} catch (IOException e) {
			// Do nothing for now
	...
	/**
	 * Reviews the catalog directory and returns any files that aren't already
	 * referenced in the catalogs in that folder.
	 *
	 * @param catFolder
	 *            path to catalog xml folder
	 * @return true if the cat was modified (and needs to be saved).
	 */
	public static List<File> getUnreferencedFiles(final File catFolder) {
		...
		for (final File f : files) {
			if (!(f.getName().endsWith(".xml") && isCatalogFile(f))) {// ignore
																		// catalog
																		// files
				// relative path is used to compare to existing catalog entries,
				// and add it if its missing. entry paths are relative to the
	...
	private static boolean formalizeCatalog(final CatCatalogI cat,
			final String catPath, String header, UserI user, EventMetaI now,
			final boolean createChecksum, final boolean removeMissingFiles) {
		boolean modified = false;

		for (CatCatalogI subSet : cat.getSets_entryset()) {
			if (formalizeCatalog(subSet, catPath,
					header + "/" + subSet.getId(), user, now, createChecksum,
					removeMissingFiles)) {
				modified = true;
			}
		}
		...
		for (CatEntryI entry : cat.getEntries_entry()) {
			...
			if (entry.getCreatedeventid() == null && now != null
					&& now.getEventId() != null) {
				((CatEntryBean) entry).setCreatedeventid(now.getEventId()
						.toString());
				modified = true;
			}
			...
			if (entry.getId() == null || !entry.getId().equals("")) {
				String entryPath = StringUtils.replace(
						FileUtils.AppendRootPath(catPath, entry.getUri()),
						"\\", "/");
				File f = getFileOnLocalFileSystem(entryPath);
				if (f != null) {
			...
			if (entry.getClass().equals(CatDcmentryBean.class)) { // CatDcmentryBeans
																	// fail to
																	// set
																	// format
																	// correctly
