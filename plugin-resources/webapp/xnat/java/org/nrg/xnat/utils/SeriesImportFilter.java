/*
 * org.nrg.xnat.utils.SeriesFilterUtils
 ...
 *
 * Last modified 12/04/13 12:10 PM
 */
...
public class SeriesImportFilter {
    ...
    public SeriesImportFilter() {
        this(XDAT.getConfigService().getConfig(SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH));
    }

    public SeriesImportFilter(long projectId) {
        this(XDAT.getConfigService().getConfig(SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, projectId));
    ...
    public static List<Pattern> compileFilterList(final List<String> candidates) {
        ...
        if (failed.size() > 0) {
            ...
            if (failed.size() == 1) {
                ...
                failedPatterns = buffer.toString().substring(0, buffer.length() - 2);
            }
    ...
    public static Map<String, String> getSeriesFilterAsMap(Configuration configuration) {
        if (configuration == null) {
    ...
    public void setProjectId(Long projectId) {
        if (!((projectId == null && _projectId == null) || (projectId != null && projectId.equals(_projectId)))) {
            _projectId = projectId;
            _isDirty = true;
        }
    }
    ...
    public void commit(String username, String reason) {
        ...
        final Configuration existing = _projectId == null
                ? XDAT.getConfigService().getConfig(SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH)
                : XDAT.getConfigService().getConfig(SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, _projectId);
        ...
        if (existing != null) {
            ...
            if (isModeChanged || isListChanged) {
                ...
                if (_projectId == null) {
                    ...
                    message.append("series import filter for project ").append(_projectId).append(" ");
                }
                if (isModeChanged) {
                ...
                if (isListChanged) {
                    message.append("list to ").append(map.get("list").trim().replaceAll("\n", ", "));
                }
                try {
                    if (_projectId == null) {
                        XDAT.getConfigService().replaceConfig(username, message.toString(), SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, filter);
                    } else {
                        XDAT.getConfigService().replaceConfig(username, message.toString(), SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, filter, _projectId);
                    }
                } catch (ConfigServiceException exception) {
            ...
            try {
                if (_projectId == null) {
                    XDAT.getConfigService().replaceConfig(username, reason, SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, true, filter);
                } else {
                    XDAT.getConfigService().replaceConfig(username, reason, SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, true, filter, _projectId);
                }
                // In reality, this shouldn't ever really happen. You can't disable the filter and send filter values, but just in case...
                if (!_enabled) {
                    if (_projectId == null) {
                        XDAT.getConfigService().disable(username, "Disabled on creation", SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH);
                    } else {
                        XDAT.getConfigService().disable(username, "Disabled on creation", SERIES_IMPORT_TOOL, SERIES_IMPORT_PATH, _projectId);
                    }
                }
            } catch (ConfigServiceException exception) {
