...
public class ItemPropBuilderTest extends BaseXDATTestCase {
	...
	private static XnatProjectdata getProject() throws Exception {
		if (proj == null) {
			...
			XnatProjectdata.createProject(proj, user, true, false,
					EventUtils.TEST_EVENT(user), "private");
		}
	...
	public XnatSubjectdata getSubject() throws Exception {
		...
		SaveItemHelper.authorizedSave(subj, user, false, true,
	...
	public void testDemographicsMod() throws Exception {
		...
		subj.setProperty(
				"xnat:subjectData/demographics[@xsi:type=xnat:demographicData]/race",
				"x");
		SaveItemHelper.authorizedSave(subj, user, false, false,
				EventUtils.TEST_EVENT(user));

		XFTItem mod = subj.getItem().getCurrentDBVersion();
		FlattenedItemI fi = ItemMerger.merge(ItemPropBuilder
				.build(mod, FlattenedItemA.GET_ALL,
						new ArrayList<FlattenedItemModifierI>()));
		...
		List<FlattenedItemI> flattened = ChangeSummaryBuilderA
				.flatten(original);
	...
	public void testInvestigatorMod() throws Exception {
		...
		SaveItemHelper.authorizedSave(mr, user, false, true,
				EventUtils.TEST_EVENT(user));
		...
		FlattenedItemI fi = ItemMerger.merge(ItemPropBuilder.build(mr.getItem()
				.getCurrentDBVersion(), FlattenedItemA.GET_ALL,
				new ArrayList<FlattenedItemModifierI>()));
		...
		assertEquals("v1",
				flattened.get(0).getFields().getParams().get("lastname"));
		assertEquals("v2",
				flattened.get(1).getFields().getParams().get("lastname"));
	}
	...
	public void testScanMod() throws Exception {
		...
		mr.setSubjectId(subj1.getId());
		mr.setProperty(
				"xnat:mrSessionData/scans/scan[@xsi:type=xnat:mrScanData][0]/ID",
				"x1");
		mr.setProperty(
				"xnat:mrSessionData/scans/scan[@xsi:type=xnat:mrScanData][0]/type",
				"v1");
		SaveItemHelper.authorizedSave(mr, user, false, true,
				EventUtils.TEST_EVENT(user));
