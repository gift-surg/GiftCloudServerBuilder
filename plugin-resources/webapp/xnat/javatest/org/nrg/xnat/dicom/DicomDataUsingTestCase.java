...
public class DicomDataUsingTestCase extends TestCase implements Iterable<File> {
	...
	static {
		...
		if (null == altPath) {
			sample1 = new File(System.getProperty("java.io.tmpdir"),
					"dicom-sample1");
		} else {
			...
			if (sample1.isDirectory()) {
				...
				try {
							...
							if (!ze.isDirectory()) {
								...
								try {
									...
									try {
										...
										if (null != ioe) {
											logger.error(
													"Unable to close sample data file",
													e);
											throw ioe;
										} else {
											throw e;
					...
					throw new RuntimeException(
							"Unable to download required sample DICOM data from "
									+ SAMPLE_1_URL, e);
				}
			}
		}
