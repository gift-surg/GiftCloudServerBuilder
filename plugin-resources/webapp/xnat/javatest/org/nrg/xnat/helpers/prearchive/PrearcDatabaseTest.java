...
public class PrearcDatabaseTest extends BaseXDATTestCase {
	...
	private static ArrayList<String> test_projects = new ArrayList<String>(
			Arrays.asList(new String[] { "proj_0", "proj_1", "proj_2",
					"proj_3", PrearcUtils.COMMON }));
	private static Collection<SessionData> sessions;

	/**
	 * Iterate through a Collection, when the end is reached start over
	 * 
	 * @author aditya
	 *
	 * @param <T>
	...
	public static void setUpBeforeClass() throws Exception {
		SessionDataProducerI sp = new SessionDataProducerI() {
			...
			public Collection<SessionData> get() {
				...
				// There are a finite set of projects in the set that get cycled
				// over.
				// This way we can guarantee that all projects are used.
				// Sessions and timestamps
				// are generated sequentially and subjects are chosen at random
				// from a pre-defined pool.
				Cycle<String> projectCycler = new Cycle<String>(
						PrearcDatabaseTest.test_projects) {
				};
				...
				Any<String> anySubj = new Any<String>(new ArrayList<String>(
						Arrays.asList(RandomPatientInfo.randomNames("subj_",
								numSessions / 4)))) {
					public String any() {
						String[] s = super.c
								.toArray(new String[super.c.size()]);
						return s[(RandomPatientInfo.randomIndex(s))];
					}
				};

				// Create the test data set. Project, timestamp, subject and
				// label are
				...
				for (int i = 0; i < numSessions; i++) {
					...
					s.setFolderName(name)
							...
							.setUploadDate(
									RandomPatientInfo.after(s.getScan_date()))
							.setLastBuiltDate(
									RandomPatientInfo.after(s.getUploadDate()))
							.setScan_time(RandomPatientInfo.randomTimestamp())
							.setUrl(StringUtils.join(new String[] {
									"/tmp/".intern(), s.getTimestamp(),
									"/".intern(), s.getFolderName() }));
	...
	public final void testGetAllSessions() {
		...
		while (i.hasNext()) {
			...
			while (j.hasNext()) {
				...
				if (!found) {
					found = (received.getFolderName().equals(existing
							.getFolderName()))
							&& (received.getTimestamp().equals(existing
									.getTimestamp()))
							&& (received.getProject().equals(existing
									.getProject()));
				}
			}
	...
	public final void testDeleteUnassigned() {
		...
		try {
			PrearcDatabase.getSession(s.getFolderName(), s.getTimestamp(),
					s.getProject());
	...
	public final void testGetProjects() {
		...
		String allProjUri = "/prearchive/projects/"
				+ StringUtils
						.join(test_projects.toArray(new String[test_projects
								.size()]), ",");
		...
		// map each project in the database to the number of sessions in that
		// project
		Map<String, Integer> cache = countSessions(projs);

		// map each project in the local store to the number of sessions in that
		// project
		...
		while (localKeys.hasNext()) {
			...
			// and check that the number of sessions in the database matches the
			// local store
	...
	public final void testSessionLock() {
		...
		for (PrearcUtils.PrearcStatus s : PrearcUtils.PrearcStatus.values()) {
			...
			try {
				...
				// the correct locked status is the current status with an '_'
				// in front of it.
				if (s != PrearcStatus.READY && s != PrearcStatus.ERROR
						&& s.toString().charAt(0) != '_') {
					// check that the session is locked with the correct value
					Assert.assertEquals(PrearcUtils.inProcessStatusMap.get(s)
							.toString(), sd.getStatus().toString());
				}
	...
	public final void testMoveToProject() {
		...
		// retrieve the session and make sure that the project name hasn't
		// changed
		...
		// retrieve the session and make sure that the project name is the new
		// project
	...
	public final void testUpdateModificationTime() {
		...
		try {
			...
			Assert.assertTrue(PrearcDatabase.setStatus(uri,
	...
	public final void testSetStatusLockedSession() {
		...
		try {
			PrearcDatabase.setStatus(uri, PrearcUtils.PrearcStatus.BUILDING);
			PrearcDatabase.lockSession(uri);
			Assert.assertFalse(PrearcDatabase.setStatus(uri,
	...
	public final void testGetOrCreateSession() {
		...
		SessionData s = new SessionData().setProject("proj_test")
				.setTimestamp("1000").setFolderName("sess_test")
	...
	public final void multipleDeleteSession() {
		...
		while (li.hasNext()) {
			...
			while (counter != 0) {
				try {
					PrearcDatabase.getSession(_s.getFolderName(),
	...
	public final void testMoveToProjectWithSyncException() {
		...
		try {
			...
			try {
				...
				Assert.assertEquals(se.getStatus(),
						PrearcUtils.PrearcStatus.ERROR);
	...
	public final void testMarkSessions() {
		...
		Map<SessionDataTriple, Boolean> m = null;
		try {
			m = PrearcDatabase.markSessions(ls,
		...
		while (di.hasNext()) {
			SessionData _s = di.next();
			Assert.assertEquals(PrearcUtils.PrearcStatus.DELETING,
					_s.getStatus());
		}
	}
	...
	public final void multipleMoveToProject() {
		...
		while (li.hasNext()) {
			...
			while (counter != 0) {
				try {
					sd = PrearcDatabase.getSession(_s.getFolderName(),
							_s.getTimestamp(), "proj_1");
					found = true;
				} catch (SQLException e) {
					fail("SQLException" + e.getMessage());
				} catch (SessionException e) {
