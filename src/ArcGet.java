...
public class ArcGet extends CommandPromptTool {
		...
		try {
			...
			if (sessions == null || sessions.size() == 0) {
				...
				if (file == null) {
					if (variables.get("xml") != null) {
						processPartialDoc((String) variables.get("xml"), user,
								pass);
						return;
					} else {
			...
			for (int i = 0; i < sessions.size(); i++) {
				...
				while (outFile.exists()) {
					outFile = new File(dir + session_id + "_" + counter++
							+ ".zip");
				}

				try {
					System.out
							.println("Requesting data for " + session_id + "");
					long startTime = Calendar.getInstance().getTimeInMillis();
					URL url = new URL(this.getSiteURL()
							+ "app/template/ArcGet.vm/session/"
					...
					if (unzip) {
						long serviceDuration = Calendar.getInstance()
						...
						serviceDuration = Calendar.getInstance()
								.getTimeInMillis() - startTime;
						System.out.println("Archive Loaded (" + serviceDuration
								+ " ms): " + dir + session_id);
					} else {
						long serviceDuration = Calendar.getInstance()
								.getTimeInMillis() - startTime;
					...
					System.out.println("Error retrieving data for "
							+ session_id);
	...
	public void processPartialDoc(String fileLocation, String user, String pass)
			throws Exception {
	...
	public void definePossibleVariables() {
		addPossibleVariable(
				"session_id",
				"MR Session ID for the desired session(s)\nFor multiple sessions, use multiple -s tags.",
				new String[] { "s", "session_id" }, false, true);
		addPossibleVariable("f",
				"File containing MRSession IDs separated by line breaks.",
				new String[] { "f", "file_name" }, false);
		addPossibleVariable("dir", "Directory to store files.", new String[] {
				"o", "dir_name" });
		addPossibleVariable("z",
				"Bundles each requested session in a zipped file.",
				new String[] { "z", "zip" });
		addPossibleVariable("xml", "Path to Partial XML document",
				new String[] { "xml" });
	}
