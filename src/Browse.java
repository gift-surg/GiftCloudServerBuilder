...
public class Browse extends CommandPromptTool {
	...
	public void definePossibleVariables() {
		addPossibleVariable("elementName", "element to browse.", new String[] {
				"e", "dataType" }, true);
		addPossibleVariable(
				"output",
				"output Format (i.e. 'xml','csv','text', or 'console') - default is output to xml.",
				new String[] { "f", "format" });

		addPossibleVariable("backup", "whether or not to duplicate files.",
				new String[] { "duplicate" });
		addPossibleVariable("dir", "Directory to store files.",
				new String[] { "dir" });
		addPossibleVariable("insert_date",
				"Get new items since this date. \"yyyy-MM-dd HH:mm:ss\"", false);
		addPossibleVariable("duration",
				"Number of days used to generate insert_date", false);
		addPossibleVariable(
				"pp",
	...
	public void run() throws DBPoolException, SQLException,
		...
		if (getService() == null) {
			...
			try {
				...
				URLExists = false;
				if (XFT.VERBOSE)
					System.out.println("Unable to connect to web service.");
			}
		...
		try {
			...
			if (output.equalsIgnoreCase("csv")) {
				...
				if (backup.equalsIgnoreCase("true")) {
					tool.XMLSearch(elementName, true, dir, false,
							pretty.booleanValue());
				} else {
					tool.XMLSearch(elementName, false, dir, false,
							pretty.booleanValue());
				}
				...
				ItemSearch search = ItemSearch.GetItemSearch(elementName,
						tool.getUser());
	...
	public void service() {
		...
		try {
			...
			URL requestSessionURL = new URL(this.getSiteURL()
			...
			long serviceDuration = Calendar.getInstance().getTimeInMillis()
					- startTime;
			if (XFT.VERBOSE)
				System.out.println("Response Received (" + serviceDuration
						+ " ms)");

			String pk = (String) o[0];
			Object[] ids = (Object[]) o[1];
			if (XFT.VERBOSE)
				System.out.println(ids.length + " Item(s) Found.");
				...
				for (int i = 0; i < ids.length; i++) {
					...
					if (output.equalsIgnoreCase("text")) {
						try {
							service = new Service();
							call = (Call) service.createCall();
							call.setTargetEndpointAddress(this.getSiteURL()
									+ "axis/VelocitySearch.jws");
							...
							call.setUsername(user);
							call.setPassword(pass);
							params = new Object[] { field, comparison, value,
									dataType };

							if (XFT.VERBOSE)
								System.out.print("Sending Request For " + id
										+ "...");
							...
							String finalName = id + ".txt";
							FileUtils
									.OutputToFile(text, dir + finalName, false);
							System.out.println("Created File: " + dir
									+ finalName);
						} catch (Throwable e) {
							System.out.println("Error retrieving text for "
									+ id);
						...
						boolean exists = FileUtils.SearchFolderForChild(dest,
								finalName);
						if (exists && !duplicate) {
							if (XFT.VERBOSE)
							...
							while (exists) {
								...
								exists = FileUtils.SearchFolderForChild(dest,
										finalName);
							}
							...
							try {
								if (XFT.VERBOSE)
									System.out.println("Requesting xml for "
											+ id + "");
								startTime = Calendar.getInstance()
										.getTimeInMillis();
								URL url = new URL(this.getSiteURL()
										+ "app/template/XMLSearch.vm/session/"
										+ session + "/id/" + id + "/data_type/"
										+ dataType);
								...
								serviceDuration = Calendar.getInstance()
										.getTimeInMillis() - startTime;
								if (XFT.VERBOSE)
									System.out.println("Response Received ("
											+ serviceDuration + " ms)");
								...
								System.out.println("Error retrieving xml for "
										+ id);
								System.out.println(e.getMessage());
			...
			if (XFT.VERBOSE)
				System.out
						.println("Total Time: "
								+ (Calendar.getInstance().getTimeInMillis() - ultimateStartTime)
								+ " ms");
		} catch (AxisFault ex2) {
			...
			System.out
					.println("Invalid date format: Try \"yyyy-MM-dd HH:mm:ss\"");
			_return = 11;
