...
public class CleanXML {
	...
	public static void process(List argsAL) {
		...
		if (index == -1) {
			...
			if (index == -1) {
				System.err
						.println("No directory or file supplied. (-file or -dir)");
				return;
			} else {
			...
			if (directory == null) {
				...
				if (fixSchemaLocation) {
					s = StringUtils.ReplaceStr(s,
							"http://cnda.wustl.edu:80/cnda_xnat/schemas",
							"C:/xdat/deployments/cnda_xnat/src/schemas");
					if (validate) {
						try {
							parse(f);
						} catch (Exception e) {
				...
				if (fixFile) {
					s = StringUtils.ReplaceStr(s,
							"xsi:type=\"xnat:imageSeries\"",
							"xsi:type=\"xnat:imageResourceSeries\"");

					s = StringUtils.ReplaceStr(s, " fileCount=", " count=");
					s = StringUtils.ReplaceStr(s,
							"xsi:type=\"xnat:imageFile\"",
							"xsi:type=\"xnat:imageResource\"");
					s = StringUtils.ReplaceStr(s, "xsi:type=\"xnat:file\"",
							"xsi:type=\"xnat:resource\"");
					...
					while (fileIndex != -1) {
						...
						if (sub.indexOf("xsi:type") == -1) {
							...
							s = start + " xsi:type=\"xnat:resource\" "
									+ s.substring(fileIndex + 11);
							...
							if (pattern == -1) {
								...
								if (nOpen != -1) {
									...
									String nString = sub.substring(nOpen + 7,
											nClose);

									int pOpen = sub.indexOf(" path=\"");
									int pClose = sub.indexOf('\"', pOpen + 7);
									String pString = sub.substring(pOpen + 7,
											pClose);
									...
									if (pOpen < nOpen) {
										sub = sub.substring(0, nOpen)
												+ sub.substring(nClose + 1);
										sub = sub.substring(0, pOpen)
												+ sub.substring(pClose + 1);
				...
				if (removeProvenance) {
					while (s.indexOf("<xnat:provenance>") != -1) {
						...
						int endIndex = s.indexOf("xnat:provenance>",
								startIndex + 4) + 11;
					...
					if (f.getName().endsWith(".xml")) {
						try {
							...
							if (fixFile) {
								...
								s = StringUtils.ReplaceStr(s, " fileCount=",
										" count=");
								s = StringUtils.ReplaceStr(s,
										"xsi:type=\"xnat:imageFile\"",
										"xsi:type=\"xnat:imageResource\"");
								...
								while (fileIndex != -1) {
									...
									if (sub.indexOf("xsi:type") == -1) {
										...
										if (pattern == -1) {
											int nOpen = sub.indexOf(" name=\"");
											if (nOpen != -1) {
												int nClose = sub.indexOf('\"',
														nOpen + 7);
												String nString = sub.substring(
														nOpen + 7, nClose);

												int pOpen = sub
														.indexOf(" path=\"");
												int pClose = sub.indexOf('\"',
														pOpen + 7);
												String pString = sub.substring(
														pOpen + 7, pClose);
							...
							if (removeNotes) {
								while (s.indexOf("<xnat:notes>") != -1) {
									...
									int endIndex = s.indexOf("xnat:notes>",
											startIndex + 4) + 11;
								...
								while (s.indexOf("<xnat:note>") != -1) {
									...
									int endIndex = s.indexOf("xnat:note>",
											startIndex + 4) + 10;
							...
							if (removeProvenance) {
								while (s.indexOf("<xnat:provenance>") != -1) {
									int startIndex = s
											.indexOf("<xnat:provenance>");

									int endIndex = s.indexOf(
											"xnat:provenance>", startIndex + 4) + 11;
							...
							if (removeTime) {
								while (s.indexOf("<xnat:time>") != -1) {
									...
									int endIndex = s.indexOf("xnat:time>",
											startIndex + 5) + 10;
	...
	public static void parse(java.io.File data) throws IOException,
