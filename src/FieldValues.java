...
public class FieldValues extends CommandPromptTool {
	...
	public void definePossibleVariables() {
		addPossibleVariable(
				"rfield",
				"field values to return - Must be specified using dot syntax from the parent element. (i.e. ClinicalAssessment.ID",
				true);
		addPossibleVariable(
				"sfield",
		...
		addPossibleVariable("comparison",
				"(i.e. '=','<','<=','>','>=', or 'LIKE').", new String[] { "c",
						"comparison" }, false);
		addPossibleVariable("order", "(i.e. 'ASC' OR 'DESC').",
				new String[] { "order" }, false);
	}
	...
	public void process() {
		...
		try {
			...
			String rfieldElementName = StringUtils
					.GetRootElementName((String) hash.get("rfield"));
			String rvalidElementName = XFTTool
					.GetValidElementName(rfieldElementName);
			if (!rvalidElementName.equals(rfieldElementName)) {
				rfield = rvalidElementName + XFT.PATH_SEPERATOR
						+ StringUtils.GetFieldText(rfield);
			}

			String sfield = (String) hash.get("sfield");
			sfield = org.nrg.xft.utils.StringUtils.StandardizeXMLPath(sfield);

			String sfieldElementName = StringUtils
					.GetRootElementName((String) hash.get("sfield"));
			String svalidElementName = XFTTool
					.GetValidElementName(sfieldElementName);
			if (!svalidElementName.equals(sfieldElementName)) {
				sfield = svalidElementName + XFT.PATH_SEPERATOR
						+ StringUtils.GetFieldText(sfield);
			}
			...
			QueryOrganizer qo = new QueryOrganizer(elementName, this.getUser(),
					ViewManager.ALL);
			qo.addField(rfield);
			qo.addField(sfield);
			...
			XFTTable table = XFTTable.Execute(query,
					SchemaElement.GetElement(elementName).getDbName(), this
							.getUser().getLogin());
