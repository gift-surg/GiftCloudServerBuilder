...
public class FilePathUpdater {
	...
	public static void showUsage() {
		...
		System.out
				.println("Function used to access particular sub-sets of the data in the database.\n");

		System.out.println("Options:");
		System.out
				.println("-xdir, File path to the xnat root directory (required with -project)");
		System.out
				.println("-project, Name of project to use. (required with -xdir)");
	...
	public static void Process(Hashtable hash) {
		...
		try {
			...
			if ((hash.get("xdir") != null) && (hash.get("project") != null)) {
				...
				hash.put("instance", xdir + "deployments" + File.separator
						+ project);
			}
			...
			if (hash.get("instance") == null) {
				if (!(new File("InstanceSettings.xml")).exists()) {
					...
					System.out
							.println("Use the -xdir and -project variables to specify the installation and project dir.");
					System.exit(0);
				}
				File f = new File("");
			...
			ItemCollection coll = ItemSearch.GetAllItems("xnat:file",
					tool.getUser(), false);

			DBItemCache cache = new DBItemCache(null, EventUtils.DEFAULT_EVENT(
					tool.getUser(), "File Path Updater"));
			String dbname = null;
			...
			// initialPath.substring(initialPath.indexOf(File.separator));
			// }else{
			// String relativePath =
			// initialPath.substring(initialPath.indexOf(File.separator + s));
			...
			FileUtils.OutputToFile(sb.toString(), tool.getWorkDirectory()
					+ "missing.csv");
