...
public class ManageProjectAccessories extends CommandPromptTool {
	...
	public void process() {
		try {
			...
			Collection<ElementSecurity> all = ElementSecurity
					.GetElementSecurities().values();
			for (ElementSecurity es : all) {
				try {
					if (es.getAccessible()) {
						GenericWrapperElement g = es.getSchemaElement()
								.getGenericXFTElement();

						if (g.instanceOf("xnat:subjectData")
								|| g.instanceOf("xnat:experimentData")) {
							usable.put(es.getElementName(), es);
						} else {
							System.out.println(es.getElementName()
									+ " not an experiment");
						}
					} else {
						System.out.println(es.getElementName()
			...
			ArrayList<XnatProjectdata> al = XnatProjectdata
					.getAllXnatProjectdatas(null, false);
			System.out.println("" + al.size() + " projects");

			EventMetaI c = EventUtils.DEFAULT_EVENT(getUser(),
					"src/Manage Project Accessories");

			for (XnatProjectdata p : al) {
				System.out.println("Reviewing " + p.getId());
				if (p.getStudyprotocol().size() == 0) {
					XnatDatatypeprotocol protocol = new XnatDatatypeprotocol(
							(UserI) getUser());
					...
					protocol.setProperty("ID", p.getProperty("ID")
							+ "_"
							+ usable.get(elementName).getSchemaElement()
									.getSQLName());
					protocol.setProperty(
							"xnat:datatypeProtocol/xnat_projectdata_id",
							p.getId());
					protocol.setProperty(
					...
					protocol.setProperty(
							...
							"false");
					SaveItemHelper.authorizedSave(protocol, getUser(), false,
							false, c);

					p.setStudyprotocol(protocol);

					Hashtable<String, Long> counts = p
							.getExperimentCountByXSIType();
					for (Map.Entry<String, Long> entry : counts.entrySet()) {
						if (entry.getValue() > 0) {
							elementName = entry.getKey();

							protocol = new XnatDatatypeprotocol(
									(UserI) getUser());
							protocol.setDataType(elementName);
							...
							protocol.setProperty("ID", p.getProperty("ID")
									+ "_"
									+ usable.get(elementName)
											.getSchemaElement().getSQLName());
							...
							SaveItemHelper.authorizedSave(protocol, getUser(),
									false, false, c);
				...
				arcP.setProperty(
						...
						ArcSpecManager.GetInstance()
								.getArcArchivespecificationId());
				arcP.setCurrentArc("arc001");

				if (arcP.getItem().getUniqueMatches(false).size() == 0) {
					if (arcP.getPaths() == null
							|| arcP.getPaths().getArchivepath() == null
							|| arcP.getPaths().getArchivepath().equals("")) {
						arcP.setProperty("arc:project/paths/archivePath",
								ArcSpecManager.GetInstance()
							...
							.getPaths()
							.getArchivepath()
							.equals(ArcSpecManager.GetInstance()
									.getGlobalArchivePath())) {
						arcP.setProperty("arc:project/paths/archivePath",
								ArcSpecManager.GetInstance()
										.getGlobalArchivePath()
					...
					if (arcP.getPaths() == null
							|| arcP.getPaths().getPrearchivepath() == null
							|| arcP.getPaths().getPrearchivepath().equals("")) {
						arcP.setProperty("arc:project/paths/prearchivePath",
								ArcSpecManager.GetInstance()
					...
					if (arcP.getPaths() == null
							|| arcP.getPaths().getCachepath() == null
							|| arcP.getPaths().getCachepath().equals("")) {
						arcP.setProperty("arc:project/paths/cachePath",
								ArcSpecManager.GetInstance()
										.getGlobalCachePath() + p.getId() + "/");
					} else if (arcP
							.getPaths()
							.getCachepath()
							.equals(ArcSpecManager.GetInstance()
					...
					if (arcP.getPaths() == null
							|| arcP.getPaths().getBuildpath() == null
							|| arcP.getPaths().getBuildpath().equals("")) {
						arcP.setProperty("arc:project/paths/buildPath",
								ArcSpecManager.GetInstance()
										.getGlobalBuildPath() + p.getId() + "/");
					}
