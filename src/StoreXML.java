...
public class StoreXML extends CommandPromptTool {
	...
	// .;XFT.jar;commons-collections-2.0.jar;commons-configuration-1.0-dev.jar;commons-dbcp-1.0-dev-20020806.jar;commons-lang-1.0.jar;commons-pool-1.0.jar;jakarta-regexp-1.2.jar;log4j-1.2.6.jar;parser.jar;pg74.214.jdbc2.jar;stratum-1.0-b3.jar;xerces-J_1.4.0.jar
	// XFTApp
	...
	public void definePossibleVariables() {
		addPossibleVariable("location", "location of xml file to insert.",
				new String[] { "l", "location" }, false);
		addPossibleVariable("dir", "directory containing files to insert.",
				new String[] { "dir" }, false);
		addPossibleVariable(
				"r",
				"in combination with the dir tag, this will cause the app to descend into sub folders looking for xml files.",
				new String[] { "r", "recursive" }, false);
		addPossibleVariable("activate", "Auto-activate all inserted data.",
				false);
		addPossibleVariable("quarantine",
				"Load all new/updated data into quaratine.", false);
		addPossibleVariable(
				"allowItemOverwrite",
				"(either 'true' or 'false'): Whether or not pre-existing data for this element which has no unique indentifiers specified, should be overwritten.  If 'true' the pre-existing rows will be removed before the new rows are inserted.  If 'false', then the new rows will be added (appended) without affect to the pre-existing rows.",
				new String[] { "allowDataDeletion", "allowItemOverwrite" },
				true);

		addPossibleVariable("stopAtException",
				"stop At Exception (Defaults to true)");
	}
	...
	public void service() {
		...
		try {
			if (hash.get("dir") == null) {
				...
				if (hash.get("r") == null) {
					storeXMLFolderService(dir, false, service, user, pass,
							quarantine, allowItemOverwrite, stopAtException);
				} else {
					storeXMLFolderService(dir, true, service, user, pass,
							quarantine, allowItemOverwrite, stopAtException);
				}
			}
		} catch (AxisFault ex2) {
	...
	private void sendFile(File f, String user, String pass, Boolean quarantine,
			Boolean allowItemOverwrite, Service service)
	...
	public void process() {
		...
		try {
			...
			Boolean quarantine = (Boolean) hash.get("quarantine");
			Boolean allowItemOverwrite = (Boolean) hash
					.get("allowItemOverwrite");
			...
			if (hash.get("dir") == null) {
				tool.info((String) hash.get("location") + "," + quarantine
						+ "," + allowItemOverwrite.booleanValue());
				tool.storeXML(((String) hash.get("location")), quarantine,
						allowItemOverwrite.booleanValue());
			} else {
				tool.info((String) hash.get("dir") + "," + hash.get("r") + ","
						+ quarantine + "," + allowItemOverwrite.booleanValue());
				File dir = new File((String) hash.get("dir"));
				if (hash.get("r") == null) {
					StoreXMLFolder(dir, false, tool, quarantine,
							allowItemOverwrite, hash, stopAtException);
				} else {
					StoreXMLFolder(dir, true, tool, quarantine,
							allowItemOverwrite, hash, stopAtException);
				}
			}
	...
	private void storeXMLFolderService(File dir, boolean recursive,
			Service service, String user, String pass, Boolean quarantine,
			Boolean allowItemOverwrite, boolean stopAtException)
			throws RemoteException, ServiceException {
		if (!dir.exists()) {
		} else {
			...
			for (int i = 0; i < list.length; i++) {
				if (list[i].getName().endsWith(".xml")) {
					try {
						sendFile(list[i], user, pass, quarantine,
								allowItemOverwrite, service);
					} catch (AxisFault ex2) {
			...
			if (recursive) {
				...
				while (iter.hasNext()) {
					...
					storeXMLFolderService(f, recursive, service, user, pass,
							quarantine, allowItemOverwrite, stopAtException);
				}
			}
		}
	...
	private static void StoreXMLFolder(File dir, boolean recursive,
			XDATTool tool, Boolean quarantine, Boolean allowItemOverwrite,
			Hashtable hash, boolean stopAtException) throws Exception {
		if (!dir.exists()) {
		} else {
			ArrayList dirs = new ArrayList();
			File[] list = dir.listFiles();
				...
				if (list[i].getName().endsWith(".xml")) {
					try {
						...
						long startTime = Calendar.getInstance()
								.getTimeInMillis();
						tool.storeXML(list[i], quarantine,
								allowItemOverwrite.booleanValue());
						if (XFT.VERBOSE)
							System.out
									.print("  "
											+ ((float) (Calendar.getInstance()
													.getTimeInMillis() - startTime) / 1000)
											+ "s ");
					} catch (ValidationException e) {
						XFT.LogError(e.VALIDATION_RESULTS.toString(), e);
						System.out
								.println("ERROR:  See log for details (logs/xdat.log).");
							...
							FileUtils.OutputToFile(
									"Error Storing (XNAT - Validation) "
											...
											+ e.VALIDATION_RESULTS
													.toFullString(),
									tool.getSettingsDirectory()
											+ "/logs/storeXMLexceptions.txt",
									true);
						}
					} catch (InvalidItemException e) {
						XFT.LogError(e.getMessage());
						...
						if (stopAtException) {
							...
							FileUtils.OutputToFile(
									"Error Storing (Invalid permissions) "
											+ list[i].getName() + " "
											+ e.getMessage(),
									tool.getSettingsDirectory()
											+ "/logs/storeXMLexceptions.txt",
						...
						if (stopAtException) {
							...
							FileUtils.OutputToFile(
									"Error Storing (XERCES - Validation) "
											+ list[i].getName() + " "
											+ e.getMessage(),
									tool.getSettingsDirectory()
											+ "/logs/storeXMLexceptions.txt",
			...
			if (recursive) {
				...
				while (iter.hasNext()) {
					...
					StoreXMLFolder(f, recursive, tool, quarantine,
							allowItemOverwrite, hash, stopAtException);
				}
			}
		}
