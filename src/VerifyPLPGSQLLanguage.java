	...
	public void execute() throws BuildException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (i == 0) {
					throw new BuildException(
							"Unable to locate 'plpgsql' language for the database '"
									+ connectionstring
									+ "'."
			...
			System.out.println("Unable to connected to '" + connectionstring
					+ "'");
			throw new BuildException(
					"Unable to instantiate Driver. (InstantiationException)");
			...
			System.out.println("Unable to connected to '" + connectionstring
					+ "'");
			throw new BuildException(
					"Unable to instantiate Driver. (ClassNotFoundException)");
		} catch (SQLException e) {
			String message = e.getMessage();
			if ((message.indexOf("FATAL: database") != -1)
					&& (message.indexOf("does not exist") != -1)) {
				String host = connectionstring.substring(0,
						connectionstring.lastIndexOf("/"));
				String db = connectionstring.substring(connectionstring
						.lastIndexOf("/") + 1);
				System.out.println("\n\nDatabase '" + db
						+ "' does not exist at '" + host + "'.");
				System.out
						.println("Please create the database and re-execute this procedure.");
				throw new BuildException(e.getMessage());
			} else {
				System.out.println("Unable to connected to '"
						+ connectionstring + "'");
				System.out
						.println("Please resolve the PostgreSQL issue (specified below).");
				throw new BuildException(e.getMessage());
			}
		} finally {
			if (con != null) {
