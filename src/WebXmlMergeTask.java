...
public class WebXmlMergeTask extends Task {
	...
	public void execute() throws BuildException {
		...
		// Modification to return rather than throw build exceptions when files
		// are missing. We don't
		// expect there to always be a project-specific merge file for an XNAT
		// build.
		...
		try {
			...
			// The cactus program expected the merging of two valid web.xml
			// files. While this might
			// be the case for XNAT, it likely we would rather just specify
			// additions to the plugin-resources
			// web.xml rather than a full file. Here we read file and add
			// surrounding webapp elements
			...
			if (force
					...
					|| (mergeFile != null && mergeFile.lastModified() > destFile
							.lastModified())) {
				// If no mergeFile, just output source to Destination and return
				if (mergeFile == null
						|| !(mergeFile.exists() && mergeFile.isFile())) {
				...
				if (sbMerge.indexOf("<web-app>") < 0
						&& sbMerge.indexOf("</web-app>") < 0) {
					createFromFragments(sbMerge, sbSrc);
				}
				WebXml srcWebXml;
				try {
					srcWebXml = WebXmlIo.parseWebXml(new ByteArrayInputStream(
							sbSrc.toString().getBytes()), this.xmlCatalog);
				...
				try {
					mergeWebXml = WebXmlIo.parseWebXml(
							new ByteArrayInputStream(sbMerge.toString()
									.getBytes()), this.xmlCatalog);
				} catch (JDOMException e) {
				...
				log("The destination file is up to date", Project.MSG_VERBOSE);
			}
	 ...
	 * 
	 * @param theSrcFile
	 *            the original <code>web.xml</code>
	 */
	 ...
	 * 
	 * @param theMergeFile
	 *            the <code>web.xml</code> to merge
	 */
	 ...
	 * 
	 * @param theDestFile
	 *            the resulting <code>web.xml</code>
	 */
	...
	/**
	 * Sets whether the merge should be performed even when the destination file
	 * is up to date.
	 * 
	 * @param isForce
	 *            Whether the merge should be forced
	 */
	...
	private static void createFromFragments(StringBuilder sbMerge,
			StringBuilder sbSrc) {
	...
	public static void outputToFile(CharSequence content, File outFile)
			throws IOException {
