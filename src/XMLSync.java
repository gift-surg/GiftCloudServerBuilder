...
public class XMLSync extends CommandPromptTool {
	...
	public void definePossibleVariables() {
		...
		addPossibleVariable("remoteURL",
				"URL of remote server (http://localhost:8080/cnda_xnat)", true);
		addPossibleVariable("remoteUsername",
				"Username for connection to remote server", true);
		addPossibleVariable("remotePassword",
				"Password for connection to remote server", true);
	}
	...
	public void run() throws DBPoolException, SQLException,
		...
		if (URLExists) {
			...
			System.out.println("Local Server is not available. "
					+ url.toString());
			System.exit(1);
		}
	}
	...
	public void service() {
		...
		File todaysDir = new File(dir + "work" + File.separator + "sync"
				+ File.separator + currentTime);
		if (!todaysDir.exists()) {
			todaysDir.mkdir();
		}

		File summaryFile = new File(dir + "work" + File.separator + "sync"
				+ File.separator + currentTime + File.separator + "summary.log");
		FileUtils.OutputToFile("XML Sync:"
						...
						try {
							...
							if (XFT.VERBOSE)
								System.out.println("Response Received ("
										+ serviceDuration + " ms)");
							FileUtils.OutputToFile("Search (" + serviceDuration
							...
							for (int j = 0; j < o.length; j++) {
								...
								FileUtils.OutputToFile(content, dir + "work"
										+ File.separator + "sync"
										+ File.separator + currentTime
										+ File.separator + name + ".xml", true);

								call.setUsername(user);
								call.setPassword(pass);
								call.setOperationName("store");
								params = new Object[] { content, Boolean.FALSE,
										Boolean.TRUE };
								...
								FileUtils.OutputToFile(name + " StoreXML ("
										+ serviceDuration + " ms)\n" + _store
										+ "\n", summaryFile.getAbsolutePath(),
										true);
