...
public class XDATXMLReader extends DefaultHandler {
	...
	public BaseElement getBaseElement(String uri, String localName)
			throws SAXException {
		return getBaseElement(uri + ":" + localName);
	}

	public BaseElement getBaseElement(String name) throws SAXException {
		String className = (String) ClassMapping.GetInstance().ELEMENTS
				.get(name);
		if (className == null) {
			...
			try {
				...
				throw new SAXException("Unable to instantiate class: "
						+ className, e);
			} catch (IllegalAccessException e) {
				throw new SAXException("Illegal access of class: " + className,
						e);
			}
	...
	public void startElement(String uri, String localName, String qName,
		...
		if (root == null) {
					...
					if (!value.equalsIgnoreCase("")) {
						try {
							...
							throw new SAXException(
									"Invalid value for attribute '" + local
											+ "'");
						}
					}
				}
			...
			if (TYPE != null
					&& (TYPE.equals(BaseElement.field_inline_repeater)
							|| TYPE.equals(BaseElement.field_multi_reference)
							|| TYPE.equals(BaseElement.field_single_reference) || TYPE
								.equals(BaseElement.field_NO_CHILD))) {
				String foreignElement = null;
				if (attributes != null) {
					for (int i = 0; i < attributes.getLength(); i++) {
						if (attributes.getURI(i).equalsIgnoreCase(
								"http://www.w3.org/2001/XMLSchema-instance")
								&& attributes.getLocalName(i).equalsIgnoreCase(
										"type")) {
							foreignElement = attributes.getValue(i);
							int index = foreignElement.indexOf(":");
							if (index != -1) {
								foreignElement = this.prefixToURIMapping
										.get(foreignElement.substring(0, index))
										+ foreignElement.substring(index);
							}
							break;
						}
					}
				...
				try {
					if (foreignElement == null) {
						foreignElement = currentItem
								.getReferenceFieldName(current_header);
					}
					...
					if (attributes != null) {
						for (int i = 0; i < attributes.getLength(); i++) {
							if (!(attributes
									.getURI(i)
									.equalsIgnoreCase(
											"http://www.w3.org/2001/XMLSchema-instance") && attributes
									.getLocalName(i).equalsIgnoreCase("type"))) {
								String local = attributes.getLocalName(i);
								String value = attributes.getValue(i);
					...
					try {
						current.getItem().setReferenceField(current_header,
								item);
						current = new SAXReaderObject(item, current, TYPE);
						if (TYPE.equals(BaseElement.field_inline_repeater)
								|| TYPE.equals(BaseElement.field_NO_CHILD)) {
							...
							if (!match) {
								if (item.getFieldType(item
										.getSchemaElementName()) != null) {
									current.addHeader(item
				...
				if (attributes != null) {
					for (int i = 0; i < attributes.getLength(); i++) {
						...
						if (!value.equalsIgnoreCase("")) {
							try {
								currentItem.setDataField(current_header + "/"
										+ local, value);
							} catch (BaseElement.UnknownFieldException e1) {
								logger.error(e1);
								throw new SAXException("Unknown field '"
	...
	public void endElement(String uri, String localName, String qName)
		...
		if (tempValue != null && !tempValue.equals("")
			...
			try {
				...
				throw new SAXException("Invalid value for field '"
						+ current_header + "'");
			} catch (RuntimeException e) {
				logger.error(e);
				throw new SAXException("Unknown Exception <" + current_header
						+ ">" + tempValue);
			} finally {
				tempValue = null;
			}
	...
	/**
	 * Convert null unicode characters into spaces. The given InputStream is
	 * iterated and mark set to the beginning afterwards.
	 * 
	 * @param i
	 * @return
	 * @throws IOException
	 */
	public static InputStream removeNullUnicodeChars(InputStream i)
			throws IOException {
	...
	public BaseElement parse(java.io.File data) throws IOException,
			SAXException {
	...
	public BaseElement parse(org.xml.sax.InputSource data) throws IOException,
			SAXException {
		SAXParserFactory spf = SAXParserFactory.newInstance();
		try {
			spf.setNamespaceAware(true);
